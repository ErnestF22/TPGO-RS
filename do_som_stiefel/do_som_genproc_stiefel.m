function [rotation_error_manopt_sep,translation_error_manopt_sep, ...
    ...
    exectime_manopt,exectime_manopt_sep] = do_som_genproc_stiefel(testdata, sigma, mu, params)
%DO_SOM_GENPROC_STIEFEL Function that executes the Shape of Motion algorithms through
%"Separated" Manopt and "Generalized Procrustes" Manopt, with formulation 
% on Stiefel manifold. The function returning 
...
transf_manopt_sep = som_manopt_stiefel(T_globalframe_nois, Tijs_vec_nois, edges, params, matStack(RT2G(R_initguess, transl_initguess)));
% manopt_end_time = tic();
exectime_manopt = toc(manopt_start_time);

% 3b) execute with step 1 through PROCRUSTES
manopt_sep_start_time = tic();
transf_manopt_gen = som_manopt_genproc_stiefel(T_globalframe_nois, Tijs_vec_nois, edges, params, matStack(transf_initguess));
