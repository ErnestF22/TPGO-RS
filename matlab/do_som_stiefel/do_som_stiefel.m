function [rotation_error_manopt,translation_error_manopt, ...
    ...
    exectime_manopt,exectime_procrustes] = do_som_stiefel(testdata, sigma, mu, params)
%DO_SOM_STIEFEL Function that executes the Shape of Motion algorithms through
%Manopt and Procrustes pipelines, with formulation on Stiefel manifold; 
%this function returns rotation and translation errors, 
%as well as the mean execution time; 
%the SoM algorithms are run on Gaussian noisy input data, 
%with sigma variance and mu mean (that are being 
%passed as arguments to the DO_SOM_STIEFEL function)
