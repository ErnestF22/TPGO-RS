function [rotation_error_procrustes,translation_error_procrustes, ...
    ...
    exectime_procrustes, ...
    exectime_manopt, ...
    exectime_manopt_sep] = do_som_cp2m_noiseinit_stiefel(testdata, ...
    sigma_noise, sigma_init, mu, params)
%DO_SOM_CP2M_NOISEINIT_STIEFEL Function that executes the Shape of Motion 
%algorithms through Procrustes, "Separated" Manopt and 
%"Generalized Procrustes" Manopt, returning  rotation and translation 
%errors, as well as the mean execution time, with formulation on Stiefel manifold;
