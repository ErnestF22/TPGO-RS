function [x, cost, info, options] = pic_trustregions(problem, x, options, testdata_plot)
...
while true
    ...
    nn = size(x.R, 3);
    for ii = 1:nn
        tmp = eye(size(x.R, 1) + 1, size(x.R, 1) + 1);
        testdata_plot.gi(:,:,ii) = tmp;
        testdata_plot.gi(:,:,ii) = RT2G(x.R(:,:,ii), x.T(:,ii));
    end
    testNetworkDisplay(testdata_plot,'member','gi')
    hold on;
