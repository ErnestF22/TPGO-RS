function [transf_out, lambdas_out]  = ssom_procrustes(T_globalframe_nois, lambdas_init, tijs, edges, params)
...
if (procrustes_mode == "som")
    ...
    rot_curr = ssom_stepone_procrustes(T_globalframe_nois, lambdas_init, tijs, edges, params);
%     r_reflections = rot_curr(:,:,1);
%     for ii = 1:N
%         r(:,:,ii) = r(:,:,ii) * r_reflections;
%     end
    transl_curr = ssom_steptwo_procrustes(rot_curr, T_globalframe_nois, lambdas_init, tijs, edges, params);
    % T = reshape(T, d, []);
...
%     transl_prev = transl_curr;
    ...
    rot_curr = ssom_stepone_procrustes(T_globalframe_nois, lambdas_curr, tijs, edges, params);    

    %COORD DESC - step 2
    transl_curr = ssom_steptwo_procrustes(rot_curr, T_globalframe_nois, lambdas_curr, tijs, edges, params);
    % T = reshape(T, d, []);
