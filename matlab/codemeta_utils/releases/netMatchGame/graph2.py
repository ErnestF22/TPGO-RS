...
class Graph(object):
    ...
    def edges_indeces(self):
        """ Edges of the graph as list of lists of indeces, but WARNING: requires external addition of vnames list"""
        e_idx=[]
        v=self.vnames
        for e in self.edges():
            e=list(e)
            e_idx.append([v.index(ve) for ve in e])
        return e_idx

    def add_vertex_list(self,vertex_list):
        """ Calls add_vertex on each element of a list """
    ...
    def add_edge_from_vertex_list(self, vertices1=None, vertices2=None):
        """ Add all edges between two sets of nodes """
        if not vertices1:
    ...
    def incidence_matrix(self):
        """ Returns incidence graph """
        a=[]
        nb_vertices=len(self.vertices())
        print nb_vertices
        for e in self.edges_indeces():
            l=np.zeros((nb_vertices,))
            l[e[0]]=1
            l[e[1]]=1
            a.append(l)
        return np.array(a).T
