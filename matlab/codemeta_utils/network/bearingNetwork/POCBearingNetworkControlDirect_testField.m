function POCBearingNetworkControlDirect_testField
...
funsBearingsControl=consensus_rot3_almostGlobal_functions('type','huber','b',0.1);
funsBearingsCost=bearingCostFunctions('angleSq');
funsRanges=bearingCostFunctions('squared');
%funsRanges=consensus_rot3_almostGlobal_functions('type','huber','b',0.01);
...
for flagUseRanges=true%[false true]
    baseFileName=fullfile(figDir, [mfilename '_']);
    if ~flagUseRanges
        disp('# Bearings only')
        dx=@(t,x) control(x,EBearings,ygBearings,funsBearingsControl);
        cEval=@(x) cost(x,EBearings,ygBearings,funsBearingsCost);
        baseFileName=[baseFileName 'B_'];
    else
        disp('# Bearings + ranges')
        dx=@(t,x) control(x,EBearings,ygBearings,funsBearingsControl,ERanges,ygRanges,nygRanges,funsRanges);
        cEval=@(x) cost(x,EBearings,ygBearings,funsBearingsCost,ERanges,ygRanges,nygRanges,funsRanges);
...
if ~flagUseRanges
    dx=bearingNetworkControlDirect(EBearings,yBearings,ygBearings,funsBearings,'methodBearing','field');
else
    ...
    dx=bearingNetworkControlDirect(EBearings,yBearings,ygBearings,funsBearings,...
        'ranges',ERanges,yRanges,ygRanges,nyRanges,nygRanges,funsRanges,'alpha',alpha,'methodBearing','field');
end
dx=dx(:);
