function essential_evaluateEpipolarConstraint_test
...
[e,De]=essential_evaluateEpipolarConstraint(Qt(t),x(:,:,1),x(:,:,2));
e=sum(e);
de=sum(De'*vVec);

function [de,dde]=derAndDder(Qt,vVec,x,t)
[~,De,He]=essential_evaluateEpipolarConstraint(Qt(t),x(:,:,1),x(:,:,2));
de=sum(De'*vVec);
Nx=size(He,3);
dde=0;
for ix=1:Nx
