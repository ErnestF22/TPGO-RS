...
for iseq=1:8
    ...
    switch s
        ...
        case '+-+'
            S1=@(theta1) Rz(theta1)*Rxpi;
            S2=@(theta2) Rz(theta2);
            S1Str='S1Solve=[];';
            S2Str='S2Solve=[];';
        case '++-'
            S1=@(theta1) Rz(theta1);
            S2=@(theta2) Rz(theta2)*Rxpi;
            S1Str='S1Solve=[];';
            S2Str='S2Solve=[];';
        case '+--'
            S1=@(theta1) Rz(theta1)*Rxpi;
            S2=@(theta2) Rz(theta2)*Rxpi;
            S1Str='S1Solve=Rz(theta)*Rxpi;';
            S2Str='S2Solve=Rz(theta)*Rxpi*Rzpi;';
        case '--+'
            S1=@(theta1) Rz(theta1)*Rxpi;
            S2=@(theta2) Rz(theta2);
            S1Str='S1Solve=[];';
        ...
        case '-++'
            ...
            S2=@(theta2) Rz(theta2);
            S1Str='S1Solve=Rz(theta);';
            S2Str='S2Solve=Rz(theta)*Rzpi;';
        case '---'
            S1=@(theta1) Rz(theta1)*Rxpi;
            S2=@(theta2) Rz(theta2)*Rxpi;
            S1Str='S1Solve=Rz(theta)*Rxpi;';
            S2Str='S2Solve=Rz(theta)*Rxpi;';
    end
