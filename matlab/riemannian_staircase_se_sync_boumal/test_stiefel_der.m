...
% manopt_data.cost = mycost_problem;
% manopt_data.egrad = myegrad_problem;
% manopt_data.grad = @(x) manopt_data.M.proj(x, myeuclgradient(x, problem_data, measurements, som_params));
...
% [stief_t,vt] = stiefel_rand_geod_fun_new(matStack(Y0));
% stief_0 = stief_t(0); % = Y0
% 
% 
% [Rnt,Rnvt,RnR0,Rnv0,RnvVec] = rot_randGeodFun(eye(num_rows_stiefel*N));
% [Rpt,Rpvt,RpR0,Rpv0,RpvVec] = rot_randGeodFun(eye(d));
% 
% stieft = @(t) Rnt(t) * stief_0 * Rpt(t);
% dstieft = @(t) Rnvt(t) * stief_0 * Rpt(t) + Rnt(t) * stief_0 * Rpvt(t);
% 
% stieft_orth = @(t) trace(stieft(t)' * stieft(t));
% dstieft_orth = @(t) 2*trace(dstieft(t)' * stieft(t));
% 
% figure("Name", "Yplus");
% funCheckDer(stieft, dstieft);
% 
% figure("Name", "Yplus_orth");
