function test_noisy_recovery_flip
...
if staircase_step_idx > d+1
    ...
    if ~any(nodes_low_deg)
        ...
        %% changing sign inside RbRecovery() for reflected HD node
        switching_node_id = 3;
        [Tij1j2, Tij1j2_tilde] = ...
            make_Tij1j2s_edges(switching_node_id, T_edges, Tijs_scaled, edges, problem_data);
        % Tij_2deg_recovery = cat(3, Tij_2deg_recovery, Tij1j2);
        % Tij_2deg_recovery = [];
        Tij_tilde_3deg_recovery = [];
        Tij_tilde_3deg_recovery = cat(switching_node_id, Tij_tilde_3deg_recovery, Tij1j2_tilde);
        RtildeEst = RbRecovery(multiprod(Qalign, R_manopt_out(:,:,3)), Tij_tilde_3deg_recovery, false);
...
disp(ssom_cost_plot(X_recovered, problem_data))
