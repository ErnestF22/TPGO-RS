function Hmat = make_ehess_Hmat_ssom_metric(X, problem_struct)
...
for ii = 1:vecsz
    for jj = 1:vecsz
        ...
        tmp_ij = d_i' * vectorizeXrtlambdas(ssom_ehess_genproc(X, U_d_j, problem_struct));

        tmp_ji = d_j' * vectorizeXrtlambdas(ssom_ehess_genproc(X, U_d_i, problem_struct));
        disp("[tmp_ij, tmp_ji]")
        disp([tmp_ij, tmp_ji])
