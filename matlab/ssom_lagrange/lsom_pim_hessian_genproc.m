function [Y0, lambda_pim_out, v_pim_out, eigenvalue_check_ok] = lsom_pim_hessian_genproc( ...
    X, problem_struct_next, thresh, num_max_iter, second_iter)
%lsom_PIM_HESSIAN_GENPROC Return a new starting point Y0 with lower cost that R
...
if problem_struct_next.relu_scale_compensation
    rhess_fun_han = @(u) lsom_rhess_genproc_relu(Xnext,u,problem_struct_next);
else    
    rhess_fun_han = @(u) lsom_rhess_genproc(Xnext,u,problem_struct_next);
end
...
% save("../matlab/data/lsom_testdata_noisy/harder/tdata_n5_mindeg2_sigma00/uStartVec.csv", "uStartVec", "-ascii");
[lambda_pim, v_pim] = lsom_pim_function_genproc(rhess_fun_han, u_start, stiefel_normalize_han, thresh, num_max_iter);
disp('Difference between lambda*v_max and H(v_max) should be in the order of the tolerance:')
eigenvalue_check_ok = lsom_eigencheck_hessian_genproc(lambda_pim, v_pim, rhess_fun_han);
...
%     lambda_pim = max(lambda_pim.R, lambda_pim.T);
    ...
    if problem_struct_next.relu_scale_compensation
        rhess_shifted_fun_han = ...
            @(u) lsom_rhess_genproc_shifted_relu(Xnext,u,mu,problem_struct_next);
    else    
        rhess_shifted_fun_han = ...
            @(u) lsom_rhess_genproc_shifted(Xnext,u,mu,problem_struct_next);
    end
    ...
    % save("../matlab/data/lsom_testdata_noisy/harder/tdata_n5_mindeg2_sigma00/uStartSecondIterVec.csv", "uStartSecondIterVec", "-ascii");
    [lambda_pim_after_shift, v_pim_after_shift] = lsom_pim_function_genproc( ...
    ...
    eigenvalue_check_ok = lsom_eigencheck_hessian_genproc(lambda_pim_after_shift, v_pim_after_shift, ...
        rhess_shifted_fun_han);
    ...
    eigenvalue_check_ok = lsom_eigencheck_hessian_genproc(highest_norm_eigenval, v_pim_after_shift, ...
        rhess_fun_han);
    highest_norm_eigenval = lambda_pim_after_shift + mu;
else
...
step2.cost = @(x) lsom_cost(x, problem_struct_next);
step2.grad = @(x) lsom_rgrad(x, problem_struct_next);
step2.hess = @(x, u) lsom_rhess_genproc(x, u, problem_struct_next);
...
%     disp([matStack(x), matStack(x_retr_ii)])
    ...
    %grad
    lsom_rg = lsom_rgrad(Xnext, problem_struct_next);
    rgt_R = alphas(ii)* ...
        sum(stiefel_metric( ...
    ...
    egt_lambdas = alphas(ii)* ...
        ...
        Xnext.lambda,v_pim_after_shift.lambda, ...
            lsom_rg.lambda,'euclidean'));


    lsom_rhess_var = lsom_rhess_genproc(Xnext, v_pim_after_shift, problem_struct_next);
    pvt_R = alphas(ii)^2/2* ...
        sum(stiefel_metric( ...
        Rnext,v_pim_after_shift.R, ...
            lsom_rhess_var.R ,'euclidean'));
    pvt_T = alphas(ii)^2/2* ...
        sum(stiefel_metric( ...
        Tnext,v_pim_after_shift.T, ...
            lsom_rhess_var.T,'euclidean'));
    pvt_lambdas = alphas(ii)^2/2* ...
        sum(stiefel_metric( ...
        Xnext.lambda,v_pim_after_shift.lambda, ...
            lsom_rhess_var.lambda,'euclidean'));
...
[~, Y0] = linesearch_decrease(step2, ...
    Xnext, v_pim_after_shift, lsom_cost(Xnext,problem_struct_next), 0, options);
...
%     [~, Y0] = linesearch_decrease(step2, ...
%         Xnext, v_ex, lsom_cost(Xnext,problem_struct_next), 0, options);
...
if is_equal_floats(vectorizeXrtlambdas(Y0), vectorizeXrtlambdas(Xnext)) && highest_norm_eigenval < 0 && ~second_iter
    second_iter = true; 
    [Y0, highest_norm_eigenval, v_pim_after_shift, eigenvalue_check_ok] = lsom_pim_hessian_genproc( ...
        X, problem_struct_next, thresh, 20000, second_iter);
end

% cost_before_ls = lsom_cost(Xnext,problem_struct_next);
% Rnext = Xnext.R(:);
% Tnext = Xnext.T(:);
...
% Y0T = Y0.T(:);
% Y0lambda = Y0.lambda(:);
% cost_after_ls = lsom_cost(Y0,problem_struct_next);
% 
