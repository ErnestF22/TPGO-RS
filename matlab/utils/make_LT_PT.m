function [L_T, P_T] = make_LT_PT(T_globalframe_nois, Tijs_vec, edges, params)
...
for edge_id = 1:num_edges
    ii = edges(edge_id, 1);
    jj = edges(edge_id, 2);
    Ti=T_globalframe_nois(:,ii);
    Tj=T_globalframe_nois(:,jj);
...
%         T_globalframe_nois(:,jj) * T_globalframe_nois(:,jj)';
    L_T(ii*d-(d-1):ii*d, ii*d-(d-1):ii*d) = L_T(ii*d-(d-1):ii*d, ii*d-(d-1):ii*d) + Lij;
