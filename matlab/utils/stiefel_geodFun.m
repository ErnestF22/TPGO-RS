...
function [Rt,dRt,R0,dR0,vVec,ddRt,dvVec]=stiefel_geodFun(R0,dR0,varargin)
s=1;
NGeodesics=size(dR0,3);

if ~exist('R0','var') || isempty(R0)
    R0=stiefel_randn(cat_zero_row(eye(3)));
end
if ~exist('dR0','var') || isempty(dR0)
    dR0=stiefel_randTangentNormVector(R0);
end
...
Rt=@(t) stiefel_exp(R0,multiprod(s(t),dR0));
