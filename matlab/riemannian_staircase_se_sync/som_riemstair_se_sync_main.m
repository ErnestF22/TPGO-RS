...
[rot_out, rot_out_stief, ...
    transl_out, transl_out_stiefel, final_cost, last_num_rows_stiefel] = som_riemstair_se_sync(measurements, Y0, transl_initguess, som_params);
...
if (just_remove_zeros)
    ...
    Ropt = zeros(d,d,N);
    for ii = 1:N
        Ropt(:,:,ii) = rot_out_stief(1:d, 1:d, ii);
    end
    transl_stiefel_resh = reshape(transl_out_stiefel, [], N);
    transl_out = transl_stiefel_resh(1:d, :);
%     transf_out = RT2G(R_out, transl_out);
else
    [Ropt, transl_out] = lowRankLocalization_solution_extractProjection( ...
        matStack(multitransp(rot_out_stief)) * reshape(transl_out_stiefel, [], N));
