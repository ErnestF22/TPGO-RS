function [Ropt, Yopt, Fval, r] = som_riemstair_se_sync_rotonly(measurements, Y0, som_params)
...
%         [stepsize, Y0T] = linesearch_decrease(manopt_data, Yplus', alpha * Ydot', SDPLRval);
        manopt_data.cost = @(x) mycost(x, problem_data, measurements, som_params);
        manopt_data.egrad = @(x) mycost(x, problem_data, measurements, som_params);
...
just_remove_zeros = lastNRowsAllZeros(Y0, som_params.d, som_params.N);
...
%     transf_out = RT2G(R_out, T_out);
    ...
    %As this script is only about R, T_stiefel is initialized with gt directly here
    T_stiefel = cat_zero_row(G2T(testdata.gitruth), r-som_params.d);
    [Ropt, ~] = lowRankLocalization_solution_extractProjection(matStack(multitransp(rot_out_stief)) * reshape(T_stiefel, [], som_params.N));
%     transf_out = RT2G(R_out, T_out);
end
