...
T_globalframe_stiefel = G2T(testdata.gitruth);
...
%SoM/testdata fields
measurements.T_globalframe_stiefel = T_globalframe_stiefel;
measurements.Tijs_vec = Tijs_vec;
...
transl_initguess = T_globalframe_stiefel + sigma.*randn(size(T_globalframe_stiefel));
...
%     Yplus = vertcat(matStackH(Yopt), zeros(1, problem_data.d * problem_data.n));
    ...
    if( lambda_min < -SE_Sync_opts.min_eig_num_tol)
        ...
        v_stiefel = zeros((rNext)*N, 1);
        idx_stief = reshape(1:(rNext)*N, rNext, N);
        idx = reshape(1:problem_data.d*N, problem_data.d, N);
        for ii = 1:N
...
%         [stepsize, Y0T] = linesearch_decrease(manopt_data, Yplus', alpha * Ydot', SDPLRval);
        manopt_data.cost = @(x) mycost(x, problem_data, measurements, som_params);
        manopt_data.egrad = @(x) myeuclgradient(x, problem_data, measurements, som_params);
        [stepsize, Y0T] = linesearch_decrease(manopt_data, matStack(Yplus), alpha * Ydot, SDPLRval);
        line_search_time = toc();
%         Y0 = Y0T';
        Y0 = matUnstack(Y0T, rNext);
