...
Hmat_ssom = make_ehess_Hmat_ssom_metric(X_cat, problem_data_next);
% disp('Hmat_ssom')
% disp(Hmat_ssom)
...
% lhs = d1' * vectorizeXrtlambdas(ssom_ehess_genproc(X, d2_struct, problem_data_next));
% rhs = d2' * vectorizeXrtlambdas(ssom_ehess_genproc(X, d1_struct, problem_data_next));
% 
% 
...
[Y0_pim, lambda_pim, v_pim] = ssom_pim_ehess_genproc(X, problem_data_next);
...
for ii = 1:vecsz
    ...
    Hgp_e_i = ssom_ehess_genproc(X, U_e_i, problem_struct);
