function test_check_ssom_test_ehess_R_R()
...
gradf=@(t) vec(problem.rgrad_R(curve.c(t), T0, lambda0));
% df=@(t) sum(stiefel_metric([],gradf(t),curve.dc(t)));
% funCheckDer(f,df)
hessf = @(t) vec(problem.ssom_rhess_R_R(curve.c(t), curve.dc(t), T0, lambda0));
