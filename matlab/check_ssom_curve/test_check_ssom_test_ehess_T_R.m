function test_check_ssom_test_ehess_T_R()
...
gradf=@(t) vec(problem.rgrad_T(curve.c(t), T0, lambda0));
% df=@(t) sum(stiefel_metric([],gradf(t),curve.dc(t)));
% funCheckDer(f,df)
hessf = @(t) vec(problem.ssom_ehess_T_R(curve.c(t), curve.dc(t), T0, lambda0));
