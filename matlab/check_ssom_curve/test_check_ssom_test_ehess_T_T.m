function test_check_ssom_test_ehess_T_T()
...
gradf=@(t) vec(problem.rgrad_T(R0, curve.c(t), lambda0));
% df=@(t) sum(stiefel_metric([],gradf(t),curve.dc(t)));
% funCheckDer(f,df)
ehessf = @(t) vec(problem.ssom_ehess_T_T(R0, curve.c(t), curve.dc(t), lambda0));
