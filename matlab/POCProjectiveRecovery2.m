...
Tij=[Tij;zeros(dimStair-d,low_deg)];
Ri=[Ri;zeros(dimStair-d,d)];
...
Rb=rot_randn(eye(dimStair-low_deg));
...
Rbrand=rot_randn(eye(dimStair - low_deg));
disp(norm(Qx'*blkdiag(eye(low_deg),Rbrand)*Qx*Ri_tilde2*tij*Lij-Ri_tilde2*tij*Lij,'fro'))
...
disp(norm(A1(end-1:end,:)-A2,'fro'))
disp('Which is the same as Qbot_right*Rb*Rcal_bot, because Qbot_left is zero')
A1_nonzero = Qbot_right*Rbrand'*Rcal_bot;
disp(norm(A1_nonzero(end-1:end,:)-A2,'fro'))
...
tmp = Qbot_right(1:2,:);
Rb_est=procrustes_R(tmp',Rcal_bot_N);
...
Ri_est1=Qx'*blkdiag(eye(2),Rb_est')*Qx*Ri_tilde2;
disp(Ri_est1)

disp('Ri_est1 should fit the measurements')
disp(Ri_est1*tij*Lij-Tij_tilde)

disp('Check that output of RbRecovery matches calculations above')
Ri_est2=RbRecovery(Ri_tilde2,Tij_tilde);
disp(norm(Ri_est1-Ri_est2,'fro'))
