function [A,b,R] = make_A_b(R, T_globalframe_nois, Tijs_vec, edges, params)
...
%     b = Tijs_vec(:);    
    for edge_id = 1:num_edges
        ...
        A(idxEdges(:,edge_id),idxNodes(:,ii)) = R(:,:,ii)';
        A(idxEdges(:,edge_id),idxNodes(:,jj)) = -R(:,:,ii)';
...
%     b = Tijs_vec(:);
    for edge_id = 1:num_edges
        ...
        A(idxEdges(:,edge_id),idxNodes(:,ii)) = R(ii*d-(d-1):ii*d, :)';
        A(idxEdges(:,edge_id),idxNodes(:,jj)) = -R(ii*d-(d-1):ii*d, :)';
        b(idxEdges(:,edge_id)) = Tijs_vec(:, edge_id);
    end
    for ii = 1:N
        R2(:,:,ii) = R(ii*d-(d-1):ii*d, :); %covert R to "3D mode"
    end
    R = R2;
end
