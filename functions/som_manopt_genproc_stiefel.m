function transf_out = som_manopt_genproc_stiefel(T_globalframe, Tijs_vec, edges, num_rows, params, transf_init_guess)
%SOM_MANOPT_GENPROC_STIEFEL Computes transformation through Procrustes 
%pipeline in only one step (rotation and translation are not separated).
...
%tested initially) on Stiefel manifold
...
% should be in.
tuple.R = stiefelfactory(num_rows, d, N);
...
function [g, store] = grad(X, store)
    ...
    %Note: variable name A is already used for the translations!
    num_rows_stiefel = d;
    [Amat,b] = make_A_b_noloops_stiefel(R, T_globalframe, Tijs_vec, edges, num_rows_stiefel, params);
