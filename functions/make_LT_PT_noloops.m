function [L_T_noloops, P_T_noloops] = make_LT_PT_noloops(T_globalframe_nois, Tijs_vec, edges, params)
...
%L_1
T_globalframe_exp_one = reshape(T_globalframe_exp_orig_weighted, d, d, N, N);
L_one = sum(T_globalframe_exp_one, 3);
% L_one_blkdiag =
% blkdiag(L_one(:,1:3),L_one(:,4:6),L_one(:,7:9),L_one(:,10:12),L_one(:,13:15))
% %works only with N=5
L_one_cells = mat2cell(matStack(squeeze(L_one)), ones(N,1)*d, d);
...
L_two_cells = mat2cell(matStack(L_two_unstacked), ones(N,1)*d, d);
...
L_3_to_be_summed = reshape(T_globalframe_exp_three, d, d, N, N);
L_3_summed = sum(L_3_to_be_summed, 3);
L_three_cells = mat2cell(matStack(squeeze(L_3_summed)), ones(N,1)*d, d);
L_three_blkdiag = blkdiag(L_three_cells{:});


%L_4
T_globalframe_exp_four = reshape(T_globalframe_exp_orig_weighted, d, d, N, N);
L_four = sum(T_globalframe_exp_four, 4);
L_four_cells = mat2cell(matStack(squeeze(L_four)), ones(N,1)*d, d);
...
P_T_noloops_weighted_exp_4d = reshape(P_T_noloops_weighted_exp, d, d, N, N);
