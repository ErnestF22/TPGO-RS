...
%   R(:,:,iN), iN=1,...,N
% where all Rs are on the Stiefel manifold where A is.
%Warning: The function does not check if A is valid point on a Stiefel manifold
%
...
function [Rt,dRt,R0,dR0,vVec,ddRt,dvVec]=stiefel_randGeodFun(A,varargin)
...
if isempty(perpTangent)
    v=stiefel_randTangentNormVector(R);
else
    error("Else");
...
s=permute(s,[1 3 2]);
[Rt,dRt,R0,dR0,vVec,ddRt,dvVec]=stiefel_geodFun(R,v,'speed',s);
