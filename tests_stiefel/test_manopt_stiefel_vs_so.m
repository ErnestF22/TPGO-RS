...
% 3) run Manopt and then manopt_stiefel
...
transf_manopt = som_manopt(T_globalframe, Tijs_vec, edges, som_params, transf_initguess);
% manopt_end_time = tic();
manopt_exec_time = toc(manopt_start_time);

% 3b) execute with step 1 through manopt_stiefel
manopt_stiefel_start_time = tic();
num_rows_stiefel = d;
transf_manopt_stiefel = som_manopt_stiefel(T_globalframe, Tijs_vec, edges, num_rows_stiefel, som_params, transf_initguess);
% manopt_stiefel_end_time = tic();
manopt_stiefel_exec_time = toc(manopt_stiefel_start_time);
...
testdata.gi = transf_manopt_stiefel;
[rotation_error_manopt_stiefel,translation_error_manopt_stiefel] = testNetworkComputeErrors(testdata)
...
fprintf("manopt_stiefel exec time %g [s]\n", (manopt_stiefel_exec_time));

% plot (sigma, manopt_exec_time, '.', 'markersize', 15)
% plot (sigma, manopt_stiefel_exec_time, '.', 'markersize', 15)
