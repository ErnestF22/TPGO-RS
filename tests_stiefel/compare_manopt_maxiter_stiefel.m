...
for iter = 1:size(maxiters, 1) 
    som_params.max_icp_iterations = maxiters(iter);
    ...
    for ii = 1:size(sigmas,1)
        ...
        for jj = 1:num_tests_per_sigma
            [manopt_rot_err, manopt_transl_err, manopt_exec_time] = ...
                do_som_manopt_maxiter(testdata, sigma, mu, som_params); % do_som();
    ...
    results(iter) = struct("manopt_rot_errs", manopt_rot_errs, ...
        "manopt_transl_errs", manopt_transl_errs, ...
        "manopt_exec_times", manopt_exec_times, ...
        "max_iterations", maxiters(iter));
end


%plot results
plot_results(sigmas, results, "manopt_icp_maxiter");
