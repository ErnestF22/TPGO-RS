function [kd, kv, beta, m, gridData] = par_rotBundle_gainGridSearch(Dist,kd_list,kv_list, magW)
...
% Grid Search
for ii = 1:length(kd_list)
    kd_test = kd_list(ii);
    parfor jj = 1:length(kv_list)
        kv_test = kv_list(jj);
%         fprintf('Checking kd=%0.3f, kv=%0.3f \n',kd_test,kv_test);
        ...
        betaData(ii,jj) = beta_test;
        mData(ii,jj,:) = m_test(:);
        gainData(ii,jj,:) = [kd_test;kv_test];
    end
end
