function [A_all_func_mdot] = rotRefOpt_allBounds_Matrix_mdot()
% Compute the relaxed gershgorin disc bounds derivative wrt the m_i 
% as real functions using trace(Ai*X)<=0 where X = x*x' 
% and x = [1;m1;m2;m3;m5;m6] as function handles  
% of (R,w,RRef,gains,M_nn,M_nn_dot,beta)
% NOTE: To do this we relaxed abs(a+bi) <= abs(a) + abs(b)
% ASSUME: m4 is constant, i.e. m4_dot = 0
% INPUTS:
% OUTPUTS:
%   A_all_func_mdot(R,w,RRef,gains,M_nn,M_nn_dot,beta) := a cell array of all possible 
...
syms m1_dot m2_dot m3_dot m4_dot m5_dot m6_dot m5m6 m5squared m6squared real;
%% Init matrices ( same as rotRefOpt_allBounds_Matrix() )
...
A_11(1,3) = -kd*mag_R/2*cot(mag_R/2); A_11 = (A_11 + A_11')/2;
A1_mat = {A_11};
A_22(1,3) = 1; A_22(1,4) = (b-kv); A_22 = (A_22 + A_22')/2;
A2_mat = {A_22};
A_33(1,1) = 0; % Removed since m4_dot := 0
A_33 = (A_33 + A_33')/2;
A3_mat = {A_33};
...
%
tempMat1 = create_empty_mat(flagSymbolic,6);
tempMat1(1,6) = sqrt(3)*(-kref/2+b); tempMat1 = (tempMat1 + tempMat1')/2;
...
%
tempMat1 = create_empty_mat(flagSymbolic,6);
tempMat1(1,5) = (-kref/2 + b); tempMat1 = (tempMat1 + tempMat1')/2;
...
%% Results
% Once we computed the sym functions, we compute the derivatives here
...
for ii = 1:length(A_all)
    % These are the standard bounds
    temp_sym = trace(A_all{ii}*X); 
    % Sub sym function with derivative terms
    temp_sym = subs(temp_sym,m5*m6,m5m6); % placeholder (m5*m6)
    temp_sym = subs(temp_sym,m5^2,m5squared); % placeholder (m5^2)
    temp_sym = subs(temp_sym,m6^2,m6squared); % placeholder (m6^2)
    temp_sym = subs(temp_sym,m1,m1_dot); % d/dt m1
    temp_sym = subs(temp_sym,m2,m2_dot); % d/dt m2
    temp_sym = subs(temp_sym,m3,m3_dot); % d/dt m3
    temp_sym = subs(temp_sym,m5,m5_dot); % d/dt m5
    temp_sym = subs(temp_sym,m6,m6_dot); % d/dt m6
    temp_sym = subs(temp_sym,m5m6,(m5_dot*m6 +m5*m6_dot)); % d\dt (m5*m6)
    temp_sym = subs(temp_sym,m5squared,2*m5*m5_dot); % d\dt (m5^2)
    temp_sym = subs(temp_sym,m6squared,2*m6*m6_dot); % d\dt (m6^2)
...
temp_func = matlabFunction(tempBounds,'Vars',...
    ...
    b]);
% For convenience, remap as function of (R,w,RRef,gains,M_nn,M_nn_dot,beta)
