function [x,t,varargout] = rotRefOptAll_gainSched_simulation(mag_R,mag_RRef,mag_W,M_nonnatural,varargin)
% LAST EDITED: Jan. 21, 2021 by Bee Vang
% Simulate the dynamics on TSO(3)xSO(3) where the gains, metric, and
% convergence rates are changed according to a schedule
% INPUTS:
...
%% Simulate
figure
% optsOde=odeset('OutputFcn',@odeplot,'MaxStep',0.01, 'Events', @stopEvent);
...
% stop simulation when R is within some distance of eye(3)
[R,w,RRef,kd,kv,kref,M_nonnatural,~] = rotDynOptAll_stateUnpack(x);
dist_R_I = norm(rot_log(R,eye(3)));
value = ( dist_R_I <= pi/4 );
