function [A_all_func] = rotRefOpt_allBounds_Matrix_simple()
% Last Edited: Jan 6, 2021 by Bee Vang
% Compute the relaxed gershgorin disc bounds as real functions
% handle  of (R,w,RRef,gains,M_nn,beta). Also generate a function
% rotRefOpt_gen_allBounds.m for use in cvx optimization.
% NOTE: To do this we relaxed abs(a+bi) <= abs(a) + abs(b)
% INPUTS:
% OUTPUTS:
%   A_all_func(R,w,RRef,gains,M_nn,beta) := a matrix handle of all possible 
...
% Fill entries
tempMat1 = m1*b;
tempMat2 = m1*sqrt(3)*b + m2*sqrt(3)*(-1/4*mag_W^2)...
    + m5*m6*sqrt(3)*(mag_W^2/(4*m4));
...
%
tempMat1 = m3*sqrt(3)*(-1/8*mag_W^2) + m5^2*sqrt(3)*(mag_W^2/(8*m4));
tempMat2 = m2*sqrt(3)*(-mag_W/4) + m5*m6*sqrt(3)*(mag_W/(4*m4)) ...
    + m3*sqrt(3)*(kv*mag_W/4) + m5^2*sqrt(3)*(-mag_W*kv/(4*m4));
...
%
tempMat1 = m6^2*sqrt(3)*mag_W/(4*m4) + m5*m6*sqrt(3)*(-mag_W*kv/(4*m4));
...
tempMat2 = m3*sqrt(3)*kd/2*mag_R/2*cot(mag_R/2) + m6*sqrt(3)*1/2 ...
    + m5*sqrt(3)*(-kv/2);
tempMat3 = m3*sqrt(3)*(kd/2*mag_R/2) + m5^2*sqrt(3)*(-kd/(4*m4)*mag_R);
...
%     -(-m5*kref/2+b*m5)};
%
tempMat1 = m5*m6*sqrt(3)*mag_W/(4*m4) + m5^2*sqrt(3)*(-kv*mag_W/(4*m4));
...
temp_func = matlabFunction(tempBounds,'Vars',[mag_R,mag_RRef,kd,kv,kref,mag_W,m1,m2,m3,m4,m5,m6,b],'File','rotRefOpt_gen_allBounds');
% For convenience, remap as function of (R,w,RRef,gains,M_nn,beta)
A_all_func = @(R,w,RRef,gains,M_nn,beta) temp_func( norm(rot_log(R,RRef)),... % mag_R
