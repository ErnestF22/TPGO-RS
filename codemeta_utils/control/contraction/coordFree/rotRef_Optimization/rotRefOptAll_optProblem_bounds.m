function [dparams, delta] = rotRefOptAll_optProblem_bounds(x,varargin)
...
while ivarargin<=len_varargin
    switch lower(varargin{ivarargin})
        case 'bounds_func'
            % Requires pre-determined function handles to compute the
            % contraction bounds (these should be a cell-array and all
            % bounds_xxx_func should be the same length)
            ivarargin=ivarargin+1;
            bounds_func = varargin{ivarargin};
        case 'bounds_func_kdot'
            % Requires pre-determined function handles to compute the
            % contraction bound derivatives wrt the gains
            % (these should be a cell-array and all
            % bounds_xxx_func should be the same length)
            ivarargin=ivarargin+1;
            bounds_func_kdot = varargin{ivarargin};
        case 'bounds_func_mdot'
            % Requires pre-determined function handles to compute the
            % contraction bound derivatives wrt the metrics
            % (these should be a cell-array and all
            % bounds_xxx_func should be the same length)
            ivarargin=ivarargin+1;
            bounds_func_mdot = varargin{ivarargin};
        case 'bounds_func_bdot'
            % Requires pre-determined function handles to compute the
            % contraction bound derivatives wrt the gains
            % (these should be a cell-array and all
            % bounds_xxx_func should be the same length)
            ivarargin=ivarargin+1;
            bounds_func_bdot = varargin{ivarargin};
...
%         maximize ( b_dot - delta*1e-5 - 2*k_dot'*k_dot) % looking to increase convergence rate
        ...
        subject to
            ...
            % NOTES: This process takes about 1minute per complete
            % iteration, TOO LONG
            tic
            % d(bound)\d(R,w,RRef) + d(bound)\dk + d(bound)dm +
            % d(bound)\db <= 0 (a ZBF) where we use linear class K
            % function
%             funApproxDer(@(t) bounds_func(R_t(t),w_t(t),RRef_t(t),gains,M_nn,b),0) ... % d(bound)\d(R,w,RRef)
%                 + bounds_func_kdot(R,w,RRef,gains_dot,M_nn,b)...
%                 + bounds_func_mdot(R,w,RRef,gains,M_nn,Mnn_dot,b)...
%                 + bounds_func_bdot(R,w,RRef,gains,M_nn,b_dot)...
%                 + bounds_func(R,w,RRef,gains,M_nn,b) <= 0 % bound
% tic
            funApproxDer(@(t) bounds_func(R_t(t),w_t(t),RRef_t(t),gains,M_nn,b),0) ... % d(bound)\d(R,w,RRef)
                + rotRefOpt_allBounds_Matrix_der_simple_cvx(R,w,RRef,gains,M_nn,b,gains_dot,Mnn_dot,b_dot)...
                + bounds_func(R,w,RRef,gains,M_nn,b) <= 0 % bound
% toc
% tic
%             funApproxDer(@(t) bounds_func(R_t(t),w_t(t),RRef_t(t),gains,M_nn,b),0) ... % d(bound)\d(R,w,RRef)
%                 + bounds_func_kdot(R,w,RRef,gains,M_nn,b,gains_dot,Mnn_dot,b_dot)...
%                 + bounds_func(R,w,RRef,gains,M_nn,b) <= 0 % bound
% toc
...
%             end
    ...
    delta
    max(bounds_func(R,w,RRef,gains,M_nn,b))
end
