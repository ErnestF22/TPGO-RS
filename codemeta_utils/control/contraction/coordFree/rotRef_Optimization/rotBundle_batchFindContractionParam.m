% rotBundle_batchFindContractionParam.m
% Script to replace par_rotBundle_contractionTest with batch jobs so that
% multiple cvx sessions can run in parallel. The goal is to find
% parameters [kd,kv,beta] that guarantees local exp. convergence on TSO(3).
...
BETA_UB = 10;
MAX_GAIN = 110;
MAG_R = pi/4; % Max initial distance from R to eye(3)
MAG_W = 1; % Max intial speed
kd_list = [.1:5:110,110]; % Define all kd gains to test
kv_list = kd_list; % Define all kv gains to test
% Generate all possible gains
init_gains = allcomb(kd_list,kv_list)';

%% Create Batch jobs
% Create a batch job per initial gain combo from gains_list
% (determine by the number of columns of init_gains)
for ii = 1:size(init_gains,2)
    tempJob = batch(@rotBundle_gainSearch, 3, {MAG_R,...
        MAG_W,...
        'gainsearch_maxiter',MAX_GAIN_ITER,...
        'init_gains',init_gains(:,ii),...
        'beta_ub',BETA_UB,...
        'beta_lb',BETA_LB,...
        'maxgain',MAX_GAIN,...
        'savedir','rotBundle_pi_over_4'});
end

%% Process Results
% To process these results use the command below the parent folder of rotBundle_contractionResults
% results=rotRef_contractionTest_ProcessResults('folderpath','rotBundle_contractionResults');
