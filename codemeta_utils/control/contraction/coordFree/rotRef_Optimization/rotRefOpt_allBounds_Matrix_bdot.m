function [A_all_func_bdot] = rotRefOpt_allBounds_Matrix_bdot()
% Last Edited: Jan 6, 2021 by Bee Vang
% Compute the relaxed gershgorin disc bounds derivative wrt the beta 
% as real functions using trace(Ai*X)<=0 where X = x*x' 
% and x = [1;m1;m2;m3;m5;m6] as function handles  
% of (R,w,RRef,gains,M_nn,beta_dot)
% NOTE: To do this we relaxed abs(a+bi) <= abs(a) + abs(b)
% INPUTS:
% OUTPUTS:
%   A_all_func_bdot(R,w,RRef,gains,M_nn,beta_dot) := a cell array of all possible 
%       bounds on the contraction matrix eigenvalue on TSO(3)xSO(3)

%% Default Parameters
flagSymbolic = true;
syms kd kv kref b_dot m1 m2 m3 m4 m5 m6 mag_R mag_RRef mag_W real;
%% Init matrices
...
A_11(1,3) = 0; A_11 = (A_11 + A_11')/2;
A1_mat = {A_11};
A_22(1,3) = 0; A_22(1,4) = b_dot; A_22 = (A_22 + A_22')/2;
A2_mat = {A_22};
A_33(1,1) = m4*b_dot;
A_33 = (A_33 + A_33')/2;
A3_mat = {A_33};
...
% Fill entries
tempMat1(1,2) = 0;
tempMat1(1,3) = sqrt(3)*b_dot;
tempMat1(1,4) = 0;
tempMat1 = (tempMat1 + tempMat1')/2;
tempMat2(1,4) = 0;
tempMat2(1,2) = 0;
tempMat2(1,3) = sqrt(3)*b_dot;
tempMat2(5,5) = 0;
tempMat2 = (tempMat2 + tempMat2')/2;
tempMat3(1,4) = 0;
tempMat3(1,2) = 0;
tempMat3(1,3) = -sqrt(3)*b_dot;
tempMat3(5,5) = 0;
...
%
tempMat1 = create_empty_mat(flagSymbolic,6);
tempMat1(1,6) = sqrt(3)*b_dot; tempMat1 = (tempMat1 + tempMat1')/2;
...
tempMat1 = create_empty_mat(flagSymbolic,6);
tempMat1(6,6) = 0;
tempMat1(5,6) = 0; tempMat1 = (tempMat1+tempMat1')/2;
...
tempMat1(1,4) = 0;
tempMat1(1,6) = 0;
tempMat1(1,5) = 0; tempMat1 = (tempMat1 + tempMat1')/2;
tempMat2(1,4) = 0;
tempMat2(1,6) = 0;
tempMat2(1,5) = 0; tempMat2 = (tempMat2 + tempMat2')/2;
tempMat3(1,4) = 0;
tempMat3(5,5) = 0; tempMat3 = (tempMat3 + tempMat3')/2;
...
%
tempMat1 = create_empty_mat(flagSymbolic,6);
tempMat1(1,5) = b_dot; tempMat1 = (tempMat1 + tempMat1')/2;
...
tempMat1 = create_empty_mat(flagSymbolic,6);
tempMat1(5,6) = 0;
tempMat1(5,5) = 0; tempMat1 = (tempMat1 + tempMat1')/2;
...
temp_func = matlabFunction(tempBounds,'Vars',[mag_R,mag_RRef,kd,kv,kref,mag_W,m1,m2,m3,m4,m5,m6,b_dot]);
% For convenience, remap as function of (R,w,RRef,gains,M_nn,beta_dot)
A_all_func_bdot = @(R,w,RRef,gains,M_nn,beta_dot) temp_func( norm(rot_log(R,RRef)),... % mag_R
