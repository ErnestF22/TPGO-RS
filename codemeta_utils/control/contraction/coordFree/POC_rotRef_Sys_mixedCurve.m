...
U_R = @(R) R*hat3(t*uVec); % Redefine U as only a function of R
LT_R = @(R,RRef) [RRef zeros(3,6);zeros(3) R zeros(3);zeros(3,6) R]; % Convient matrix for Left translation

% Define curves based on velocities
U = @(R,RRef,t) R*hat3(t*uVec) + J(3,1)*R*RRef'*dRRef(RRef);
R = rot_geodFun(R0,dR(R0)+R0*RRef0'*dRRef(RRef0));

%% Compute the change of the nonnatural metric (ie d/dt<X,Y>)
R_t = R(t); RRef_t = RRef(t); U_t = U(R_t,RRef_t,t); % Evaluated at t
Xt = @(t) X(R(t),U(R(t),RRef(t),t),RRef(t));
Yt = @(t) Y(R(t),U(R(t),RRef(t),t),RRef(t));
Zt = @(t) Z(R(t),U(R(t),RRef(t),t),RRef(t));
...
%%%%%%%RESULTS INDICATE BELOW SHOULD BE 0
% Compute at (RRef,0)
...
% Chain rule
dJX_dt_RRef = funApproxDer(@(t) X_TSO3_RRef_all(R(t),U(R(t),RRef(t),t),RRef_t),t);
...
% Chain rule
dJY_dt_RRef = funApproxDer(@(t) Y_TSO3_RRef_all(R(t),U(R(t),RRef(t),t),RRef_t),t);
...
g_natural_TSO3 = @(t) rotBundle_metric_nonNatural([R(t);U(R(t),RRef(t),t)],...
    Xm_TSO3(R(t),U(R(t),RRef(t),t),RRef(t)), Ym_TSO3(R(t),U(R(t),RRef(t),t),RRef(t)),...
    m_natural_TSO3);
