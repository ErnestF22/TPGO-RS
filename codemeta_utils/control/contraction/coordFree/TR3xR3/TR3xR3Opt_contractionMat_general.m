function [M_contract] = TR3xR3Opt_contractionMat_general(hessf_R_Rd, hessf_Rd_0, M_nn)
...
% NOTE: This contraction matrix assumes \beta == 0, \beta is intended to be
%   solved for in the optimization problem!
% INPUTS:
...
M11 = @(A,B,C,kd,kv,kref) -2*m2*kd*hessf_R_Rd(A,C);
M21 = @(A,B,C,kd,kv,kref) -m3*kd*hessf_R_Rd(A,C)+(m1-m2*kv)*eye(3);
M22 = @(A,B,C,kd,kv,kref) (2*m2-2*m3*kv)*eye(3);
M31 = @(A,B,C,kd,kv,kref) -m5*kd*hessf_R_Rd(A,C)...
    +m2*kd*hessf_R_Rd(A,C)-m6*kref*hessf_Rd_0(C,zeros(3,1));
M32 = @(A,B,C,kd,kv,kref) m3*kd*hessf_R_Rd(A,C)...
    -m5*kref*hessf_Rd_0(C,zeros(3,1))+(m6-m5*kv)*eye(3);
M33 = @(A,B,C,kd,kv,kref) 2*m5*kd*hessf_R_Rd(A,C)-2*m4*kref*hessf_Rd_0(C,zeros(3,1));

M_contract = @(A,B,C,kd,kv,kref) [M11(A,B,C,kd,kv,kref) M21(A,B,C,kd,kv,kref)' M31(A,B,C,kd,kv,kref)'; ...
    M21(A,B,C,kd,kv,kref) M22(A,B,C,kd,kv,kref) M32(A,B,C,kd,kv,kref); ...
    M31(A,B,C,kd,kv,kref) M32(A,B,C,kd,kv,kref) M33(A,B,C,kd,kv,kref)];
end
