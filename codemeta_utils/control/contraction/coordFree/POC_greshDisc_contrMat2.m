...
%     plot(theta,sum(A,2),'x');
    ...
    % Find Omega_D bounds
%     norm_omega = norm(w_t(theta));
    omega = w_t(t_w(i));
    norm_omega = norm(omega);
    alpha = norm_omega*(m3*kv-m2)/4;
    O_D11 = m2/4*norm_omega^2*diag([0, -1, -1]);
    O_D21 = m3/8*norm_omega^2*diag([0, -1, -1]) + alpha*diag([0,1j,-1j]);
    O_D12 = conj(O_D21);
    O_D22 = 0*(m3*beta + m2 -m3*kv)*eye(3);
    O_D = [O_D11 O_D12; O_D21 O_D22];
    [B,maxODEval(i)] = gershgorinDisc(O_D);
    
    subplot(2,2,2);
    plot(norm_omega,sum(B,2),'rx');
    title('OD Matrix');
    hold on
    plot(norm_omega,-m2/4*norm_omega^2+m3/8*norm_omega^2+abs(m2-m3*kv)/4*norm_omega,'go')
    
    C = gershgorinDisc(O_D11);
    subplot(2,2,3);
    plot(norm_omega,sum(C,2),'rx');
    title('OD11');
    hold on
    
    D = gershgorinDisc(abs(O_D12));
    subplot(2,2,4);
    plot(norm_omega,sum(D,2),'rx');
    title('OD12');
    hold on
