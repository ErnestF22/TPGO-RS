...
import numpy as np
from dynamics_simulation_example3 import Dynamics_sim
import time
...
class STL_CBF_Planning:
    ...
    def STL_predicates(self):
        '''
        F[t1,t4] x1 <= -1 and x3 >= 1
        predicate 0: -x1 - 1 >= 0
        predicate 1: x3 - 0.5 > 0

        F[t6,t9] x1 >= 1 and x3 >= 1
        predicate 2: x1 - 1 > 0
        predicate 3: x3 - 1 > 0
        '''
        # Predicate Encoding
        ...
        for t_step in range(0,self.N):
            self.model.addConstr(self.mu[t_step,0] == (-self.X[t_step,0] - 0.5),"Eventually_Predicate_0")
            self.model.addConstr(self.mu[t_step,1] == (self.X[t_step,2] - 0.5),"Eventually_Predicate_1")
            self.model.addConstr(self.mu[t_step,2] == (self.X[t_step,0] - 1),"Eventually_Predicate_2")
            self.model.addConstr(self.mu[t_step,3] == (self.X[t_step,2] - 1),"Eventually_Predicate_3")
            self.model.addConstr(self.mu[t_step,4] == (self.X[t_step,0]),"Always_Predicate_4")
            self.model.addConstr(self.mu[t_step,5] == (self.X[t_step,2]),"Always_Predicate_5")

        # Integer Encoding
        self.model.addConstr(self.z_STL <= quicksum(self.Z[t_step,0] for t_step in [1,2,3,4,5,6]),"Eventually_Integer_Constraint")
        self.model.addConstr(self.z_STL <= quicksum(self.Z[t_step,1] for t_step in [1,2,3,4,5,6]),"Eventually_Integer_Constraint")

        self.model.addConstr(self.z_STL <= quicksum(self.Z[t_step,2] for t_step in [7,8,9]),"Eventually_Integer_Constraint")
        self.model.addConstr(self.z_STL <= quicksum(self.Z[t_step,3] for t_step in [7,8,9]),"Eventually_Integer_Constraint")

        # Enforcing Discrete Time Safety using traditional robustness encoding, Comment out when CBF constraints are enabled
        #for t_step in range(0,self.N):
        #    self.model.addConstr(self.z_STL <= self.Z[t_step,4]+self.Z[t_step,5])
        #    self.model.addConstr(self.z_STL >= self.Z[t_step,4])
        #    self.model.addConstr(self.z_STL >= self.Z[t_step,5])


        for t_step in [1,2,3,4,5,6]:
    ...
    def CBF_Constraints(self):
        '''
        G_[t_0,t_f] x1 > 0 or G_[t_0,t_f] x3 > 0
        '''
        ...
        for t_step in range(0,self.N-1):
            # x1(t) >= 0
            self.model.addConstr((self.z_h1[t_step] == 1) >> (self.k_cbf[0]*self.X[t_step,1]+self.k_cbf[1]*self.X[t_step,1]*self.tau_list[t_step]+ self.k_cbf[1]*self.X[t_step,0]+self.beta_X_h1[t_step] >= 0) ,"CBF_x_constr_step"+str(t_step))
            ...
            # x3(t) >= 0
            self.model.addConstr((self.z_h3[t_step] == 1) >> (self.k_cbf[0]*self.X[t_step,3]+self.k_cbf[1]*self.X[t_step,3]*self.tau_list[t_step]+ self.k_cbf[1]*self.X[t_step,2]+self.beta_X_h3[t_step] >= 0) ,"CBF_x_constr_step"+str(t_step))
        ...
        for t_step in range(0,self.N-1):
            ...
            self.model.addConstr((self.k_cbf[0]*self.U[t_step,1]) <= self.M * (1-self.z_h3[t_step]))
            self.model.addConstr(-(self.k_cbf[0]*self.U[t_step,1]) <= self.M * self.z_h3[t_step])
    ...
    def save_model(self):
        self.model.write("example3_model.lp")
