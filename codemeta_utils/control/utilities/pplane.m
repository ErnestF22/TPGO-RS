function output = pplane(action,input1,input2,input3)

% pplane  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane is executed, a pplane Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane
...
%	the pplane Display window, the solution to the system with that
...
if strcmp(action,'initialize')
   ...
   % First we make sure that there is no other copy of pplane
   % running, since this causes problems.	
   pph = findobj('tag','pplane');
   if ~isempty(pph);
      qstring = {'There are some pplane figures open although they may be invisible.  ';...
            'What do you want to do?'};
      tstring = 'Only one copy of pplane can be open at one time.';
      b1str = 'Restart pplane.';
   ...
   % Next we look for old files created by pplane.
   ...
   for k = 1:length(oldfiles)
      ...
      ll = fgetl(fid);
      fclose(fid);
      if strcmp(ll,'%% Created by pplane')
         delete(fn)
      else
	l= length(ofiles);
	ofiles{l+1} = fn;
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane Setup','numb','off',...
		 'tag','pplane','visible','off',...
		 'user',ud);
  
  pplane('figdefault',ppset);
  ...
  callnfpts = [
      'ppset = findobj(''name'',''pplane Setup'');',...
      'ud = get(ppset,''user'');'...
      'me = ud.h.npts;',...
...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'user',sud);
   he = findobj('name','pplane Equilibrium point data');
   hl = findobj('name','pplane Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
   ...
   if (sud.flag == 1)
     ...
     NumbFPts = sud.c.npts;
     ppdisp = findobj('name','pplane Display');
     dud = get(ppdisp,'user');
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
	 ...
	 if isempty(pv) 
	   ...
	   xxderivstr = pplane('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane('paraeval',pn,pv,yderivstr);
     ...
     % Find pplane Display if it exists.	
     % If pplane Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
       ...
       ppdisp = figure('name','pplane Display',...
		       'numb','off',...
		       'interrupt','on',...
		       'visible','off',...
		       'tag','pplane');
       pplane('figdefault',ppdisp);
       dud = get(ppdisp,'user');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       merestart = uimenu(fmenu,'label',...
			  ...
			  'call','pplane(''restart'')',...
			  'separator','on');
       mequit = uimenu(fmenu,'label','Quit pplane',...
		       'call','pplane(''quit'')','separator','off');
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			'call','pplane(''hotcold'')','separator','on');
       ...
       set(gcf,'WindowButtonDownFcn','pplane(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','pplane(''cdisp'')');
	...
	case 6
     ...
     ppkbd = findobj('name','pplane Keyboard input','vis','on');
     if ~isempty(ppkbd),pplane('kbd'),end
     pplevel = findobj('name','pplane Level sets');
...
elseif strcmp(action,'linear')
   ...
   % Get the information from pplane Equilibrium ...
   ...
   % Find pplane Linearization if it exists.	
   % If pplane Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','pplane Linearization');
   if (~isempty(pplin))
      figure(pplin);
      dud = get(pplin,'user');
      dud.syst = system;
      ...
      pplin = figure('name','pplane Linearization',...
         'numb','off',...
         'interrupt','on',...
         'visible','off',...
         'tag','pplane');
      pplane('figdefault',pplin);
      dud = get(pplin,'user');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      mequit = uimenu(fmenu,'label','Quit pplane',...
	  'call','pplane(''quit'')','separator','on','pos',9);
	...
	chkd = 'on';
      ...
      set(pplin,'WindowButtonDownFcn',['pplane(''down'',',num2str(pplin),')']);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane Display and get the user data.
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'pplane Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','pplane(''down'')');
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      pplane('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      pplane('solution',initpt,ppdisp);
   end  
   pplane('solution',[1,-1],ppdisp);
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % button on the pplane Keyboard figure.
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane('paraeval',pn,pv,xstr);
	 ystr = pplane('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'kbd')
   ...
   % sets up the pplane Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','pplane Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','pplane Keyboard input',...
		  'vis','off',...
		  'numb','off','tag','pplane');
   
   pplane('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','call','pplane(''kcompute'')');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','pplane Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','pplane Equilibrium point data',...
 		    'numb','off','tag','pplane');
     pplane('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   set(gcf,'WindowButtonDownFcn','pplane(''zoom'')',...
...
elseif strcmp(action,'zoom')
   ...
   set(axh,'user',aud);	
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane(''down'')',...
      'WindowButtonUpFcn','');
   pplane('dirfield',disph);
   ppset = findobj('name','pplane Setup');
   if isempty(ppset)
      pplane('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
	...
	'clickedcallback','pplane(''print'');');
    switch name
     case 'pplane Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedcallback',['pplane(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['pplane(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'pplane Linearization'
...
elseif strcmp(action,'settings')
   ...
   % up the pplane Settings window, which allows the user to interactively
   ...
   setfig = findobj('name','pplane Settings');
   if ~isempty(setfig);
     delete(setfig);
   end
   
   setfig = figure('name','pplane Settings',...
		   'numb','off',...
		   'tag','pplane','vis','off');
   
   pplane('figdefault',setfig);
...
elseif strcmp(action,'plotxy')  % Start a graph 
  ...
  x=get(objh,'xdata');
  y=get(objh,'ydata');
  hh = pplane('plotxyfig',type,gcf);
  set(ppdisp,'WindowButtonDownFcn',oldcall);
  aud = get(hh(2),'user');
  ud = get(hh(1),'user');
  ud.data = [t;x;y];
  set(hh(1),'user',ud);
  pplane('plxy',aud.rad,hh(1));
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('number','off',...
		'tag','pplane','visible','off');
  name = ['pplane t-plot # ',int2str(ppxy)];
  ud.color = dud.color;
  ud.type = 0;
  set(ppxy,'name',name,'user',ud);
  pplane('figdefault',ppxy);
  ...
  set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
  set(findobj(fmenu,'label','&Print...'),'pos',7);
  qmenu = uimenu(fmenu,'label','Quit pplane',...
		 'call','pplane(''quit'')',...
		 'separator','on');
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','pplane(''plxybdf'')');
    case 2
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane(''plxybmf'')',...
      'windowbuttonupfcn','pplane(''plxybuf'')',...
      'inter','on');
   aud = get(dispa,'user');
   delete(aud.h);
   aud.h = [];
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the pplane Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the pplane Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'figdefault')
  ...
  fig = input1;
  set(fig,'CloseRequestFcn','pplane(''closefcn'')');
  ppset = findobj('name','pplane Setup');
  sud = get(ppset,'user');
  ud = get(fig,'user');
  ud.ssize = sud.ssize;
...
elseif strcmp(action,'zoominsq')
   ...
   set(gcf,'WindowButtonDownFcn','pplane(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','inter','on');
   set(gca,'inter','on');
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'user',aud);
   set(gcf,'windowbuttonmotionfcn','pplane(''zoomsqm'')',...
      'windowbuttonupfcn','pplane(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane(''down'')',...
      'WindowButtonMotionFcn','pplane(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane('dirfield',disph);
   ppset = findobj('name','pplane Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','pplane Level sets',...
		 'vis','off',...
		 'numb','off','tag','pplane');
  
  pplane('figdefault',pplevel);
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
elseif strcmp(action,'closefcn')
   ...
   fig = gcf;  
   name = get(fig,'name');
   if strcmp(name,'pplane Setup') | strcmp(name,'pplane Display')
      quest = ['Closing this window will cause all pplane ',...
            'windows to close, and pplane will stop.  ',...
            'Do you want to quit pplane?'];
      butt = questdlg(quest,'Quit pplane?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         pplane('quit');
      end
   elseif strcmp(name,'pplane Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
...
elseif strcmp(action,'confused')
   ...
   tstring = 'pplane is totally confused';
   qstring = {['You will have to restart pplane from '...
...
% PPOUT  is the output function for pplane.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','pplane Display'); %gca;   % 
dud = get(ppdisp,'user');
ppdispa = dud.axes;
dfcn = dud.function;
ud = get(ppdispa,'user');
