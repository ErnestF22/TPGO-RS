function [basic,result,A,P]=dual_simplex_vertices_old(A,basic,d,PreviousACT)
%initialization:
...
id = (d+1:size(A,2)-1);
act_new = id(~ismember(id,basic));
if size(act_new,2)<d/2
    flagNotRepeat = false;
elseif ismember(sort(act_new),PreviousACT,'rows')
    flagNotRepeat = false;
else
    flagNotRepeat = true;
end
        
while (~(flagXinBasic && flagNotRepeat) && ~flagNotFind)
