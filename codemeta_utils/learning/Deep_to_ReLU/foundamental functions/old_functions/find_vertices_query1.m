...
% a = basic(basic<(size(Ar,2)-d));
% a = a(a>d);
% idx = (d+1:size(Ar,2)-d-1);
% a = idx(~ismember(idx,a));
ACT = sort(a,'ascend');
    ...
    for xs = basic % for every variable in basic
        ...
        for j=1:k
            ...
            elseif (xs<=(d) && j>(d)) % if try to pivot basic to slack
                ...
                a_new = basic_new(basic_new<(size(Ar,2)-d));
                a_new = a_new(a_new>d);
                a_new = idx(~ismember(idx,a_new));
                ...
                if A(pivotRow,pivotColumn)<= 1e-5 % if the pivot value =0, decided by A
                    ...
                    if ~ismember(sort(a_new,'ascend'),ACT,'rows')
                        ...
                        if any(A_new(1:end-1,end)<0)
                            [basic_new,~,A_new] = dual_simplex2_ori(A_new,basic_new,d);
