function [basic,A]=dual_simplex_vertices(A,basic,d)
%initialization:
    ...
    for i=1:size(xs,2)
        ...
        if all(A(pivotRow,1:d)<=0) % all x's negative, cannot pivot
            ...
            for j=1:sizeColumn % try every x's
                ...
                % try to pivot
                Anew = pivotA(A,pivotRow,pivotColumn);
                basic_new = basic;
                basic_new(pivotRow)=pivotColumn;
                [Anew,basic_new] = pivot_correct_xpn_A(Anew,basic_new,d);
            ...
            if all(A(pivotRow,pivotColumnSet)<=0) % all negative, cannot pivot
                ...
                for j=1:sizeColumn % try every x's
                    ...
                    if all(Anew(1:end-1,end)>=0)% new solution is feasible
                        A = Anew;
                        basic = basic_new;
                        PreviousBasic = [PreviousBasic;sort(basic_new)];
