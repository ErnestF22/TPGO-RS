function [vertices,zs]=find_vertices1(basic,Ar,d)
k = size(Ar,2)-1; % number of all variables
Q = sort(basic,'ascend'); % basic set tried before
V = basic;
T = {Ar}; % tableu set
result = zeros(1,size(Ar,2)-1);
result(1,basic(:))=Ar(1:(size(Ar,1)-1),end);
num_val = length(result);
zs = find_active_constraints(basic,d,num_val);
v = result(1:d);
vertices = [v];
i=1;
while i <= size(V,1) % check if visit all corners
    basic = V(i,:);
    base = sort(V(i,:),'ascend'); % reorder
    A = T{i};
    
    for xs = base % for every variable in basic
        pivotRow = find(basic==xs);
        % find pivot column
        for j=1:k
            if any(base==j) % if already in basic
                continue
            elseif (xs<=(d) && j>(d)) % if try to pivot basic to slack
                continue
            else
                % find new base
                base_new = base;
                base_new(base_new == xs)=j;
                base_new = sort(base_new,'ascend');
                
                if sum(sum(Q-base_new,2)==0)% if already in Q
                    continue
                else % if not in Q
                    Q = [Q;base_new]; % put in Q
                    pivotColumn = j;
                    if A(pivotRow,pivotColumn)<= 1e-10 % if the pivot value =0, decided by A
                        continue
                    else %pivot
                        [flagFind,A_new,Q,basic_new] = pivot1(A,pivotRow,pivotColumn,basic,d,Q);
                        if flagFind % if find a new vertex
                            V = [V;basic_new];
                            T{end+1} = A_new;
                            result = zeros(1,size(A_new,2)-1);
                            result(1,basic_new)=A_new(1:(size(A_new,1)-1),end);
                            v = result(1:d);
                            if sum(sum(round(vertices-v,10),2)~=0)
                                vertices = [vertices;v];
                                zs_new = find_active_constraints(basic_new,d,num_val);
                                zs = [zs;zs_new];
                            end
                        end
                    end
                end
            end
        end
    end
    i = i+1;
end
end














