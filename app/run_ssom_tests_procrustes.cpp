...
int main(int argc, char **argv)
{
    ...
    for (const auto &entry : sortedByName)
    {
        ...
        for (int testjd = 0; testjd < numTestsPerInstance; ++testjd)
        {
            ...
            SomUtils::MatD startX = SomUtils::MatD::Random(somSzD.d_ * somSzD.d_ * somSzD.n_ + somSzD.d_ * somSzD.n_ + numEdges, 1);

            if (readStartingPtFromFile)
                if (!SomUtils::readCsvVecEigen(entry.string() + "/ssom_x_start.csv", startX))
                {
                    ROFL_ERR("Error opening file")
                    ROFL_ASSERT(0)
                }
            ...
            auto LambdasStart = startX.block(somSzD.d_ * somSzD.d_ * somSzD.n_ + somSzD.d_ * somSzD.n_, 0, numEdges, 1);
            ROFL_VAR1(LambdasStart)
            ...
            { // RSOM Procrustes execution scope
                ...
                SsomProcrustes sp(somSzD, Tijs, edges);

                SomUtils::MatD Xgt = SomUtils::MatD::Random(somSzD.d_ * somSzD.d_ * somSzD.n_ + somSzD.d_ * somSzD.n_ + numEdges, 1);
                ...
                SomUtils::MatD LambdasGt = SomUtils::MatD::Random(numEdges, 1);
                LambdasGt = Xgt.block(somSzD.d_ * somSzD.d_ * somSzD.n_ + somSzD.d_ * somSzD.n_, 0, numEdges, 1);

                // auto Tstart = Tgt;
                sp.setTstart(Tstart);
                sp.setLambdasStart(LambdasStart);

                sp.setTgt(Tgt);
                sp.setRgt(Rgt);
                sp.setLambdasGt(LambdasGt);

                sp.run();

                SomUtils::MatD Tout = sp.getTout();
                SomUtils::VecMatD Rout = sp.getRout();
                SomUtils::MatD LambdasOut = sp.getLambdasOut();
