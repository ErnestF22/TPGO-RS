function transf_out = som_manopt(T_globalframe, Tijs_vec, edges, params, transf_init_guess)
% Compute transformation through Manopt pipeline
...
[R, R_cost, R_info, R_options] = som_stepone_manopt(T_globalframe, Tijs_vec, edges, cost_const_term_tij, params, R_init_guess);
...
while (norm(matStack(transf_prev) - matStack(transf_out))>=transf_end_thresh && num_iterations<max_icp_iterations)
    ...
    %COORD DESC - step 1
    [R, R_cost, R_info, R_options] = som_stepone_manopt(T_globalframe, Tijs_vec, edges, cost_const_term_tij, params, rot_prev);
