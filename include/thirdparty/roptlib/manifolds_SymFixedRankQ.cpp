...
namespace ROPTLIB
{
    ...
    realdp SymFixedRankQ::Metric(const Variable &x, const Vector &etax, const Vector &xix) const
    {
        ...
        { /*2 \trace(x^T etax x^T xix + x^T x etax^T xix)*/
            Vector tmp1(p, p);
            tmp1.AlphaABaddBetaThis(1, etax, GLOBAL::T, x, GLOBAL::N, 0);
            Vector tmp2(p, p);
            tmp2.AlphaABaddBetaThis(1, x, GLOBAL::T, xix, GLOBAL::N, 0);
    ...
    {
        Vector XTX(p, p);
        XTX.AlphaABaddBetaThis(1, x, GLOBAL::T, x, GLOBAL::N, 0);
        Vector XTV(p, p);
        XTV.AlphaABaddBetaThis(1, x, GLOBAL::T, etax, GLOBAL::N, 0);
    ...
    Variable &SymFixedRankQ::Retraction(const Variable &x, const Vector &etax, Variable *result) const
    {
        if (IsIntrApproach)
        {
            ...
            tmp1.AlphaABaddBetaThis(1, x, GLOBAL::T, x, GLOBAL::N, 0);
            Vector tmp2(p, p);
            tmp2.AlphaABaddBetaThis(1, x, GLOBAL::T, exetax, GLOBAL::N, 0);
            Vector S = tmp1 % tmp2; /* S = (x.GetTranspose() * x) % (x.GetTranspose() * exetax); */
            Vector Yp(exetax);
            Yp.AlphaABaddBetaThis(-1, x, GLOBAL::N, S, GLOBAL::N, 1); /* Yp = etax - x * S; */
            ...
            Vector XtY(p, p);
            XtY.AlphaABaddBetaThis(1, x, GLOBAL::T, Yp, GLOBAL::N, 0); /* XtY = x.GetTranspose() * Yp; */
            XtY.SVDDecom();
    ...
    Vector &SymFixedRankQ::EucGradToGradEUC(const Variable &x, const Vector &egf, const Problem *prob, Vector *result) const
    {
        ...
        *result = (egf / XTX) / 2; /* result = (egf / (x.GetTranspose() * x)) / 2 */
        Vector tmp1(p, p);
        tmp1.AlphaABaddBetaThis(1, x, GLOBAL::T, *result, GLOBAL::N, 0); /*tmp1 = x.GetTranspose() * tmp1 */
        tmp1 = XTX % tmp1;
    ...
    Vector &SymFixedRankQ::EucHvToHvEUC(const Variable &x, const Vector &etax, const Vector &exix, const Problem *prob, Vector *result) const
    {
        ...
        Vector xTEGYYinv(p, p);
        xTEGYYinv.AlphaABaddBetaThis(1, x, GLOBAL::T, EGYYinv, GLOBAL::N, 0);
        Vector YYinvYTEGYYinv = XTX % xTEGYYinv;

        Vector EtaTEGYYinv(p, p);
        EtaTEGYYinv.AlphaABaddBetaThis(1, etax, GLOBAL::T, EGYYinv, GLOBAL::N, 0);
        Vector YTeta(p, p);
        YTeta.AlphaABaddBetaThis(1, x, GLOBAL::T, etax, GLOBAL::N, 0);
        Vector YTxix(p, p);
        YTxix.AlphaABaddBetaThis(1, x, GLOBAL::T, exix, GLOBAL::N, 0);
    ...
    Vector &SymFixedRankQ::EucHvToHvHGZ(const Variable &x, const Vector &etax, const Vector &exix, const Problem *prob, Vector *result) const
    {
        ...
        XTX.AlphaABaddBetaThis(1, x, GLOBAL::T, x, GLOBAL::N, 0);
        Vector EGYYinv = egf / XTX;
        Vector YTEGYYinv(p, p);
        YTEGYYinv.AlphaABaddBetaThis(1, x, GLOBAL::T, EGYYinv, GLOBAL::N, 0);
        Vector EtaTEGYYinv(p, p);
        EtaTEGYYinv.AlphaABaddBetaThis(1, etax, GLOBAL::T, EGYYinv, GLOBAL::N, 0);
        Vector EtaTY(p, p);
        EtaTY.AlphaABaddBetaThis(1, etax, GLOBAL::T, x, GLOBAL::N, 0);
