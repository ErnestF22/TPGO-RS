...
#include "thirdparty/roptlib/manifolds_CFixedRankQ2F.h"

/*Define the namespace*/
namespace ROPTLIB
{

    /* the numbers of rows are double because complexRopt numbers need double memory compared to realRopt numbers*/
    CFixedRankQ2F::CFixedRankQ2F(integer inm, integer inn, integer inr) : ProductManifold(2,
                                                                                          new Euclidean(inm, inr, "complexRopt"), static_cast<integer>(1), new Euclidean(inn, inr, "complexRopt"), static_cast<integer>(1))
    {
        m = inm;
        n = inn;
        r = inr;
        name.assign("complexRopt Fixed-rank manifold by 2-factor form");
        IsIntrApproach = true;
        IntrinsicDim = 2 * (m + n - r) * r;
        ExtrinsicDim = 2 * (m + n) * r;
        Vector F1(m, r, "complexRopt"), F2(n, r, "complexRopt");
    ...
    Vector &CFixedRankQ2F::ExtrProjection(const Variable &x, const Vector &etax, Vector *result) const
    {
        ...
        /*tmp1 = (H^T H)^{-1} (H^T Z)*/
        Vector tmp1(r, r, "complexRopt");
        tmp1.AlphaABaddBetaThis(GLOBAL::ZONE, x.GetElement(1), GLOBAL::C, inetax.GetElement(1), GLOBAL::N, GLOBAL::ZZERO);
        tmp1 = tmp1.TriangleLinSol(LH, GLOBAL::N);
        tmp1 = tmp1.TriangleLinSol(LH, GLOBAL::C);

        /*tmp2 = (G^T G)^{-1} (G^T W)*/
        Vector tmp2(r, r, "complexRopt");
        tmp2.AlphaABaddBetaThis(GLOBAL::ZONE, x.GetElement(0), GLOBAL::C, inetax.GetElement(0), GLOBAL::N, GLOBAL::ZZERO);
    ...
    realdp CFixedRankQ2F::Metric(const Variable &x, const Vector &etax, const Vector &xix) const
    {
        ...
        realdp result = 0;
        Vector tmp1(r, r, "complexRopt");
        tmp1.AlphaABaddBetaThis(GLOBAL::ZONE, G, GLOBAL::C, G, GLOBAL::N, GLOBAL::ZZERO);
        Vector tmp2(r, r, "complexRopt");
        tmp2.AlphaABaddBetaThis(GLOBAL::ZONE, xixH, GLOBAL::C, etaxH, GLOBAL::N, GLOBAL::ZZERO);
        result += tmp1.DotProduct(tmp2);
        tmp1.AlphaABaddBetaThis(GLOBAL::ZONE, H, GLOBAL::C, H, GLOBAL::N, GLOBAL::ZZERO);
        tmp2.AlphaABaddBetaThis(GLOBAL::ZONE, etaxG, GLOBAL::C, xixG, GLOBAL::N, GLOBAL::ZZERO);
        result += tmp1.DotProduct(tmp2);
        tmp1.AlphaABaddBetaThis(GLOBAL::ZONE, H, GLOBAL::C, etaxH, GLOBAL::N, GLOBAL::ZZERO);
        tmp2.AlphaABaddBetaThis(GLOBAL::ZONE, xixG, GLOBAL::C, G, GLOBAL::N, GLOBAL::ZZERO);
        result += tmp1.DotProduct(tmp2);
        tmp1.AlphaABaddBetaThis(GLOBAL::ZONE, xixH, GLOBAL::C, H, GLOBAL::N, GLOBAL::ZZERO);
        tmp2.AlphaABaddBetaThis(GLOBAL::ZONE, G, GLOBAL::C, etaxG, GLOBAL::N, GLOBAL::ZZERO);
        result += tmp1.DotProduct(tmp2);
    ...
    { /* G H^* + dG H^* + G dH^* = [G dG] [I, I; I, 0] * [H dH]^* */
        ...
        Vector tmp1(m, 2 * r, "complexRopt"), tmp2(n, 2 * r, "complexRopt");
