...
#include "problems_FRankEWeightApprox.h"
...
namespace ROPTLIB
{
	...
	{
		Vector xmA(x);
		xmA.AlphaXaddThis(-1, A); /* xmA = x - A; */
		Vector WxmA(m * n, 1);
		WxmA.AlphaABaddBetaThis(1, W, GLOBAL::N, xmA.Reshape(m * n, 1), GLOBAL::N, 0);
	...
	Vector &FRankEWeightApprox::EucHessianEta(const Variable &x, const Vector &etax, Vector *result) const
	{
		...
		etaxreshape.Reshape(m * n, 1);
		result->Reshape(m * n, 1);
		result->AlphaABaddBetaThis(2.0, W, GLOBAL::N, etaxreshape, GLOBAL::N, 0);
		result->Reshape(m, n);
