...
#include "others_SparseMatrix.h"
#include "others_def.h"
#include "others_fftw_fftw3.h"
#include <sstream>
...
namespace ROPTLIB{
    ...
    public:
        ...
        /*add two elements entrywise, they have to be both realRopt or complexRopt*/
        friend Element operator+(Element left, Element right);

        /*add a realRopt element to a realRopt number*/
        friend Element operator+(Element left, realdp right);
        
        /*add a complexRopt element to a complexRopt number*/
        friend Element operator+(Element left, realdpcomplex right);
        
        /*add a realRopt number to a realRopt element*/
        friend Element operator+(realdp left, Element right);
        
        /*add a complexRopt number to a complexRopt element*/
        friend Element operator+(realdpcomplex left, Element right);
        
        /*left = left + right*/
        friend Element operator+=(Element left, Element right);
        
        /*substract an element from another. They have to be both realRopt or complexRopt*/
        friend Element operator-(Element left, Element right);
        
        /*a realRopt element minus a realRopt number*/
        friend Element operator-(Element left, realdp right);
        
        /*a complexRopt element minus a complexRopt number*/
        friend Element operator-(Element left, realdpcomplex right);
        
        /*a realRopt number minus a realRopt element*/
        friend Element operator-(realdp left, Element right);
        
        /*a complexRopt number minus a complexRopt element*/
        friend Element operator-(realdpcomplex left, Element right);
        
        /*left = left - right*/
        friend Element operator-=(Element left, Element right);
        
        /*a element times another, they have to be both realRopt or complexRopt
         This is matrix multiplication*/
        friend Element operator*(Element left, Element right);

        /*a realRopt number times a realRopt element, this is an entrywise multiplication*/
        friend Element operator*(realdp value, Element mat);
        
        /*a complexRopt number times a complexRopt element, this is an entrywise multiplication*/
        friend Element operator*(realdpcomplex value, Element mat);

        /*a realRopt element times a realRopt number, this is an entrywise multiplication*/
        friend Element operator*(Element mat, realdp value);
        
        /*a complexRopt element times a complexRopt number, this is an entrywise multiplication*/
        friend Element operator*(Element mat, realdpcomplex value);
        
        /*The left element times an inverse of the right element, they have to be both realRopt or complexRopt*/
        friend Element operator/(Element left, Element right);
        
        /*The inverse of the left element times the right element, they have to be both realRopt or complexRopt*/
        friend Element operator%(Element left, Element right);

        /*entrywise divide an realRopt element by a realRopt number*/
        friend Element operator/(Element mat, realdp value);
        
        /*entrywise divide an complexRopt element by a complexRopt number*/
        friend Element operator/(Element mat, realdpcomplex value);
        
        /*entrywise divide an realRopt number by a realRopt element*/
        friend Element operator/(realdp value, Element mat);
        
        /*entrywise divide an complexRopt number by a complexRopt element*/
        friend Element operator/(realdpcomplex value, Element mat);
...
//            right.Print("right:", false);//--
        ...
        Element(integer r, integer l = 1, integer n = 1, const char * = "realRopt");
        ...
        */
        ...
        this element = L * L^T (realRopt) or element = L * L^H (complexRopt).
        ...
        /*Use *syev_ to compute the eigenvalue decomposition for a realRopt symmetric matrix.
         Use *heev_ to compute the eigenvalue decomposition for a complexRopt Hermitian matrix.
        ...
        /*Return the transpose of the Element if it is a matrix, without changing the original element
         If the matrix is complexRopt, the it is the conjugate transpose of the element*/
        ...
        /*Compute realRopt(this^T M), T donotes conjuage transpose */
        ...
        /*Return the transpose of the Element if it is a matrix, and transpose the original element
         If the matrix is complexRopt, the it is the conjugate transport of the element*/
        ...
        inline void Setiscomplex(bool iniscomplex) const { if (iniscomplex && ((std::floor(size[0]/2)*2 - size[0]) !=0)) {printf("Warning: cannot set to be complexRopt!\n"); return;}; iscomplex = iniscomplex; };
    ...
    protected:
        ...
        /*the entries in element are realRopt or complexRopt*/
