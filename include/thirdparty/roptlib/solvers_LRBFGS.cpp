...
namespace ROPTLIB
{
    ...
    void LRBFGS::SetParams(PARAMSMAP params)
    {
        ...
        for (iter = params.begin(); iter != params.end(); iter++)
        {
            ...
            else if (iter->first == static_cast<std::string>("LengthSY"))
            {
            ...
            else if (iter->first == static_cast<std::string>("nu"))
            {
                nu = iter->second;
            }
            else if (iter->first == static_cast<std::string>("mu"))
            {
                mu = iter->second;
            }
        }
    ...
    {
        Mani->VectorTransport(x1, eta2, x2, eta2, &s);
        nV++;
        Vector Tgf1(gf1);
        Mani->VectorTransport(x1, eta2, x2, gf1, &Tgf1);
        nVp++;
                                                    ...
                                                    (inpss > std::numeric_limits<realdp>::epsilon() && inpsy > std::numeric_limits<realdp>::epsilon())))
        {
            ...
            else if (LengthSY > 0)
            {
                ...
                {
                    idx = i % LengthSY;
                    Mani->VectorTransport(x1, eta2, x2, Y[idx], &Y[idx]);
                    nVp++;
                    Mani->VectorTransport(x1, eta2, x2, S[idx], &S[idx]);
                    nVp++;
                }
            }
            isupdated = true;
