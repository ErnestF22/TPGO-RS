...
namespace NIST_SPBLAS
{
  ...
  */
  ...
  void Sp_mat::print() const
  {
    ...
    std::cout << "State : " << (is_void() ? "void" : is_new() ? "new"
                                                 : is_open()  ? "open"
                                                 : is_valid() ? "valid"
                                                              : "unknown")
              << "\n";

    std::cout << "M = " << num_rows() << "  N = " << num_cols() << "  nz = " << num_nonzeros() << "\n";

#define yesno(exp) ((exp) ? "yes" : "no")

    std::cout << "realRopt: " << yesno(is_real()) << "\n";
    std::cout << "complexRopt: " << yesno(is_complex()) << "\n";
...
#define TSp_MAT_SET_FLOAT(A)   \
  {                            \
    A->set_single_precision(); \
    A->set_real();             \
  }
#define TSp_MAT_SET_DOUBLE(A)  \
  {                            \
    A->set_double_precision(); \
    A->set_real();             \
  }
#define TSp_MAT_SET_COMPLEX_FLOAT(A) \
  {                                  \
    A->set_single_precision();       \
    A->set_complex();                \
  }
#define TSp_MAT_SET_COMPLEX_DOUBLE(A) \
  {                                   \
    A->set_double_precision();        \
    A->set_complex();                 \
  }
...
int BLAS_usgp(blas_sparse_matrix A, int pname)
{
  ...
  case (blas_num_cols):
    return S->num_cols();
  case (blas_num_nonzeros):
    return S->num_nonzeros();
  ...
  case (blas_real):
    return S->is_real();
  case (blas_single_precision):
    return S->is_single_precision();
  case (blas_double_precision):
    return S->is_double_precision();

  case (blas_lower_triangular):
    return S->is_lower_triangular();
    break;
  case (blas_upper_triangular):
    return S->is_upper_triangular();
    break;

  case (blas_general):
    return S->is_general();
  case (blas_symmetric):
    return S->is_symmetric();
  case (blas_hermitian):
    return S->is_hermitian();

  case (blas_zero_base):
    return S->is_zero_base();
  case (blas_one_base):
    return S->is_one_base();

  case (blas_rowmajor):
    return S->is_rowmajor();
  case (blas_colmajor):
    return S->is_colmajor();
  ...
  case (blas_open_handle):
    return S->is_open();
  case (blas_invalid_handle):
    return S->is_void();

  case (blas_regular):
    return S->is_opt_regular();
  case (blas_irregular):
    return S->is_opt_irregular();
  case (blas_block):
    return S->is_opt_block();
  case (blas_unassembled):
    return S->is_opt_unassembled();
...
int BLAS_xuscr_insert_clique(blas_sparse_matrix A, int k, int l, const T *Val,
                             ...
                             const int *jndx)
{
  return ((TSp_mat<T> *)Table[A])->insert_clique(k, l, Val, row_stride, col_stride, indx, jndx);
}
...
{
  return ((TSp_mat<T> *)Table[A])->insert_block(Val, row_stride, col_stride, bi, bj);
}
...
int BLAS_DOUBLE_NAME(usmv)(enum blas_trans_type transa,
                           SPBLAS_DOUBLE_IN alpha,
                           blas_sparse_matrix A,
                           SPBLAS_VECTOR_DOUBLE_IN x,
                           int incx,
...
int BLAS_COMPLEX_DOUBLE_NAME(usmv)(enum blas_trans_type transa,
                                   SPBLAS_COMPLEX_DOUBLE_IN alpha,
                                   blas_sparse_matrix A,
                                   SPBLAS_VECTOR_COMPLEX_DOUBLE_IN x,
                                   int incx,
...
int BLAS_FLOAT_NAME(usmv)(enum blas_trans_type transa,
                          SPBLAS_FLOAT_IN alpha,
                          blas_sparse_matrix A,
                          SPBLAS_VECTOR_FLOAT_IN x,
                          int incx,
...
int BLAS_COMPLEX_FLOAT_NAME(usmv)(enum blas_trans_type transa,
                                  SPBLAS_COMPLEX_FLOAT_IN alpha,
                                  blas_sparse_matrix A,
                                  SPBLAS_VECTOR_COMPLEX_FLOAT_IN x,
                                  int incx,
