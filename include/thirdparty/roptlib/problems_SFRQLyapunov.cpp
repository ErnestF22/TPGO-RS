...
#include "problems_SFRQLyapunov.h"
...
namespace ROPTLIB
{
        ...
        realdp SFRQLyapunov::f(const Variable &x) const
        {
                ...
                Vector MY = (*Mptr) * x;
                Vector CTY(Cp, p);
                CTY.AlphaABaddBetaThis(1, C, GLOBAL::T, x, GLOBAL::N, 0); /* CTY = C.GetTranspose() * x; */
                Vector YTAY(p, p);
                YTAY.AlphaABaddBetaThis(1, x, GLOBAL::T, AY, GLOBAL::N, 0); /* YTAY = x.GetTranspose() * AY; */
                Vector YTMY(p, p);
                YTMY.AlphaABaddBetaThis(1, x, GLOBAL::T, MY, GLOBAL::N, 0); /* YTMY = x.GetTranspose() * MY; */
        ...
        Vector &SFRQLyapunov::EucHessianEta(const Variable &x, const Vector &etax, Vector *result) const
        {
                ...
                Vector YTMeta(p, p);
                YTMeta.AlphaABaddBetaThis(1, x, GLOBAL::T, Meta, GLOBAL::N, 0); /* YTMeta = x.GetTranspose() * Meta; */
                Vector Aeta = (*Aptr) * etax;
                Vector YTAeta(p, p);
                YTAeta.AlphaABaddBetaThis(1, x, GLOBAL::T, Aeta, GLOBAL::N, 0); /* YTAeta = x.GetTranspose() * Aeta; */
                ...
                Vector tmp(Cp, p);
                tmp.AlphaABaddBetaThis(1, C, GLOBAL::T, etax, GLOBAL::N, 0);
        ...
        Vector &SFRQLyapunov::ActionEH(const Variable &x, const Vector &intreta, Vector *result) const
        {
                Vector AY = x.Field("AY"), MY = x.Field("MY"), YTAY = x.Field("YTAY"), YTMY = x.Field("YTMY");
                Vector eta(n, p);
                Domain->ObtainExtr(x, intreta, &eta);
