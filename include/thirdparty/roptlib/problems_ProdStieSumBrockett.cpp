...
namespace ROPTLIB
{
        ...
        {
                Vector B1x1D1(n, p);
                B1x1D1.AlphaABaddBetaThis(1, B1, GLOBAL::N, x.GetElement(0), GLOBAL::N, 0); /* B1x1D1 = B1 * x.GetElement(0); */
                D1.DiagTimesM(B1x1D1, GLOBAL::R);
                realdp result = x.GetElement(0).DotProduct(B1x1D1);
                x.AddToFields("B1x1D1", B1x1D1);

                Vector B2x2D2(n, p);
                B2x2D2.AlphaABaddBetaThis(1, B2, GLOBAL::N, x.GetElement(1), GLOBAL::N, 0); /* B2x2D2 = B2 * x.GetElement(1); */
                D2.DiagTimesM(B2x2D2, GLOBAL::R);
                result += x.GetElement(1).DotProduct(B2x2D2);
                x.AddToFields("B2x2D2", B2x2D2);

                Vector B3x3D3(m, q);
                B3x3D3.AlphaABaddBetaThis(1, B3, GLOBAL::N, x.GetElement(2), GLOBAL::N, 0); /* B3x3D3 = B3 * x.GetElement(2); */
