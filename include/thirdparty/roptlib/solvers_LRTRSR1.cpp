...
namespace ROPTLIB
{
    ...
    void LRTRSR1::SetParams(PARAMSMAP params)
    {
        ...
        for (iter = params.begin(); iter != params.end(); iter++)
        {
            ...
            else if (iter->first == static_cast<std::string>("LMrestart"))
            {
    ...
    { /*This method is only used when the Euclidean metric or an intrinsic approach is used.*/
        /*note that if the manifold is a complexRopt, then we still view it as a realRopt manifold with realRopt reparameterizations
        therefore, we first set the types of vectors to be realRopt and then recover the types of them at the end of this function*/
        bool gf1iscomplex = gf1.Getiscomplex();
        gf1.Setiscomplex(false);
        ...
        if (k > 0)
        {
            ...
            Vector gf1reshape(gf1);
            gf1reshape.Reshape(n);
        ...
        else if (lambda_min <= 0 && PhiBar_fg(-lambda_min, Delta, Lambda, a_j) > 0)
        {
    ...
    { /*gamma, gf1, Psig, YMGS, PMGQ, R*/
        if (Currentlength != 0)
        {
            ...
            {
                eta2 = gf1;
                eta2.ScalarTimesThis(static_cast<realdp>(-1) / tauStar); /* eta2 = (static_cast<realdp> (-1) / tauStar) * gf1; */
                return;
            }
    ...
    realdp LRTRSR1::PhiBar_fg(realdp nlambda_min, realdp Delta, const Vector &Lambda, const Vector &a_j, realdp *gf)
    {
        ...
        if (flag)
        {
            ...
            for (integer i = 0; i < k + 1; i++)
            {
                ...
                else if (fabs(a_jptr[i]) > eps_tol && fabs(tmpptr[i]) > eps_tol)
                {
    ...
    void LRTRSR1::UpdateDataLRTRSR1(void)
    {
        ...
        ng++;
        s = eta2;
        Mani->InverseVectorTransport(x1, eta2, x2, gf2, &eta1);
        nV++;
        ...
        Vector zeta(y);
        Mani->ScalarVectorAddVector(x1, -1, Heta2, y, &zeta);
        denorminator = Mani->Metric(x1, s, zeta);
        norm2ymBs = Mani->Metric(x1, zeta, zeta);
        inpss = Mani->Metric(x1, s, s);
        inpsy = Mani->Metric(x1, s, y);

        if (denorminator * denorminator >= mintolsq * inpss * norm2ymBs && (norm2ymBs >= mintolsq || ngf2 / ngf0 < 1e-3) && (iter != 0 || fabs(gamma - inpsy / inpss) > mintol)) /* This is for the robustness when the cost
