/*
This file defines the class for the complexRopt Stiefel manifold \CSt(p, n) = \{X \in C^{n \times p} | X^H X = I_p\}
...
namespace ROPTLIB
{
	...
	complexRopt versions of the retractions in Stiefel.h */
	enum CStieRetraction
	{
		CSTIE_QF,
		CSTIE_POLAR,
		CSTIE_EXP,
		CSTIERETRACTIONLENGTH
	};

	/*Vector transport for the Stiefel manifold
	complexRopt versions of the vector transports in Stiefel.h */
	enum CStieVectorTransport
	{
		CSTIE_PARALLELIZATION,
		CSTIE_PARALLELTRANSLATION,
		CSTIE_PROJECTION,
		CSTIEVECTORTRANSPORTLENGTH
	};
