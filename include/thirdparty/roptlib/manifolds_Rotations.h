/*
This file defines the class for the Rotations manifold \SO(n) = \{X \in R^{n \times n} | X^T X = I_n, det(X) = 1\}
...
namespace ROPTLIB{
	...
	/* Riemannian Metric for the Rotations manifold:
	Eucldean: g_x(etax, xix) = \trace(etax^T xix);
	Canonical: g_x(etax, xix) = \trace(etax^T (I_n - x x^T / 2) xix); */
	enum RotMetric{ ROT_EUCLIDEAN, ROT_CANONICAL, ROTMETRICLENGTH };

	/*Retraction for the Rotations manifold
	...
	// enum RotRetraction{ ROT_QF, ROT_POLAR, ROT_EXP, ROT_CAYLEYR, ROTRETRACTIONLENGTH };
	enum RotRetraction{ ROT_QF, ROT_POLAR, ROT_EXP, ROTRETRACTIONLENGTH };

	/*Vector transport for the Rotations manifold
	...
	[Zhu2016]: Xiaojing Zhu, A Riemannian conjugate gradient method for optimization on the Stiefel Manifold */
	enum RotVectorTransport{ ROT_PARALLELIZATION, ROT_RIGGING, ROT_PARALLELTRANSLATION, ROT_PROJECTION, ROT_CAYLEYVT, ROTVECTORTRANSPORTLENGTH };

	class Rotations : public Manifold{
	public:
		/*Construct the Rotations manifold: St(p, n) and set up default parameters*/
		Rotations(integer n);

		/*Delete EMPTYINTR and EMPTYEXTR*/
		virtual ~Rotations(void);
	...
	protected:
		mutable integer n; /*The number of rows and columns*/
		RotMetric metric; /*Riemannian metric*/
		RotRetraction retraction; /*The used retraction*/
		RotVectorTransport VecTran; /*The used vector transport*/
		...
		[Zhu2016]: Xiaojing Zhu, A Riemannian conjugate gradient method for optimization on the Rotations Manifold.*/
