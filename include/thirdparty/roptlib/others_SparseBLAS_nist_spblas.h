...
#else
#define ASSERT_RETURN(x, ret_val) \
  {                               \
    if (!(x))                     \
      return ret_val;             \
  }
...
{
  /* dummy routines for realRopt version of usdot to compile. */
  ...
  */
  ...
  public:
    Sp_mat(int M, int N) : num_rows_(M), /* default construction */
                           num_cols_(N),
                           num_nonzeros_(0),
    ...
    {
      void_ = 1;
      nnew_ = open_ = valid_ = 0;
    }
    void set_new()
    {
      nnew_ = 1;
      void_ = open_ = valid_ = 0;
    }
    void set_open()
    {
      open_ = 1;
      void_ = nnew_ = valid_ = 0;
    }
    void set_valid()
    {
      valid_ = 1;
  ...
  public:
    ...
    */
             ...
             T *y, int incy) const
    {
      ...
      else if (transa == blas_conj_trans)
        mult_vec_conj_transpose(alpha, x, incx, y, incy);
      else if (transa == blas_trans)
    ...
    void print() const
    {
      ...
      for (int i = 0; i < num_rows(); i++)
        for (unsigned int j = 0; j < S[i].size(); j++)
          std::cout << i << "    " << col_index(S[i][j]) << "        " << val(S[i][j]) << "\n";
