...
{
    ////////////////////////////////////////RS////////////////////////////////////////
    bool SsomProblem::checkIsStiefelTg(const SomUtils::MatD &m, const SomUtils::MatD &mTg) const
    {
    ...
    bool SsomProblem::checkIsStiefelTg(const SomUtils::VecMatD &m, const SomUtils::VecMatD &mTg) const
    {
        int n = m.size();
    ...
    bool SsomProblem::checkIsStiefelTgNorm(const SomUtils::MatD &m, const SomUtils::MatD &mTg) const
    {
        // For any matrix representative $U \in St(n, p)$, the tangent space of $St(n, p)$ at $U$ is represented by
        // U\transpose \Delta = -\Delta\transpose U
        double diff = (m.transpose() * mTg + mTg.transpose() * m).cwiseAbs().maxCoeff();
    ...
    void SsomProblem::stiefelRandTgNormVector(const SomUtils::MatD &mIn, SomUtils::MatD &mOut) const
    {
        mOut.setIdentity();
    ...
    void SsomProblem::stiefelRandTgNormVector(const SomUtils::VecMatD &mIn, SomUtils::VecMatD &mOut) const
    {
        int n = mIn.size();
    ...
    bool SsomProblem::eigencheckHessianGenproc(const double &lambda,
    ...
    bool SsomProblem::eigencheckHessianGenprocShifted(const double &lambda,
                                                      const SomUtils::VecMatD &xR, const SomUtils::VecMatD &uR,
                                                      const SomUtils::MatD &xT, const SomUtils::MatD &uT,
                                                      const SomUtils::MatD &xLambdas, const SomUtils::MatD &uLambdas,
    ...
    void SsomProblem::ssomPimFunctionGenproc(const SomUtils::VecMatD &xR, const SomUtils::MatD &xT, const SomUtils::MatD &xLambdas,
                                             ...
                                             double thresh) const
    {
        ROFL_VAR1("Running ssomPimFunctionGenproc()")
    ...
    void SsomProblem::ssomPimFunctionGenprocShifted(const SomUtils::VecMatD &xR, const SomUtils::MatD &xT, const SomUtils::MatD &xLambdas,
                                                    const SomUtils::VecMatD &uR, const SomUtils::MatD &uT, const SomUtils::MatD &uLambdas,
                                                    double mu,
                                                    double &lambdaMax,
                                                    SomUtils::VecMatD &uOutR, SomUtils::MatD &uOutT, SomUtils::MatD &uOutLambdas,
                                                    double thresh) const
    {
        ROFL_VAR1("Running ssomPimFunctionGenprocShifted()")
        // Note: normalization is done across entire ProdMani vector through simple eucl. metric
    ...
    void SsomProblem::ssomPimHessianGenprocSmall(double thresh, const SomUtils::VecMatD &R, const SomUtils::MatD &T) const
    {
        // Implement later if needed for debug
    }

    void SsomProblem::ssomPimHessianGenprocEigen(double thresh,
    ...
    void SsomProblem::ssomEscapeHessianGenprocEigen(const SomUtils::VecMatD &R, const SomUtils::MatD &T, const SomUtils::MatD &Lambdas,
                                                    ...
                                                    SomUtils::VecMatD &vRout, SomUtils::MatD &vTout, SomUtils::MatD &vLambdasOut) const
    {
        ...
        ProbNext.makeHmatSsom(Xvec, somSzNext, Hmat);
        ROFL_VAR1(Hmat)
                                            ...
                                            bool armijo) const
    {
        ...
        ssomPimFunctionGenproc(Rnext, Tnext, LambdasNext, RnextTgNorm, TnextTgNorm, LambdasNextTgNorm, lambdaPim, vPimR, vPimT, vPimLambdas);
        std::cout << "Difference between lambda_pim_after_shift*v_pim_after_shift"
                  << " and H_SH(v_pim_after_shift) should be in the order of the tolerance:" << std::endl;
        eigencheckHessianGenproc(lambdaPim, Rnext, vPimR, Tnext, vPimT, LambdasNext, vPimLambdas);
        ROFL_VAR1(lambdaPim)
    ...
    void SsomProblem::linesearchArmijoROPTLIB(const Vector &xIn, const SomUtils::SomSize &somSzLocal, Vector &Y0) const
    {
    ...
    // void SsomProblem::stiefelRetractionQR(const SomUtils::VecMatD &x, const SomUtils::VecMatD &e, SomUtils::VecMatD &rxe) const
    // {
    //     // N = size(x, 3);
    //     // p = size(x, 2);
    //     // if N > 1
    ...
    // void SsomProblem::QRunique(const SomUtils::MatD &a, SomUtils::MatD &Q, SomUtils::MatD &R) const
    // {
    //     // [q, r] = qr(A(:, :, k), 0);
    ...
    void SsomProblem::QRunique(const SomUtils::VecMatD &A, SomUtils::VecMatD &Q, SomUtils::VecMatD &R) const
    {
        // [m, n, N] = size(A);
    ...
    bool SsomProblem::checkIsOn3dStiefel(const SomUtils::VecMatD &m) const
    {
        int p = m[0].rows();
        int d = m[0].cols();
    ...
    bool SsomProblem::checkIsOnStiefel(const SomUtils::MatD &m) const
    {
        int p = m.rows();
        int d = m.cols();
    ...
    void SsomProblem::stiefelRetractionQR(const SomUtils::VecMatD &x, const SomUtils::VecMatD &e, SomUtils::VecMatD &rxe, double t) const
    {
    ...
    void SsomProblem::stiefelRetractionPolar(const SomUtils::MatD &xIn, const SomUtils::MatD &e, SomUtils::MatD &rxe, double t) const
    {
    ...
    void SsomProblem::stiefelRetractionPolar(const SomUtils::VecMatD &xIn, const SomUtils::VecMatD &e, SomUtils::VecMatD &rxe, double t) const
    {
        // N = size(x, 3);
        // p = size(x, 2);
    ...
    void SsomProblem::computeNodeDegrees(Eigen::ArrayXi &nodeDegrees) const
    {
    ...
    void SsomProblem::makeTedges(const SomUtils::MatD &T, SomUtils::MatD &Tedges, SomUtils::MatD &T1offset) const
    {
        int nrs = T.rows();
        Tedges.setZero();
    ...
    void SsomProblem::makeTedges(const SomUtils::MatD &T, SomUtils::MatD &Tedges) const
    {
        int nrs = T.rows();
        Tedges.setZero();
        ROFL_ASSERT(Tedges.rows() == nrs && Tedges.cols() == numEdges_)
    ...
    void SsomProblem::POCRotateToMinimizeLastEntries(const SomUtils::MatD &x, SomUtils::MatD &Qtransp) const
    {
    ...
    void SsomProblem::orthComplement(const SomUtils::MatD &v, SomUtils::MatD &vOrth) const
    {
    ...
    void SsomProblem::orthCompleteBasis(const SomUtils::MatD &Q, SomUtils::MatD &Qout) const
    {
    ...
    void SsomProblem::fliplr(const SomUtils::MatD &Ain, SomUtils::MatD &Aout) const
    {
    ...
    void SsomProblem::flipud(const SomUtils::MatD &Ain, SomUtils::MatD &Aout) const
    {
        ROFL_ASSERT(Ain.rows() == Aout.rows() && Ain.cols() == Aout.cols())
        int nrows = Ain.rows();
    ...
    void SsomProblem::procrustesRb(const SomUtils::MatD &c, const SomUtils::MatD &q, SomUtils::MatD &RbEst) const
    {
        // [U,~,V]=svd(c*q');
    ...
    void SsomProblem::recoverRiTilde(const SomUtils::MatD &RiTilde2, const SomUtils::MatD &Tijtilde,
    ...
    void SsomProblem::dijkstraBT(int src, int n,
    ...
    void SsomProblem::dijkstraBTedges(int src, int n,
    ...
    void SsomProblem::dijkstraSP(int n, int src,
    ...
    void SsomProblem::edgeDiffs2T(int src,
    ...
    bool SsomProblem::recoverySEdN(int staircaseStepIdx,
    ...
    bool SsomProblem::globalize(int src,
