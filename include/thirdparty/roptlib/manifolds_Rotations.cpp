...
namespace ROPTLIB
{
    ...
    Rotations::Rotations(integer inn)
    {
        HasHHR = false;

        metric = ROT_EUCLIDEAN;
        retraction = ROT_QF;
        VecTran = ROT_PARALLELIZATION;
        IsIntrApproach = true;

        n = inn;
        ExtrinsicDim = n * n;
        IntrinsicDim = n * n - n * (n + 1) / 2;
        name.assign("Rotations");
    ...
    void Rotations::ChooseParamsSet1(void)
    {
        metric = ROT_EUCLIDEAN;
        retraction = ROT_QF;
        VecTran = ROT_PARALLELIZATION;
    ...
    void Rotations::ChooseParamsSet2(void)
    {
        metric = ROT_EUCLIDEAN;
        retraction = ROT_QF;
        VecTran = ROT_PROJECTION;
        IsIntrApproach = false;
        HasHHR = false;
    };

    void Rotations::ChooseParamsSet3(void)
    {
        metric = ROT_EUCLIDEAN;
        retraction = ROT_POLAR;
        VecTran = ROT_PARALLELIZATION;
        IsIntrApproach = true;
        HasHHR = false;
    };

    void Rotations::ChooseParamsSet4(void)
    {
        metric = ROT_EUCLIDEAN;
        retraction = ROT_POLAR;
        VecTran = ROT_PROJECTION;
    ...
    // void Rotations::ChooseParamsSet5(void)
    // {
    //     metric = ROT_EUCLIDEAN;
    //     retraction = ROT_CAYLEYR;
    //     VecTran = ROT_CAYLEYVT;
    //     IsIntrApproach = false;
    //     HasHHR = false;
    // };

    void Rotations::CheckParams(void) const
    {
        std::string StieMetricnames[ROTMETRICLENGTH] = {"EUCLIDEAN", "CANONICAL"};
        // std::string StieRetractionnames[ROTRETRACTIONLENGTH] = { "QF", "POLAR", "EXP", "CAYLEYR" };
        std::string StieRetractionnames[ROTRETRACTIONLENGTH] = {"QF", "POLAR", "EXP"};
        // std::string StieVectorTransportnames[ROTVECTORTRANSPORTLENGTH] = { "PARALLELIZATION", "RIGGING", "PARALLELTRANSLATION", "PROJECTION", "CAYLEYVT" };
        std::string StieVectorTransportnames[ROTVECTORTRANSPORTLENGTH] = {"PARALLELIZATION", "RIGGING", "PARALLELTRANSLATION", "PROJECTION"};
    ...
    realdp Rotations::Metric(const Variable &x, const Vector &etax, const Vector &xix) const
    {
        if (metric == ROT_EUCLIDEAN || IsIntrApproach)
            return Manifold::Metric(x, etax, xix);
        else if (metric == ROT_CANONICAL)
        {
            Vector tmp1(n, n);
            tmp1.AlphaABaddBetaThis(1, etax, GLOBAL::T, x, GLOBAL::N, 0); /* tmp1 = etax.GetTranspose() * x;*/
            Vector tmp2(n, n);
            tmp2.AlphaABaddBetaThis(1, xix, GLOBAL::T, x, GLOBAL::N, 0); /* tmp2 = xix.GetTranspose() * x;*/
    ...
    Variable Rotations::RandominManifold(void) const
    {
        Variable result(n, n);
        result.RandGaussian();
        result.QRDecom();
    ...
    Vector &Rotations::Projection(const Variable &x, const Vector &etax, Vector *result) const
    {
        if (IsIntrApproach)
    ...
    Vector &Rotations::IntrProjection(const Variable &x, const Vector &etax, Vector *result) const
    {
        *result = etax;
        return *result;
    };

    Vector &Rotations::ExtrProjection(const Variable &x, const Vector &etax, Vector *result) const
    {
        Vector tmp(n, n);
    ...
    Variable &Rotations::Retraction(const Variable &x, const Vector &etax, Variable *result) const
    {
        if (retraction == ROT_QF)
            return qfRetraction(x, etax, result);

        if (retraction == ROT_POLAR)
            return PolarRetraction(x, etax, result);

        // if (retraction == ROT_CAYLEYR)
    ...
    Vector &Rotations::InvRetraction(const Variable &x, const Variable &y, Vector *result) const
    {
        if (retraction == ROT_POLAR)
    ...
    Vector &Rotations::coTangentVector(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
        if (retraction == ROT_QF)
    ...
    Vector &Rotations::DiffRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        if (retraction == ROT_QF)
    ...
    realdp Rotations::Beta(const Variable &x, const Vector &etax) const
    {
        if (!HasHHR)
            return 1;
    ...
    Vector &Rotations::VectorTransport(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result) const
    {
        if (VecTran == ROT_PARALLELIZATION && !HasHHR)
        {
            return Manifold::VectorTransport(x, etax, y, xix, result);
        }

        if (VecTran == ROT_PROJECTION && !HasHHR)
            return Projection(y, xix, result);

        // if (VecTran == ROT_CAYLEYVT && !HasHHR)
    ...
    Vector &Rotations::InverseVectorTransport(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
        if (VecTran == ROT_PARALLELIZATION && !HasHHR)
        ...
        if (VecTran == ROT_PROJECTION && !HasHHR)
        {
            printf("Rotations::InverseVectorTransport: inverse vector transport by projection has not been done!\n");
            return Manifold::InverseVectorTransport(x, etax, y, xiy, result);
        }
        ...
        printf("Error: Rotations::InverseVectorTransport has not been done!\n");
        return Manifold::InverseVectorTransport(x, etax, y, xiy, result);
    };

    LinearOPE &Rotations::HInvTran(const Variable &x, const Vector &etax, const Variable &y, const LinearOPE &Hx, integer start, integer end, LinearOPE *result) const
    {
        if (VecTran == ROT_PARALLELIZATION && !HasHHR)
        ...
        if (VecTran == ROT_PROJECTION && !HasHHR)
        {
            printf("Rotations::HInvTran for vector transport by projection has not been done!\n");
            return Manifold::HInvTran(x, etax, y, Hx, start, end, result);
        }

        // if (VecTran == ROT_CAYLEYVT && !HasHHR)
        // {
        // 	printf("Rotations::HInvTran for Cayley vector transport has not been done!\n");
        //     return Manifold::HInvTran(x, etax, y, Hx, start, end, result);
        // }
    ...
    LinearOPE &Rotations::TranH(const Variable &x, const Vector &etax, const Variable &y, const LinearOPE &Hx, integer start, integer end, LinearOPE *result) const
    {
        if (VecTran == ROT_PARALLELIZATION && !HasHHR)
        ...
        if (VecTran == ROT_PROJECTION && !HasHHR)
        {
            printf("Rotations::TranH for vector transport by projection has not been done!\n");
            return Manifold::TranH(x, etax, y, Hx, start, end, result);
        }

        // if (VecTran == ROT_CAYLEYVT && !HasHHR)
        // {
        // 	printf("Rotations::TranH for Cayley vector transport has not been done!\n");
        //     return Manifold::TranH(x, etax, y, Hx, start, end, result);
        // }
        ...
        printf("Error: Rotations::TranH has not been done!\n");
        return Manifold::TranH(x, etax, y, Hx, start, end, result);
    };

    LinearOPE &Rotations::TranHInvTran(const Variable &x, const Vector &etax, const Variable &y, const LinearOPE &Hx, LinearOPE *result) const
    {
        if (VecTran == ROT_PARALLELIZATION && !HasHHR)
        ...
        if (VecTran == ROT_PROJECTION && !HasHHR)
        {
            printf("Warning: Rotations::TranHInvTran for vector transport by projection has not been done!\n");
            return Manifold::TranHInvTran(x, etax, y, Hx, result);
        }

        // if (VecTran == ROT_CAYLEYVT && !HasHHR)
        // {
        // 	printf("Warning: Rotations::TranHInvTran for Cayley vector transport has not been done!\n");
        //     return Manifold::TranHInvTran(x, etax, y, Hx, result);
        // }
        ...
        printf("Error: Rotations::TranHInvTran has not been done!\n");
        return Manifold::TranHInvTran(x, etax, y, Hx, result);
    };

    Vector &Rotations::EucGradToGrad(const Variable &x, const Vector &egf, const Problem *prob, Vector *result) const
    {
        if (prob->GetUseHess())
        {
        ...
        /*Canonical metric*/
        Vector tmp(n, n);
        tmp.AlphaABaddBetaThis(1, egf, GLOBAL::T, x, GLOBAL::N, 0);
        *result = egf;
    ...
    Vector &Rotations::EucHvToHv(const Variable &x, const Vector &etax, const Vector &exix, const Problem *prob, Vector *result) const
    {
        Vector EGrad = x.Field("EGrad");
        if (metric == ROT_EUCLIDEAN)
        {
            Vector tmp(n, n);
            tmp.AlphaABaddBetaThis(1, x, GLOBAL::T, EGrad, GLOBAL::N, 0); /* tmp = x.GetTranspose() * EGrad; */
            tmp = (tmp + tmp.GetTranspose()) / 2;
            *result = exix;
            result->AlphaABaddBetaThis(-1, etax, GLOBAL::N, tmp, GLOBAL::N, 1);
            return ExtrProjection(x, *result, result);
        }
        /*Canonical metric*/
        Vector tmp(n, n);
        tmp.AlphaABaddBetaThis(1, EGrad, GLOBAL::T, etax, GLOBAL::N, 0); /*tmp = EGrad.GetTranspose() * etax;*/
        Vector tmp2(n, n);
        tmp2.AlphaABaddBetaThis(1, x, GLOBAL::T, EGrad, GLOBAL::N, 0); /*tmp2 = x.GetTranspose() * EGrad;*/
    ...
    Vector &Rotations::ObtainIntr(const Variable &x, const Vector &etax, Vector *result) const
    {
        if (retraction == ROT_QF || retraction == ROT_POLAR)
    ...
    Vector &Rotations::ObtainExtr(const Variable &x, const Vector &intretax, Vector *result) const
    {
        if (retraction == ROT_QF || retraction == ROT_POLAR)
    ...
    Vector &Rotations::ObtainNorVerIntr(const Variable &x, const Vector &etax, Vector *result) const
    {

        Vector xTetax(n, n);
        xTetax.AlphaABaddBetaThis(1, x, GLOBAL::T, etax, GLOBAL::N, 0); /*xTetax  = x.GetTranspose() * etax; */
        ...
        for (integer i = 0; i < n; i++)
        {
            ...
            {
                resultptr[idx] = (xTetaxptr[i + j * n] + xTetaxptr[j + i * n]) / r2;
                idx++;
            }
        }
        return *result;
    ...
    Vector &Rotations::ObtainNorVerExtr(const Variable &x, const Vector &intretax, Vector *result) const
    {
        ...
        for (integer i = 0; i < n; i++)
        {
            xTetaxptr[i + i * n] = intretaxptr[idx];
            idx++;
        }
        for (integer i = 0; i < n; i++)
        {
            for (integer j = i + 1; j < n; j++)
            {
                xTetaxptr[i + j * n] = intretaxptr[idx] / r2;
                xTetaxptr[j + i * n] = xTetaxptr[i + j * n];
                idx++;
            }
        }
        *result = x;
        result->AlphaABaddBetaThis(1, x, GLOBAL::N, xTetax, GLOBAL::N, 0);
        return *result;
    };

    Variable &Rotations::qfRetraction(const Variable &x, const Vector &etax, Variable *result) const
    {
    ...
    Vector &Rotations::qfcoTangentVector(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
        ...
        Vector ytxiy(n, n);
        ytxiy.AlphaABaddBetaThis(1, y, GLOBAL::T, exxiy, GLOBAL::N, 0); /* ytxiy = y.GetTranspose() * exxiy;*/
        realdp *ytxiyptr = ytxiy.ObtainWritePartialData();
        for (integer i = 0; i < n; i++)
        {
            for (integer j = i; j < n; j++)
            {
                ytxiyptr[i + j * n] = -ytxiyptr[i + j * n];
            }
        }

        Vector exresult = exxiy;
        exresult.AlphaABaddBetaThis(1, y, GLOBAL::N, ytxiy, GLOBAL::N, 1); /*exresult = y * ytxiy + exxiy;*/
        ...
        trsm_(GLOBAL::R, GLOBAL::U, GLOBAL::T, GLOBAL::N, &n, &n, &GLOBAL::DONE, const_cast<realdp *>(HHRptr), &n, exresultptr, &n);
    ...
    Vector &Rotations::DiffqfRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        ...
        trsm_(GLOBAL::R, GLOBAL::U, GLOBAL::N, GLOBAL::N, &n, &n, &GLOBAL::DONE, const_cast<realdp *>(HHRptr), &n, exxixptr, &n);
        for (integer i = 0; i < n; i++)
        {
            if (HHRptr[i + i * n] < 0)
        ...
        Vector YtVRinv(n, n);
        YtVRinv.AlphaABaddBetaThis(1, y, GLOBAL::T, exxix, GLOBAL::N, 0); /*YtVRinv = y.GetTranspose() * exxix; */
        realdp *YtVRinvptr = YtVRinv.ObtainWritePartialData();

        for (integer i = 0; i < n; i++)
        {
            YtVRinvptr[i + n * i] = -YtVRinvptr[i + n * i];
            for (integer j = i + 1; j < n; j++)
            {
                YtVRinvptr[i + n * j] = -YtVRinvptr[j + n * i] - YtVRinvptr[i + n * j];
                YtVRinvptr[j + n * i] = 0;
            }
        }
        Vector exresult(exxix);
        exresult.AlphaABaddBetaThis(1, y, GLOBAL::N, YtVRinv, GLOBAL::N, 1); /*exresult = exxix + y * YtVRinv; */
    ...
    Variable &Rotations::PolarRetraction(const Variable &x, const Vector &etax, Variable *result) const
    {
        Vector exetax(EMPTYEXTR);
        if (IsIntrApproach)
            ObtainExtr(x, etax, &exetax);
    ...
    Vector &Rotations::InvPolarRetraction(const Variable &x, const Variable &y, Vector *result) const
    {
        Vector A(n, n);
        A.AlphaABaddBetaThis(1, x, GLOBAL::T, y, GLOBAL::N, 0); /* A = x.GetTranspose() * y; */
        ...
        *result = x;
        result->AlphaABaddBetaThis(1, y, GLOBAL::N, soln, GLOBAL::N, -1); /* y * tIp.SYL(A, A.GetTranspose()) - x; */

        return *result;
    };

    Vector &Rotations::PolarcoTangentVector(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
    ...
    Vector &Rotations::DiffPolarRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        if (IsEtaXiSameDir)
        {
            ...
            Vector tmp2(n, n);
            tmp2.AlphaABaddBetaThis(1, tmp, GLOBAL::T, tmp, GLOBAL::N, 0); /*tmp2 = tmp.GetTranspose() * tmp*/
            Vector exresult(tmp);
            exresult.AlphaABaddBetaThis(-alpha, y, GLOBAL::N, tmp2, GLOBAL::N, alpha); /* exresult = (tmp - y * (tmp.GetTranspose() * tmp)) * alpha; */
        ...
        printf("Warning: Rotations::DiffPolarRetraction: The differentiated retraction of the polar retraction has not been implemented!\n");
        return Manifold::DiffRetraction(x, etax, y, xix, result);
    };

    Vector &Rotations::ObtainIntrHHR(const Variable &x, const Vector &etax, Vector *result) const
    {
        ...
        {
            if (HHRptr[i + n * i] < 0)
                scal_(&n, &GLOBAL::DNONE, tmpptr + i, &n);
        }
    ...
    Vector &Rotations::ObtainExtrHHR(const Variable &x, const Vector &intretax, Vector *result) const
    {
        if (!x.FieldsExist("_HHR"))
        {
            x.HHRDecom();
        ...
        {
            if (HHRptr[i + n * i] < 0)
                scal_(&n, &GLOBAL::DNONE, resultptr + i, &n);
        }
    ...
    void Rotations::SetParams(PARAMSMAP params)
    {
