...
namespace ROPTLIB{
	...
	realdp Rotations::Metric(const Variable &x, const Vector &etax, const Vector &xix) const
	{
        ...
        if (metric == ROT_CANONICAL)
        {
            Vector tmp1(n, n); tmp1.AlphaABaddBetaThis(1, etax, GLOBAL::T, x, GLOBAL::N, 0); /* tmp1 = etax.GetTranspose() * x;*/
            Vector tmp2(n, n); tmp2.AlphaABaddBetaThis(1, xix, GLOBAL::T, x, GLOBAL::N, 0); /* tmp2 = xix.GetTranspose() * x;*/
    ...
    {
        Vector tmp(n, n); tmp.AlphaABaddBetaThis(1, x, GLOBAL::T, etax, GLOBAL::N, 0); /*tmp = x.GetTranspose() * etax*/
	...
	Vector &Rotations::EucGradToGrad(const Variable &x, const Vector &egf, const Problem *prob, Vector *result) const
	{
        ...
        /*Canonical metric*/
        Vector tmp(n, n); tmp.AlphaABaddBetaThis(1, egf, GLOBAL::T, x, GLOBAL::N, 0);
	...
	Vector &Rotations::EucHvToHv(const Variable &x, const Vector &etax, const Vector &exix, const Problem *prob, Vector *result) const
	{
		...
		if (metric == ROT_EUCLIDEAN)
		{
            Vector tmp(n, n); tmp.AlphaABaddBetaThis(1, x, GLOBAL::T, EGrad, GLOBAL::N, 0); /* tmp = x.GetTranspose() * EGrad; */
            tmp = (tmp + tmp.GetTranspose()) / 2;
            *result = exix; result->AlphaABaddBetaThis(-1, etax, GLOBAL::N, tmp, GLOBAL::N, 1);
            return ExtrProjection(x, *result, result);
		}
        /*Canonical metric*/
        Vector tmp(n, n); tmp.AlphaABaddBetaThis(1, EGrad, GLOBAL::T, etax, GLOBAL::N, 0); /*tmp = EGrad.GetTranspose() * etax;*/
        Vector tmp2(n, n); tmp2.AlphaABaddBetaThis(1, x, GLOBAL::T, EGrad, GLOBAL::N, 0); /*tmp2 = x.GetTranspose() * EGrad;*/
    ...
    Vector &Rotations::ObtainNorVerIntr(const Variable &x, const Vector &etax, Vector *result) const
    {
        ...
        Vector xTetax(n, n); xTetax.AlphaABaddBetaThis(1, x, GLOBAL::T, etax, GLOBAL::N, 0); /*xTetax  = x.GetTranspose() * etax; */
    ...
    Vector &Rotations::qfcoTangentVector(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
        ...
        Vector ytxiy(n, n); ytxiy.AlphaABaddBetaThis(1, y, GLOBAL::T, exxiy, GLOBAL::N, 0); /* ytxiy = y.GetTranspose() * exxiy;*/
    ...
    Vector &Rotations::DiffqfRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        ...
        Vector YtVRinv(n, n); YtVRinv.AlphaABaddBetaThis(1, y, GLOBAL::T, exxix, GLOBAL::N, 0); /*YtVRinv = y.GetTranspose() * exxix; */
    ...
    {
        Vector A(n, n); A.AlphaABaddBetaThis(1, x, GLOBAL::T, y, GLOBAL::N, 0); /* A = x.GetTranspose() * y; */
        
        Vector tIp(n, n); tIp.SetToIdentity(); tIp = 2 * tIp;
	...
	Vector &Rotations::DiffPolarRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
	{
		if (IsEtaXiSameDir)
		{
            ...
            Vector tmp2(n, n); tmp2.AlphaABaddBetaThis(1, tmp, GLOBAL::T, tmp, GLOBAL::N, 0); /*tmp2 = tmp.GetTranspose() * tmp*/
