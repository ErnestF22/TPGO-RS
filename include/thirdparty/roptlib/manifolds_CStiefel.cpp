...
namespace ROPTLIB
{
    ...
    CStiefel::CStiefel(integer inn, integer inp)
    {
        ...
        EMPTYEXTR = Vector(n, p, "complexRopt");
    ...
    realdp CStiefel::Metric(const Variable &x, const Vector &etax, const Vector &xix) const
    {
        ...
        else if (metric == CSTIE_CANONICAL)
        {
            Vector tmp1(p, p, "complexRopt");
            tmp1.AlphaABaddBetaThis(1, etax, GLOBAL::C, x, GLOBAL::N, 0); /* tmp1 = etax.GetTranspose() * x;*/
            Vector tmp2(p, p, "complexRopt");
            tmp2.AlphaABaddBetaThis(1, xix, GLOBAL::C, x, GLOBAL::N, 0); /* tmp2 = xix.GetTranspose() * x;*/
    ...
    {
        Variable result(n, p, "complexRopt");
        result.RandGaussian();
        result.QRDecom();
    ...
    {
        Vector tmp(p, p, "complexRopt");
        tmp.AlphaABaddBetaThis(GLOBAL::ZONE, x, GLOBAL::C, etax, GLOBAL::N, GLOBAL::ZZERO); /*tmp = x.GetTranspose() * etax*/
    ...
    Vector &CStiefel::EucGradToGrad(const Variable &x, const Vector &egf, const Problem *prob, Vector *result) const
    {
        ...
        /*Canonical metric*/
        Vector tmp(p, p, "complexRopt");
        tmp.AlphaABaddBetaThis(1, egf, GLOBAL::C, x, GLOBAL::N, 0);
        *result = egf;
    ...
    Vector &CStiefel::EucHvToHv(const Variable &x, const Vector &etax, const Vector &exix, const Problem *prob, Vector *result) const
    {
        ...
        {
            Vector tmp(p, p, "complexRopt");
            tmp.AlphaABaddBetaThis(1, x, GLOBAL::C, EGrad, GLOBAL::N, 0); /* tmp = x.GetTranspose() * EGrad; */
            tmp = (tmp + tmp.GetTranspose()) / 2;
            *result = exix;
        ...
        /*Canonical metric*/
        Vector tmp(p, p, "complexRopt");
        tmp.AlphaABaddBetaThis(1, EGrad, GLOBAL::C, etax, GLOBAL::N, 0); /*tmp = EGrad.GetTranspose() * etax;*/
        Vector tmp2(p, p, "complexRopt");
        tmp2.AlphaABaddBetaThis(1, x, GLOBAL::C, EGrad, GLOBAL::N, 0); /*tmp2 = x.GetTranspose() * EGrad;*/
    ...
    Vector &CStiefel::ObtainNorVerIntr(const Variable &x, const Vector &etax, Vector *result) const
    {
        ...
        Vector xTetax(p, p, "complexRopt");
        xTetax.AlphaABaddBetaThis(1, x, GLOBAL::C, etax, GLOBAL::N, 0); /*xTetax  = x.GetTranspose() * etax; */
    ...
    Vector &CStiefel::DiffqfRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        ...
        Vector YtVRinv(p, p, "complexRopt");
        YtVRinv.AlphaABaddBetaThis(1, y, GLOBAL::C, exxix, GLOBAL::N, 0); /*YtVRinv = y.GetTranspose() * exxix; */
    ...
    {
        Vector A(p, p);
        A.AlphaABaddBetaThis(1, x, GLOBAL::C, y, GLOBAL::N, 0); /* A = x.GetTranspose() * y; */
    ...
    Vector &CStiefel::DiffPolarRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        if (IsEtaXiSameDir)
        {
            ...
            Vector tmp2(p, p, "complexRopt");
            tmp2.AlphaABaddBetaThis(1, tmp, GLOBAL::C, tmp, GLOBAL::N, 0); /*tmp2 = tmp.GetTranspose() * tmp*/
            Vector exresult(tmp);
