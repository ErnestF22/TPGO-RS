...
namespace ROPTLIB
{
    ...
    Vector &Stiefel::EucGradToGrad(const Variable &x, const Vector &egf, const Problem *prob, Vector *result) const
    {
        ...
        {
            std::cout << "STIE_EUCLIDEAN" << std::endl;
            return ExtrProjection(x, egf, result);
        }

        /*Canonical metric*/
        std::cout << "STIE_CANONICAL" << std::endl;
        Vector tmp(p, p);
    ...
    { /* assume extrinsic representation is used for etax */
        ...
        Vector Mk211(p, p);
        Mk211.AlphaABaddBetaThis(0.5, x, GLOBAL::T, etax, GLOBAL::N, 0); /* Mk211 = 0.5 * x.GetTranspose() * etax; */
