...
namespace ROPTLIB
{
	...
	void SolversSMLS::Run(void)
	{
		...
		while ((((!isstop) && iter < Max_Iteration) || iter < Min_Iteration) && LSstatus == LSSM_SUCCESS)
		{
			...
			else
			{
				...
				{
					stepsize = (Prob->GetDomain()->GetIsIntrinsic()) ? LinesearchInput(iter, x1, Prob->GetDomain()->ObtainExtr(x1, eta1, &exeta1), stepsize, initialslope, Prob, this) : LinesearchInput(iter, x1, eta1, stepsize, initialslope, Prob, this);
	...
	void SolversSMLS::ChooseLinesearch(void)
	{
		...
		else if (LineSearch_LS == LSSM_WOLFE)
			Linesearch = &SolversSMLS::LinesearchWolfe;
		else if (LineSearch_LS == LSSM_STRONGWOLFE)
			Linesearch = &SolversSMLS::LinesearchStrongWolfe;
		else if (LineSearch_LS == LSSM_EXACT)
			Linesearch = &SolversSMLS::LinesearchExact;
		else if (LineSearch_LS == LSSM_INPUTFUN)
		{
	...
	void SolversSMLS::InitialStepSize(void)
	{
		...
		else
		{
			...
			case LSSM_BBSTEP:
				...
				if (BBratio == 0)
					stepsize = BB1;
				else if (BBratio == 1 && Num_pre_BB == 0)
				{
					stepsize = BB2;
				}
				else
	...
	void SolversSMLS::LinesearchExact(void)
	{
		...
		{					 /* quasi-Newton for optimizing the scalar function */
			...
			stepsize = a1 + t * dir;
			sf2 = h(); /* evaluete the scalar function at (a1 + t * dir) */
	...
	void SolversSMLS::SetParams(PARAMSMAP params)
	{
		...
		for (iter = params.begin(); iter != params.end(); iter++)
		{
			...
			else if (iter->first == static_cast<std::string>("LS_beta"))
			{
				LS_beta = iter->second;
			}
			...
			else if (iter->first == static_cast<std::string>("Maxstepsize"))
			{
				Maxstepsize = iter->second;
			}
			else if (iter->first == static_cast<std::string>("LS_ratio1"))
			{
				LS_ratio1 = iter->second;
			}
			else if (iter->first == static_cast<std::string>("LS_ratio2"))
			{
			...
			else if (iter->first == static_cast<std::string>("Finalstepsize"))
			{
				Finalstepsize = iter->second;
			}
			else if (iter->first == static_cast<std::string>("Num_pre_funs"))
			{
				Num_pre_funs = static_cast<integer>(iter->second);
			}
			else if (iter->first == static_cast<std::string>("InitSteptype"))
			{
				InitSteptype = static_cast<InitStepsizeSetSM>(static_cast<integer>(iter->second));
			}
			else if (iter->first == static_cast<std::string>("IsPureLSInput"))
			{
				IsPureLSInput = ((static_cast<integer>(iter->second)) != 0);
			}
			else if (iter->first == static_cast<std::string>("PreFunsAccuracy"))
			{
				PreFunsAccuracy = iter->second;
			}
			else if (iter->first == static_cast<std::string>("Num_pre_BB"))
			{
				Num_pre_BB = static_cast<integer>(iter->second);
			}
			else if (iter->first == static_cast<std::string>("BBratio"))
			{
