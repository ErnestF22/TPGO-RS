...
#include "manifolds_SPDManifold.h"
...
namespace ROPTLIB
{
    ...
    Vector &SPDManifold::ObtainExtrAF(const Variable &x, const Vector &intretax, Vector *result) const
    {
        ...
        Vector tmp(n, n);
        tmp.AlphaABaddBetaThis(1, x.Field("_L"), GLOBAL::N, *result, GLOBAL::N, 0);
        result->AlphaABaddBetaThis(1, tmp, GLOBAL::N, x.Field("_L"), GLOBAL::T, 0);
    ...
    { /* dis(x, y) = |log(Lx^{-1} y Lx^{-T})|_F */
        ...
        Vector tmp2(EMPTYEXTR);
        tmp2.AlphaABaddBetaThis(1, tmp, GLOBAL::N, tmp, GLOBAL::T, 0);
        tmp2.EigenDecomSym();
