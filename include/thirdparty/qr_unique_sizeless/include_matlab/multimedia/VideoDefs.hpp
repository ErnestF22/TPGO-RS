...
/* Ratio of the total image size in bytes to the size of a full band for
 * different FourCC Types */
static const double TotalSizeToOneFullBandSizeInBytesRatio[] = {0.0, 3.0, 1.0,
                                                                2.0};
...
typedef struct {
  ...
  /* The following fields refer to the format of the video data as encoded in
   * the file/device */
  double frameRate; /* video num frames per second */
  double
      frameRateComputed; /* video num frames per second, as computed by the
                          * video framework. The reason that this field is
                          * necessary is that on Windows, this number is less
                          * accurate than the frameRate field, yet it is
                          * essential to use this value in methods that
                          * determine which frame to emit. The frameRate field
                          * determines the Sample Time on the Simulink wire, to
                          * be consistent with the UNIX implementation.
                          */
  char fourcc[4]; /* set to the FOURCC code of the video encoding (e.g. RGB,
                     YUV)*/
  ...
  int bandWidth[3];  /* width of the video bands (RGB or intensity/chroma if
                        YCbCr) */
  int bandHeight[3]; /* height of the video bands (RGB or intensity/chroma if
                        YCbCr) */
