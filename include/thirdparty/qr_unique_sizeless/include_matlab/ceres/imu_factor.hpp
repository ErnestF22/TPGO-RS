...
//     Vol. 33, No. 1, pp. 1-21, Feb. 2017, doi: 10.1109/TRO.2016.2597321
//
// See more details at https://ieeexplore.ieee.org/document/7557075

class IMUCostAD {
public:
  IMUCostAD(const Eigen::Vector3d &dPos, const Eigen::Quaterniond &dQuat,
            const Eigen::Vector3d &dVel, const Eigen::Vector3d &g, double dT,
            const Eigen::Matrix3d &dPdBg, const Eigen::Matrix3d &dPdBa,
            const Eigen::Matrix3d &dRdBg, const Eigen::Matrix3d &dVdBg,
            const Eigen::Matrix3d &dVdBa, const Eigen::Vector3d &biasGBar,
            const Eigen::Vector3d &biasABar,
            const Eigen::Matrix<double, 9, 9> &dCov,
            const Eigen::Matrix<double, 6, 6> &bCov,
            const Eigen::Quaterniond &sensorTformQ,
            const Eigen::Vector3d &sensorTformT)
      : m_DeltaPos(dPos), m_DeltaQ(dQuat), m_DeltaVel(dVel), m_G(g),
        m_Duration(dT), m_dPdBg(dPdBg), m_dPdBa(dPdBa), m_dRdBg(dRdBg),
        m_dVdBg(dVdBg), m_dVdBa(dVdBa), m_BiasGyroBar(biasGBar),
        m_BiasAccBar(biasABar), m_SensorTransform_q(sensorTformQ),
        m_SensorTransform_t(sensorTformT)
  {
    m_SqrtDeltaInformation =
        dCov.inverse()
            .llt()
            .matrixL(); // already is discrete-time noise covariance
    m_SqrtBiasProcessInformation =
        (bCov)
            .inverse()
            .llt()
            .matrixL(); // already is discrete-time noise covariance
  ...
  bool operator()(const T *const pose_i, const T *const vel_i,
                  const T *const bias_i, const T *const pose_j,
                  const T *const vel_j, const T *const bias_j,
                  const T *const g_dir, const T *const pose_scale,
                  const T *const sf, T *residuals) const
  {
    ...
    Eigen::Quaternion<T> correctedDeltaQ =
        m_DeltaQ.cast<T>() * deltaQCorrection;

    Eigen::Matrix<T, 3, 1> correctedDeltaVel = m_DeltaVel.cast<T>() +
                                               m_dVdBg.cast<T>() * dBg +
                                               m_dVdBa.cast<T>() * dBa;
    Eigen::Matrix<T, 3, 1> correctedDeltaPos = m_DeltaPos.cast<T>() +
                                               m_dPdBg.cast<T>() * dBg +
                                               m_dPdBa.cast<T>() * dBa;
    ...
    resNavState.template block<3, 1>(0, 0) =
        (qi1.inverse() *
         (s * (pj1 - pi1 - vi * dT) - static_cast<T>(0.5) * g * dT * dT)) -
        correctedDeltaPos; // position
    resNavState.template block<3, 1>(3, 0) =
        static_cast<T>(2.0) *
        (correctedDeltaQ.inverse() * (qi1.inverse() * qj1))
            .vec(); // orientation
    resNavState.template block<3, 1>(6, 0) =
        (qi1.inverse() * ((s * (vj - vi)) - g * dT)) -
        correctedDeltaVel; // velocity
                  ...
                  T *residuals) const
  {
    return this->operator()(
        pose_i, vel_i, bias_i, pose_j, vel_j, bias_j, static_cast<T *>(nullptr),
        static_cast<T *>(nullptr), static_cast<T *>(nullptr), residuals);
  }
  ...
  bool operator()(const T *const pose_i, const T *const vel_i,
                  ...
                  T *residuals) const
  {
    return this->operator()(pose_i, vel_i, bias_i, pose_j, vel_j, bias_j, g_dir,
                            pose_scale, static_cast<T *>(nullptr), residuals);
  }
...
// FactorIMU has the same interface as gtsam::CombinedImuFactor but it does not
// do the same thing. FactorIMU is equivalent to gtsam::ImuFactor plus an
// embedded "between" factor for biases
...
public:
  ...
  FactorIMU(int *ids, double sampleRate, double *gravityAcceleration,
            double *gyroBiasNoise, double *accelBiasNoise, double *gyroNoise,
            double *accelNoise, double *gyroReadings, double *accelReadings,
            size_t numReadings, double *sensorTform = nullptr)
      : Factor()
  {

    m_VariableIDs = std::vector<int>(ids, ids + 6);
    auto dims = std::vector<int>{7, 3, 6, 7, 3, 6};
    auto varTypes = std::vector<int>{
        VariableType::Pose_SE3, VariableType::Vel_3, VariableType::IMU_Bias,
        VariableType::Pose_SE3, VariableType::Vel_3, VariableType::IMU_Bias};
    ...
    setData(sampleRate, gravityAcceleration, gyroBiasNoise, accelBiasNoise,
            gyroNoise, accelNoise, gyroReadings, accelReadings, numReadings,
            sensorTform);
  }

  // set imu readings and imu parameters
  void setData(double sampleRate, double *gravityAcceleration,
               double *gyroBiasNoise, double *accelBiasNoise, double *gyroNoise,
               double *accelNoise, double *gyroReadings, double *accelReadings,
               size_t numReadings, double *sensorTform)
  {
    ...
    m_GyroReadings =
        std::vector<double>(gyroReadings, gyroReadings + numReadings);
    m_AccelReadings =
        std::vector<double>(accelReadings, accelReadings + numReadings);

    m_DeltaT = 1.0 / sampleRate;

    m_GyroscopeNoise = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(
        gyroNoise); // input data vector is assumed row-major
    m_AccelerometerNoise = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(
        accelNoise); // input data vector is assumed row-major

    m_GyroscopeBiasNoise = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(
        gyroBiasNoise); // input data vector is assumed row-major
    m_AccelerometerBiasNoise = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(
        accelBiasNoise); // input data vector is assumed row-major
    if (sensorTform == nullptr) {
      m_SensorTransform =
          Eigen::Matrix<double, 4, 4, Eigen::RowMajor>::Identity();
      m_SensorTransform_q =
          Eigen::Quaterniond(m_SensorTransform.block<3, 3>(0, 0));
      m_SensorTransform_t = m_SensorTransform.block<3, 1>(0, 3);
    } else {
      m_SensorTransform = Eigen::Matrix<double, 4, 4, Eigen::RowMajor>(
          sensorTform); // input data vector is assumed row-major
      m_SensorTransform_q =
          Eigen::Quaterniond(m_SensorTransform.block<3, 3>(0, 0));
      m_SensorTransform_t = m_SensorTransform.block<3, 1>(0, 3);
    }
  ...
  void
  preOptimizationUpdate(std::vector<double *> &currVariableStates) override;
  ...
  ceres::CostFunction *createFactorCostFcn() const override
  {
    return new ceres::AutoDiffCostFunction<IMUCostAD, 15, 7, 3, 6, 7, 3, 6>(
        new IMUCostAD(m_PreIntegratedPos, m_PreIntegratedQuat,
                      m_PreIntegratedVel, m_GravityAcceleration, m_SumDeltaT,
                      m_dPos_dBg, m_dPos_dBa, m_dRot_dBg, m_dVel_dBg,
                      m_dVel_dBa, m_NominalBiasGyro, m_NominalBiasAcc,
                      m_PreIntegCovariance, m_BiasRandWalkCovariance,
                      m_SensorTransform_q, m_SensorTransform_t));
  }
  ...
  getVariableLocalParameterization(int variableID) override
  {
    ...
    else if (m_MapVariableLPTypes.find(variableID)->second ==
             VariableType::Vel_3)
  ...
  void predict(const double *prevBias, const double *prevPose,
               const double *prevVel, double *predictedPose,
               double *predictedVel) const;

  void predictInternal(const double *prevBias, Eigen::Matrix3d &R,
                       Eigen::Vector3d &v, Eigen::Vector3d &p,
...
public:
  FactorIMUGS(int *ids, double sampleRate, double *gravityAcceleration,
              double *gyroBiasNoise, double *accelBiasNoise, double *gyroNoise,
              double *accelNoise, double *gyroReadings, double *accelReadings,
              size_t numReadings, double *sensorTform = nullptr)
  {
  ...
  ceres::CostFunction *createFactorCostFcn() const override
  {
    return new ceres::AutoDiffCostFunction<IMUCostAD, 15, 7, 3, 6, 7, 3, 6, 4,
                                           1>(new IMUCostAD(
        m_PreIntegratedPos, m_PreIntegratedQuat, m_PreIntegratedVel,
        m_GravityAcceleration, m_SumDeltaT, m_dPos_dBg, m_dPos_dBa, m_dRot_dBg,
        m_dVel_dBg, m_dVel_dBa, m_NominalBiasGyro, m_NominalBiasAcc,
        m_PreIntegCovariance, m_BiasRandWalkCovariance, m_SensorTransform_q,
        m_SensorTransform_t));
  }
...
public:
  FactorIMUGST(int *ids, double sampleRate, double *gravityAcceleration,
               double *gyroBiasNoise, double *accelBiasNoise, double *gyroNoise,
               double *accelNoise, double *gyroReadings, double *accelReadings,
               size_t numReadings, double *sensorTform = nullptr)
  {
  ...
  ceres::CostFunction *createFactorCostFcn() const override
  {
    ...
    return new ceres::AutoDiffCostFunction<
        IMUCostAD, numResiduals, poseLength, velLength, biasLength, poseLength,
        velLength, biasLength, quatLength, scaleLength, poseLength>(
        new IMUCostAD(m_PreIntegratedPos, m_PreIntegratedQuat,
                      m_PreIntegratedVel, m_GravityAcceleration, m_SumDeltaT,
                      m_dPos_dBg, m_dPos_dBa, m_dRot_dBg, m_dVel_dBg,
                      m_dVel_dBa, m_NominalBiasGyro, m_NominalBiasAcc,
  ...
  // the internal states of this factor should not be affected
  void predict(const double *prevBias, const double *prevPose,
               const double *prevVel, const double *gRot, const double *scale,
               const double *sensorTform, double *predictedPose,
               double *predictedVel) const;
};
