...
static std::unordered_map<std::string, int> NodeTypeInt{
    {"POSE_SE3", 0},         {"POSE_SE2", 1},      {"POINT_XY", 2},
    {"POINT_XYZ", 3},        {"VEL3", 4},          {"IMU_BIAS", 5},
...
static std::unordered_map<VariableType, std::string> VariableTypeString{
    {VariableType::Pose_SE3, "POSE_SE3"},
    {VariableType::Pose_SE2, "POSE_SE2"},
    {VariableType::Point_XY, "POINT_XY"},
    {VariableType::Point_XYZ, "POINT_XYZ"},
    {VariableType::Vel_3, "VEL3"},
    {VariableType::IMU_Bias, "IMU_BIAS"},
...
public:
  ...
  getVariableLocalParameterization(int variableID) = 0;
  ...
  /// This method is optional, use it only when the factor's internal data needs
  /// to be updated based on the current variable states
...
/// <summary>
///  Base class for non-sensor-specific factors whose measurements follow a
///  zero-mean Gaussian noise model. The factor can involve one ore multiple
///  variable.
/// </summary>
class CERESCODEGEN_API FactorGaussianNoiseModel : public Factor {
public:
  FactorGaussianNoiseModel(std::vector<int> ids, std::vector<int> dims,
  ...
  {
    m_InformationMatrix =
        std::vector<double>(infoMat, infoMat + m_InfoMatLength);
    return true;
  }
