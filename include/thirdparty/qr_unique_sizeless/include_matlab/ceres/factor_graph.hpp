...
struct CERESCODEGEN_API CeresSolverOptions {
  ...
  /// LEVENBERG_MARQUARDT or DOGLEG (default). Note this default is different
  /// from Ceres
  ...
  /// |new_cost - old_cost| < FunctionTolerance * old_cost (cost is always > 0).
  /// (default 1e-6)
  ...
  /// 0 - no printing, 1 - only summary, 2 - per iteration + summary. (default
  /// 1)
  int VerbosityLevel;

  std::unordered_map<int, int> LinearSolverOrdering;
  /// Whether Ceres Solver should update solver parameters after each iteration,
  /// irrespective of solver success This is the case if the
  /// UpdateStateEveryIteration is set to true (default false)
...
// NOTE Ceres problem definition only supports double as "too much pain and
// suffering in single precision land"

/// <summary>
/// A factor graph is a bipartite graph consisting of factors connected to
/// variables. The variables represent the unknown random variables in the
/// estimation problem, whereas the factors represent probabilistic constraints
/// on those variables, derived from measurements or prior knowledge.
/// </summary>
...
public:
  ...
  {
    m_FactorType = {
        {"Two_SE2_F", FactorTypeEnum::Two_SE2_F},
  ...
  std::vector<int> validateFactor(std::unique_ptr<Factor> fctr,
                                  std::vector<int> IDs, int size);

  /// Store node IDs with node type
  void storeNodeIDs(std::unordered_map<int, std::set<int>> &map, int type,
                    std::set<int> IDs);

  /// Store node with factor type
  void storeFactorTypes(
      std::unordered_map<int, std::unordered_map<int, std::set<int>>> &map,
      int fctrType, int node_type, std::set<int> IDs);

  /// Store factor and node with group ID
  void storeGroups(
      std::unordered_map<
          int, std::unordered_map<int, std::unordered_map<int, std::set<int>>>>
          &map,
      int group, int fctrType, int node_type, std::set<int> IDs);

  /// Store information for the factor which can accept two group ID
  void storeByTwoGroupID(std::vector<int> GroupID, const size_t numGroupID,
                         std::vector<int> IDs, const size_t numIds,
                         int fctrType, int nodeType1, int nodeType2);

  /// Store information for the factor which can only accept one group ID
  void storeByOneGroupID(std::vector<int> GroupID, const size_t numGroupID,
                         std::vector<int> IDs, int fctrType, int nodeType,
                         std::set<int> IDset);
  ...
  /// Store factorIMU information
  int storeIMUArray(std::unique_ptr<Factor> fctr, const int *groupID,
                    const size_t numGroupID)
  {
  ...
  std::vector<int> getAllVariableIDs(std::vector<int> groupID,
                                     std::string nodeType,
                                     std::string factorType) const;

  /// Add a gaussian noise model factor
  std::vector<int>
  addGaussianFactor(const char *factorType, const size_t factorTypeLen,
                    const int *ids, const size_t numIds,
                    const double *measurement, const size_t numMeasurement,
                    const double *information, const size_t numInformation,
                    const size_t numFactors, const int *groupID,
                    const size_t numGroupID);

  /// Add factors and get output as an array
  void addGaussianFactorArray(
      const char *factorType, const size_t factorTypeLen, const int *ids,
      const size_t numIds, const double *measurement,
      const size_t numMeasurement, const double *information,
      const size_t numInformation, const size_t numFactors, const int *groupID,
      const size_t numGroupID, double *output, double *outputLen)
  {
    std::vector<int> v = addGaussianFactor(
        factorType, factorTypeLen, ids, numIds, measurement, numMeasurement,
        information, numInformation, numFactors, groupID, numGroupID);
    // fill output array
  ...
  std::vector<int> addCameraProjectionFactor(
      const char *factorType, const size_t factorTypeLen, const int *ids,
      const size_t numIds, const double *measurement,
      const size_t numMeasurement, const double *information,
      const size_t numInformation, const size_t numFactors, const int *groupID,
      const size_t numGroupID, const double *sensorTform);

  /// Add camera projection factors and get output as an array
  void addCameraProjectionFactorArray(
      const char *factorType, const size_t factorTypeLen, const int *ids,
      const size_t numIds, const double *measurement,
      const size_t numMeasurement, const double *information,
      const size_t numInformation, const size_t numFactors, const int *groupID,
      const size_t numGroupID, const double *sensorTform, double *output,
      double *outputLen)
  {
    std::vector<int> v = addCameraProjectionFactor(
        factorType, factorTypeLen, ids, numIds, measurement, numMeasurement,
        information, numInformation, numFactors, groupID, numGroupID,
        sensorTform);
    // fill output array
  ...
  std::vector<int> setVariableState(const std::vector<int> &id,
                                    const std::vector<double> &var, int size);

  template <typename VarType>
  std::vector<int> setVariableState(const std::vector<int> &id,
                                    const VarType &&var, int size)
  {
  ...
  std::vector<double> getVariableState(const std::vector<int> &ids)
  {
    ...
    size_t sz = m_MapOfVariableDims[ids[0]];
  ...
  void getVariableStateArray(const int *vid, int vidNum, double *state,
                             double *stateLen)
  {
  ...
  void setVariableStateArray(const int *vid, int vidNum, double *state,
                             int stateLen, double *output, double *outputLen)
  {
    std::vector<int> id(vid, vid + size_t(vidNum));
  ...
  void getVariableIDsArray(double *output, double *outputLen,
                           const int *groupID, const size_t numgroupID,
                           const char *nodeType, const size_t nodeTypeLen,
                           const char *factorType, const size_t factorTypeLen)
  {
    std::vector<int> gID(groupID, groupID + numgroupID);
  ...
  /// Validate whether all nodes exist in the graph and return -1 for invalid
  /// ones
  ...
  /// Validate whether all factors exist in the graph and return -1 for invalid
  /// ones
  ...
  void fixVariableArray(const int *vid, int vidNum, double *output,
                        double *outputLen)
  {
    std::vector<int> id(vid, vid + size_t(vidNum));
  ...
  void freeVariableArray(const int *vid, int vidNum, double *output,
                         double *outputLen)
  {
    std::vector<int> id(vid, vid + size_t(vidNum));
  ...
  void isVariableFixedArray(const int *vid, int vidNum, double *output,
                            double *outputLen)
  {
    std::vector<int> id(vid, vid + size_t(vidNum));
  ...
  void removeFactorArray(const int *vid, int vidNum, double *removedNodeID,
                         double *removedNodeLen)
  {
    std::vector<int> ids(vid, vid + size_t(vidNum));
  ...
  void removeNodeArray(const int *vid, int vidNum, double *removedID,
                       double *removedIDLen)
  {
    std::vector<int> ids(vid, vid + size_t(vidNum));
  ...
  CeresSolutionInfo optimize(const CeresSolverOptions &optStruct,
                             ...
                             bool storeProblem = false);
  void optimize(double *opts, double *info, int *ids, int vidNum,
                int covarianceTypeNum, double *OptimizedIDs,
                double *OptimizedIDsLen, double *FixedIDs,
                double *FixedIDsLen); // for codegen call

  // Get which nodes are connected by the edges
  std::vector<int> getEdge(int fctrType);

  // Get node IDs for covariance calculation
  std::set<int> getCovarianceNodeID(const std::vector<int> &CovarianceType,
                                    const std::vector<int> &ids);

  // Calculate covariance
  std::vector<double> computeCovariance(ceres::Problem &problem,
                                        const std::set<int> &covarianceNodeID);
  ...
  void getVariableCovarianceArray(const int *vid, int vidNum,
                                  double *covariance, double *covarianceLen)
  {
    std::vector<int> ids(vid, vid + size_t(vidNum));
  ...
  bool isUnselectedPoseNodeIncluded(const std::vector<int> &selectedIDs,
                                    int fID,
                                    const std::vector<int> &includedNodeIDs);

  // Get individual factor residuals for the given node IDs
  std::unordered_map<int, std::unordered_map<int, std::vector<double>>>
  getIndividualFactorResidual(const std::set<int> &ids, int factorType);

  // Get individual factor residuals for the given node IDs
  std::unordered_map<int, std::unordered_map<int, std::vector<double>>>
  getIndividualFactorResidualAssumingStateUnchanged(const std::set<int> &ids,
                                                    int factorType);
...
protected:
  ...
  /// This is currently the map from variable ID to variable (local
  /// parameterization) type
  std::unordered_map<int, int> m_MapOfVariableTypes;

  /// This is currently the map from variable ID to whether this variable is
  /// held constant during graph optimization or not
  ...
  /// Map from factor ID to residual block ID. (Should be reset for each new
  /// problem)
...
FactorGraph::isVariableFixed(const std::vector<int> &ids)
{
  std::vector<int> validness = validateExistence(ids);
  // If all ids are valid
  if (validness.back() != -1) {
...
public:
  AbortSolverCallback(const bool *abortIteration)
      : abortIteration_(abortIteration)
  {
  }
  ~AbortSolverCallback() override = default;
  ceres::CallbackReturnType
  operator()(const ceres::IterationSummary &summary) final;
