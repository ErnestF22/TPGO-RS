...
/// A class that provides function to compute the residual error between two
/// SE(3) poses, expressed as a single composite vector, respectively (i.e.
/// [position, orientation]). The Jacobians are computed analytically using
/// first-order approximation
class PoseSE3CostComposite
    : public ceres::SizedCostFunction<6, 7,
                                      7> { // 6 - residual dimension, 7 - param1
                                           // dimension, 7, param2 dimension
public:
  PoseSE3CostComposite(const std::vector<double> &measuredRelPose,
                       const std::vector<double> &information)
  {
    ...
    m_MeasuredRot = Eigen::Quaterniond(measuredRelPose.data() + 3)
                        .normalized()
                        .toRotationMatrix();
                ...
                double **jacobians) const override
  {
    ...
    Eigen::Matrix3d rot_i =
        Eigen::Quaterniond(parameters[0] + 3).normalized().toRotationMatrix();
    Eigen::Vector3d pos_j(parameters[1]);
    Eigen::Matrix3d rot_j =
        Eigen::Quaterniond(parameters[1] + 3).normalized().toRotationMatrix();
    ...
    if (jacobians) {
      if (jacobians[0]) {
        Eigen::Map<Eigen::Matrix<double, 6, 7, Eigen::RowMajor>> jac_i(
            jacobians[0]); // Jacobian w.r.t. to pose i
        // Jac_i:
        //     [ ddeltaP/dpos_i, ddeltaP/dphi_i]
        //     [ ddeltaR/dpos_i, ddeltaR/dphi_i]
        jac_i.block<3, 3>(3, 3) =
            -mw_ceres::SO3::rightJacobianLogm(residualsVec.tail(3)) *
        ...
        jac_i.block<3, 3>(0, 3) =
            mw_ceres::SO3::hat<double>(tmp); // ddeltaP / dphi_i
        jac_i.block<3, 3>(0, 0) =
            -Eigen::Matrix3d::Identity(); // ddeltaP / dpos_i
        jac_i.col(6).setZero();
      ...
      if (jacobians[1]) {
        ...
        Eigen::Map<Eigen::Matrix<double, 6, 7, Eigen::RowMajor>> jac_j(
            jacobians[1]); // Jacobian w.r.t. to pose j
        jac_j.block<3, 3>(3, 3) = mw_ceres::SO3::rightJacobianLogm(
            residualsVec.tail(3));                           // ddeltaR/dphi_j
...
/// A factor that relates between two SE(3) poses (each pose is expressed as a
/// single composite vector) using PoseSE3CostComposite as residual cost
/// function
class FactorTwoPoseSE3Composite : public FactorGaussianNoiseModel {
public:
  FactorTwoPoseSE3Composite(std::vector<int> ids)
      : FactorGaussianNoiseModel(
            ids, {7, 7},
            {VariableType::Pseudo_Pose_SE3, VariableType::Pseudo_Pose_SE3})
  {
...
/// Functor to compute the residual cost between two SE(3) poses (expressed as
/// split variables) based on quaternion formulation. The Jacobians are computed
/// through auto-diff.
class PoseSE3CostSplitAD {
public:
  PoseSE3CostSplitAD(const vector<double> &measuredRelPose,
  ...
  template <typename T>
  bool operator()(const T *const pos_i, const T *const quat_i,
                  const T *const pos_j, const T *const quat_j,
                  T *residuals) const
  {
    ...
    // compute the difference between the estimated orientation change and the
    // measured orientation change
    Eigen::Quaternion<T> q = qi.conjugate() * qj;
    Eigen::Quaternion<T> dq = m_MeasuredQuat.template cast<T>() * q.conjugate();
    // also possible
    // Eigen::Quaternion<T> dq = m_MeasuredQuat.template cast<T>().conjugate() *
    // q;
    ...
    Eigen::Matrix<T, 3, 3> R =
        (m_MeasuredRot.transpose().template cast<T>()) * Ri.transpose() * Rj;
...
public:
  FactorTwoPosesSE3SplitAD(std::vector<int> ids)
      : FactorGaussianNoiseModel(
            ids, {3, 4, 3, 4},
...
/// Alternative functor to compute the residual cost between two SE(3) poses
/// (expressed as split variables) based on matrix exponentials. The Jacobians
/// are computed through auto-diff.
class PoseSE3CostSplitAD_Alt {
public:
  PoseSE3CostSplitAD_Alt(const vector<double> &measuredRelPose,
                         const vector<double> &information)
  {
...
public:
  FactorTwoPosesSE3SplitAD_Alt(std::vector<int> ids)
      : FactorTwoPosesSE3SplitAD(ids)
  {
  ...
  ceres::CostFunction *createFactorCostFcn() const override
  {
    return new ceres::AutoDiffCostFunction<PoseSE3CostSplitAD_Alt, 6, 3, 4, 3,
                                           4>(
  ...
  {
    if (m_MapVariableLPTypes.find(variableID)->second ==
        VariableType::Orientation_SO3_AD)
...
/// A class that provides function to compute the residual error between two
/// SE(3) poses. Each SE(3) pose is expressed as two split variables (i.e. one
/// for position one for orientation quaternion). The Jacobians are computed
/// analytically using first-order approximation
class PoseSE3CostSplit : public ceres::SizedCostFunction<6, 3, 4, 3, 4> {
public:
  PoseSE3CostSplit(const std::vector<double> &measuredRelPose,
                   const std::vector<double> &information)
  {
    // relative pose: [x, y, z, qx, qy, qz, qw]
    m_MeasuredPos << measuredRelPose[0], measuredRelPose[1], measuredRelPose[2];
    ...
    // assuming information matrix is stored row-major
    m_SqrtInformation =
        Eigen::Matrix<double, 6, 6, Eigen::RowMajor>(information.data()).sqrt();
  }
                ...
                double **jacobians) const override
  {
    Eigen::Vector3d pos_i(parameters[0]);
    Eigen::Matrix3d rot_i =
        Eigen::Quaterniond(parameters[1]).normalized().toRotationMatrix();
    Eigen::Vector3d pos_j(parameters[2]);
    Eigen::Matrix3d rot_j =
        Eigen::Quaterniond(parameters[3]).normalized().toRotationMatrix();
    ...
    if (jacobians) {
      if (jacobians[0]) { // Jacobian w.r.t. to pos_i
        Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_pos_i(
            jacobians[0]);
        // jac_pos_i:
        //     [ ddeltaP/dpos_i]
        //     [ ddeltaR/dpos_i]
        ...
        jac_pos_i.block<3, 3>(3, 0) =
            Eigen::Matrix3d::Zero(); // ddeltaR / dpos_i

        jac_pos_i.applyOnTheLeft(m_SqrtInformation);
      }

      if (jacobians[1]) { // Jacobian w.r.t. to quat_i
        Eigen::Map<Eigen::Matrix<double, 6, 4, Eigen::RowMajor>> jac_quat_i(
            jacobians[1]);
        // Jac_quat_i:
        //     [ ddeltaP/dphi_i]
        //     [ ddeltaR/dphi_i]
        Eigen::Vector3d tmp = rot_i.transpose() * (pos_j - pos_i);
        jac_quat_i.block<3, 3>(0, 0) =
            mw_ceres::SO3::hat<double>(tmp); // ddeltaP / dphi_i
        jac_quat_i.block<3, 3>(3, 0) =
            -mw_ceres::SO3::rightJacobianLogm(residualsVec.tail(3)) *
            rot_j.transpose() * rot_i; // ddeltaR / dphi_i
      ...
      if (jacobians[2]) { // Jacobian w.r.t. to pos_j
        Eigen::Map<Eigen::Matrix<double, 6, 3, Eigen::RowMajor>> jac_pos_j(
            jacobians[2]);
        // Jac_pos_j:
        //     [ ddeltaP/dpos_j]
        //     [ ddeltaR/dpos_j]
        jac_pos_j.block<3, 3>(0, 0) = rot_i.transpose(); // ddeltaP / dpos_j
        jac_pos_j.block<3, 3>(3, 0) =
            Eigen::Matrix3d::Zero(); // ddeltaR / dpos_j

        jac_pos_j.applyOnTheLeft(m_SqrtInformation);
      }
      if (jacobians[3]) { // Jacobian w.r.t. to quat_j
        Eigen::Map<Eigen::Matrix<double, 6, 4, Eigen::RowMajor>> jac_quat_j(
            jacobians[3]);
        jac_quat_j.block<3, 3>(0, 0) =
            Eigen::Matrix3d::Zero(); // ddeltaP / dphi_j
        jac_quat_j.block<3, 3>(3, 0) = mw_ceres::SO3::rightJacobianLogm(
            residualsVec.tail(3)); // ddeltaR/dphi_j
...
/// A factor that relates between two SE(3) poses (each pose is expressed as two
/// split variables) using PoseSE3CostSplit as residual cost function
class FactorTwoPosesSE3Split : public FactorGaussianNoiseModel {
public:
  FactorTwoPosesSE3Split(std::vector<int> ids)
      : FactorGaussianNoiseModel(
            ids, {3, 4, 3, 4},
            {VariableType::Point_XYZ, VariableType::Orientation_SO3,
             VariableType::Point_XYZ, VariableType::Orientation_SO3})
  {
    m_MeasurementLength = 7;
    m_InfoMatLength = 36;
...
/// Functor to compute the residual cost between two SE(2) poses (each SE(2)
/// pose is expressed as two split variables). The Jacobians are computed
/// through auto-diff.
class PoseSE2CostSplitAD {
public:
  PoseSE2CostSplitAD(const std::vector<double> &measuredRelPose,
                     const std::vector<double> &information)
  {
  ...
  bool operator()(const T *const pos_i, const T *const theta_i,
                  const T *const pos_j, const T *const theta_j,
                  T *residuals) const
  {
...
/// pose is expressed as two split variables) using PoseSE2CostSplitAD as
/// residual cost function
class FactorTwoPosesSE2SplitAD : public FactorGaussianNoiseModel {
public:
  FactorTwoPosesSE2SplitAD(std::vector<int> ids)
      : FactorGaussianNoiseModel(ids, {2, 1, 2, 1},
                                 {VariableType::Point_XY, VariableType::Angle,
                                  VariableType::Point_XY, VariableType::Angle})
  {
  ...
  getVariableLocalParameterization(int variableID) override
  {
    if (m_MapVariableLPTypes.find(variableID)->second == VariableType::Point_XY)
      ...
      return new ceres::AutoDiffLocalParameterization<mw_ceres::AngleOPlus, 1,
                                                      1>();
  }
