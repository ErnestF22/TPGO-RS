...
template <typename T> T wrapToPi(T theta)
{
  T twoPi = T(2.0 * M_PI);
  return theta - ceres::floor((theta + T(M_PI)) / twoPi) *
                     twoPi; // must use ceres::floor (cannot use %)
}
...
class PseudoSE3LocalParameterization : public ceres::LocalParameterization {
  ...
  virtual bool Plus(const double *x, const double *delta,
                    double *x_oplus_delta) const override;

  /**
   * @brief Computes Jacobian D(Oplus(x, delta_x))/D(delta_x), i.e.
   *        global-to-local Jacobian (from manifold variable to local tangent
   * space variable)
...
/// Functor to compute oplus(x, delta) for SO(3) orientation (through matrix
/// exponential multiplied on the right) To be used with
...
public:
  ...
  bool operator()(const T *x, const T *delta, T *x_oplus_delta) const
  {
    ...
    Eigen::Matrix<T, 3, 3> R =
        quat.normalized().toRotationMatrix() * mw_ceres::SO3::expm<T>(domega);
...
inline bool mw_ceres::PseudoSE3LocalParameterization::Plus(
    const double *x, const double *delta, double *x_oplus_delta) const
{
  ...
  // while internally the coefficients are stored in the following order: [x, y,
  // z, w]
...
inline bool mw_ceres::PseudoSE3LocalParameterization::ComputeJacobian(
...
inline bool
mw_ceres::SO3LocalParameterization::Plus(const double *x, const double *delta,
                                         double *x_oplus_delta) const
{
  // x comes in as [qx, qy, qz, qw]
  Eigen::Map<const Eigen::Quaterniond> quat(x);
  const Eigen::Vector3d domega(delta);
  Eigen::Matrix3d R =
      quat.normalized().toRotationMatrix() * mw_ceres::SO3::expm(domega);
  Eigen::Quaterniond quatOut(R);
  quatOut.normalize();
  x_oplus_delta[0] = quatOut.x();
  x_oplus_delta[1] = quatOut.y();
...
inline bool
mw_ceres::SO3LocalParameterization::ComputeJacobian(const double *x,
