...
public:
  PinholeCameraSE3Point3ReprojectionCost(
      const std::vector<double> &observedPosition,
      const std::vector<double> &information,
      const std::vector<double> &sensorTform)
  {
    // m_Observed is the observed position of the projection point in camera
    // frame
  ...
  bool operator()(const T *const camera, const T *const pos, T *residuals) const
  {
    ...
    Eigen::Matrix<T, 3, 1> p =
        qs * qi.conjugate() * (xi - pi) + m_SensorTransform_t.cast<T>();
    ...
    residuals[0] =
        r_x * m_SqrtInformation(0, 0) + r_y * m_SqrtInformation(1, 0);
    residuals[1] =
        r_y * m_SqrtInformation(0, 1) + r_y * m_SqrtInformation(1, 1);
...
// This class represents a factor that relates between an camera and an R(3)
// point using PinholeCameraSE3Point3ReprojectionCost as residual cost function
class CERESCODEGEN_API FactorCameraSE3AndPointXYZ
    : public FactorGaussianNoiseModel {
public:
  FactorCameraSE3AndPointXYZ(std::vector<int> ids)
            ...
            ids, {7, 3}, {VariableType::Pose_SE3, VariableType::Point_XYZ})
  {
    ...
    Eigen::Map<Eigen::Matrix<double, 4, 4, Eigen::RowMajor>> matS(
        m_SensorTransform.data());
  ...
  ceres::CostFunction *createFactorCostFcn() const override
  {
    return new ceres::AutoDiffCostFunction<
        PinholeCameraSE3Point3ReprojectionCost, 2, 7, 3>(
        new PinholeCameraSE3Point3ReprojectionCost(
            m_Measurement, m_InformationMatrix, m_SensorTransform));
  }
  ...
  ceres::LossFunctionWrapper *createFactorLossFcn() const override
  {
    if (lossParameter < 0)
      ...
      return new ceres::LossFunctionWrapper(new ceres::HuberLoss(lossParameter),
  ...
  // Loss function parameter used to define the loss function. Most loss
  // functions supported in Ceres only takes one parameter which defines how
  // large residuals are reduced. To create a valid loss function, lossParameter
  // must be postive. Otherwise, no loss function is created.
