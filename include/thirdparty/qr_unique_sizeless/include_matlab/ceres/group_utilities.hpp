...
inline Eigen::Matrix<T, 3, 3> hat(const Eigen::Matrix<T, 3, 1> &phi)
{
  ...
  S << static_cast<T>(0), -phi[2], phi[1], phi[2], static_cast<T>(0), -phi[0],
...
/// i.e. Jr as in (Exp(phi + delta_phi)) /approx Exp(phi) *
/// Exp(Jr(phi)*delta_phi)
inline Eigen::Matrix3d rightJacobianExpm(const Eigen::Vector3d &phi)
{
  ...
  if (thetaSq < std::numeric_limits<double>::epsilon()) {
    ...
    return Eigen::Matrix3d::Identity() - (oneMinusCosTheta / theta) * K +
           (1 - ceres::sin(theta) / theta) * K * K;
  }
}
...
inline Eigen::Matrix<T, 3, 1> logm(const Eigen::Matrix<T, 3, 3> &R)
{
  ...
  T mag;
  if (tr <
      static_cast<T>(-1.0 + 1e-10)) { // when trace(R) is near -1, cos(theta) ->
                                      // -1, i.e. theta = +/- pi
    // In such case we cannot extract the rotation axis using the regular
    // formula if n is the rotation axis (unit vector), and v is an arbitrary
    // vector to be rotated by pi, we have identity (R + I)v = 2*n*(n.dot(v)) =
    // 2*(n*n')*v i.e. 0.5*(R + I) = n*n'
    Eigen::Matrix<T, 3, 3> M =
        static_cast<T>(0.5) * (R + Eigen::Matrix<T, 3, 3>::Identity());
...
/// i.e. Jr_inv as in Logm(Exp(phi)*Exp(delta_phi)) /approx phi +
/// Jr_inv*delta_phi
inline Eigen::Matrix3d rightJacobianLogm(const Eigen::Vector3d &phi)
{
  ...
  return Eigen::Matrix3d::Identity() + 0.5 * phiHat +
         (1 / thetaSq -
          (1 + ceres::cos(theta)) / (2 * theta * ceres::sin(theta))) *
             phiHat * phiHat;
}
} // namespace SO3
