...
public:
  IMUPreIntegratorBase(const Eigen::Vector3d &biasGyro,
                       const Eigen::Vector3d &biasAcc, double dT)
      : m_BiasGyro(biasGyro), m_BiasAcc(biasAcc), m_DeltaT(dT)
  {
...
public:
  OnManifoldPreIntegrator(const Eigen::Vector3d &biasGyro,
                          const Eigen::Vector3d &biasAcc, double dT)
      : IMUPreIntegratorBase(biasGyro, biasAcc, dT)
  {
                            ...
                            const Eigen::Vector3d &accMeasurement) override
  {
    ...
    Eigen::Vector3d correctedAccMeasurement =
        accMeasurement - m_BiasAcc; // Note that accMeasurement here includes
                                    // the gravity acceleration in local frame
    ...
    Eigen::Matrix3d tmp1 =
        m_Rot * mw_ceres::SO3::hat<double>(correctedAccMeasurement);
    ...
    m_dRot_dBiasG =
        dRot.transpose() * m_dRot_dBiasG - rightJac_dTheta * m_DeltaT;

    // update covariance for pre-integrated measurements
    // noise propagation is determined by the linear system dx_(i+1) = A * dx_i
    // + B * u where dx = [ dPos, dphi, dvel]' and u = [ ita_gd, ita_ad]'
    ...
    // raw IMU measurement noise covariance matrix, combined from
    // m_GyroscopeNoise & m_AccelerometerNoise
    Eigen::Matrix<double, 6, 6> imuMeasurementNoise =
        Eigen::Matrix<double, 6, 6>::Identity();
    imuMeasurementNoise.block<3, 3>(0, 0) = m_GyroscopeNoise;
    imuMeasurementNoise.block<3, 3>(3, 3) = m_AccelerometerNoise;
    m_Covariance = m_A * m_Covariance * m_A.transpose() +
                   m_B * imuMeasurementNoise * m_B.transpose();

    // we could further consider the error introduced by integrating positions
    // from velocities assuming integrationErrorCov represents the
    // continuous-time integration uncertainty (typically 1e-8 on diagonal),
    ...
    m_Pos +=
        m_Vel * m_DeltaT + 0.5 * m_Rot * correctedAccMeasurement * m_DeltaT_Sq;
  ...
  /** The name dR_dBiasGryo is consistent with the paper notation, but it's a
   * bit misleading. As it does not reflect the linearized change of rotation
   * w.r.t. gyro bias, but instead it is used in this expression: Rot * expm(
   * dR_dBiasGyro * delta_BiasGyro)*/
  ...
  /** Extract Jacobian: change of pre-integrated velocity w.r.t. accelerometer
   * bias*/
  ...
  /** Extract Jacobian: change of pre-integrated position w.r.t. accelerometer
   * bias*/
