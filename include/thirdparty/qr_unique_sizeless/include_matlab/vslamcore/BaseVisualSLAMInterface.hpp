...
public:
  ...
  /**
   * @brief Pure virtual function to close all internal processes and clear all
   * data
   *
   */
   ...
   *
   */
  virtual std::pair<std::vector<int>, std::vector<cv::Matx44d>>
  getCameraPoses() = 0;
  ...
  getViewIMUMeasurements(const int viewId) const = 0;
