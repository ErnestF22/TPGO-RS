...
class LIBMWVSLAMCORE_API MonoVisualSLAMImpl
    : public BaseVisualSLAMImpl<QueueInstanceBase, ConfigurationMono> {
...
public:
  ...
  MonoVisualSLAMImpl(
      ...
      const ConfigurationMono &configuration, const char *vocabFile,
   ...
   * cv::Mat
   */
  void addFrame(const cv::Mat &frame, const cv::Mat &imuG = cv::Mat(),
                const cv::Mat &imuA = cv::Mat());

  /**
   * @brief Stores the results of the scale and gravity direction estimation
   *
   * param[in]: camToLocalIMU, transform containing the rotation needed to align
   * the camera frame with IMU frame. param[in]: poseScale, number used to scale
   * the camera poses to the same metric units as the IMU measurements.
   */
  void storeGravityRotationAndScale(const cv::Matx44d &camToLocalIMU,
...
private:
   ...
   */
  void addNewMapPoints(QueueInstanceBase &qObj,
                       std::vector<int> &refinedViewIds,
                       std::vector<int> &fixedViewIds) override;

  /**
   * @brief Remove recently-created map points that are observed by less than 2
   * views.
   */
  ...
  /**
   * IDs of map points created recently using triangulation from the last key
   * frame
   */
  std::vector<int> lastMapPointIds;

  /**
   * IDs of map points created recently using triangulation from the second last
   * key frame
   */
