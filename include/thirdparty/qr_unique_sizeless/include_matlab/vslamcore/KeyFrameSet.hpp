...
public:
   ...
   */
  std::shared_ptr<Connection> findConnection(const int viewId1,
                                             const int viewId2);

  /**
   * @brief Find a connection by Id
   *
   * @param[in] pairId Id of view1 and view2  in the connection, specified as a
   * pair of integers.
   * @return a pointer to the connection.
   */
   ...
   * @param[out] distances distances associated with the connected views,
   * returned as a vector of integers.
   * @param[in] maxDistance maximum distance from the query view to the
   * connected views, specified as an integer.
   * @param[in] minNumMatches minimum number of matched features between two
   * views for the views to be considered as connected, specified as an integer.
   */
  void connectedViews(const int viewId, std::vector<int> &viewIds,
                      std::vector<int> &distances, const int maxDistance = 1,
                      const int minNumMatches = 0);

  /**
   * @brief Get poses of all views
   *
   * @return a pair of vectors containing the view Ids and the view poses,
   * respectively.
   */
  std::pair<std::vector<int>, std::vector<cv::Matx44d>> getPoses();

  /**
   * @brief Get all the views Ids
   *
   * @return a vector containing the view Ids.
   */
  std::vector<int> getViewIds();
  ...
  /**
   * @brief Get frame index associated with view Id
   *
   * @return frame index, returned as an integer.
   */
  int getFrameIndexByViewId(const int viewId);

  /**
   * @brief Get all the IMU measurements
   *
   * @return all IMU measurements, returned as a map (KF-viewIds), of pairs
   * (frame) of vectors (IMU).
   */
  std::unordered_map<int, std::pair<std::vector<double>, std::vector<double>>>
  getIMUMeasurements();

  /**
   * @brief Get key frame index (not viewId)
   *
   * @return list of key frame indices.
   */
  std::vector<int> getKeyFrameIndex() const;
   ...
   *
   * @param[in] conn connection to be added, specified as a pointer to
   * Connection.
   */
   ...
   */
  void updateViewPose(const int viewId, const cv::Matx33d &ori,
                      const cv::Vec3d &loc);

  /**
   * @brief Update velocity and bias of a view
   *
   * @param[in] viewId view Id, specified as an integer.
   * @param[in] velo new velocity, specified as a std::vector<double>.
   * @param[in] bs new bias, specified as a std::vector<double>.
   */
  void updateVelocityAndBias(const int viewId, const std::vector<double> &velo,
   ...
   */
  void updateConnectionPose(const int viewId1, const int viewId2,
                            const cv::Matx33d &rot, const cv::Vec3d &trans);
   ...
   * @param[in] viewId1 Id of view1, specified as an integer.
   * @param[in] viewId1 Id of view2, specified as an integer.
   * @param[in] matches matched feature pairs specified as a vector of integer
   * pairs.
   */
  void addConnectionMatches(const int viewId1, const int viewId2,
...
private:
   ...
   */
  std::unordered_map<std::pair<int, int>, std::shared_ptr<Connection>, pairHash>
      connections;
