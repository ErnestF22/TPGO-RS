 ...
 * the last key frame.
 * @param[in] qObj, a QueueInstanceBase struct containing data for the current
 * frame.
 * @param[in] config configuration parameters of the SLAM system.
 */

void addNewKeyFrame(MapPointSet &mpSet, KeyFrameSet &kfSet,
                    const QueueInstanceBase &qObj, const Configuration &config);
} // namespace vslam
} // namespace vision
