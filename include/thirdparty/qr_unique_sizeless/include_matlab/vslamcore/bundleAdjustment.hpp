 ...
 * The following class is required to capture all cout statements from ceres
 * library.
 */
struct CoutRedirectForCeres {
  CoutRedirectForCeres(std::streambuf *new_buffer)
 ...
 * @param[in] viewIds Id of camera poses to be refined in kfSet, specified as a
 * vector integers.
 * @param[in] fixedViewIds Id of cameras to be fixed during the optimization,
...
void bundleAdjustment(MapPointSet &mpSet, KeyFrameSet &kfSet,
                      const cv::Matx33d &intrinsics,
                      const std::vector<int> &viewIds,
                      const std::vector<int> &fixedViewIds,
                      const Configuration &config, bool *abortBA = nullptr,
 ...
 * @param[in] currExtrinsics_R rotation matrix from world coordinate to camera
 * coordinate, specified as a cv::Matx33d.
 * @param[in] currExtrinsics_t translation vector from world coordinate to
 * camera coordinate, specified as a cv::Vec3d.
 * @param[in] config contains the configuration of the optimizer.
 * @param[in] imuInfo a struct containing information related to the state of
 * IMU fusion.
...
void bundleAdjustmentMotionOnly(
    const cv::Matx33d &intrinsics, const std::vector<cv::Vec3d> &worldPoints,
    const std::vector<cv::Vec2f> &imagePoints, cv::Matx33d &currExtrinsics_R,
    cv::Vec3d &currExtrinsics_t, const Configuration &config,
    const IMUInfo &imuInfo = IMUInfo(),
