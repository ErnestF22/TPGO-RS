...
public:
   ...
   */
  StereoReconstructor(const cv::Matx33d &intrinsics, const double baseline,
   ...
   *
   * @param[in] qObj a QueueInstanceStereo struct containing data for the
   * current frame
   * @param[out] matchedPairs Indices of matched features, returned as a vector
   * of pairs.
   * @param[out] xyzPoints 3-D world points in the left camera coordinate
   * constructed from stereo images, specified as vector of cv::Vec3d.
   * @param[in] config Configuration struct
   *
   * Notes:
   *   If qObj.disparity is empty, the function will calculate the disparity
   * using cv::StereoSGBM. If qObj.disparity is not empty, it must be a cv::Mat
   * of type CV_32FC1 with the same size as qObj.frameL and qObj.frameR.
   */
  void reconstruct(const QueueInstanceStereo &qObj,
...
private:
  ...
  const int disp12MaxDiff =
      -1; // disable maximum allowed difference in left-right disparity check
