...
public:
   ...
   * @brief Constructor.
   *
   * @param[in] orbVocabularyFilename Name of the file containing OrbVocabulary.
   * File extension is .yml.gz created using class OrbVocabulary from 3p/DBoW2.
   */
  LoopClosureDatabase(const std::string &orbVocabularyFilename)
      : database(orbVocabularyFilename)
  {
  }
   ...
   * @param[out] outFeatures feature descriptors of current key frame in vector
   * form.
   */
  void changeFeaturesStructure(const cv::Mat &plainFeatures,
                               std::vector<cv::Mat> &outFeatures);

  /**
   * @brief Find loop candidate key frames, which are visually
   *        similar to but not connected to the current key frame.
   *
   * @param[in] kfSet keyFrameSet object containing views that observe the world
   * point.
   * @param[in] currKeyFrameId frame ID of the current key frame, specified as
   * an integer.
   * @param[in] currFeatures feature descriptors of current key frame.
   * @param[in] config configuration parameters of the system.
   * @param[out] loopKeyFrameIds Groups of consecutive keyframe IDs forming loop
   * with current key frame.
   * @return true if a loop is detected, otherwise false.
   */
  bool checkLoopClosure(KeyFrameSet &kfSet, const int currKeyFrameId,
                        const std::vector<cv::Mat> &currFeatures,
                        const Configuration &config,
                        std::vector<std::vector<int>> &loopKeyFrameIds);

  /**
   * @brief Add connections between the current key frame and the valid loop
   * candidate key frames.
   *
   * @param[in/out] mpSet a MapPointSet object containing 3-D world points.
   * @param[in/out] kfSet keyFrameSet object containing views that observe the
   * world point.
   * @param[in] intrinsics camera intrinsics matrix, specified as a cv::Matx33d
   * @param[in] currKeyFrameId frame ID of the current key frame, specified as
   * an integer.
   * @param[in] loopKeyFrameIds Groups of consecutive keyframe IDs forming loop
   * with current key frame.
   * @param[in] config configuration parameters of the system.
   * @param[out] loopConnections loop closure connections, specified as a vector
   * of pairs. Each pair contains a pointer to Connection, which contains the
   * relative orientation R and relative translation t, and the corresponding
   * scale s. The relative pose of a loop closure connection is represented as a
   * 3-D similarity transform: | s*R, t | | 0,   1 |
   * @return true if the loop connection is successfully added, otherwise false.
   */
  bool
  addLoopConnections(MapPointSet &mpSet, KeyFrameSet &kfSet,
                     const cv::Matx33d &intrinsics, const int currKeyFrameId,
   ...
   */
  unsigned int addImageFeatures(const int currKeyFrameId,
