...
public:
   ...
   */
  VSLAMLoggerBase(const int _verboseLevel)
      : verboseLevel(_verboseLevel), filename("")
  {
  }
  ...
  VSLAMLoggerBase(const int _verboseLevel, const std::string &_filename)
      : std::ofstream(_filename.c_str(), std::ofstream::out),
        verboseLevel(_verboseLevel), filename(_filename)
  {
  }
