 ...
 *
 * @param[in/out]  points  [in]: M-by-N matrix, containing N M-dimension
 * homogeneous coordinates. [out]: (M-1)-by-N matrix, containing N
 * (M-1)-dimension cartesian coordinates.
 */
template <typename T> void hom2cart(cv::Mat &points)
{
  ...
  for (int i = 0; i < points.cols; i++) {
    T scale = (std::abs(divPtr[i]) > std::numeric_limits<T>::epsilon())
                  ? 1. / divPtr[i]
                  : 1.;
    std::for_each(rowPtrs.begin(), rowPtrs.end(),
                  [&](T *&p) { p[i] *= scale; });
  }
 ...
 *
 * @param[in]   extrinsics_R  rotation matrix from world coordinate to camera
 * coordinate, represented as a cv::Matx<T,3,3>.
 * @param[in]   extrinsics_t  translation vector from world coordinate to camera
 * coordinate, represented as a cv::Vec<T,3>.
 * @param[out]  pose_R     relative rotation between the two views, represented
 * as a cv::Matx<T,3,3>.
 * @param[out]  pose_t     relative translation between the two views,
 * represented as a cv::Vec<T,3>.
 */
template <typename T>
void extr2pose(const cv::Matx<T, 3, 3> &extrinsics_R,
               const cv::Vec<T, 3> &extrinsics_t, cv::Matx<T, 3, 3> &pose_R,
               cv::Vec<T, 3> &pose_t)
{
 ...
 */
template <typename T>
void pose2rt(const cv::Matx<T, 4, 4> &pose, cv::Matx<T, 3, 3> &R,
             cv::Vec<T, 3> &t)
{
  R(0, 0) = pose(0, 0);
  R(0, 1) = pose(0, 1);
...
/**
 * @brief Returns string form of pose
 *
 * @param[in] pose     pose, represented as a cv::Matx<T,4,4>.
 * @returns   String representation of pose.
 */
template <typename T> std::string pose2String(const cv::Matx<T, 4, 4> &pose)
{
  std::string buffer;
...
std::string convertVectorToString(const std::vector<T> &vec)
{
  ...
  for (int i = 0; i < static_cast<int>(vec.size()); i++) {
    ...
    retval = retval + std::to_string(vec[i]);
  }
  retval = retval + " ]";
  return retval;
}
...
template <typename T>
std::string
convertVectorOfVectorToString(const std::vector<std::vector<T>> &vec)
{
  std::string retval = "[ ";
  for (int i = 0; i < static_cast<int>(vec.size()); i++) {
    if (i != 0) {
    ...
    retval = retval + convertVectorToString<T>(vec[i]);
  }
  retval = retval + " ]";
  return retval;
}

inline void unoptimized2OptimizedCurrExtrinsicsUsingLastPose(
    QueueInstanceBase &qObj,
    const cv::Matx<decltype(QueueInstanceBase::lastPose_R)::value_type, 4, 4>
        &lastPose_opt)
{
  // 1. Get inverse of last pose
  decltype(QueueInstanceBase::lastPose_R) lastPose_R_unopt_inv;
  decltype(QueueInstanceBase::lastPose_t) lastPose_t_unopt_inv;
  extr2pose(qObj.lastPose_R, qObj.lastPose_t, lastPose_R_unopt_inv,
            lastPose_t_unopt_inv);
  const auto lastPose_inv(rt2pose(lastPose_R_unopt_inv, lastPose_t_unopt_inv));

  // 2. Get transformation between last and current frames
  decltype(QueueInstanceBase::lastPose_R) currPose_R;
  decltype(QueueInstanceBase::lastPose_t) currPose_t;
  extr2pose(qObj.currExtrinsics_R, qObj.currExtrinsics_t, currPose_R,
            currPose_t);
  const auto currPose(rt2pose(currPose_R, currPose_t));
  const auto transH = lastPose_inv * currPose;

  // 3. Apply transformation to get optimized pose for current frame
  const auto currPose_opt = lastPose_opt * transH;

  // 4. Get optimized extrinsics for current frame
  decltype(QueueInstanceBase::lastPose_R) currPose_opt_R;
  decltype(QueueInstanceBase::lastPose_t) currPose_opt_t;
  pose2rt(currPose_opt, currPose_opt_R, currPose_opt_t);
  extr2pose(currPose_opt_R, currPose_opt_t, qObj.currExtrinsics_R,
