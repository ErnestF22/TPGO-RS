...
class LIBMWVSLAMCORE_API StereoVisualSLAMImpl
    : public BaseVisualSLAMImpl<QueueInstanceStereo, ConfigurationStereo> {
...
public:
  ...
  StereoVisualSLAMImpl(
      ...
      const double baseline, const ConfigurationStereo &configuration,
      const char *vocabFile,
      const VSlamConcurrency desiredLevel = VSlamConcurrency::THREAD_L2);

  /**
   * @brief Add an image frame with optional disparity map to the system and
   ...
   *
   * Notes:
   *   If non-empty, disparity must be of same size as frameL and frameR and be
   * of type CV_32FC1.
   *
   *
   */
  void addFrame(const cv::Mat &frameL, const cv::Mat &frameR,
                const cv::Mat &disparity = cv::Mat());
...
private:
   ...
   */
  void addNewMapPoints(QueueInstanceStereo &qObj,
  ...
  /**
   * StereoReconstructor object used to construct 3-D world points using
   * disparity
   */
