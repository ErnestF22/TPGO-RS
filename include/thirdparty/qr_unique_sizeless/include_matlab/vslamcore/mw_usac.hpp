...
/*
 * MW: This file contains the code derived from
 * //3rdparty/R2023a/OpenCV/modules/calib3d/src/usac.hpp
 */
...
// Symmetric Geometric Distance (to epipolar lines) for Fundamental and
// Essential matrix
...
public:
   ...
   */
  virtual void getNormTransformation(Mat &norm_points,
                                     const std::vector<int> &sample,
                                     int sample_number, Matx33d &T1,
                                     Matx33d &T2) const = 0;
...
public:
  // Estimate models from minimal sample. models.size() == number of found
  // solutions
  virtual int estimate(const std::vector<int> &sample,
                       std::vector<Mat> &models) const = 0;
...
public:
  static Ptr<P3PSolver> create(const Mat &points_, const Mat &calib_norm_pts,
                               const Mat &K);
};
...
public:
  // Estimate models from non minimal sample. models.size() == number of found
  // solutions
...
public:
  static Ptr<DLSPnP> create(const Mat &points_, const Mat &calib_norm_pts,
                            const Mat &K);
};
...
public:
  ...
  // Compare two scores. Objective is minimization of score. Lower score is
  // better.
...
public:
  ...
  virtual int getInliers(const Mat &model, std::vector<int> &inliers,
                         double thr) const = 0;
  ...
  virtual int getInliers(const Mat &model,
                         std::vector<bool> &inliers_mask) const = 0;
...
public:
  static Ptr<RansacQuality> create(int points_size_, double threshold_,
...
public:
  static Ptr<MsacQuality> create(int points_size_, double threshold_,
                                 const Ptr<Error> &error_);
};
...
public:
  static Ptr<MagsacQuality>
  create(double maximum_thr, int points_size_, const Ptr<Error> &error_,
...
public:
  static Ptr<LMedsQuality> create(int points_size_, double threshold_,
                                  const Ptr<Error> &error_);
};
...
public:
   ...
   */
  virtual bool isModelValid(const Mat & /*model*/,
   ...
   */
  virtual bool recoverIfDegenerate(const std::vector<int> & /*sample*/,
                                   const Mat & /*best_model*/,
                                   Mat & /*non_degenerate_model*/,
                                   Score & /*non_degenerate_model_score*/)
  {
    return false;
  }
  virtual Ptr<Degeneracy> clone(int /*state*/) const
  {
    return makePtr<Degeneracy>();
  }
};
...
public:
  ...
  static Ptr<EpipolarGeometryDegeneracy> create(const Mat &points_,
                                                int sample_size_);
};
...
public:
  ...
  static Ptr<FundamentalDegeneracy> create(int state,
                                           const Ptr<Quality> &quality_,
                                           const Mat &points_, int sample_size_,
...
public:
   ...
   */
  virtual int estimateModels(const std::vector<int> &sample,
                             std::vector<Mat> &models) const = 0;
  /*
...
public:
  static Ptr<HomographyEstimator>
  create(const Ptr<MinimalSolver> &min_solver_,
...
public:
  static Ptr<FundamentalEstimator>
  create(const Ptr<MinimalSolver> &min_solver_,
         const Ptr<NonMinimalSolver> &non_min_solver_,
         const Ptr<Degeneracy> &degeneracy_);
};
...
////////////////////////////////////////////
/*
 * Matas, Jiri, and Ondrej Chum. "Randomized RANSAC with sequential probability
 * ratio test." Tenth IEEE International Conference on Computer Vision (ICCV'05)
...
public:
  ...
  static Ptr<SPRT> create(int state, const Ptr<Error> &err_, int points_size_,
                          ...
                          double prob_pt_of_good_model,
                          double prob_pt_of_bad_model, double time_sample,
                          double avg_num_models, ScoreMethod score_type_);
};
...
public:
  static Ptr<RadiusSearchNeighborhoodGraph>
  create(const Mat &points, int points_size, double radius_,
         int flann_search_params, int num_kd_trees);
};

class FlannNeighborhoodGraph : public NeighborhoodGraph {
public:
  static Ptr<FlannNeighborhoodGraph>
  create(const Mat &points, int points_size, int k_nearest_neighbors_,
         bool get_distances, int flann_search_params, int num_kd_trees);
...
public:
  static Ptr<GridNeighborhoodGraph>
  create(const Mat &points, int points_size, int cell_size_x_img1_,
         int cell_size_y_img1_, int cell_size_x_img2_, int cell_size_y_img2_,
         int max_neighbors);
};
...
public:
  static Ptr<UniformSampler> create(int state, int sample_size_,
                                    int points_size_);
};
...
public:
  static Ptr<ProsacSimpleSampler> create(int state, int points_size_,
                                         int sample_size_,
...
public:
  static Ptr<ProsacSampler> create(int state, int points_size_,
                                   int sample_size_, int growth_max_samples);
  // return number of samples generated (for prosac termination).
  virtual int getKthSample() const = 0;
  // return constant reference of growth function of prosac sampler (for prosac
  // termination)
...
public:
  static Ptr<NapsacSampler>
  create(int state, int points_size_, int sample_size_,
...
public:
  static Ptr<StandardTerminationCriteria> create(double confidence,
                                                 int points_size_,
                                                 int sample_size_,
                                                 int max_iterations_);
};
...
public:
  static Ptr<SPRTTermination>
  create(const std::vector<SPRT_history> &sprt_histories_, double confidence,
...
public:
  static Ptr<SPRTPNapsacTermination>
  create(const std::vector<SPRT_history> &sprt_histories_, double confidence,
         int points_size_, int sample_size_, int max_iterations_,
         double relax_coef_);
};
...
public:
  static Ptr<ProsacTerminationCriteria>
  create(const Ptr<ProsacSampler> &sampler_, const Ptr<Error> &error_,
         int points_size_, int sample_size, double confidence, int max_iters,
         int min_termination_length, double beta, double non_randomness_phi,
 ...
 */
void calibratePoints(const Mat &K1, const Mat &K2, const Mat &points,
                     Mat &norm_points);
void calibrateAndNormalizePointsPnP(const Mat &K, const Mat &pts,
                                    Mat &calib_norm_pts);
...
public:
  ...
  // fill @sample of size @subset_size with random numbers in range <0,
  // @max_range)
  virtual void generateUniqueRandomSet(std::vector<int> &sample,
                                       int subset_size, int max_range) = 0;
  // fill @sample of size @sample.size() with random numbers in range <0,
  // @max_range)
  virtual void generateUniqueRandomSet(std::vector<int> &sample,
                                       int max_range) = 0;
  ...
  // return random number from <0, max_range), where max_range is from
  // constructor
  virtual int getRandomNumber() = 0;
  // return random number from <0, max_rng)
  virtual int getRandomNumber(int max_rng) = 0;
  virtual const std::vector<int> &
  generateUniqueRandomSubset(std::vector<int> &array1, int size1) = 0;
...
public:
  ...
  static Ptr<UniformRandomGenerator> create(int state, int max_range,
                                            int subset_size_);
};
...
public:
  static Ptr<GraphCut> create(const Ptr<Estimator> &estimator_,
                              const Ptr<Error> &error_,
                              const Ptr<Quality> &quality_,
                              const Ptr<NeighborhoodGraph> &neighborhood_graph_,
                              const Ptr<RandomGenerator> &lo_sampler_,
                              double threshold_, double spatial_coherence_term,
                              int gc_iters);
};
...
public:
  ...
  create(const Ptr<Estimator> &estimator_, const Ptr<Quality> &quality_,
         ...
         int lo_inner_iterations, int lo_iter_max_iterations,
...
public:
  ...
  create(const Ptr<Estimator> &estimator_, const Ptr<Error> &error_,
         ...
         double sigma_quantile, double upper_incomplete_of_sigma_quantile,
...
public:
   ...
   */
  virtual bool polishSoFarTheBestModel(const Mat &model,
                                       const Score &best_model_score,
                                       Mat &new_model,
                                       Score &new_model_score) = 0;
};
...
public:
  ...
  static Ptr<Model> create(double threshold_, EstimationMethod estimator_,
                           SamplingMethod sampler_, double confidence_ = 0.95,
                           int max_iterations_ = 5000,
                           ScoreMethod score_ = ScoreMethod::SCORE_METHOD_MSAC);
};
...
Mat findFundamentalMat(InputArray points1, InputArray points2, int method,
...
bool solvePnPRansac(InputArray objectPoints, InputArray imagePoints,
                    ...
                    double confidence, OutputArray inliers, int flags);
...
void setParameters(Ptr<Model> &params, EstimationMethod estimator,
                   const UsacParams &usac_params, bool mask_need);
bool run(const Ptr<const Model> &params, InputArray points1, InputArray points2,
         int state, Ptr<RansacOutput> &ransac_output, InputArray K1_,
         InputArray K2_, InputArray dist_coeff1, InputArray dist_coeff2);
} // namespace usac
} // namespace vslam
} // namespace vision
