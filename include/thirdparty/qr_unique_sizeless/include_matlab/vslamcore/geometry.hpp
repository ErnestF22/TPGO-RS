...
template <typename T>
bool checkParallax(const cv::Vec<T, 3> &point3d,
                   const cv::Vec<T, 3> &camLocation1,
                   const cv::Vec<T, 3> &camLocation2, const T threshold)
{

  const cv::Vec<T, 3> ray1 = point3d - camLocation1,
                      ray2 = point3d - camLocation1;
...
template <typename T>
double getMaxDepth(MapPointSet &mpSet, KeyFrameSet &kfSet,
                   const int currKeyFrameId, const cv::Matx<T, 3, 3> &R,
                   const cv::Vec<T, 3> &t)
{
  double maxDepth = 0;
...
bool checkParallax(const cv::Mat &point3d, const cv::Mat &camLocation1,
                   const cv::Mat &camLocation2, const double threshold);
} // namespace vslam
} // namespace vision
