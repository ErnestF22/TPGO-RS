 ...
 *
 *   Note: This is the base class and it is not intended to be initialized
 * explicitly
 */
...
protected:
  ...
  Configuration(const BaseParams &base, const IMUParams &imu,
                const bool isMonoIn);
};

/**
 * @brief Configuration struct containing all parameters for monocular visual
 * SLAM
 */
struct LIBMWVSLAMCORE_API ConfigurationMono : public Configuration {
  ...
  ConfigurationMono();
  ConfigurationMono(const BaseParams &base, const IMUParams &imu,
                    const MonoParams &mono);
...
struct LIBMWVSLAMCORE_API ConfigurationStereo : public Configuration {
  // Constructors for ConfigurationStereo used in StereoVisualSLAMImpl
  ConfigurationStereo();
  ConfigurationStereo(const BaseParams &base, const IMUParams &imu,
...
struct LIBMWVSLAMCORE_API ConfigurationRGBD : public Configuration {
  ...
  ConfigurationRGBD();
  ConfigurationRGBD(const BaseParams &base, const IMUParams &imu,
                    const RGBDParams &rgbd);
