...
private:
  ...
  public:
    UnderlyingDataMgrOwn(BufferType *src) : fUnderlyingData(nullptr)
    {
      if (src == nullptr) {
        return;
      }
    ...
    BufferType *getUnderlyingData() const override
    {
      return fUnderlyingData.get();
    }
  ...
  public:
    UnderlyingDataMgrUseOnly(BufferType *src) : fUnderlyingData(src)
    {
    }
  ...
  public:
    ...
    BufferType *releaseUnderlyingData() override
    {
      return fUnderlyingData.release();
    }
...
public:
   ...
   *
   * The image created with RelinquishDataUnderlying::YES can be copied and
   * assigned. However, when copied or assigned, the target image will contain a
   * deep copy of the data of the source image. For IM1 created with this flag =
   * TRUE Image im2 = im1; % IM2 will have its own copy of the data buffer i.e
   ...
   */
  Image(DimType height, DimType width, DimType numChannels,
        UnderlyingType dtype, ColorFormat format, Layout layout,
   ...
   *                        underlying data buffer without a data copy
   */
  Image(void *src, DimType height, DimType width, DimType numChannels,
        UnderlyingType dtype, ColorFormat format, Layout layout,
   ...
   * @param - layout - Layout of the image data
   */
  Image(void const *dataUnderlying, DimType height, DimType width,
        DimType numChannels, UnderlyingType dtype, ColorFormat format,
        Layout layout);
   ...
   *                        underlying data buffer without a data copy
   */
  Image(void *src, const ImageMetadata &metadata,
        SourceMgmt mgmt = SourceMgmt::COPY_SRC,
        RelinquishDataUnderlying relinquish = RelinquishDataUnderlying::NO);
   ...
   */
  Image deepCopy(
      RelinquishDataUnderlying relinquish = RelinquishDataUnderlying::NO) const;
   ...
   *
   * @param - none
   * @return - bool. TRUE if a deep copy will be done when calling
  ...
  // ------------------------------------------------
  DimType getHeight() const
  {
    return fMetadata.getHeight();
  }
  DimType getWidth() const
  {
    return fMetadata.getWidth();
  }
  DimType getNumChannels() const
  {
    return fMetadata.getNumChannels();
  }

  UnderlyingType getUnderlyingType() const
  {
    return fMetadata.getUnderlyingType();
  }
  ColorFormat getColorFormat() const
  {
    return fMetadata.getColorFormat();
  }
  Layout getLayout() const
  {
    return fMetadata.getLayout();
  }
...
private:
  ...
  static IUnderlyingDataMgr *createDataUnderlyingMgr(
      BufferType *src, SourceMgmt mgmt = SourceMgmt::COPY_SRC,
      RelinquishDataUnderlying relinquish = RelinquishDataUnderlying::NO);

  std::unique_ptr<BufferType, RawDataDeleter>
  copyRawData(const BufferType *src) const;
...
inline Image makeImage(void const *dataUnderlying, DimType height,
                       DimType width, DimType numChannels,
                       UnderlyingType datatype, ColorFormat colorFormat,
                       Layout layout)
{
  return Image(dataUnderlying, height, width, numChannels, datatype,
               colorFormat, layout);
}

inline Image makeImage(DimType height, DimType width, DimType numChannels,
                       UnderlyingType datatype, ColorFormat colorFormat,
                       Layout layout)
{
  return Image(height, width, numChannels, datatype, colorFormat, layout);
}
} // namespace datatypes
} // namespace images
