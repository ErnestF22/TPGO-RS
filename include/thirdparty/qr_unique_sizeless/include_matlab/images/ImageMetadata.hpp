...
public:
   ...
   */
  ImageMetadata()
      : ImageMetadata(0, 0, 3, UnderlyingType::Uint8, ColorFormat::RGB,
                      Layout::RowMajor)
  {
  }
   ...
   */
  ImageMetadata(DimType rows, DimType columns, DimType numChannels,
                UnderlyingType dtype, ColorFormat format, Layout layout)
      : fHeight(rows), fWidth(columns), fNumChannels(numChannels),
        fUnderlyingType(dtype), fColorFormat(format), fLayout(layout)
  {
  }
  ...
  void setHeight(DimType rows)
  {
    fHeight = rows;
  }
  void setWidth(DimType cols)
  {
    fWidth = cols;
  }
  void setNumChannels(DimType chans)
  {
    fNumChannels = chans;
  }

  void setUnderlyingType(UnderlyingType c)
  {
    fUnderlyingType = c;
  }
