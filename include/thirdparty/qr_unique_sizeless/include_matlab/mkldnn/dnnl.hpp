...
private:
  static dnnl_status_t dummy_destructor(T)
  {
    return dnnl_success;
  }
...
template <> struct handle_traits<dnnl_primitive_desc_iterator_t> {
...
struct engine : public handle<dnnl_engine_t> {
  ...
  engine(kind kind, size_t index)
  {
    ...
    error::wrap_c_api(dnnl_engine_create(&engine, convert_to_c(kind), index),
  ...
  engine(kind kind, cl_device_id device, cl_context context)
  {
    dnnl_engine_t engine;
    error::wrap_c_api(
        dnnl_engine_create_ocl(&engine, convert_to_c(kind), device, context),
        "could not create an engine");
    reset(engine);
  }
#endif
  ...
  engine(const handle<dnnl_primitive_desc_t> &pd)
  {
    ...
    error::wrap_c_api(
        dnnl_primitive_desc_query(
            pd.get(), dnnl::convert_to_c(dnnl::query::engine), 0, &c_engine),
...
private:
  ...
  static engine query(const primitive_desc &pd, dnnl::query what)
  {
    ...
    error::wrap_c_api(dnnl_primitive_desc_query(
                          pd.get(), dnnl::convert_to_c(what), 0, &c_engine),
                      "could not get an engine from a primitive_desc");
...
template <> struct handle_traits<dnnl_stream_attr_t> {
...
struct stream : public handle<dnnl_stream_t> {
         ...
         const stream_attr &attr = stream_attr())
  {
    ...
    error::wrap_c_api(
        dnnl_stream_create_v2(&stream, engine.get(),
...
struct memory : public handle<dnnl_memory_t> {
  ...
  struct desc {
        ...
        : data()
    {
      ...
      dnnl_status_t status = dnnl_memory_desc_init_by_tag(
          &data, (int)dims.size(), dims.data(), convert_to_c(data_type),
        ...
        : data()
    {
      validate_dims(dims);
      if (!strides.empty())
        validate_dims(strides, (int)dims.size());
      dnnl_status_t status = dnnl_memory_desc_init_by_strides(
          &data, (int)dims.size(), dims.data(), convert_to_c(data_type),
    ...
    desc submemory_desc(const memory::dims &dims, const memory::dims &offsets,
                      ...
                      bool allow_empty = false) const
    {
      ...
      dnnl_memory_desc_t out_md = dnnl_memory_desc_t();
      dnnl_status_t status =
          dnnl_memory_desc_permute_axes(&out_md, &data, permutation.data());
      if (!allow_empty)
        error::wrap_c_api(status,
    ...
    {
      return dnnl_memory_desc_get_size(&data);
    }
  ...
  {
    error::wrap_c_api(dnnl_memory_set_data_handle_v2(get(), handle, nullptr),
                      "could not set native handle of a memory object");
  }
  ...
  cl_mem get_ocl_mem_object() const
  {
    cl_mem mem_object;
    error::wrap_c_api(
        dnnl_memory_get_ocl_mem_object(get(), &mem_object),
  ...
  {
    error::wrap_c_api(
        dnnl_memory_set_ocl_mem_object(get(), mem_object),
...
struct post_ops : public handle<dnnl_post_ops_t> {
  ...
  void append_eltwise(float scale, algorithm algorithm, float alpha, float beta)
  {
    error::wrap_c_api(dnnl_post_ops_append_eltwise(
                          get(), scale, convert_to_c(algorithm), alpha, beta),
                          ...
                          float &alpha, float &beta) const
  {
    ...
    error::wrap_c_api(dnnl_post_ops_get_params_eltwise(get(), index, &scale,
                                                       &c_alg, &alpha, &beta),
  ...
  void append_dw_k3s1p1(memory::data_type weights_data_type,
                        ...
                        memory::data_type dst_data_type, int mask,
                        const std::vector<float> &scales)
  {

    error::wrap_c_api(dnnl_post_ops_append_dw_k3s1p1(
                          get(), memory::convert_to_c(weights_data_type),
                          memory::convert_to_c(bias_data_type),
                          memory::convert_to_c(dst_data_type), scales.size(),
                          mask, &scales[0]),
                            ...
                            std::vector<float> &scales) const
  {
    ...
    error::wrap_c_api(dnnl_post_ops_get_params_dw_k3s1p1(
                        ...
                        const std::vector<float> &scales)
  {
    ...
    error::wrap_c_api(dnnl_post_ops_append_dw_k3s2p1(
                          get(), memory::convert_to_c(weights_data_type),
                          memory::convert_to_c(bias_data_type),
                          memory::convert_to_c(dst_data_type), scales.size(),
                          mask, &scales[0]),
                            ...
                            std::vector<float> &scales) const
  {
    ...
    int c_mask;
    const float *c_scales;
    error::wrap_c_api(dnnl_post_ops_get_params_dw_k3s2p1(
                          get(), index, &c_weights_data_type, &c_bias_data_type,
                          &c_dst_data_type, &count, &c_mask, &c_scales),
                      "could not get parameters of depthwise post-op");
...
struct primitive_attr : public handle<dnnl_primitive_attr_t> {
  ...
  {
    error::wrap_c_api(
        dnnl_primitive_attr_set_output_scales(get(), (dnnl_dim_t)scales.size(),
                                              mask, scales.data()),
  ...
  void get_scales(int arg, int &mask, std::vector<float> &scales) const
  {
    ...
    const float *c_scales;
    error::wrap_c_api(
        dnnl_primitive_attr_get_scales(get(), arg, &count, &c_mask, &c_scales),
  ...
  void get_zero_points(int arg, int &mask,
                       ...
                       const std::vector<int32_t> &zero_points)
  {
    error::wrap_c_api(dnnl_primitive_attr_set_zero_points(
                          get(), arg, (dnnl_dim_t)zero_points.size(), mask,
                          zero_points.data()),
...
struct reorder : public primitive {
  ...
  struct primitive_desc : public primitive_desc_base {
                   ...
                   const primitive_attr &attr = primitive_attr())
    {
      ...
      error::wrap_c_api(dnnl_reorder_primitive_desc_create(
                            &result, &src_md.data, src_engine.get(),
                            &dst_md.data, dst_engine.get(), attr.get()),
                   ...
                   const primitive_attr &attr = primitive_attr())
    {
      ...
      error::wrap_c_api(dnnl_reorder_primitive_desc_create(
                            &result, &src_md.data, src.get_engine().get(),
                            &dst_md.data, dst.get_engine().get(), attr.get()),
                        "could not create a primitive descriptor for a reorder "
                        "primitive");
      reset(result);
    }
...
/// @cond DO_NOT_DOCUMENT_THIS
inline std::vector<dnnl_memory_desc_t>
convert_to_c(const std::vector<memory::desc> &mems)
{
  ...
  struct primitive_desc : public primitive_desc_base {
                   ...
                   const primitive_attr &attr = primitive_attr())
    {
      ...
      error::wrap_c_api(dnnl_concat_primitive_desc_create(
                            &result, &dst.data, (int)c_srcs.size(),
    ...
    /// @param engine Engine to perform the operation on.
    /// @param attr Primitive attributes to use (optional).
    primitive_desc(int concat_dimension, const std::vector<memory::desc> &srcs,
                   const engine &engine,
                   const primitive_attr &attr = primitive_attr())
    {
...
struct sum : public primitive {
  ...
  struct primitive_desc : public primitive_desc_base {
    ...
    /// @param engine Engine to perform the operation on.
    /// @param attr Primitive attributes to use (optional).
    primitive_desc(const memory::desc &dst, const std::vector<float> &scales,
                   const std::vector<memory::desc> &srcs, const engine &engine,
                   const primitive_attr &attr = primitive_attr())
    {
...
struct primitive_desc : public primitive_desc_base {
      ...
      : allow_empty_(allow_empty)
  {
    ...
    dnnl_status_t status = dnnl_primitive_desc_iterator_create(
        &iterator, desc, attr ? attr->get() : nullptr, engine.get(),
        hint_fwd_pd);
    if (!allow_empty)
      error::wrap_c_api(status,
  ...
  bool next_impl()
  {
    dnnl_status_t status = dnnl_primitive_desc_iterator_next(pd_iterator.get());
...
private:
  ...
  void fetch_impl()
  {
    dnnl_primitive_desc_t pd =
        dnnl_primitive_desc_iterator_fetch(pd_iterator.get(allow_empty_));
...
struct convolution_forward : public primitive {
  ...
  struct desc {
    ...
    desc(prop_kind prop_kind, algorithm algorithm, const memory::desc &src_desc,
         ...
         const memory::dims &padding_l, const memory::dims &padding_r)
    {
      ...
      error::wrap_c_api(
          dnnl_convolution_forward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), convert_to_c(algorithm),
         ...
         const memory::dims &padding_r)
    {
      ...
      error::wrap_c_api(
          dnnl_convolution_forward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), convert_to_c(algorithm),
              &src_desc.data, &weights_desc.data, nullptr, &dst_desc.data,
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const desc &desc, const engine &engine,
                   bool allow_empty = false)
        : dnnl::primitive_desc(&desc.data, nullptr, engine, nullptr,
                               allow_empty)
    {
    }
    ...
    primitive_desc(const desc &desc, const primitive_attr &attr,
        ...
        : dnnl::primitive_desc(&desc.data, &attr, engine, nullptr, allow_empty)
    {
    }
...
struct convolution_backward_data : public primitive {
  ...
  struct desc {
    ...
    desc(algorithm algorithm, const memory::desc &diff_src_desc,
         const memory::desc &weights_desc, const memory::desc &diff_dst_desc,
         const memory::dims &strides, const memory::dims &padding_l,
         const memory::dims &padding_r)
    {
      ...
      error::wrap_c_api(
          dnnl_convolution_backward_data_desc_init(
              &data, convert_to_c(algorithm), &diff_src_desc.data,
              &weights_desc.data, &diff_dst_desc.data, &strides[0],
         ...
         const memory::dims &padding_l, const memory::dims &padding_r)
    {
      ...
      error::wrap_c_api(
          dnnl_dilated_convolution_backward_data_desc_init(
              &data, convert_to_c(algorithm), &diff_src_desc.data,
              &weights_desc.data, &diff_dst_desc.data, &strides[0], &dilates[0],
              &padding_l[0], &padding_r[0]),
          "could not create a descriptor for a dilated convolution "
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(const desc &desc, const engine &engine,
                   ...
                   bool allow_empty = false)
        : dnnl::primitive_desc(&desc.data, nullptr, engine, hint_fwd_pd.get(),
                               allow_empty)
    {
    }
...
struct convolution_backward_weights : public primitive {
  ...
  struct desc {
    ...
    desc(algorithm algorithm, const memory::desc &src_desc,
         ...
         const memory::desc &diff_bias_desc, const memory::desc &diff_dst_desc,
         const memory::dims &strides, const memory::dims &padding_l,
         const memory::dims &padding_r)
    {
      memory::validate_dims(strides, src_desc.data.ndims - 2);
      ...
      error::wrap_c_api(
          dnnl_convolution_backward_weights_desc_init(
              &data, convert_to_c(algorithm), &src_desc.data,
              &diff_weights_desc.data, &diff_bias_desc.data,
              &diff_dst_desc.data, &strides[0], &padding_l[0], &padding_r[0]),
         ...
         const memory::dims &padding_l, const memory::dims &padding_r)
    {
      ...
      error::wrap_c_api(
          dnnl_dilated_convolution_backward_weights_desc_init(
              &data, convert_to_c(algorithm), &src_desc.data,
              &diff_weights_desc.data, &diff_bias_desc.data,
              &diff_dst_desc.data, &strides[0], &dilates[0], &padding_l[0],
              &padding_r[0]),
          "could not create a descriptor for a dilated convolution "
...
struct deconvolution_backward_data : public primitive {
  ...
  struct desc {
         ...
         const memory::dims &padding_r)
    {
      ...
      memory::validate_dims(padding_l, diff_src_desc.data.ndims - 2);
      memory::validate_dims(padding_r, diff_src_desc.data.ndims - 2);
      error::wrap_c_api(dnnl_deconvolution_backward_data_desc_init(
                            &data, convert_to_c(algorithm), &diff_src_desc.data,
...
struct lrn_forward : public primitive {
  ...
  struct desc {
    ...
    desc(prop_kind prop_kind, algorithm algorithm,
         const memory::desc &data_desc, memory::dim local_size, float alpha,
         float beta, float k = 1.f)
    {
      error::wrap_c_api(
          dnnl_lrn_forward_desc_init(&data, dnnl::convert_to_c(prop_kind),
...
struct eltwise_forward : public primitive {
  ...
  struct desc {
    ...
    {
      error::wrap_c_api(
          dnnl_eltwise_forward_desc_init(&data, dnnl::convert_to_c(prop_kind),
  ...
  struct desc {
         ...
         const memory::desc &data_desc, float alpha = 0, float beta = 0)
    {
      error::wrap_c_api(dnnl_eltwise_backward_desc_init(
                            &data, dnnl::convert_to_c(algorithm),
...
struct softmax_forward : public primitive {
  ...
  struct desc {
    ...
    {
      error::wrap_c_api(
          dnnl_softmax_forward_desc_init(&data, dnnl::convert_to_c(prop_kind),
...
struct logsoftmax_forward : public primitive {
  ...
  struct desc {
    ...
    {
      error::wrap_c_api(
          dnnl_logsoftmax_forward_desc_init(&data,
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(dnnl_primitive_desc_t pd)
        : dnnl::primitive_desc(
              ...
              dnnl::primitive::kind::softmax, dnnl::prop_kind::forward_training,
              dnnl::prop_kind::forward_inference)
    {
    }
...
struct logsoftmax_backward : public primitive {
  ...
  struct desc {
    ...
    {
      error::wrap_c_api(
          dnnl_logsoftmax_backward_desc_init(&data, &diff_data_desc.data,
                                             &data_desc.data, logsoftmax_axis),
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    primitive_desc(dnnl_primitive_desc_t pd)
        : dnnl::primitive_desc(
              ...
              // currently report the same primitive kind. Hence this
              // must be softmax and not logsoftmax.
              dnnl::primitive::kind::softmax, dnnl::prop_kind::backward_data)
    {
    }
...
struct batch_normalization_forward : public primitive {
         ...
         normalization_flags flags)
    {
      error::wrap_c_api(
          dnnl_batch_normalization_forward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), &data_desc.data, epsilon,
              convert_to_c(flags)),
  ...
  private:
    ...
    memory::desc stat_desc(int kind) const
    {
      ...
      error::wrap_c_api(
          dnnl_primitive_desc_query(
              get(), dnnl::convert_to_c(query::batch_normalization_d), 0, &p),
...
struct batch_normalization_backward : public primitive {
         ...
         normalization_flags flags)
    {
      error::wrap_c_api(
          dnnl_batch_normalization_backward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), &diff_data_desc.data,
  ...
  struct primitive_desc : public dnnl::primitive_desc {
    ...
    ///     defaults to false.
    primitive_desc(
        const desc &desc, const engine &engine,
    ...
    ///     defaults to false.
    primitive_desc(
        const desc &desc, const primitive_attr &attr, const engine &engine,
        const batch_normalization_forward::primitive_desc &hint_fwd_pd,
        bool allow_empty = false)
        : dnnl::primitive_desc(&desc.data, &attr, engine, hint_fwd_pd.get(),
                               allow_empty)
...
struct layer_normalization_forward : public primitive {
  ...
  struct desc {
         ...
         normalization_flags flags)
    {
      error::wrap_c_api(
          dnnl_layer_normalization_forward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), &data_desc.data,
         ...
         normalization_flags flags)
    {
      error::wrap_c_api(
          dnnl_layer_normalization_forward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), &data_desc.data, nullptr,
  ...
  private:
    ...
    memory::desc stat_desc(int kind) const
    {
      ...
      error::wrap_c_api(
          dnnl_primitive_desc_query(
              get(), dnnl::convert_to_c(query::layer_normalization_d), 0, &p),
          "could not retrieve a descriptor from a primitive "
  ...
  struct desc {
         ...
         float epsilon, normalization_flags flags)
    {
      error::wrap_c_api(
          dnnl_layer_normalization_backward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), &diff_data_desc.data,
              &data_desc.data, &stat_desc.data, epsilon, convert_to_c(flags)),
          "could not create a descriptor for a batch normalization "
          "backward propagation primitive");
    }
         ...
         normalization_flags flags)
    {
      error::wrap_c_api(
          dnnl_layer_normalization_backward_desc_init(
              &data, dnnl::convert_to_c(prop_kind), &diff_data_desc.data,
              &data_desc.data, nullptr, epsilon, convert_to_c(flags)),
          "could not create a descriptor for a batch normalization "
          "backward propagation primitive");
    }
  };
...
struct inner_product_backward_data : public primitive {
  ...
  struct desc {
    ...
    desc(const memory::desc &diff_src_desc, const memory::desc &weights_desc,
         const memory::desc &diff_dst_desc)
    {
      error::wrap_c_api(dnnl_inner_product_backward_data_desc_init(
                            &data, &diff_src_desc.data, &weights_desc.data,
                            &diff_dst_desc.data),
                        "could not create a descriptor for an inner product "
                        "backward propagation primitive");
    }
...
struct inner_product_backward_weights : public primitive {
  ...
  struct desc {
    ...
    /// @param diff_dst_desc Memory descriptor for diff dst.
    desc(const memory::desc &src_desc, const memory::desc &diff_weights_desc,
         const memory::desc &diff_bias_desc, const memory::desc &diff_dst_desc)
    {
...
protected:
  ...
  rnn_primitive_desc_base(dnnl_primitive_desc_t pd, dnnl::prop_kind prop_kind1,
                          dnnl::prop_kind prop_kind2, dnnl::algorithm cell_kind)
  {
    dnnl_rnn_desc_t *rnn_d;
    dnnl_status_t rc;
    rc = dnnl_primitive_desc_query(pd, dnnl_query_rnn_d, 0, &rnn_d);
    error::wrap_c_api(
        rc, "could not retrieve a descriptor from a primitive descriptor "
...
struct vanilla_rnn_forward : public primitive {
         ...
         float beta = 0.0f)
    {
      error::wrap_c_api(
          dnnl_vanilla_rnn_forward_desc_init(
              ...
              &bias_desc.data, &dst_layer_desc.data, &dst_iter_desc.data,
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(const desc &desc, const engine &engine,
                   bool allow_empty = false)
        : rnn_primitive_desc_base(&desc.data, nullptr, engine, nullptr,
                                  allow_empty)
    {
    }
    ...
    primitive_desc(const desc &desc, const primitive_attr &attr,
                   const engine &engine, bool allow_empty = false)
        : rnn_primitive_desc_base(&desc.data, &attr, engine, nullptr,
                                  allow_empty)
    {
    }
...
struct vanilla_rnn_backward : public primitive {
         ...
         float beta = 0.0f)
    {
      error::wrap_c_api(
          dnnl_vanilla_rnn_backward_desc_init(
              ...
              &diff_weights_layer_desc.data, &diff_weights_iter_desc.data,
              &diff_bias_desc.data, &diff_dst_layer_desc.data,
              &diff_dst_iter_desc.data, dnnl::convert_to_c(flags), alpha, beta),
  ...
  struct primitive_desc : public rnn_primitive_desc_base {
    ...
    primitive_desc(const desc &desc, const primitive_attr &attr,
                   ...
                   const vanilla_rnn_forward::primitive_desc &hint_fwd_pd,
                   bool allow_empty = false)
        : rnn_primitive_desc_base(&desc.data, &attr, engine, hint_fwd_pd.get(),
                                  allow_empty)
    {
    }
...
struct lstm_forward : public primitive {
  ...
  struct desc {
    ...
    {
      error::wrap_c_api(dnnl_lstm_forward_desc_init_v3(
                            &data, dnnl::convert_to_c(prop_kind),
    ...
    desc(prop_kind prop_kind, rnn_direction direction,
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(dnnl_lstm_forward_desc_init_v2(
                            &data, dnnl::convert_to_c(prop_kind),
                            dnnl::convert_to_c(direction), &src_layer_desc.data,
                            &src_iter_desc.data, &src_iter_c_desc.data,
                            &weights_layer_desc.data, &weights_iter_desc.data,
    ...
    desc(prop_kind prop_kind, rnn_direction direction,
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(dnnl_lstm_forward_desc_init(
                            &data, dnnl::convert_to_c(prop_kind),
                            dnnl::convert_to_c(direction), &src_layer_desc.data,
                            &src_iter_desc.data, &src_iter_c_desc.data,
                            &weights_layer_desc.data, &weights_iter_desc.data,
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(
          dnnl_lstm_backward_desc_init_v3(
              ...
              &src_iter_desc.data, &src_iter_c_desc.data,
              &weights_layer_desc.data, &weights_iter_desc.data,
              &weights_peephole_desc.data, &weights_projection_desc.data,
              &bias_desc.data, &dst_layer_desc.data, &dst_iter_desc.data,
              &dst_iter_c_desc.data, &diff_src_layer_desc.data,
              &diff_src_iter_desc.data, &diff_src_iter_c_desc.data,
              &diff_weights_layer_desc.data, &diff_weights_iter_desc.data,
              &diff_weights_peephole_desc.data,
              &diff_weights_projection_desc.data, &diff_bias_desc.data,
              &diff_dst_layer_desc.data, &diff_dst_iter_desc.data,
              &diff_dst_iter_c_desc.data, dnnl::convert_to_c(flags)),
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(
          dnnl_lstm_backward_desc_init_v2(
              ...
              &diff_src_layer_desc.data, &diff_src_iter_desc.data,
              &diff_src_iter_c_desc.data, &diff_weights_layer_desc.data,
              &diff_weights_iter_desc.data, &diff_weights_peephole_desc.data,
              &diff_bias_desc.data, &diff_dst_layer_desc.data,
              &diff_dst_iter_desc.data, &diff_dst_iter_c_desc.data,
              dnnl::convert_to_c(flags)),
          "could not create a descriptor for an LSTM backward "
          "propagation primitive");
    }
...
struct gru_forward : public primitive {
  ...
  struct desc {
    ...
    desc(prop_kind prop_kind, rnn_direction direction,
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(dnnl_gru_forward_desc_init(
                            &data, dnnl::convert_to_c(prop_kind),
                            dnnl::convert_to_c(direction), &src_layer_desc.data,
...
struct gru_backward : public primitive {
  ...
  struct desc {
    ...
    desc(prop_kind prop_kind, rnn_direction direction,
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(dnnl_gru_backward_desc_init(
                            &data, dnnl::convert_to_c(prop_kind),
                            dnnl::convert_to_c(direction), &src_layer_desc.data,
                            &src_iter_desc.data, &weights_layer_desc.data,
                            &weights_iter_desc.data, &bias_desc.data,
...
struct lbr_gru_forward : public primitive {
  ...
  struct desc {
    ...
    desc(prop_kind prop_kind, rnn_direction direction,
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(dnnl_lbr_gru_forward_desc_init(
                            &data, dnnl::convert_to_c(prop_kind),
                            dnnl::convert_to_c(direction), &src_layer_desc.data,
                            &src_iter_desc.data, &weights_layer_desc.data,
                            &weights_iter_desc.data, &bias_desc.data,
...
struct lbr_gru_backward : public primitive {
  ...
  struct desc {
    ...
    desc(prop_kind prop_kind, rnn_direction direction,
         ...
         rnn_flags flags = rnn_flags::undef)
    {
      error::wrap_c_api(dnnl_lbr_gru_backward_desc_init(
                            &data, dnnl::convert_to_c(prop_kind),
                            dnnl::convert_to_c(direction), &src_layer_desc.data,
                            &src_iter_desc.data, &weights_layer_desc.data,
                            &weights_iter_desc.data, &bias_desc.data,
...
struct shuffle_forward : public primitive {
  ...
  struct desc {
    ...
    {
      error::wrap_c_api(
          dnnl_shuffle_forward_desc_init(&data, dnnl::convert_to_c(prop_kind),
  ...
  struct desc {
    ...
    desc(const memory::desc &diff_data_desc, int axis, int group_size)
    {
      error::wrap_c_api(dnnl_shuffle_backward_desc_init(
                            &data, &diff_data_desc.data, axis, group_size),
...
struct resampling_forward : public primitive {
  ...
  struct desc {
    ...
    desc(prop_kind prop_kind, algorithm algorithm, const memory::desc &src_desc,
         const memory::desc &dst_desc)
    {
      error::wrap_c_api(dnnl_resampling_forward_desc_init(
                            &data, dnnl::convert_to_c(prop_kind),
                            convert_to_c(algorithm), nullptr, &src_desc.data,
                            &dst_desc.data),
...
struct resampling_backward : public primitive {
  ...
  struct desc {
    ...
    {
      error::wrap_c_api(
          dnnl_resampling_backward_desc_init(&data, convert_to_c(algorithm),
                                             nullptr, &diff_src_desc.data,
                                             &diff_dst_desc.data),
         ...
         const memory::desc &diff_src_desc, const memory::desc &diff_dst_desc)
    {
      ...
      error::wrap_c_api(
          dnnl_resampling_backward_desc_init(
              &data, convert_to_c(algorithm), factors.data(),
...
inline status sgemm(char transa, char transb, dnnl_dim_t M, dnnl_dim_t N,
                    dnnl_dim_t K, float alpha, const float *A, dnnl_dim_t lda,
                    const float *B, dnnl_dim_t ldb, float beta, float *C,
                    dnnl_dim_t ldc, dnnl::threadpool_iface *tp)
{
  return static_cast<status>(dnnl_sgemm_tp(transa, transb, M, N, K, alpha, A,
                                           lda, B, ldb, beta, C, ldc, tp));
}
...
inline status gemm_u8s8s32(char transa, char transb, char offsetc, dnnl_dim_t M,
                           ...
                           const int8_t *B, dnnl_dim_t ldb, int8_t bo,
                           float beta, int32_t *C, dnnl_dim_t ldc,
                           const int32_t *co, dnnl::threadpool_iface *tp)
{
  return static_cast<status>(dnnl_gemm_u8s8s32_tp(transa, transb, offsetc, M, N,
                                                  K, alpha, A, lda, ao, B, ldb,
                                                  bo, beta, C, ldc, co, tp));
}
...
inline status gemm_s8s8s32(char transa, char transb, char offsetc, dnnl_dim_t M,
                           ...
                           const int32_t *co, dnnl::threadpool_iface *tp)
{
  return static_cast<status>(dnnl_gemm_s8s8s32_tp(transa, transb, offsetc, M, N,
                                                  K, alpha, A, lda, ao, B, ldb,
                                                  bo, beta, C, ldc, co, tp));
}
#endif
...
inline void
primitive::execute(const stream &stream,
                   const std::unordered_map<int, memory> &args) const
{
