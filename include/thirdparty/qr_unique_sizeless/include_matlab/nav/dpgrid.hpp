...
private:
  ...
  // Store grid cell data for the open set. We store the location and the cost
  // estimate
  struct _cellData {
    std::array<uint32_t, 2> location;
    double cost;
    _cellData(std::array<uint32_t, 2> loc, double costval)
        : location(loc), cost(costval)
    {
    }
  };
  ...
  // Grid surrounding directions - south, north, west, east and diagonal
  // directions
  const std::vector<std::array<int8_t, 2>> _directions = {
  ...
  getValidNeighbors(const std::array<uint32_T, 2> &current);
  ...
  double distance2D(const std::array<uint32_T, 2> &cellI,
