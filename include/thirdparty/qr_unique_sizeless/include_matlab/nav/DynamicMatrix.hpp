...
///     NOTE: This does not create a new object like std::make_unique. This
///     function
...
template <typename T> auto raw2unique(T *&rawPtr) -> std::unique_ptr<T>
{
  return std::unique_ptr<T>(rawPtr);
}

/// @brief A convenience class which packages arbitrarily nested C-style arrays
/// with a custom deallocator
template <typename T, size_t N> class CWrapper {
protected:
  T m_arr; // Dynamically allocated (potentially nested) C-style array
  std::array<size_t, N>
      m_dims; // Number of dimensions in each layer of the nested array
public:
...
/// @return A unique_ptr with custom deallocator for all arrays in the nested
/// hierarchy
template <typename T, size_t N>
std::unique_ptr<CWrapper<T, N>> wrap_unique_CArray(T arr,
                                                   size_t const (&dims)[N])
{
 ...
 *      linear array.
 *
 *      This helper is used in conjunction with nav::DynamicMatrixVoidWrapper
 * and the C-compliant API in dynamicmatrix_api.hpp to return
 * dynamically-allocated matrices to MATLAB during codegen.
 *
 *      Example:
 *          MATLAB:
 *              ...
 *              % Create container for dynamically allocated matrix
 *              matWrapper = nav.algs.internal.coder.dynamicMatrixBuildable([inf
 * 3]); % Voxel center(s)
 *
 *              % The C++ function "foo" returns a wrapper around some
 * dynamically allocated C++ object representing [inf 3] matrix
 * coder.ceval('foo', ..., coder.ref(matWrapper.Ptr));
 *
 *              % Copy data back to matlab
 *              mat = matWrapper.getData();
 *
 *              % C++ object will get cleaned up automatically when matWrapper
 * goes out of scope
 *
 *          C++:
 ...
 *                  auto* ctrMat = new
 * nav::DynamicMatrix<std::vector<std::array<double, 3>>,
 * 2>(std::move(nav::raw2unique(ctrs)), { ctrs->size(), 3});
 *
 *                  // Generate a new DynamicMatrixVoidWrapperBase and point the
 * void** to it nav::createWrapper<double>(ctrMat,*ctrPtr);
 *              }
 */
...
protected:
  ...
  // Mapping between each layer of C++ structure and corresponding dimension of
  // MATLAB matrix
  //  Example 1:
  //      Assume m_data points to a 2D array, with m_dataSize = [3 10] (e.g:
  //      double[3][10]) Assume each row contains all X values for a set of 10
  //      [x y z] points
  //          - Specify m_unpackOrder = [0 1] if the data corresponds to a 3x10
  //          matrix in MATLAB (default)
  //          - Specify m_unpackOrder = [1 0] if the data corresponds to a 10x3
  //          matrix in MATLAB
  //  Example 2:
  //      Assume m_data points to a 3-layer object, with m_dataSize = [10 3 3]
  //      (e.g: std::vector<std::array<std::array<T,3>>>) Assume the innermost
  //      container (std::array<T,3>) represents an [x y z] point
  //          - Specify m_unpackOrder = [2 0 1] to map the C++ data to a 3x3x10
  //          matrix in MATLAB, where
  //              - Each 3x3 "page" in the matrix contains an element of the
  //              std::vector
  //              - Each row in the 3x3 matrix corresponds to an [x y z]
  //              std::array<T,3>
  std::array<size_t, N> m_unpackOrder;

public:
  /// @brief Creates a wrapper capable of marshalling data from a nested
  /// container to linear array
  /// @param data A unique_ptr pointing to a dynamically allocated container or
  /// nested C-array
  /// @param dataSize An N-element initializer representing the size of each
  /// nested container
  /// @param unpackOrder An N-element std::array that maps the layers in the
  /// container to dimensions of a matrix in MATLAB
  DynamicMatrix(std::unique_ptr<T, DEALLOC> &&data, size_t const (&dataSize)[N],
                std::array<size_t, N> unpackOrder =
                    sequence2array<N>(nav::make_index_sequence<N>{}))
      : m_data(std::move(data)), m_unpackOrder(unpackOrder)
  {
  ...
  /// @param data A unique_ptr pointing to a dynamically allocated container or
  /// nested C-array
  /// @param dataSize An N-element std::array representing the size of each
  /// nested container
  /// @param unpackOrder An N-element std::array that maps the layers in the
  /// container to dimensions of a matrix in MATLAB
  DynamicMatrix(std::unique_ptr<T, DEALLOC> &&data,
                std::array<size_t, N> dataSize,
                std::array<size_t, N> unpackOrder =
                    sequence2array<N>(nav::make_index_sequence<N>{}))
      : m_data(std::move(data)), m_sizes(dataSize), m_unpackOrder(unpackOrder)
  {
  }

  /// @brief Copies data from the nested resource to a (pre-allocated) linear
  /// array
  /// @tparam Tout The type of the output array
  /// @param[out] out Pointer to head of output array
  template <typename Tout> void copyData(Tout *out) const;
  ...
  /// @brief Copy size of MATLAB matrix represented by the C++ structure to a
  /// (pre-allocated) linear array
  /// @tparam Tout Type of destination array
  /// @param[out] out Pointer to head of output array
  template <typename Tout> const void copySizeMatrix(Tout *out) const
  {
...
protected:
  ...
  // Recursively traverses the stored/output data structures with appropriate
  // stride until base case is hit
  template <typename Tout, typename C, typename CurSize, typename CurStride,
            typename... REST> // REST contains pairs of dimension size and
                              // stride
  void copyElement(Tout *dest, C &elem, CurSize cSize, CurStride stride,
                   REST... rest) const;
  ...
  /// @brief Calculate matrix size and stride information when copying C++ data
  /// to linear MATLAB array
  ///     This method returns a tuple containing the following:
  ///         1) matlabSize       : The C++ dimensions (m_sizes) rearranged
  ///         based on the m_unpackOrder (essentially the size of the matrix in
  ///         MATLAB) 2) cppStride        : The distance between elements in the
  ///         linear output array for a step along each dimension of the C++
  ///         data-structure
  std::tuple<std::array<size_t, N>, std::array<size_t, N>>
  createOrderedSizeAndStride(void) const;
};
...
nav::DynamicMatrix<T, N, DEALLOC>::createOrderedSizeAndStride(void) const
{
  ...
  std::tuple<std::array<size_t, N>, std::array<size_t, N>> cpp2matlabMap(
      matlabDims, orderedStride);
...
void DynamicMatrix<T, N, DEALLOC>::copyData(Tout *out) const
{
  ...
  // Expand and extract
  copyDataExpansion(out, interlacedSizeAndStride,
                    nav::make_index_sequence<2 * N>{});
}

// Expand template parameter pack and begin recursive copy
template <typename T, size_t N, typename DEALLOC>
template <typename Tout, std::size_t... Ts>
void DynamicMatrix<T, N, DEALLOC>::copyDataExpansion(
    Tout *out, std::array<std::size_t, 2 * N> SizeStridePairs,
    nav::index_sequence<Ts...>) const
{
...
template <typename Tout, typename C, typename CurSize, typename CurStride,
          typename... REST>
void nav::DynamicMatrix<T, N, DEALLOC>::copyElement(Tout *dest, C &elem,
                                                    CurSize cSize,
                                                    CurStride stride,
                                                    REST... rest) const
{
