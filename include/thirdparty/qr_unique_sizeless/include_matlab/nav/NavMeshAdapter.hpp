...
/**
 * This file contains a lightweight wrapper which adapts a simple vertice/face
 * collection to OpenVDB algorithms
 */
...
namespace nav {
/// @brief An adaptor which converts vertices defined w.r.t a geometry's LOCAL
/// frame (herein defined as "world"-space) to axially-aligned grid coordinates
/// (herein defined as "index"-space). Once this "local" mesh has been
/// discretized, the geometry's world->local transformation may be applied to
/// the TSDF, situating it atop the mesh in world coordinates.
...
struct MeshDataAdapter {
  ...
  MeshDataAdapter(openvdb::math::Transform::Ptr tform, const Sz_T nV,
                  const Vert_T *const vertices, const Sz_T nF,
