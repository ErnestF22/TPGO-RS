...
/**
 * @brief A base-class which provides an interface to a type-erased
 * nav::DynamicMatrix object This object manages a dynamically allocated,
 * type-erased "delegate" - specifically, it manages a pointer to a
 * DynamicMatrix that has been cast to void.
 *
 *      Handles to DynamicMatrix member functions generated before the type was
 * erased allow this object to redirect calls to the managed pointer without
 * knowledge of the underlying type.
 *
 *      Use nav::createWrapper to convert a nav::DynamicMatrix* to a
 * DynamicMatrixVoidWrapperBase
 *
 *      Example:
 *          MATLAB:
 *              ...
...
protected:
  void *m_dynamicMatrix;   // Pointer to a nav::DynamicMatrix that has been cast
                           // to void*
...
public:
  ...
  DynamicMatrixVoidWrapperBase(void *dynamicMat, NumelFcn &&fNumel,
                               NumDimFcn &&fNumDim, CopySizeFcn &&fSize,
                               CleanupFcn &&fCleanup)
      : m_dynamicMatrix(dynamicMat), m_numelFcn(fNumel), m_numDimFcn(fNumDim),
        m_sizeFcn(fSize), m_CleanupFcn(fCleanup)
  {
  }
...
protected:
  copyDataFcn<Tout> m_dataFcn; // Handle for copying data out of the
                               // nav::DynamicMatrix to a linear array
public:
  /// @brief Constructor
  /// @param dynamicMat Pointer to a nav::DynamicMatrix
  /// @param fNumel Handle for calling nav::DynamicMatrix::getNumel()
  ...
  DynamicMatrixVoidWrapper(void *dynamicMat, NumelFcn &&fNumel,
                           NumDimFcn &&fNumDim, CopySizeFcn &&fSize,
                           CleanupFcn &&fCleanup, copyDataFcn<Tout> &&fData)
      : DynamicMatrixVoidWrapperBase(std::move(dynamicMat), std::move(fNumel),
                                     std::move(fNumDim), std::move(fSize),
                                     std::move(fCleanup)),
        m_dataFcn(fData)
  {
  }
...
/// @param dynamicMatrix Object managing some nested/dynamically-allocated C++
/// matrix
/// @param[out] wrapper DynamicMatrixVoidWrapper cast to void*
template <typename Tout, typename T, std::size_t N, typename DEALLOC>
void createWrapper(nav::DynamicMatrix<T, N, DEALLOC> *&dynamicMatrix,
                   void *&wrapper)
{
  ...
  auto *pDerived = new DynamicMatrixVoidWrapper<Tout>(
      dynamicMatrix, std::move(f0), std::move(f1), std::move(f2), std::move(f3),
      std::move(f4));
