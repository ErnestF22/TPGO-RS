 ...
 *
 */
template <typename T> class CommonCSMetric : public DistanceMetric<T> {
private:
...
public:
  ...
  CommonCSMetric(std::size_t dim, const std::vector<int32_T> &topologies,
  ...
  void configure(const std::vector<int32_T> &topologies,
                 const std::vector<T> &weights)
  {
    assert(topologies.size() == this->m_dim);
    assert(weights.size() == this->m_dim);
...
protected:
  ...
  /// distance between two states as the Cartesian product of the three metric
  /// spaces
                     ...
                     const typename std::vector<T>::const_iterator &state2It)
  {
    ...
    for (std::size_t k = 0; k < this->m_dim; k++) {
      ...
      case REALPROJECTIVE: /* quaternion, [k, k+3] */
        ...
        k += 3; // RP3 (quaternion) always has 4 (3+1) elements, another k
                // addition will happen as part of the for-loop
        break;
