...
namespace nav {
template <typename T> class DistanceMetric {
public:
  DistanceMetric()
  {
  }
  ...
  virtual std::vector<T> distance(const std::vector<T> &treeStates,
                                  const std::vector<T> &queryState) = 0;

protected:
  /// convert row major matrix to column major matrix
  static std::vector<real64_T>
  rowMajorToColumnMajor(const std::vector<real64_T> &states, std::size_t dim)
  {
...
protected:
  ...
  /**
   * from treeStates to queryState [false, outward] or the other way around
   * [true, inward]
   */
  boolean_T m_reverseDirection;
};

template <typename T> class EuclideanMetric : public DistanceMetric<T> {
public:
  ...
  std::vector<T> distance(const std::vector<T> &states,
                          const std::vector<T> &queryState)
  {
    std::vector<T> dists;
...
protected:
                     ...
                     const typename std::vector<T>::const_iterator &state2It)
  {
    ...
    for (size_t i = 0; i < this->m_dim; i++) {
      sum += (*(state2It + i) - *(state1It + i)) *
             (*(state2It + i) - *(state1It + i));
    }
    return std::sqrt(sum);
  }
};
