...
/**
 * @brief Implements Data Class to be passed to global localization sampling
 * function
 */
...
public:
  ...
  /* Using separate random number generators for XY and Theta produces very bad
     results. The MCL algorithm converges to wrong locations. Need to figure out
     why this happens. */
  std::minstd_rand0 _randGenerator; ///< Random number generator.

  /* Using separate distribution -PI to PI for Theta produces inferior results.
     In addition, using common distribution will produce comparable results with
     ROS implementation */
  std::uniform_real_distribution<double>
      _zeroToOne; ///< Uniform distribution for random number generation.

  std::vector<std::pair<int, int>>
      _freeSpaceIndices; ///< Free indices in the map

  GlobalSamplingData(unsigned int seed = 0)
      : _mapPtr(nullptr), _randGenerator(seed), _zeroToOne(0.0, 1.0)
  {
  }
};

/**
 * @brief Data storage container for one instance of the Monte Carlo
 * Localization Filter
 */
class MCLData {
public:
  ...
  boolean_T _isPFInitialized; ///< Has particle filter been initialized?
                              ///< (mclInitialize has been called)
  boolean_T _isPFUpdated; ///< Has particle filter been updated? (mclUpdate has
                          ///< been called)
  boolean_T _isResampled; ///< Has particle filter been resampled? (mclResample
                          ///< has been called)
  boolean_T _updateLikelihood;

  uint32_T _resampleCount;    ///< Current number of correction steps that have
                              ///< been called. If _resampleCount %
                              ///< _resampleInterval == 0, resampling occurs
  uint32_T _resampleInterval; ///< How often particle filter should be
                              ///< resampled. =1 means every correction step.

  real64_T
      _xThreshold; ///< Threshold for detecting non-static motion in x-direction
  real64_T
      _yThreshold; ///< Threshold for detecting non-static motion in y-direction
  real64_T _thetaThreshold; ///< Threshold for detecting non-static motion in
                            ///< theta angle
  ...
  nav::GlobalSamplingData
      _globalLocalizationData; ///< Map data for global localization

  boolean_T _testHookForMemory; ///< Test hook for memory testing

  MCLData(unsigned int seed)
      : _map(nullptr), _particleFilter(nullptr), _isPFInitialized(false),
        _isPFUpdated(false), _isResampled(false), _updateLikelihood(false),
        _resampleCount(0), _resampleInterval(1), _xThreshold(0.2),
        _yThreshold(0.2), _thetaThreshold(0.2), _minSensorRange(0.0),
        _maxSensorRange(6.0),
