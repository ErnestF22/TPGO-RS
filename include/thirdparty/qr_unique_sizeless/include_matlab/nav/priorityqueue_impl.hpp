...
/**
 * @brief This class is a simple wrapper of std::unordered_map<int,
 * std::vector<double>>
 */
class SimpleMap {
private:
...
public:
  ...
  std::vector<double> getData(int id)
  {
    if (IdDataMap.find(id) == IdDataMap.end()) {
      return std::vector<double>(DataDim,
...
/**
 * @brief This class represents the map between nodeIds and the map between a
 * nodeId and its data
 */
class NodeMap {
...
public:
  ...
  /// Adds a new node as the child of parentId node (i.e. append). The new
  /// nodeId is automatically assigned and returned. parentId may or may not
  /// already exist in the nodeMap.
  /**
  ...
  std::vector<double> getNodeData(int nodeId, int &parentId)
  {
    if (NodeDataMap.find(nodeId) == NodeDataMap.end()) {
      parentId =
          nodeId; // in normal case, parentId should never be the same as nodeId
      return std::vector<double>(DataDim,
                                 std::numeric_limits<double>::quiet_NaN());
    }
  ...
  /**
   * @return A vector of data from nodes along the path in reverse order (data
   * of the root node last)
   */
  std::vector<std::vector<double>> traceBack(int idx)
  {
    ...
    if (res.empty()) {
      res.emplace_back(DataDim, std::numeric_limits<double>::quiet_NaN());
    }
    return res;
  }
...
/**
 * @brief This class represents a priority queue of "node" data structure (min
 * heap assumed). A node is a pair of nodeId (int) and nodeData (vector of
 * double). PriorityQueueImpl is a thin wraper around the std::priority_queue
 * data structure.
 */
  ...
  struct comparator {
    ...
    bool operator()(const Node &n1, const Node &n2) const
    {
      if (n1.second[idx] != n2.second[idx]) {
        ...
        return n1.first > n2.first; // if the scores are the same, the one
                                    // inserted first takes priority
      } // the secondary comparison is NOT necessary, just for backward
        // compatibility, but it's extra work
    }
...
public:
  ...
  /**
   * @return If the queue is empty, top() returns a dummy node {-1, [nan, ...,
   * nan]}
   */
  Node top()
  {
