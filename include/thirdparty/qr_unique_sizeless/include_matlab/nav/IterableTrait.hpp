...
auto is_iterable_impl(int a) // Automatically attempt this implementation first
    -> std::decay_t<
        decltype(void(*begin(std::declval<T &>())), // Has a begin iterator that
                                                    // can be dereferenced
                 begin(std::declval<T &>()) !=
                     end(std::declval<T &>()), // input is not empty
                 void(),                       // Handle evil operator
                 ++std::declval<decltype(begin(
                     std::declval<T &>())) &>(), // operator++ is defined
                 std::true_type{})>; // If all previous conditions do not error
                                     // during inference, this type is true

// Any input which does not pass conditions of above specialization will show as
// false
...
namespace nav {
// is_iterable A template specialization trait for determining whether input of
// type T is iterable
template <typename T>
