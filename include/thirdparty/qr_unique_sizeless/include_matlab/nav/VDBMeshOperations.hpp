 ...
 * @return MeshDataAdapter<Data_T, Data_T, Size_T> A mesh adapter for the given
 * mesh data.
 */
template <class Data_T, class Size_T>
MeshDataAdapter<Data_T, Data_T, Size_T>
createMesh(float resolution, Data_T const *const pose, const Size_T nVert,
           Data_T const *const vertices, const Size_T nFace,
           Data_T const *const faces);
 ...
 * @param resolution The resolution of the voxels (in cells/meters).
 * @param fillInterior Flag indicating whether to fill the interior of the mesh
 * with distance values.
 * @param fastSweep Flag indicating whether to use a fast sweeping algorithm for
 * TSDF updates.
 * @return DistGridType::Ptr A pointer to a grid containing the discretized
 * mesh.
 */
template <class Mesh>
DistGridType::Ptr discretizeMesh(const Mesh &mesh, float truncDist,
                                 float resolution, bool fillInterior,
                                 bool fastSweep);
 ...
 * @param pose Pointer to the array representing a transformation matrix (1x16
 * array of doubles).
 * @param nVert The number of vertices (size_t).
 * @param vertices Pointer to the vertices data (Mx3 array of doubles,
 * representing M vertices in 3D space).
 * @param nFace The number of faces (size_t).
 * @param faces Pointer to the face indices data (Nx3 array of doubles,
 * representing N faces with indices to vertices).
 * @return double Returns 1.0 if a new ID was added, 0.0 otherwise.
 */
double addMesh(VDBManager &mgr, const size_t id, const double *const pose,
               const size_t nVert, const double *const vertices,
               const size_t nFace, const double *const faces);
 ...
 * array of doubles).
 */
void transformGrid(DistGridType::Ptr grid, float resolution,
