...
namespace nav {
template <typename T> class DistanceRadiusFunctor {
...
template <typename T> class AscendingNodeFunctor {
 ...
 * @tparam T               Data type
 */
template <typename T> class NearestNeighborFinder {
  // tree is a friend
  template <typename> friend class SearchTree;
...
public:
  ...
  virtual TreeNode<T> *nearestNeighbor(const std::vector<T> &state,
                                       T &distNN) = 0;
  virtual std::vector<TreeNode<T> *> nearK(const std::vector<T> &state,
                                           std::size_t num) = 0;
  virtual std::vector<TreeNode<T> *> near(const std::vector<T> &state,
                                          T dist) = 0;
...
template <typename T> class ExhaustiveNN : public NearestNeighborFinder<T> {
public:
  ExhaustiveNN(std::size_t dim) : NearestNeighborFinder<T>(dim)
  { /*ExhaustiveNN constructor*/
  }
  void insert(TreeNode<T> *nodePtr) override
  {
    m_nodePtrs.push_back(nodePtr);
    m_currentNodeStates.insert(m_currentNodeStates.end(),
                               nodePtr->getState().begin(),
  ...
  {
    std::vector<T> dists =
        this->m_queryMetric->distance(m_currentNodeStates, state);
    auto minIter =
        std::min_element(dists.begin(), dists.end(), NaNDistanceComparator());

    distNN = *minIter;
    auto nodePtrIter =
        m_nodePtrs.begin() + std::distance(dists.begin(), minIter);
    return *nodePtrIter;
  }

  /// return the nearest node (as node pointer) to a given state, alternative
  /// implementation
  TreeNode<T> *nearestNeighbor_alternative(const std::vector<T> &state,
                                           T &distNN)
  {
    ...
    for (auto &&nodePtr : m_nodePtrs) {
      std::vector<T> dists =
          this->m_queryMetric->distance(nodePtr->getState(), state);
      d = dists[0];
      if (d < dNN) {
        dNN = d;
        out = nodePtr;
  ...
  std::vector<TreeNode<T> *> nearK(const std::vector<T> &state,
                                   std::size_t num) override
  {
    std::vector<TreeNode<T> *> out;

    // TODO
    auto comparator = [this, &state](TreeNode<T> *node1,
                                     TreeNode<T> *node2) -> bool {
      std::vector<T> d1 =
          this->m_queryMetric->distance(state, node1->getState());
      std::vector<T> d2 =
          this->m_queryMetric->distance(state, node2->getState());
      return d1[0] > d2[0];
    };

    std::priority_queue<TreeNode<T> *, std::vector<TreeNode<T> *>,
                        decltype(comparator)>
        pq(comparator);
    for (auto &&nodePtr : m_nodePtrs) {
      pq.push(nodePtr);
  ...
  /// return the list of nodes (as node pointers) within a radius of a given
  /// state
  std::vector<TreeNode<T> *> near(const std::vector<T> &state,
                                  T radius) override
  {
  ...
  /// return the list of nodes (as node pointers) within a radius of a given
  /// state (alternative implementation)
  std::vector<TreeNode<T> *> near_alternative(const std::vector<T> &state,
                                              T radius)
  {
    std::vector<TreeNode<T> *> out;
    ...
    auto comparator = [this,
                       &state](std::pair<TreeNode<T> *, T> &pair1,
                               std::pair<TreeNode<T> *, T> &pair2) -> bool {
      if (pair1.second < 0) // compute distance if it has not been done yet
      {
        std::vector<T> d1 =
            this->m_queryMetric->distance(pair1.first->getState(), state);
        pair1.second = d1[0];
      }
      if (pair2.second < 0) {
        std::vector<T> d2 =
            this->m_queryMetric->distance(pair2.first->getState(), state);
        pair2.second = d2[0];
      }
      return pair1.second > pair2.second;
    };

    // each pair contains the tree node pointer and the node's distance to given
    // state
    std::priority_queue<std::pair<TreeNode<T> *, T>,
                        std::vector<std::pair<TreeNode<T> *, T>>,
                        decltype(comparator)>
        pq(comparator);
    for (auto &&nodePtr : m_nodePtrs) {
  ...
  std::size_t nearestNeighborIdx(const std::vector<T> &state, T &distNN)
  {
    ...
    for (std::size_t i = 0; i < m_nodePtrs.size(); i++) {
      std::vector<T> dists =
          this->m_queryMetric->distance(m_nodePtrs[i]->getState(), state);
      d = dists[0];
      if (d < dNN) {
        dNN = d;
        idxNN = i;
  ...
  /// find the IDs of the near K neighboring nodes for a given state, used for
  /// test only
  std::vector<std::size_t> nearKIdx(const std::vector<T> &state,
