...
class OCTOMAP_CODEGEN_API octomapcodegen {
...
public:
  ...
  real64_T octomapGetOccupiedThreshold();

  void octomapSetOccupiedThreshold(real64_T probability);

  real64_T octomapGetFreeThreshold();

  void octomapSetFreeThreshold(real64_T probability);

  uint32_T octomapGetNumLeafNodes();

  void octomapGetMapDimensions(real64_T *dimensions);

  void octomapUpdateNodeBoolean(real64_T *xyz, boolean_T occupied,
                                boolean_T lazyEval);

  void octomapUpdateNodeDouble(real64_T *xyz, real64_T probUpdate,
                               boolean_T lazyEval);

  void octomapSetNodeValue(real64_T *xyz, real64_T prob, boolean_T lazyEval);

  void octomapGetOccupancy(real64_T *xyz, uint32_T nRows,
                           real64_T *occupancyValues);

  void octomapCheckOccupancy(real64_T *xyz, uint32_T nRows,
  ...
  void octomapSetClampingThreshold(real64_T clampingThresMin,
  ...
  void octomapSetProbHit(real64_T probHit);

  void octomapSetProbMiss(real64_T probMiss);
  ...
  void octomapDeserializationFullROSMsgData(real64_T resolution, char *data,
                                            uint32_T n);

  void octomapDeserializationBinaryROSMsgData(real64_T resolution, char *data,
                                              uint32_T n);

  void octomapGetRayIntersection(real64_T *ptStart, real64_T *ptDirections,
                                 uint32_T nPtDirection,
                                 real64_T occupiedThreshold,
                                 boolean_T ignoreUnknownCells,
                                 real64_T maxRange, real64_T *outData);

  bool insertPointCloudImpl_real64(real64_T *origin, real64_T *points,
                                   uint32_T nPoints, real64_T maxRange,
                                   boolean_T lazyEval, boolean_T discretize);

  bool insertPointCloudImpl_real32(real64_T *origin, real32_T *points,
                                   uint32_T nPoints, real64_T maxRange,
                                   boolean_T lazyEval, boolean_T discretize);

  std::shared_ptr<octomap::OcTree> getTree()
  {
    return _map;
  }

private:
  std::shared_ptr<octomap::OcTree> _map; ///< Octomap Map Object

  double occ_free_thres; /// free threshold (probability)
  ...
  bool insertPointCloudImpl(double const *origin, TfloatPoint const *points,
                            uint32_T rows, double maxRange, bool lazyEval,
                            bool discretize);
};
} // namespace nav
