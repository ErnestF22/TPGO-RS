...
/// @details A wrapper class for the octomap::OcTree, which provides an
/// interface for collision
///         checking against nav::AABB and shared_robotics::CollisionGeometry
///         objects.
class COLLISIONMAP_CODEGEN_API CollisionMap {
public:
  /// @brief Creates a lightweight collision-checking wrapper around an OcTree
  /// object
  ...
  /// @param depth An unsigned int between [0 nav::MAX_TREE_DEPTH], specifying
  /// the depth in the tree (root = 0)
  ...
  /// @details During collision checking, only voxels which fall within this
  /// AABB region
  ///             will be checked for collision
  /// @param box A nav::AABB whose volume defines the bounded region on the tree
  /// @param depth An unsigned int between [0 nav::MAX_TREE_DEPTH], specifying
  /// the depth in the tree (root = 0)
  /// @return Boolean indicating whether any portion of the BBOX falls within
  ...
  /// @details Takes in an OcTreeKey and a depth-offset from the base depth. The
  /// function
  ///          then floors the key by the offset, resulting in a key lying at
  ///          the min corner of a voxel located at
  ///          maxQueryDepth-levelFromMaxDepth. The max corner key is created by
  ///          shifting the min key by the size of a voxel at the indicated
  ///          depth.
  /// @param key The original octomap::OcTreeKey
  /// @param levelFromMaxDepth The number of levels above the base depth
  /// @return Returns a pair of octomap::OcTreeKey objects, representing the min
  /// and max keys
  inline std::tuple<OcKey, OcKey>
  shiftKeyToVoxelCorners(const OcKey &key,
                         const unsigned short levelFromMaxDepth);

  /// @brief Returns whether an octomap::OcTreeKey at a given depth lies within
  /// the AABB region
  /// @param key An octomap::OcTreeKey object
  /// @param currentDepth An unsigned int between [0 nav::MAX_TREE_DEPTH],
  /// specifying the depth in the tree (root = 0)
  /// @return True if the key touches the region, false otherwise
  bool inBaseBBX(const OcKey &key, const depth_t currentDepth);

  /// @brief Performs a broad-phase pre-check between a key and the map at a
  /// given depth
  /// @param broadPhase When true, the check is performed, otherwise the
  /// pre-check always returns true
  /// @param key An octomap::OcTreeKey object
  /// @param currentDepth An unsigned int between [0 nav::MAX_TREE_DEPTH],
  /// specifying the depth in the tree (root = 0)
  /// @return Returns true when the broad-phase check indicates a collision,
  /// false otherwise
  bool broadPhasePrecheck(const bool broadPhase, const OcKey &key,
                          const depth_t currentDepth);

  /// @brief Performs a non-exhaustive check between the tree and a simple
  /// AABB-bounded object
  /// @param box A nav::AABB object
  /// @param[out] key An octomap::OcTreeKey object corresponding to the first
  /// voxel in collision with the nav::AABB
  ...
  bool checkCollision_BroadPhase(const AABB &box, std::vector<OcKey> &key,
                                 std::vector<depth_t> &collDepth);

  /// @brief Performs a non-exhaustive AABB-only check, limited to the specified
  /// depth in the tree
  /// @param box A nav::AABB object
  /// @param[out] key An octomap::OcTreeKey object corresponding to the first
  /// voxel in collision with the nav::AABB
  /// @param[out] collDepth The depth at which the collision occurs
  /// @param maxQueryDepth The maximum depth allowed during the search (unsigned
  /// int between [0 nav::MAX_TREE_DEPTH])
  /// @return Returns true when a collision has been found, false otherwise
  ...
  /// @param box A nav::AABB object
  /// @param[out] keys A vector of keys corresponding to all voxels in collision
  /// with the nav::AABB
  ...
  /// @brief Performs an exhaustive AABB-only check, limited to the specified
  /// depth in the tree
  /// @param box A nav::AABB object
  /// @param[out] keys A vector of keys corresponding to all voxels in collision
  /// with the nav::AABB
  ...
  /// int between [0 nav::MAX_TREE_DEPTH])
  /// @return Returns true when a collision has been found, false otherwise
  bool checkCollision_BroadPhaseExhaustiveAtDepth(
      const AABB &box, std::vector<OcKey> &keys,
      std::vector<depth_t> &collDepth, const depth_t maxQueryDepth);

  /// @brief Generates a new key located at the center of the voxel containing
  /// the input key at the specified depth
  /// @param key An octomap::OcTreeKey object
  ...
  /// @brief Calculates the closest distance between voxels in the map and a
  /// nav::AABB object
  ...
  double broadPhaseDistance(AABB &aabb, double *p1Vec, double *p2Vec,
                            const depth_t maxQueryDepth);
  ...
  OcKey minKey(void)
  {
    return m_tree->coordToKey(m_tree->getBBXMin());
  }
  ...
  OcKey maxKey(void)
  {
    return m_tree->coordToKey(m_tree->getBBXMax());
  }
...
inline std::tuple<OcKey, OcKey>
CollisionMap::shiftKeyToVoxelCorners(const OcKey &key,
                                     const unsigned short levelFromMaxDepth)
{
  ...
  std::for_each(minNodeKey.k, minNodeKey.k + 3u, [&](octomap::key_type &kVal) {
    kVal = static_cast<unsigned short>(
        static_cast<unsigned short>(kVal >> levelFromMaxDepth)
        << levelFromMaxDepth);
  });

  // Shift up by current level's node dimension (results in topRight corner)
  OcKey maxNodeKey = minNodeKey;
  std::for_each(maxNodeKey.k, maxNodeKey.k + 3u, [&](octomap::key_type &kVal) {
    kVal += ((1u << levelFromMaxDepth) - 1u);
  });
...
std::vector<Tout> castVec(const std::vector<Tin> &vIn)
{
  ...
  std::transform(vIn.begin(), vIn.end(), out.begin(),
                 [](Tin x) { return static_cast<Tout>(x); });
  return out;
}
...
/// templated helper functions for casting between datatypes
template <class OutContainer, typename Tout = typename OutContainer::value_type,
          class InContainer, class t_size>
...
/// templated helper functions for casting between datatypes
template <typename T> inline octomap::point3d castVec(const std::vector<T> &vIn)
{
  return octomap::point3d(static_cast<float>(vIn[0]),
                          static_cast<float>(vIn[1]),
                          static_cast<float>(vIn[2]));
}

/// templated helper functions for casting between datatypes
template <typename T> inline const std::vector<T> castVec(octomap::point3d vIn)
{
