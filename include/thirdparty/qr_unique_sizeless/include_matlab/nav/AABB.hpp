 ...
 * @details An axially-aligned bounding box with helper utilities used for
 * simple collision-checking. This class is primarily used by nav::CollisionMap
 * to support geometry vs octomap collision-checks.
 */
class AABB {
public:
  ...
  AABB()
      : m_localMin({-0.5, -0.5, -0.5}), m_localMax({0.5, 0.5, 0.5}),
        m_localOrigin({0, 0, 0})
  {
  }

  /// @brief Constructs a nav::AABB with all sides of equal length
  /// (2*halfLength), centered at location
  /// @param halfLength Distance of local min/max from center point in all 3
  /// dimensions
  ...
  /// @brief Construct a nav::AABB using local min/max coordinates. Box is
  /// centered at [0,0,0]
  ...
  AABB(const double xMin, const double yMin, const double zMin,
       const double xMax, const double yMax, const double zMax)
      : m_localMin({xMin, yMin, zMin}), m_localMax({xMax, yMax, zMax}),
        m_localOrigin({0, 0, 0})
  {
  }
  ...
  {
    return (&obj == &other || (obj.m_localMin == other.m_localMin &&
                               obj.m_localMax == other.m_localMax &&
  ...
  inline double distance(const AABB &other, double *p1, double *p2)
  {
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      if (d != 0) {
        ...
        p1[i] = (std::max)(worldMin(i),
                           other.worldMin(
                               i)); // parenthesis surrounding std::min/max to
...
protected:
  ...
  {
    return !(worldMax(dim) < other.worldMin(dim) ||
             worldMin(dim) > other.worldMax(dim));
  }

  // Returns min/max corner of overlapping cuboid region in given dimension
  inline bool overlapInDim(const AABB &other, size_t dim, double p1[],
                           double p2[])
  {
    bool overlaps = !(worldMax(dim) < other.worldMin(dim) ||
                      worldMin(dim) > other.worldMax(dim));
    if (overlaps) {
