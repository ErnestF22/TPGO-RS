...
template <typename T, typename... REMSIZE> void deallocateArr(T arr, REMSIZE...)
{
  if (arr != nullptr) {
    // Free self
...
/// @param sizes Parameter pack containing dimensions of arrays below current
/// array
...
void deallocateCArrayExpand(T arr, std::array<size_t, NDIM> sizes,
                            nav::index_sequence<I...>)
{
...
/// @brief Convenience function for wrapping a raw pointer to an object inside a
/// unique_ptr
///     NOTE: This does not create a new object like std::make_unique. This
///     function expects
///           that the user relinquishes deallocation responsibilities to the
///           unique_ptr.
...
std::unique_ptr<T> make_unique(Args &&...args)
{
  return std::unique_ptr<T>(new T(std::forward<Args>(
      args)...)); // All arguments are forwarded to T-style constructor
}
} // namespace nav
