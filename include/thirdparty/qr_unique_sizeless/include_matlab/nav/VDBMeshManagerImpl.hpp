 ...
 * @brief This file contains the interface of the MCOS wrapper for the
 * VDBManager object.
...
public:
  VDBMeshManagerImpl(const double resolution, const double truncDist,
                     const bool fillInterior, const bool fastSweep);
  ...
  /**
   * @brief Retrieves the active voxels information from all VDB volumes managed
   * by the VDBMeshManager.
   *
   * This method compiles information about the active voxels from each VDB
   * volume into a structured array. Each element of the structured array
   * corresponds to a single VDB volume and contains the following fields:
   * - 'ID': A scalar double representing the unique identifier of the VDB
   * volume.
   * - 'Centers': An N-by-3 matrix where each row contains the (x, y, z)
   * coordinates of an active voxel's center.
   * - 'Distances': An N-by-1 column vector containing the signed distance field
   * (SDF) values of the active voxels.
   * - 'Sizes': An N-by-1 column vector containing the size of each active
   * voxel.
   *
   * Here, N is the number of active voxels in the respective VDB volume.
   *
   * @return An mxArrayProxy structure representing a structured array with
   * active voxels data for each VDB volume.
   */
  mxArrayProxy getActiveVoxels(void) const;

  /**
   * @brief Retrieves the pose matrices of all VDB volumes managed by the
   * VDBMeshManager.
   *
   * This method compiles the pose information for each VDB volume into a
   * structured array. Each element of the array corresponds to a single VDB
   * volume and contains the following fields:
   * - 'ID': A scalar double representing the unique identifier of the VDB
   * volume.
   * - 'Pose': A 4-by-4 matrix representing the transformation matrix (pose) of
   * the VDB volume in homogeneous coordinates.
   *
   * The 'Pose' matrix encodes the position and orientation of the VDB volume in
   * a global reference frame.
   *
   * @return An mxArrayProxy structure representing a structured array with the
   * ID and pose data for each VDB volume.
   */
   ...
   * @param serializedMgrData Serialized data as an mxArrayProxy storing a
   * 1-by-N uint8 array.
   */
