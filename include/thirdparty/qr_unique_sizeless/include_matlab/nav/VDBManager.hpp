...
class VDBManager {
public:
  VDBManager(const double resolution, const double truncDist,
  ...
  /**
   * @brief Computes the minimum distances from a set of points to all managed
   * grids.
   ...
   */
  void distance(const std::vector<openvdb::Vec3d> &pts, double *dists,
   ...
   * @param interpMethod Interpolation method identifier.
   */
  void gradient(const std::vector<openvdb::Vec3d> &pts, double *grads,
                const size_t interpMethod) const;
   ...
   */
  void getActiveVoxelFrom(const size_t id, double *ctrs, double *vals,
                          double *sizes) const;

  /**
   * @brief Retrieves the pose of a specified grid.
   * @param id Identifier for the grid.
   * @param poses Array to store the 4x4 transformation matrix representing the
   * grid's pose.
   */
   ...
   * @return A binary string representing the serialized state of the
   * VDBManager.
   */
   ...
   * @param vdbManagerString The binary string representing a serialized
   * VDBManager.
  ...
  /**
   * @brief Retrieves the total number of active voxels across all managed VDB
   * grids.
   ...
   * @param id Identifier for the grid.
   * @return The number of active voxels in the specified grid, or 0 if the grid
   * is not found.
   */
   ...
   * @param lims Array to store the minimum and maximum coordinates of the world
   * limits.
   */
