...
namespace nav {
/// template class that represents the underlying tree data structure for
/// RRT-like planning algorithms
/**
 * @tparam T               Data type
 */
...
public:
  ...
  public:
    BFSIterator(TreeNode<T> *ptr) : Iterator<BFSIterator>(ptr)
    {
      m_queue.push_front(ptr);
    }

    BFSIterator(const BFSIterator &bit) : Iterator<BFSIterator>(bit)
    {
      m_queue = bit.m_queue;
    }
  ...
  public:
    DFSIterator(TreeNode<T> *ptr) : Iterator<DFSIterator>(ptr)
    {
      m_stack.push(ptr);
    }

    DFSIterator(const DFSIterator &dit) : Iterator<DFSIterator>(dit)
    {
      m_stack = dit.m_stack;
    }
...
public:
  ...
  TreeNode<T> *insertNode(TreeNode<T> *parent, const std::vector<T> &state)
  {
    std::vector<T> dist = m_nnFinder->m_queryMetric->distance(
        parent->getState(), state); // compute distance first, if it throws
                                    // exception, the rest will not be executed.
  ...
  boolean_T insertNodeByID(std::size_t idx, const std::vector<T> &state,
                           std::size_t &idxNew)
  {
    idxNew = 0;
    if (idx < m_nodePtrs.size()) {
      ...
      std::vector<T> dist = m_nnFinder->m_queryMetric->distance(
          parent->getState(),
          state); // compute distance first, if it throws exception, the
  ...
  int32_T rewireNodeByID(std::size_t nodeID, std::size_t newParentNodeID,
                         T distanceBetweenNodes = -99)
  {
    if (nodeID < m_nodePtrs.size() && newParentNodeID < m_nodePtrs.size()) {
      ...
      // normal case: node's parent is not newParentNode, and newParentNode is
      // not a descendant of node.
      ...
      auto distDiff =
          pNewParentNode->m_costFromRoot + newDist - pNode->m_costFromRoot;
  ...
  std::vector<std::size_t> nearKNeighborIDs(const std::vector<T> &state,
  ...
  /// Returns the IDs of the nodes within the closed ball of radius r centered
  /// at the given state. r is computed adaptively.
  std::vector<std::size_t> nearNeighborIDs(const std::vector<T> &state)
  {
    std::vector<std::size_t> indices;

    std::vector<TreeNode<T> *> nodePtrs =
        m_nnFinder->near(state, computeBallRadius());
