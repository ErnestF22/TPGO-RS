...
CODEINSTRHOSTAPPSVC_API_EXTERN_C int codeInstrHostAppSvcCreate(
    void **const ppCodeInstrService, const CodeInstrServiceData_T *const pData,
    void *const pCSVoid, const void *const pMemUnitXformer,
    const int idTypeSize, void *const pXILUtils, const int memUnitSize,
    const boolean_T isUsingCommService, void *multiRunData);

CODEINSTRHOSTAPPSVC_API_EXTERN_C void
codeInstrHostAppSvcSetTime(void *const pCodeInstrService, const time_T simTime);

CODEINSTRHOSTAPPSVC_API_EXTERN_C void
codeInstrHostAppSvcDestroy(void *const pCodeInstrService,
