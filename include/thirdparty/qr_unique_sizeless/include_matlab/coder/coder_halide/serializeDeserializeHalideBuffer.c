...
halide_buffer_t matlabArrayToHalideBuffer_size_numDim(
    const void *wrapee, const int *size, const int dim,
    const uint8_t halide_type_t_code, const uint8_t halide_type_t_bits,
    const uint16_t halide_type_t_lanes)
{
...
halide_buffer_t matlabArrayToHalideBuffer_min_size_stride_numDim(
    const void *wrapee, const int *min, const int *size, const int *stride,
    const int dim, const uint8_t halide_type_t_code,
    const uint8_t halide_type_t_bits, const uint16_t halide_type_t_lanes)
{
  halide_buffer_t hbuffer;
  initialize_halide_buffer(&hbuffer);
  hbuffer.dimensions = dim;
...
void halideBufferToMatlabArray(halide_buffer_t *wrapee, void *out,
                               const int size)
{
