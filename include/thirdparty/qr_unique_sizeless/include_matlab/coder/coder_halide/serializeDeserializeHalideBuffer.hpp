...
#if defined(RTWTYPES_H) && defined(__APPLE__)
template <> HALIDE_ALWAYS_INLINE halide_type_t halide_type_of<unsigned long>()
{
  return halide_type_t(halide_type_uint, 64);
}

template <> HALIDE_ALWAYS_INLINE halide_type_t halide_type_of<long>()
{
  return halide_type_t(halide_type_int, 64);
}
#endif

template <class T> halide_type_t getHalideType(const T *in)
{
...
template <class T>
halide_buffer_t matlabArrayToHalideBuffer(const T *wrapee, const int *size,
                                          const int dim)
{
  ...
  hbuffer.host =
      const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(wrapee));
...
template <class T>
halide_buffer_t matlabArrayToHalideBuffer(const T *wrapee, const int *min,
...
template <class T> void deallocateHalideBuffer(T *wrapee)
{
  if (wrapee->dim) {
    delete[] (wrapee->dim);
  }
