...
// Custom shared intrusive pointer. Counts the number of items pointing to an
// object without storing a separate reference count. Requires the corresponding
// object to store a field called "referenceCount" as well as friend this class.
...
template <typename T> shared_ptr<T>::shared_ptr(T *p) : fPtr(p)
{
  if (fPtr != nullptr) {
    fPtr->referenceCount++;
  }
...
template <typename T>
shared_ptr<T>::shared_ptr(const shared_ptr<T> &sp) : fPtr(sp.fPtr)
{
  if (fPtr) {
    fPtr->referenceCount++;
  }
...
template <typename T> shared_ptr<T> &shared_ptr<T>::operator=(T *rhs)
{
...
template <typename T> unsigned int shared_ptr<T>::use_count()
{
...
template <typename T> bool shared_ptr<T>::unique() const
{
