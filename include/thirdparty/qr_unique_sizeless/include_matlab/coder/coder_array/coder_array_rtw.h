...
#define CODER_ALLOC(T, N)                                                      \
  reinterpret_cast<T *>(new char[sizeof(T) * static_cast<size_t>(N)])
...
public:
  ...
  array_base(array_base const &_other) : data_(_other.data_)
  {
  ...
  array_base<T, SZ, 10> reshape(SZ _n1, SZ _n2, SZ _n3, SZ _n4, SZ _n5, SZ _n6,
                                SZ _n7, SZ _n8, SZ _n9, SZ _n10) const
  {
...
template <> class array<char, 2> : public array_base<char, SizeType, 2> {
private:
...
public:
  ...
  array(array<char, 2> const &_other) : Base(_other)
  {
  }
  ...
  array(char *_data, SizeType const *_sz) : Base(_data, _sz)
  {
  }
