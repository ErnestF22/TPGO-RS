...
public:
  ...
  void *operator new(size_t size, void *place) throw()
  {
  ...
  const MessageHeader &header() const
  {
    ...
    const MessageHeader *p = reinterpret_cast<const MessageHeader *>(
        payload() - sizeof(MessageHeader));
    assert((reinterpret_cast<size_t>(p) & 3) == 0);
    return *p;
  }
  MessageHeader &header()
  {
    return const_cast<MessageHeader &>(
  ...
  void payloadSize(uint16_t v)
  {
    header().payloadSize(v);
  }
  ...
  void appId(uint8_t v)
  {
    header().appId(v);
  }

  uint8_t appFun() const
  {
    return header().appFun();
  }
  void appFun(uint8_t v)
  {
    header().appFun(v);
  }
  ...
  uint8_t *payload()
  {
    return const_cast<uint8_t *>(static_cast<const Message *>(this)->payload());
  }

  uint16_t transmitSize() const
  {
    return header().payloadSize() + (uint16_t)sizeof(MessageHeader);
  }

  const uint8_t *transmitStart() const
  {
    return payload() - sizeof(MessageHeader);
  }
  uint8_t *transmitStart()
  {
    return const_cast<uint8_t *>(
        static_cast<const Message *>(this)->transmitStart());
  }

  static uint16_t memoryNeeded(uint16_t payloadSize)
  {
    return sizeof(Message) + payloadSize;
  }
