...
template <typename valueType> struct circular_iterator {
  ...
  explicit circular_iterator(valueType *it, valueType *begin, valueType *end)
      : it_(it), begin_(begin), end_(end)
  {
  }

  bool operator==(const circular_iterator<valueType> &rhs) const
  {
    return rhs.it_ == it_;
  }
  bool operator!=(const circular_iterator<valueType> &rhs) const
  ...
  circular_iterator<valueType> &operator+=(int dist)
  {
    advance(dist);
    return *this;
  }
  ...
  circular_iterator<valueType> &operator++()
  {
    advance();
    return *this;
  }
  ...
  circular_iterator<valueType> operator++(int)
  {
    coder::tgtsvc::detail::circular_iterator<valueType> temp(*this);
    advance();
    return temp;
  }
  circular_iterator<valueType> operator--(int)
  ...
  circular_iterator<valueType> operator+(ptrdiff_t dist) const
  {
    coder::tgtsvc::detail::circular_iterator<valueType> temp(*this);
    temp.advance(dist);
    return temp;
...
public:
  ...
  bool empty() const
  {
    return space_ == contents_;
  }
  bool full() const
  {
    return contents_size() == N - 1;
  }
  void clear()
  {
    contents_ = buff_;
  ...
  circular_iterator<valueType> contents_begin()
  {
    return circular_iterator<valueType>(const_cast<T *>(contents_), buff_,
                                        buff_ + N);
  }
  circular_iterator<valueType> contents_end()
  {
    return circular_iterator<valueType>(const_cast<T *>(space_), buff_,
                                        buff_ + N);
  }

  circular_iterator<valueType> space_begin()
  {
    return circular_iterator<valueType>(const_cast<T *>(space_), buff_,
                                        buff_ + N);
  }
  circular_iterator<valueType> space_end()
  {
