...
struct Chunk : public SListBaseHook<> {
  ...
  uint8_t poolIndex() const
  {
    return header()[POOL_INDEX_OFFSET];
  }
  void poolIndex(uint8_t v)
  {
    header()[POOL_INDEX_OFFSET] = v;
  }

  bool allocated() const
  {
    return header()[ALLOCATED_OFFSET] == 1;
  }
  void allocated(bool v)
  {
    header()[ALLOCATED_OFFSET] = (uint8_t)v;
  }
...
public:
      ...
      : poolSizes_(poolSizes), poolCount_(poolCnt)
  {
    ...
    for (uint8_t i = 0; i < poolCount(); ++i) {
      ...
      if (i > 0)
        assert(poolSize(i) > poolSize(i - 1));
      else
        assert(poolSize(i) >= sizeof(void *));
    }
  }
...
private:
  ...
  {
    uint8_t r = 0;
    while (r < poolCount() && poolSize(r) < requestSize)
      ++r;
    return r;
  }
