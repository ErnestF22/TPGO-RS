...
/* Conversion from unsigned __int64 to double is not implemented in Visual
 * Studio and results in a compile error, thus the value must first be cast to
...
#else
#define double_to_uint64(d)                                                    \
  (((d) >= 18446744073709551616.0) ? (unsigned long long)0xffffffffffffffffULL \
   : ((d) >= 0)                    ? (unsigned long long)(d)                   \
