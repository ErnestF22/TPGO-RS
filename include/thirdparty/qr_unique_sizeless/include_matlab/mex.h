...
typedef mexLocalFunctionTable (*fn_mex_set_local_function_table)(
...
#define mexGet(a, b)                                                           \
  NULL;                                                                        \
  do {                                                                         \
    mexGetIsDeprecated x;                                                      \
  } while (0)
#define mexSet(a, b, c)                                                        \
  0;                                                                           \
  do {                                                                         \
    mexSetIsDeprecated x;                                                      \
  } while (0)
#endif /* MEX_DOUBLE_HANDLE */
#define mexSetTrapFlag(a)                                                      \
  0;                                                                           \
  do {                                                                         \
    mexSetTrapFlagIsDeprecated x;                                              \
  } while (0)
 ...
 */
LIBMWMEX_API_EXTERN_C void
mexErrMsgTxt(const char *error_msg /* string with error message */
);

/*
 * Issue formatted error message with corresponding error identifier and return
 * to MATLAB prompt.
 */
...
/*
 * Invoke an unidentified warning. Such warnings can only be affected by the
 * M-code 'warning * all', since they have no specific identifier. See also
 * mexWarnMsgIdAndTxt.
 */
LIBMWMEX_API_EXTERN_C void
mexWarnMsgTxt(const char *warn_msg /* string with warning message */
);

/*
 * Invoke a warning with message identifier 'identifier' and message derived
 * from 'fmt' and subsequent arguments. The warning may either get printed as is
 * (if it is set to 'on'), or not actually get printed (if set to 'off'). See
 * 'help warning' in MATLAB for more details.
 */
 ...
 */
LIBMWMEX_API_EXTERN_C int mexPrintf(const char *fmt, /* printf style format */
                                    ... /* any additional arguments */
);
 ...
 */
LIBMWMEX_API_EXTERN_C void
mexMakeArrayPersistent(mxArray *pa /* pointer to array */
);
 ...
 */
LIBMWMEX_API_EXTERN_C int
mexCallMATLAB(int nlhs,            /* number of expected outputs */
 ...
 */
LIBMWMEX_API_EXTERN_C mxArray *
mexCallMATLABWithTrap(int nlhs,            /* number of expected outputs */
                      mxArray *plhs[],     /* pointer array to outputs */
                      int nrhs,            /* number of inputs */
                      mxArray *prhs[],     /* pointer array to inputs */
                      const char *fcn_name /* name of function to execute */
 ...
 */
LIBMWMEX_API_EXTERN_C int
mexPutVariable(const char *workspace, const char *name,
 ...
 */
LIBMWMEX_API_EXTERN_C const mxArray *
mexGetVariablePtr(const char *workspace, const char *name /* name of symbol */
);
...
LIBMWMEX_API_EXTERN_C mxArray *mexGetVariable(
    const char *workspace, const char *name /* name of variable in question */
);
 ...
 */
LIBMWMEX_API_EXTERN_C int
mexEvalString(const char *str /* matlab command string */
);
 ...
 */
LIBMWMEX_API_EXTERN_C mxArray *
mexEvalStringWithTrap(const char *str /* matlab command string */
);
...
LIBMWMEX_API_EXTERN_C const mxArray *mexGet(double handle,
                                            const char *property);

/*
 * mex equivalent to MATLAB's "set" function
 */
LIBMWMEX_API_EXTERN_C int mexSet(double handle, const char *property,
                                 mxArray *value);
