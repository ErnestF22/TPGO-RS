...
/**
* @param[in] VisitOrderSpecified  flag indicating if Visit Order of Polygons is
specified.
* @param[out] SpecifiedVisitSequenceC a pointer to the Visiting order.
* @param[in] MinAdjacencyCount  Minimum Adjacency Count required for permutation
to be considered valid.
* @param[in] StartPosesC  row vector containing start positions corresponding to
four different sweep patterns in each polygon.
* @param[out] EndPosesC row vector containing exit positions corresponding to
four different sweep patterns in each polygon.
...
* @param[out] TransitionCost  Cost to transition between polygons for given
sequence and sweep sequence.

*/
// Exhaustive Planner API
EXTERN_C LIBMWCOVERAGE_API void planExhaustivePathAPI(
    size_t *SequenceC, size_t *SweepSequenceC,
    double *TransitionCost, // Outputs
    bool VisitOrderSpecified, size_t const *SpecifiedVisitSequenceC,
    int MinAdjacencyCount, // Unique Inputs
    size_t const *AdjacencyInfoC, double const *StartPosesC,
    double const *EndPosesC, double *TakeoffArr, double *LandingArr,
    size_t NumPolygons, size_t NumPoses, size_t NumAdjacency);
...
*/
EXTERN_C LIBMWCOVERAGE_API void planMinTraversalPathAPI(
    size_t *SequenceC, size_t *SweepSequenceC,
    double *TransitionCost, // Outputs
    bool VisitOrderSpecified, size_t const *SpecifiedVisitSequenceC,
