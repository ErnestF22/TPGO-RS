...
constructCoverageOptions(const std::vector<double> &startPoses,
...
class PolygonGraph { // Utility functions that are common to both MinTraversal
                     // and Exhaustive Path Optimizer.
public:
  // AdjacencyMatrix representing the connections.
  std::vector<std::vector<int>> AdjacencyMatrix;
  // Represent the neighbours
  std::vector<std::vector<size_t>> AdjacencyGraph;
  PolygonGraph(const std::vector<size_t> &AdjacencyInfo,
               const std::vector<size_t> &PolygonIndices,
               const std::vector<double> &startPoses,
               const std::vector<double> &endPoses, const double TakeoffArr[],
               const double LandingArr[]);
