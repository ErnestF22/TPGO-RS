 ...
 */
MAVLINK_HELPER bool
mavlink_signature_check(mavlink_signing_t *signing,
                        ...
                        const mavlink_message_t *msg)
{
  ...
  mavlink_sha256_update(
      &ctx, p, MAVLINK_NUM_HEADER_BYTES); /*This fix is to make the signature
                                             check compiler agnostic.*/
    ...
    uint8_t crc_extra)
{
  ...
  if (signing) {
    mavlink_sign_packet(
        status->signing, msg->signature, (const uint8_t *)buf, header_len,
 ...
 */
MAVLINK_HELPER void
_mav_finalize_message_chan_send(mavlink_channel_t chan, uint32_t msgid,
                                const char *packet, uint8_t min_length,
