...
mavlink_get_message_info_by_id(uint32_t msgid)
{
  ...
  */
  uint32_t low = 0, high = sizeof(mavlink_message_info) /
                           sizeof(mavlink_message_info[0]);
  while (low < high) {
...
mavlink_get_message_info_by_name(const char *name)
{
  ...
  */
  uint32_t low = 0, high = sizeof(mavlink_message_names) /
                           sizeof(mavlink_message_names[0]);
  while (low < high) {
    uint32_t mid = (low + 1 + high) / 2;
