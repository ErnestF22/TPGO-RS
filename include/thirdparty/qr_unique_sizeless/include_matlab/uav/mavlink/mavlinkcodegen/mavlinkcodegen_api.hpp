 ...
 */
EXTERN_C MAVLINK_CODEGEN_API void
MAVLinkCodegen_constructMAVLinkCodegenPointer(void **ptrObj);

/// fill the MAVLinkML struct with channel specific information and encode the
/// MAVLink message structure to a byte array
/**
 * @param[in] ptrObj			a pointer to the
 * MAVLinkSerializerCodegenImpl class
 ...
 * @param[in] msgid				Msg ID of the message to be
 * serialized
 * @param[in] minlen			minimum Length of message
 * @param[in] len				maximum Length of message
 * @param[in] crcExtra			CRC of the message
 * @param[in] buffer			pointer to the output buffer to be
 * filled
 * @param[in] bufferLength		length of serialized message
 */
EXTERN_C MAVLINK_CODEGEN_API void MAVLinkCodegen_finalizeAndSerializeMsg(
    void *ptrObj, void *payloadBusObj, uint8_T sysid, uint8_T compid,
    uint32_T msgid, uint8_T minlen, uint8_T len, uint8_T crcExtra,
 ...
 * @param[in] ptrObj				a pointer to the
 * MAVLinkSerializerCodegenImpl class
 * @param[in] msgID					Msg ID of the decoded
 * message
 * @param[in] compid				Component ID of the decoded
 * message
 * @param[in] seq					Sequence of the decoded
 * message
 * @param[in] payloadObj			pointer to output payload
 * structure to be filled
 * @param[in] isNew					Is a new message for the
 * desired MsgID received
 ...
 */
EXTERN_C MAVLINK_CODEGEN_API void
MAVLinkCodegen_deserialize(void *ptrObj, uint32_T *msgID, uint8_T *sysid,
                           uint8_T *compid, uint8_T *seq, void *payloadObj,
                           boolean_T *isNew, uint8_T const *buffer, size_t len,
                           uint32_T desiredMsgID, bool filterBySystemID,
                           uint8_T desiredSystemID, bool filterByComponentID,
 ...
 */
EXTERN_C MAVLINK_CODEGEN_API void MAVLinkCodegen_setVersion(void *ptrObj,
                                                            double version);
 ...
 * @param[in] version   MAVLink protocol version
 */
EXTERN_C MAVLINK_CODEGEN_API void MAVLinkCodegen_getVersion(void *ptrObj,
                                                            double *version);

/// update the dialect with CRC_EXTRA information
/**
 * @param[in] ptrObj		a pointer to the MAVLinkSerializerCodegenImpl
 *class
 * @param[in] messageInfo   a 7*N vector containing [msgid, msgck, minlen,
 *maxlen, flags, target_sys_offset, target_comp_offset]
 * @param[in] size			Number of MAVLink messages in the parser
 */
EXTERN_C MAVLINK_CODEGEN_API void
MAVLinkCodegen_updateMessageEntries(void *ptrObj, double *messageInfo,
                                    double size);
 ...
 * @param[in] ptrObj			a pointer to the
 MAVLinkSerializerCodegenImpl class
 * @param[in] serializerObj		a pointer to the final
 ...
 */
EXTERN_C MAVLINK_CODEGEN_API void
MAVLinkCodegen_fetchSerializerObj(void *ptrObj, void **serializerObj,
 ...
 */
EXTERN_C MAVLINK_CODEGEN_API void
MAVLinkCodegen_createMessageQueue(void *ptrObj, uint16_T queueSize);
