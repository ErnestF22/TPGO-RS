...
/**
 * Interface between MATLAB struct definition and MAVLink internal packet
 * definition Support switching between MAVLink dialects at run time using
...
public:
   ...
   * thread safe
   */
  void finalizeMsg(mavlink_message_t &msg, uint8_t sysid, uint8_t compid,
                   uint8_t minlen, uint8_t len, uint8_t crcExtra) override;

  /// convert byte array to mavlink_message_t struct
  /**
   * thread safe, called during message callback
   */
  std::vector<MAVLinkMsgResult> deserialize(uint8_t const *buffer,
                                            size_t len) override;

  /// convert finalized mavlink_message_t struct to byte array
  std::vector<uint8_t>
  serialize(std::vector<mavlink_message_t> const &msg) const override;
  ...
  /**
   * @param[in] entries   a 7*N row vector containing [msgid, msgck, minlen,
   * maxlen, flags, target_sys_offset, target_comp_offset] repeatedly
   *
   * thread safe
   */
  ...
  /**
   * @return     A struct containing [msgid, checksum, msgLength, flag,
   * sysid_offset, compid_offset]
   */
  ...
  void setSigningChannel(mavlink_signing_t *signingChannel,
                         mavlink_signing_streams_t *signingStream);
...
private:
  ...
  /// map from msgid to message info entries, default value includes common
  /// heartbeat message and system status
  /**
   * Each entry includes [msgid, checksum, minLength, maxLength, flag,
   * sysid_offset, compid_offset] For details, check the mavlink_msg_entry_t
   * struct definition in mavlink/mavlink_types.h
   */
  MessageEntryRepository::MsgEntryLookupTbl m_table = {
      {0, {0, 50, 9, 9, 0, 0, 0}}, {1, {1, 124, 31, 31, 0, 0, 0}}};
};
} // namespace mavlink
} // namespace uavlib
