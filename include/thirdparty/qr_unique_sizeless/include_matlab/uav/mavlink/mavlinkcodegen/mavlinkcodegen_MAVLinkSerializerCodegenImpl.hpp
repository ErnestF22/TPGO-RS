 ...
 * @brief Header for API of MAVLinkSerializerCodegenImpl to be exposed using
 * codegen.
 */
...
public:
   ...
   */
  void finalizeMsg(mavlink_message_t &msgs, uint8_t sysid, uint8_t compid,
                   uint8_t minlen, uint8_t len, uint8_t crcExtra) const;
   ...
   * @param[in] filterBySystemID		Filter received messages by
   * SystemID
   ...
   * @param[in] isNew					determine if the message
   * is New
   */
  void deserialize(uint8_t const *buffer, size_t len, uint32_t desiredMsgID,
                   bool filterBySystemID, uint8_t desiredSystemID,
                   bool filterByComponentID, uint8_t desiredComponentID,
                   mavlink_message_t &msg, bool &isNew) const;
  ...
  /**
   * @param[in] entries   a 7*N vector containing [msgid, msgck, minlen, maxlen,
   * flags, target_sys_offset, target_comp_offset]
   */
  ...
  MAVLinkCodegenSerializerImpl *fetchSerializerObj(uint8_t SystemID,
                                                   uint8_t ComponentID);
...
private:
  ...
  /// extract the message with requested MsgID from the received MAVLink
  /// messages
  /**
   ...
   * @param[in] desiredMsgID		MsgID to look for
   */
  void filterReceivedMessages(
      std::vector<mavlink_message_t> &filteredMsgs,
      std::vector<uavlib::mavlink::MAVLinkMsgResult> const &receivedMsgs,
      uint32_t desiredMsgID) const;

  /// extract the message with requested MsgID and SystemID from the received
  /// MAVLink messages
  /**
   * @param[in] filteredMsgs			Vector to store the filtered
   * messages
   * @param[in] receivedMsgs			Vector of the deserialized
   * messages
  ...
  void filterReceivedMessages(
      std::vector<mavlink_message_t> &filteredMsgs,
      std::vector<uavlib::mavlink::MAVLinkMsgResult> const &receivedMsgs,
      uint32_t desiredMsgID, uint8_t desiredSystemID) const;

  /// extract the message with requested MsgID, SystemID and ComponentID from
  /// the received MAVLink messages
  /**
   * @param[in] filteredMsgs			Vector to store the filtered
   * messages
   * @param[in] receivedMsgs			Vector of the deserialized
