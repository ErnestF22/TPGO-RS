...
public:
  ...
  virtual void finalizeMsg(mavlink_message_t &msg, uint8_t sysid,
                           uint8_t compid, uint8_t minlen, uint8_t len,
                           uint8_t crcExtra) = 0;

  /// convert byte array to mavlink_message_t struct
  virtual std::vector<MAVLinkMsgResult> deserialize(uint8_t const *buffer,
                                                    size_t len) = 0;

  /// convert finalized mavlink_message_t struct to byte array
  virtual std::vector<uint8_t>
  serialize(std::vector<mavlink_message_t> const &msg) const = 0;
