...
public:
  UavDubinsConnection(double airSpeed, double maxRollAngle,
                      double flightPathAngleLimits[2],
                      std::vector<double> disabledPathTypes);

  UavDubinsConnection(double airSpeed, double maxRollAngle,
                      double flightPathAngleLimits[2],
                      double disabledPathTypes[],
                      uint32_T mNumDisabledPathTypes);
  ...
  void connectAirplaneXYZ(const double startPose[4], const double goalPose[4],
                          const bool flagOptimal, const double *idx,
                          double motionLengths[3][6], double motionCosts[6],
                          UavDubinsPathsegments *psegObjs);
...
private:
  ...
  const std::vector<std::string> AllPathTypes{
      "LSLN",  "LSRN",  "RSLN",  "RSRN",  "RLRN",  "LRLN",  "HlLSL",
      "HlLSR", "HrRSL", "HrRSR", "HrRLR", "HlLRL", "LSLHl", "LSRHr",
      "RSLHl", "RSRHr", "RLRHr", "LRLHl", "LRSL",  "LRSR",  "LRLR",
  ...
  void optimizeMidAltitude(double startPoseDubins2D[3],
                           double goalPoseDubins2D[3], double theta,
                           int optStartCurve, double maxFlightPathAngle,
                           double absAltitude, double motionLength[2][3],
                           double motionCost[2], char motionType[2][3],
                           double lm[2], int *qualityFlag);

  void addArc(std::string tempMotionType, double tempMotionCost,
              double *tempMotionLength, double maxFlightPathAngle, double lm,
              char strMotion, double &lowestMotionCost,
              UavDubinsPathsegments *psegObjs, int optStartCurve,
              bool flagOptimal, int signFlightPathAngle);
};
} // namespace uav
