...
namespace dubins {
/**
 * @brief Functor for calculating the distance along UAV Dubins motion primitive
 * segments
 *
 * This function is used to allow both parallel (with TBB) and serial execution.
 */
template <typename T> class DubinsDistanceFunctor {
private:
  void *m_connObj;
  T *m_startPose;
...
public:
  DubinsDistanceFunctor(void *connObj, T *startPose,
                        const uint32_T numStartPoses, T *goalPose,
                        const uint32_T numGoalPoses, const T *idx1,
                        const boolean_T flagOptimal, const T minTurningRadius,
                        real64_T *sp, real64_T *gp, real64_T *fpa, real64_T *a,
                        real64_T *mtr, real64_T *h, real64_T *mt, real64_T *ml)
      : m_connObj(connObj), m_startPose(startPose),
        m_maxNumPoses(numStartPoses > numGoalPoses ? numStartPoses
                                                   : numGoalPoses),
        m_numStartPoses(numStartPoses), m_numGoalPoses(numGoalPoses),
        m_goalPose(goalPose), idx(idx1), m_flagOptimal(flagOptimal),
        m_minTurningRadius(minTurningRadius), m_sp(sp), m_gp(gp), m_fpa(fpa),
        m_a(a), m_mtr(mtr), m_h(h), m_mt(mt), m_ml(ml),
        m_nRows(flagOptimal != 1 ? 28 : 1)
  {
    ...
    if (m_flagOptimal) {
      for (uint32_T j = 0; j < m_maxNumPoses; j++) {
        pathSegObjs[j].setMethod(s, s, 0, obj->getAirSpeed(),
                                 m_minTurningRadius, 0,
                                 static_cast<int>(idx[0]), tempVar);
      }
    } else {
      for (uint32_T i = 0; i < m_nRows; i++) {
        for (uint32_T j = 0; j < m_maxNumPoses; j++) {
          pathSegObjs[i * m_maxNumPoses + j].setMethod(
              s, s, 0, obj->getAirSpeed(), m_minTurningRadius, 0,
              static_cast<int>(idx[i]), tempVar);
        }
      }
    }
    ...
    for (uint32_T i = 0; i < m_numStartPoses; i++) {
      ...
      startPoseDubins2D[i + m_numStartPoses] =
          -m_startPose[i + 1 * m_numStartPoses];
      startPoseDubins2D[i + 2 * m_numStartPoses] =
          -m_startPose[i + 3 * m_numStartPoses];
    }
  ...
  void operator()(const uav::ParallelRange<uint32_T> &range) const
  {
    ...
    if (m_numStartPoses >= m_numGoalPoses && m_numGoalPoses == 1) {
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        obj->connectAirplaneXYZ(&a[0], &b[0], m_flagOptimal, idx,
                                tempMotionLengths, tempMotionCosts,
                                &tempPathSegObjs[0]);
        for (uint32_T j = 0; j < static_cast<uint32_T>(tempPathSegObjs.size());
             ++j) {
          ...
          m_ml[4 * (j * m_maxNumPoses + i) + 0] =
              tempPathSegObjs[j].MotionLengths[0];
          m_ml[4 * (j * m_maxNumPoses + i) + 1] =
              tempPathSegObjs[j].MotionLengths[1];
          m_ml[4 * (j * m_maxNumPoses + i) + 2] =
              tempPathSegObjs[j].MotionLengths[2];
          m_ml[4 * (j * m_maxNumPoses + i) + 3] =
              tempPathSegObjs[j].MotionLengths[3];
        }
      }
