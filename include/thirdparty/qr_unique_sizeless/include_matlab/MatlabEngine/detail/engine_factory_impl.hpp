...
startMATLAB(const std::vector<std::u16string> &options)
{
  return startMATLABAsync(options).get();
}

inline FutureResult<std::unique_ptr<MATLABEngine>>
startMATLABAsync(const std::vector<std::u16string> &options)
{
  initSession();
  ...
  std::future<std::unique_ptr<MATLABEngine>> stdF =
      std::async(std::launch::async, startMATLABType);
...
{
  std::future<std::vector<std::u16string>> stdFuture =
      std::async(std::launch::async, findMATLAB);
  FutureResult<std::vector<std::u16string>> future(
      std::move(stdFuture), std::make_shared<TaskReference>(&cancelFind));
  return future;
}
...
inline FutureResult<std::unique_ptr<MATLABEngine>> connectMATLABAsync()
{
  ...
  if (!engines.empty()) {
    ...
    std::u16string option2 =
        convertUTF8StringToUTF16String("matlab.engine.shareEngine");
...
connectMATLABAsync(const std::u16string &name)
{
  ...
  std::future<std::unique_ptr<MATLABEngine>> stdF =
      std::async(std::launch::async, connectMATLABType);
  FutureResult<std::unique_ptr<MATLABEngine>> future(std::move(stdF));
  return future;
}
...
{
  // initialize the session and load the library so the call to
  // engine_terminate_session will not crash.
  initSession();
