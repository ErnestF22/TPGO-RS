 ...
 * @return std::unique_ptr<MATLABEngine> - A unique pointer of MATLABEngine
 * object
 *
 * @throw EngineException if failed to start MATLAB
 */
std::unique_ptr<MATLABEngine> startMATLAB(
 ...
 *
 * @param options -  Startup options used to launch MATLAB
 * @return FutureResult<std::unique_ptr<MATLABEngine>> - A future of a unique
 * pointer of a MATLABEngine object
 *
 * @throw none
 */
FutureResult<std::unique_ptr<MATLABEngine>> startMATLABAsync(
    const std::vector<std::u16string> &options = std::vector<std::u16string>());

/**
 * Find all shared MATLAB sessions synchronously
 *
 * @return std::vector<std::u16string> - A vector of names of shared MATLAB
 * sessions
 *
 * @throw none
 */
 ...
 * Find all shared MATLAB sessions synchronously
 *
 * @return FutureResult<std::vector<std::u16string>> - A future of a vector of
 ...
 *
 * @return std::unique_ptr<MATLABEngine> - A unique pointer of a MATLABEngine
 * object
 *
 * @throw EngineException if failed to start or connect to a shared MATLAB
 * session
 */
...
connectMATLABAsync(const std::u16string &name);

/**
 * Terminate the engine client.  The Engine application will be no longer able
 * to start MATLAB after calling this function.
 *
 * @return void
 *
 * @throw none
