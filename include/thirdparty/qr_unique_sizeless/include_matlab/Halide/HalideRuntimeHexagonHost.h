...
extern int halide_hexagon_wrap_device_handle(void *user_context,
 ...
 * on return. */
extern int halide_hexagon_detach_device_handle(void *user_context,
                                               struct halide_buffer_t *buf);

/** Return the underlying device handle for a halide_buffer_t. If there is
 * no device memory (dev field is NULL), this returns 0. */
extern void *halide_hexagon_get_device_handle(void *user_context,
                                              struct halide_buffer_t *buf);
extern uint64_t halide_hexagon_get_device_size(void *user_context,
                                               struct halide_buffer_t *buf);
...
extern void halide_hexagon_power_hvx_off_as_destructor(void *user_context,
                                                       void * /* obj */);
// @}
 ...
 * @param mipsPerThread - mips requested per thread, to establish a minimal
 * clock frequency per HW thread
 * @param mipsTotal - Total mips requested, to establish total number of MIPS
 * required across all HW threads
 * @param set_bus_bw - Set to TRUE to request bus_bw
 * @param bwMeagabytesPerSec - Max bus BW requested (megabytes per second)
 * @param busbwUsagePercentage - Percentage of time during which bwBytesPerSec
 * BW is required from the bus (0..100)
 ...
 *                  take to awaken. Only values > 0 are supported (1 microsecond
...
// @{
extern int
halide_hexagon_set_performance_mode(void *user_context,
                                    halide_hexagon_power_mode_t mode);
extern int halide_hexagon_set_performance(void *user_context,
...
// @{
extern int halide_hexagon_initialize_kernels(
    void *user_context, void **module_ptr, const uint8_t *code,
    uint64_t code_size, const uint8_t *runtime, uint64_t runtime_size);
extern int halide_hexagon_run(void *user_context, void *module_ptr,
                              const char *name,
                              halide_hexagon_handle_t *function,
                              uint64_t arg_sizes[], void *args[],
                              int arg_flags[]);
extern void halide_hexagon_finalize_kernels(void *user_context,
                                            void *state_ptr);
