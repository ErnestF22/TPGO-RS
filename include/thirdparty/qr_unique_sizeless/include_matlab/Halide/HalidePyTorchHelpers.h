...
#define HLPT_CHECK_DEVICE(x, dev)                                              \
  AT_ASSERTM(x.is_cuda() && x.get_device() == dev, #x " must be a CUDA "       \
                                                      "tensor")
...
template <class scalar_t> inline void check_type(at::Tensor &tensor)
{
  AT_ERROR("Scalar type ", tensor.scalar_type(),
...
#define HL_PT_DEFINE_TYPECHECK(ctype, ttype)                                   \
  template <> inline void check_type<ctype>(at::Tensor & tensor)               \
  {                                                                            \
    AT_ASSERTM(tensor.scalar_type() == at::ScalarType::ttype,                  \
               "scalar type do not match");                                    \
  }
...
template <class scalar_t> inline Buffer<scalar_t> wrap(at::Tensor &tensor)
{
...
template <class scalar_t> inline Buffer<scalar_t> wrap_cuda(at::Tensor &tensor)
{
  check_type<scalar_t>(tensor);
  std::vector<int> dims = get_dims(tensor);
#if HL_PYTORCH_API_VERSION >= 13
  ...
  const halide_device_interface_t *cuda_interface =
