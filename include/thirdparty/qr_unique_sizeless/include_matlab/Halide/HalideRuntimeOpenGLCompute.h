 ...
 *  compiles src OpenGL compute shader into OpenGL program and stores it for
 * future use.
 */
extern int halide_openglcompute_initialize_kernels(void *user_context,
                                                   void **state_ptr,
                                                   const char *src, int size);

/** This function triggers execution of OpenGL program built around compute
 * shader. Execution of the shader is parallelized into given number of blocks
 * and threads.
 *
 *  This function doesn't wait for the completion of the shader, but it sets
 * memory barrier which forces successive retrieval of output data to wait until
 * shader is done.
 */
extern int halide_openglcompute_run(void *user_context, void *state_ptr,
                                    ...
                                    void *args[], int8_t is_buffer[]);

extern void halide_openglcompute_finalize_kernels(void *user_context,
                                                  void *state_ptr);
// @}

/** This function retrieves pointers to OpenGL API functions.
 *
 *  You may have to implement this yourself. Halide only provides
