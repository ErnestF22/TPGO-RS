...
static_assert(
    ((HALIDE_RUNTIME_BUFFER_ALLOCATION_ALIGNMENT &
      (HALIDE_RUNTIME_BUFFER_ALLOCATION_ALIGNMENT - 1)) == 0),
...
template <typename T, int Dims, int InClassDimStorage> class Buffer;

// A helper to check if a parameter pack is entirely implicitly
// int-convertible to use with std::enable_if
template <typename... Args> struct AllInts : std::false_type {};

template <> struct AllInts<> : std::true_type {};

template <typename T, typename... Args> struct AllInts<T, Args...> {
  static const bool value =
      std::is_convertible<T, int>::value && AllInts<Args...>::value;
};
...
// it here.
template <typename... Args> struct AllInts<float, Args...> : std::false_type {};
...
namespace Internal {
template <typename Container> bool any_zero(const Container &c)
{
  for (int i : c) {
    if (i == 0) {
      return true;
...
{
  Allocated, ///> halide_device_free will be called when device ref count goes
             /// to zero
  WrappedNative, ///> halide_device_detach_native will be called when device ref
                 /// count goes to zero
  Unmanaged, ///> No free routine will be called when device ref count goes to
             /// zero
  AllocatedDeviceAndHost, ///> Call device_and_host_free when DevRefCount goes
                          /// to zero.
...
class Buffer {
  ...
  using add_const_if_T_is_const =
      typename std::conditional<std::is_const<T>::value, const T2, T2>::type;

  /** T unless T is (const) void, in which case (const)
   * uint8_t. Useful for providing return types for operator() */
  using not_void_T =
      typename std::conditional<T_is_void, add_const_if_T_is_const<uint8_t>,
                                T>::type;
   ...
   * systems. */
  using storage_T = typename std::conditional<std::is_pointer<T>::value,
...
private:
  ...
  /** Setup the device ref count for a buffer to indicate it is a crop (or
   * slice, embed, etc) of cropped_from */
    ...
    if (new_count == 0) {
      if (buf.device) {
        assert(!(alloc && device_dirty()) &&
               "Implicitly freeing a dirty device allocation while a host "
               "allocation still lives. "
               "Call device_free explicitly if you want to drop dirty "
               "device-side data. "
               "Call copy_to_host explicitly if you want the data copied to "
               "the host allocation "
        ...
        } else if (dev_ref_count &&
                   dev_ref_count->ownership ==
                       BufferDeviceOwnership::AllocatedDeviceAndHost) {
          result = buf.device_interface->device_and_host_free(nullptr, &buf);
        } else if (dev_ref_count &&
                   dev_ref_count->ownership == BufferDeviceOwnership::Cropped) {
          result = buf.device_interface->device_release_crop(nullptr, &buf);
        } else if (dev_ref_count == nullptr ||
                   dev_ref_count->ownership ==
                       BufferDeviceOwnership::Allocated) {
          result = buf.device_interface->device_free(nullptr, &buf);
        }
        // No reasonable way to return the error, but we can at least
        // assert-fail in debug builds.
        assert((result == 0) && "device_interface call returned a nonzero "
                                "result in Buffer::decref()");
        (void)result;
      }
      if (dev_ref_count) {
  ...
  template <int DimsSpecified> void make_static_shape_storage()
  {
    static_assert(
        Dims == AnyDims || Dims == DimsSpecified,
  ...
  void initialize_shape_from_array_shape(int next, Array (&vals)[N])
  {
    ...
    if (next == 0) {
      ...
      buf.dim[next].stride =
          buf.dim[next - 1].stride * buf.dim[next - 1].extent;
    }
  }

  /** Base case for the template recursion above. */
  template <typename T2> void initialize_shape_from_array_shape(int, const T2 &)
  {
  }
  ...
  template <typename T2> static int dimensionality_of_array(const T2 &)
  {
    return 0;
  }
  ...
  template <typename T2> static halide_type_t scalar_type_of_array(const T2 &)
  {
  ...
  /** Crop as many dimensions as are in rect, without handling device
   * allocation. */
  void crop_host(const std::vector<std::pair<int, int>> &rect)
  {
    assert(rect.size() <=
           static_cast<decltype(rect.size())>(std::numeric_limits<int>::max()));
  ...
  void complete_device_crop(
      Buffer<T, Dims, InClassDimStorage> &result_host_cropped) const
  {
    assert(buf.device_interface != nullptr);
    if (buf.device_interface->device_crop(nullptr, &this->buf,
                                          &result_host_cropped.buf) == 0) {
      const Buffer<T, Dims, InClassDimStorage> *cropped_from = this;
      // TODO: Figure out what to do if dev_ref_count is nullptr. Should incref
      // logic run here? is it possible to get to this point without incref
      // having run at least once since the device field was set? (I.e. in the
      // internal logic of crop. incref might have been called.)
  ...
  void complete_device_slice(
      Buffer<T, AnyDims, InClassDimStorage> &result_host_sliced, int d,
      int pos) const
  {
    assert(buf.device_interface != nullptr);
    if (buf.device_interface->device_slice(nullptr, &this->buf, d, pos,
                                           &result_host_sliced.buf) == 0) {
      const Buffer<T, Dims, InClassDimStorage> *sliced_from = this;
      // TODO: Figure out what to do if dev_ref_count is nullptr. Should incref
      // logic run here? is it possible to get to this point without incref
      // having run at least once since the device field was set? (I.e. in the
      // internal logic of slice. incref might have been called.)
      if (dev_ref_count != nullptr &&
          dev_ref_count->ownership == BufferDeviceOwnership::Cropped) {
...
public:
  ...
  {
    assert(buf.host !=
           nullptr); // Cannot call begin() on an unallocated Buffer.
    return (T *)buf.begin();
  }
  ...
  /** Give Buffers access to the members of Buffers of different
   * dimensionalities and types. */
  template <typename T2, int D2, int S2> friend class Buffer;
...
private:
  ...
  static void static_assert_can_convert_from()
  {
    ...
    static_assert(Dims == AnyDims || D2 == AnyDims || Dims == D2,
                  "Can't convert from a Buffer with static dimensionality to a "
                  "Buffer with different static dimensionality");
  }

public:
  /** Determine if a Buffer<T, Dims, InClassDimStorage> can be constructed from
   * some other Buffer type. If this can be determined at compile time, fail
   * with a static assert; otherwise return a boolean based on runtime typing.
   */
  template <typename T2, int D2, int S2>
  ...
  /** Fail an assertion at runtime or compile-time if an Buffer<T, Dims,
   * InClassDimStorage> cannot be constructed from some other Buffer type. */
  template <typename T2, int D2, int S2>
  ...
  Buffer(const Buffer<T, Dims, InClassDimStorage> &other)
      : buf(other.buf), alloc(other.alloc)
  {
    other.incref();
   ...
   * Buffer<const T>. */
  template <typename T2, int D2, int S2>
  Buffer(const Buffer<T2, D2, S2> &other) : buf(other.buf), alloc(other.alloc)
  {
  ...
  Buffer(Buffer<T, Dims, InClassDimStorage> &&other) noexcept
      : buf(other.buf), alloc(other.alloc), dev_ref_count(other.dev_ref_count)
  {
  ...
  operator=(const Buffer<T, Dims, InClassDimStorage> &other)
  {
  ...
  Buffer<T, Dims, InClassDimStorage> &
  operator=(Buffer<T, Dims, InClassDimStorage> &&other) noexcept
  {
    decref();
    alloc = other.alloc;
    other.alloc = nullptr;
                ...
                void (*deallocate_fn)(void *) = nullptr)
  {
    ...
    // alignment for all the platforms we might use. Also ensure that the
    // allocation is such that the logical size is an integral multiple of 128
    // bytes (or a bit more).
    ...
    if (!allocate_fn && !deallocate_fn) {
      // As a practical matter, sizeof(AllocationHeader) is going to be no more
      // than 16 bytes on any supported platform, so we will just overallocate
      // by 'alignment' so that the user storage also starts at an aligned
      // point. This is a bit wasteful, but probably not a big deal.
      static_assert(sizeof(AllocationHeader) <= alignment);
      void *alloc_storage =
          ::aligned_alloc(alignment, align_up(size) + alignment);
    ...
    // alignof(std::max_align_t); make sure this is OK for AllocationHeader,
    // since it always goes at the start
  ...
  explicit Buffer(int first)
  {
    static_assert(!T_is_void,
                  "To construct an Buffer<void>, pass a halide_type_t as the "
...
private:
  ...
  static std::vector<int> make_ordered_sizes(const std::vector<int> &sizes,
...
public:
  /** Allocate a new image of unknown type using a vector of ints as the size
   * and a vector of indices indicating the storage order for each dimension.
   * The length of the sizes vector and the storage-order vector must match. For
   * instance, to allocate an interleaved RGB buffer, you would pass {2, 0, 1}
   * for storage_order. */
  Buffer(halide_type_t t, const std::vector<int> &sizes,
  ...
  template <typename Array, size_t N> explicit Buffer(Array (&vals)[N])
  {
  ...
  explicit Buffer(T *data, int first, Args &&...rest)
  {
    ...
    buf.type = static_halide_type();
    buf.host =
        (uint8_t *)const_cast<typename std::remove_const<T>::type *>(data);
    constexpr int buf_dimensions = 1 + (int)(sizeof...(rest));
    make_static_shape_storage<buf_dimensions>();
    initialize_shape(extents);
  }
  ...
  explicit Buffer(halide_type_t t, add_const_if_T_is_const<void> *data, int d,
  ...
  Buffer<typename std::add_const<T>::type, Dims, InClassDimStorage> &
  as_const() &
  {
    // Note that we can skip the assert_can_convert_from(), since T -> const T
    // conversion is always legal.
    return *((Buffer<typename std::add_const<T>::type, Dims, InClassDimStorage>
                  *)this);
  }

  HALIDE_ALWAYS_INLINE
  const Buffer<typename std::add_const<T>::type, Dims, InClassDimStorage> &
  as_const() const &
  {
    return *((const Buffer<typename std::add_const<T>::type, Dims,
                           InClassDimStorage> *)this);
  }
   ...
   */
  Buffer<not_const_T, Dims, InClassDimStorage>
  copy(void *(*allocate_fn)(size_t) = nullptr,
       void (*deallocate_fn)(void *) = nullptr) const
  {
    Buffer<not_const_T, Dims, InClassDimStorage> dst =
        Buffer<not_const_T, Dims, InClassDimStorage>::make_with_shape_of(
            *this, allocate_fn, deallocate_fn);
    dst.copy_from(*this);
    return dst;
  }
   ...
   */
  Buffer<not_const_T, Dims, InClassDimStorage>
  copy_to_interleaved(void *(*allocate_fn)(size_t) = nullptr,
                      void (*deallocate_fn)(void *) = nullptr) const
  {
    static_assert(Dims == AnyDims || Dims == 3);
    assert(dimensions() == 3);
    Buffer<not_const_T, Dims, InClassDimStorage> dst =
        Buffer<not_const_T, Dims, InClassDimStorage>::make_interleaved(
            nullptr, width(), height(), channels());
   ...
   */
  Buffer<not_const_T, Dims, InClassDimStorage>
  copy_to_planar(void *(*allocate_fn)(size_t) = nullptr,
                 void (*deallocate_fn)(void *) = nullptr) const
  {
    ...
    Buffer<not_const_T, Dims, InClassDimStorage> dst =
        Buffer<not_const_T, Dims, InClassDimStorage>(nullptr, extents);
    dst.set_min(mins);
    dst.allocate(allocate_fn, deallocate_fn);
    dst.copy_from(*this);
    return dst;
   ...
   * non-const copy (e.g. to make a call into AOT-generated Halide code), and
   * want a terse inline way to create a temporary. \code void
   * call_my_func(const Buffer<const uint8_t>& input) { my_func(input.alias(),
   * output);
   * }\endcode
   */
   ...
   */
  template <typename T2, int D2, int S2> void copy_from(Buffer<T2, D2, S2> src)
  {
    static_assert(!std::is_const<T>::value,
                  "Cannot call copy_from() on a Buffer<const T>");
    assert(!device_dirty() && "Cannot call Halide::Runtime::Buffer::copy_from "
                              "on a device dirty destination.");
    assert(!src.device_dirty() &&
           "Cannot call Halide::Runtime::Buffer::copy_from on a device dirty "
           "source.");
    ...
    if (T_is_void ? (type().bytes() == 1) : (sizeof(not_void_T) == 1)) {
      ...
      typed_dst.for_each_value([&](MemType &dst, MemType src) { dst = src; },
                               typed_src);
  ...
  Buffer<T, Dims, InClassDimStorage>
  cropped(const std::vector<std::pair<int, int>> &rect) const
  {
    // Make a fresh copy of the underlying buffer (but not a fresh
    // copy of the allocation, if there is one).
    Buffer<T, Dims, InClassDimStorage> im = *this;
  ...
  Buffer<T, Dims, InClassDimStorage>
  translated(const std::vector<int> &delta) const
  {
    Buffer<T, Dims, InClassDimStorage> im = *this;
    im.translate(delta);
    return im;
  ...
  {
    device_deallocate();
    assert(delta.size() <= static_cast<decltype(delta.size())>(
  ...
  template <typename... Args> void set_min(Args... args)
  {
  ...
  Buffer<T, Dims, InClassDimStorage>
  transposed(const std::vector<int> &order) const
  {
    Buffer<T, Dims, InClassDimStorage> im = *this;
    im.transpose(order);
    return im;
   ...
   *
   ...
   \endcode
   */
  Buffer<T, (Dims == AnyDims ? AnyDims : Dims + 1)> embedded(int d,
                                                             int pos = 0) const
  {
    Buffer<T, AnyDims, InClassDimStorage> im(*this);
    im.embed(d, pos);
  ...
  void add_dimension()
  {
    ...
    if (dims == 0) {
      ...
      buf.dim[dims].stride =
          buf.dim[dims - 1].extent * buf.dim[dims - 1].stride;
    }
  }
  ...
  {
    assert((!v || !device_dirty()) &&
           "Cannot set host dirty when device is already dirty. Call "
           "copy_to_host() before accessing the buffer from host.");
  ...
  int copy_to_device(const struct halide_device_interface_t *device_interface,
                     void *ctx = nullptr)
  {
    if (host_dirty()) {
  ...
  int device_malloc(const struct halide_device_interface_t *device_interface,
                    void *ctx = nullptr)
  {
  ...
  int device_free(void *ctx = nullptr)
  {
    if (dev_ref_count) {
      assert(
          dev_ref_count->ownership == BufferDeviceOwnership::Allocated &&
          "Can't call device_free on an unmanaged or wrapped native device "
          "handle. "
  ...
  int device_wrap_native(
      const struct halide_device_interface_t *device_interface, uint64_t handle,
      void *ctx = nullptr)
  {
  ...
  int device_and_host_malloc(
      ...
      void *ctx = nullptr)
  {
    return device_interface->device_and_host_malloc(ctx, &buf,
                                                    device_interface);
  }

  int device_and_host_free(
      const struct halide_device_interface_t *device_interface,
      void *ctx = nullptr)
  {
    if (dev_ref_count) {
      assert(
          dev_ref_count->ownership ==
              BufferDeviceOwnership::AllocatedDeviceAndHost &&
          "Can't call device_and_host_free on a device handle not allocated "
          "with device_and_host_malloc. "
          "Free the source allocation or call device_detach_native instead.");
      // Multiple people may be holding onto this dev field
      assert(dev_ref_count->count == 1 &&
             "Multiple Halide::Runtime::Buffer objects share this device "
             "allocation. Freeing it would create dangling references. "
             "Don't call device_and_host_free on Halide buffers that you have "
             "copied or "
             "passed by value.");
    }
    int ret = 0;
  ...
  static Buffer<void, Dims, InClassDimStorage>
  make_interleaved(halide_type_t t, int width, int height, int channels)
  {
    static_assert(Dims == AnyDims || Dims == 3,
                  "make_interleaved() must be called on a Buffer that can "
                  "represent 3 dimensions.");
   ...
   * generator has been compiled with support for interleaved (also
   * known as packed or chunky) memory layouts. */
  static Buffer<T, Dims, InClassDimStorage>
  make_interleaved(int width, int height, int channels)
  {
  ...
  make_interleaved(halide_type_t t, T *data, int width, int height,
                   int channels)
  {
    static_assert(Dims == AnyDims || Dims == 3,
                  "make_interleaved() must be called on a Buffer that can "
                  "represent 3 dimensions.");
    Buffer<add_const_if_T_is_const<void>, Dims, InClassDimStorage> im(
        t, data, channels, width, height);
    im.transpose(0, 1);
    im.transpose(1, 2);
    return im;
  }
  ...
  /** Wrap an existing interleaved image. */
  static Buffer<T, Dims, InClassDimStorage>
  make_interleaved(T *data, int width, int height, int channels)
  {
    return make_interleaved(static_halide_type(), data, width, height,
                            channels);
  }

  /** Make a zero-dimensional Buffer */
  static Buffer<add_const_if_T_is_const<void>, Dims, InClassDimStorage>
  make_scalar(halide_type_t t)
  {
    static_assert(Dims == AnyDims || Dims == 0,
                  "make_scalar() must be called on a Buffer that can represent "
                  "0 dimensions.");
  ...
  template <typename T2, int D2, int S2>
  static Buffer<T, Dims, InClassDimStorage>
  make_with_shape_of(Buffer<T2, D2, S2> src,
                     void *(*allocate_fn)(size_t) = nullptr,
                     void (*deallocate_fn)(void *) = nullptr)
  {
    static_assert(Dims == D2 || Dims == AnyDims);
    const halide_type_t dst_type =
        T_is_void ? src.type()
                  : halide_type_of<typename std::remove_cv<not_void_T>::type>();
    return Buffer<>::make_with_shape_of_helper(
        dst_type, src.dimensions(), src.buf.dim, allocate_fn, deallocate_fn);
  }
...
private:
  ...
  template <typename... Args>
  HALIDE_ALWAYS_INLINE ptrdiff_t offset_of(int d, int first, Args... rest) const
  {
...
public:
  ...
  template <typename... Args,
            typename = typename std::enable_if<AllInts<Args...>::value>::type>
  HALIDE_ALWAYS_INLINE const not_void_T &operator()(int first,
                                                    Args... rest) const
  {
    static_assert(!T_is_void, "Cannot use operator() on Buffer<void> types");
    constexpr int expected_dims = 1 + (int)(sizeof...(rest));
    static_assert(Dims == AnyDims || Dims == expected_dims,
                  "Buffer with static dimensions was accessed with the wrong "
                  "number of coordinates in operator()");
    assert(!device_dirty());
  ...
  HALIDE_ALWAYS_INLINE
  const not_void_T &operator()(const int *pos) const
  {
    static_assert(!T_is_void, "Cannot use operator() on Buffer<void> types");
    assert(!device_dirty());
  ...
  HALIDE_ALWAYS_INLINE
  not_void_T &operator()(const int *pos)
  {
    static_assert(!T_is_void, "Cannot use operator() on Buffer<void> types");
    set_host_dirty();
...
private:
  ...
  // @{
  template <int N> struct for_each_value_task_dim {
  ...
  HALIDE_ALWAYS_INLINE static void advance_ptrs(const std::ptrdiff_t *stride,
                                                Ptr &ptr, Ptrs &...ptrs)
  {
    ptr += *stride;
  ...
  HALIDE_NEVER_INLINE static void
  for_each_value_helper(Fn &&f, int d, bool innermost_strides_are_one,
                        const for_each_value_task_dim<sizeof...(Ptrs) + 1> *t,
                        Ptr ptr, Ptrs... ptrs)
  {
    if (d == 0) {
      ...
      for (std::ptrdiff_t i = t[d].extent; i != 0; i--) {
        for_each_value_helper(f, d - 1, innermost_strides_are_one, t, ptr,
                              ptrs...);
  ...
  template <int N>
  HALIDE_NEVER_INLINE static bool
  for_each_value_prep(for_each_value_task_dim<N> *t,
                      const halide_buffer_t **buffers)
  {
    ...
    for (int i = 0; i < N; i++) {
      if (buffers[i]->device) {
        assert(buffers[i]->host &&
               "Buffer passed to for_each_value has device allocation but no "
               "host allocation. Call allocate() and copy_to_host() first");
        assert(!buffers[i]->device_dirty() &&
               "Buffer passed to for_each_value is dirty on device. Call "
               "copy_to_host() first");
      } else {
        assert(
            buffers[i]->host &&
    ...
    for (int i = 0; i < dimensions; i++) {
      ...
      // Order the dimensions by stride, so that the traversal is
      // cache-coherent. Use the last dimension for this, because this is the
      // source in copies. It appears to be better to optimize read order than
      // write order.
      for (int j = i; j > 0 && t[j].stride[N - 1] < t[j - 1].stride[N - 1];
           j--) {
        std::swap(t[j], t[j - 1]);
      }
    }
  ...
  void for_each_value_impl(Fn &&f, Args &&...other_buffers) const
  {
    if (dimensions() > 0) {
      Buffer<>::for_each_value_task_dim<N> *t =
          (Buffer<>::for_each_value_task_dim<N> *)HALIDE_ALLOCA(
              (dimensions() + 1) * sizeof(for_each_value_task_dim<N>));
      ...
      bool innermost_strides_are_one =
          Buffer<>::for_each_value_prep(t, buffers);

      Buffer<>::for_each_value_helper(f, dimensions() - 1,
                                      innermost_strides_are_one, t, data(),
                                      (other_buffers.data())...);
    } else {
...
public:
  ...
  // @{
  template <typename Fn, typename... Args, int N = sizeof...(Args) + 1>
  HALIDE_ALWAYS_INLINE const Buffer<T, Dims, InClassDimStorage> &
  for_each_value(Fn &&f, Args &&...other_buffers) const
  {
  ...
  template <typename Fn, typename... Args, int N = sizeof...(Args) + 1>
  HALIDE_ALWAYS_INLINE Buffer<T, Dims, InClassDimStorage> &
...
private:
  ...
  HALIDE_ALWAYS_INLINE static void
  for_each_element_variadic(int, int, const for_each_element_task_dim *, Fn &&f,
                            Args... args)
  {
    f(args...);
  }
  ...
  template <typename Fn, typename... Args>
  HALIDE_ALWAYS_INLINE static void
  for_each_element_variadic(double, int d, const for_each_element_task_dim *t,
                            Fn &&f, Args... args)
  {
  ...
  HALIDE_ALWAYS_INLINE static int num_args(double, Fn &&f, Args... args)
  {
    static_assert(
        ...
        " or up to 256 ints. No such operator found. Expect infinite template "
        "recursion.");
  ...
  HALIDE_ALWAYS_INLINE static void
  for_each_element_array_helper(int, const for_each_element_task_dim *t, Fn &&f,
                                int *pos)
  {
  ...
  template <int d, typename Fn,
            typename = typename std::enable_if<(d < 0)>::type>
  HALIDE_ALWAYS_INLINE static void
  for_each_element_array_helper(double, const for_each_element_task_dim *t,
                                Fn &&f, int *pos)
  {
    f(pos);
  }
  ...
  static void for_each_element_array(int d, const for_each_element_task_dim *t,
                                     Fn &&f, int *pos)
  {
    if (d == -1) {
      f(pos);
  ...
  static void for_each_element(int, int dims,
                               const for_each_element_task_dim *t, Fn &&f,
                               int check = 0)
  {
    const int size = dims * sizeof(int);
    int *pos = (int *)HALIDE_ALLOCA(size);
    // At least one version of GCC will (incorrectly) report that pos "may be
    // used uninitialized". Add this memset to silence it.
    memset(pos, 0, size);
  ...
  template <typename Fn>
  HALIDE_ALWAYS_INLINE static void
  for_each_element(double, int dims, const for_each_element_task_dim *t, Fn &&f)
  {
  ...
  template <typename Fn> void for_each_element_impl(Fn &&f) const
  {
    for_each_element_task_dim *t = (for_each_element_task_dim *)HALIDE_ALLOCA(
        dimensions() * sizeof(for_each_element_task_dim));
    for (int i = 0; i < dimensions(); i++) {
      t[i].min = dim(i).min();
      t[i].max = dim(i).max();
    }
...
private:
  template <typename Fn> struct FillHelper {
    Fn f;
...
public:
  ...
  template <typename Fn, typename = typename std::enable_if<!std::is_arithmetic<
   ...
   * are initialized under MSAN. Note that by default, we use for_each_value()
   * here so that we skip any unused padding that isn't part of the Buffer; this
   * isn't efficient, but in MSAN mode, it doesn't matter. (Pass true for the
   * flag to force check the entire Buffer storage.) */
  void msan_check_mem_is_initialized(bool entire = false) const
  {
    ...
    if (entire) {
      ...
      for_each_value([](T &v) {
        __msan_check_mem_is_initialized(&v, sizeof(T));
        ;
      });
    }
#endif
