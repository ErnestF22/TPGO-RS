...
#ifdef _MSC_VER
// Note that (for MSVC) you should not use "inline" along with
// HALIDE_ALWAYS_INLINE; it is not necessary, and may produce warnings for some
// build configurations.
#define HALIDE_ALWAYS_INLINE __forceinline
#define HALIDE_NEVER_INLINE __declspec(noinline)
#else
// Note that (for Posixy compilers) you should always use "inline" along with
// HALIDE_ALWAYS_INLINE; otherwise some corner-case scenarios may erroneously
// report link errors.
...
extern halide_error_handler_t
halide_set_error_handler(halide_error_handler_t handler);
// @}
...
extern int halide_mutex_array_unlock(struct halide_mutex_array *array,
                                     int entry);
//@}
...
extern halide_do_par_for_t
halide_set_custom_do_par_for(halide_do_par_for_t do_par_for);
...
typedef int (*halide_do_loop_task_t)(void *, halide_loop_task_t, int, int,
                                     uint8_t *, void *);
extern halide_do_loop_task_t
halide_set_custom_do_loop_task(halide_do_loop_task_t do_task);
...
extern int
halide_default_do_parallel_tasks(void *user_context, int num_tasks,
                                 struct halide_parallel_task_t *tasks,
                                 void *task_parent);
...
extern halide_get_symbol_t
halide_set_custom_get_symbol(halide_get_symbol_t user_get_symbol);
extern halide_load_library_t
halide_set_custom_load_library(halide_load_library_t user_load_library);
...
struct halide_type_t {
  ...
  HALIDE_ALWAYS_INLINE constexpr halide_type_t(halide_type_code_t code,
                                               uint8_t bits, uint16_t lanes = 1)
  ...
  HALIDE_ALWAYS_INLINE constexpr halide_type_t
  with_lanes(uint16_t new_lanes) const
  {
  ...
  HALIDE_ALWAYS_INLINE constexpr bool
  operator==(const halide_type_t &other) const
  {
  ...
  HALIDE_ALWAYS_INLINE constexpr uint32_t as_u32() const
  {
    ...
    // on Clang, the compiler knows this and optimizes this into a single 32-bit
    // move. (At -O0 it will look awful.)
    return static_cast<uint8_t>(code) | (static_cast<uint16_t>(bits) << 8) |
...
#if (__cplusplus >= 201103L || _MSVC_LANG >= 201103L)
static_assert(sizeof(halide_type_t) == sizeof(uint32_t),
              "size mismatch in halide_type_t");
#endif
 ...
 * calling this. */
extern void
halide_device_release(void *user_context,
                      const struct halide_device_interface_t *device_interface);
 ...
 * error is returned. */
extern int
halide_copy_to_device(void *user_context, struct halide_buffer_t *buf,
                      const struct halide_device_interface_t *device_interface);
...
extern int
halide_buffer_copy(void *user_context, struct halide_buffer_t *src,
                   const struct halide_device_interface_t *dst_device_interface,
                   struct halide_buffer_t *dst);
...
extern int
halide_device_malloc(void *user_context, struct halide_buffer_t *buf,
                     const struct halide_device_interface_t *device_interface);
...
extern int halide_memoization_cache_store(
    void *user_context, const uint8_t *cache_key, int32_t size,
    struct halide_buffer_t *realized_bounds, int32_t tuple_count,
    struct halide_buffer_t **tuple_buffers, bool has_eviction_key,
 ...
 */
extern void halide_memoization_cache_evict(void *user_context,
                                           uint64_t eviction_key);
...
/** Verify that a given range of memory has been initialized; only used when
 * Target::MSAN is enabled.
 *
 * The default implementation simply calls the LLVM-provided
 * __msan_check_mem_is_initialized() function.
 *
 * The return value should always be zero.
 */
extern int halide_msan_check_memory_is_initialized(void *user_context,
                                                   const void *ptr,
                                                   uint64_t len,
                                                   const char *name);

/** Verify that the data pointed to by the halide_buffer_t is initialized (but
 * *not* the halide_buffer_t itself), using
 * halide_msan_check_memory_is_initialized() for checking.
 *
 * The default implementation takes pains to only check the active memory ranges
 * (skipping padding), and sorting into ranges to always check the smallest
 ...
 * The return value should always be zero.
 */
extern int halide_msan_check_buffer_is_initialized(
    void *user_context, struct halide_buffer_t *buffer, const char *buf_name);

/** Annotate that a given range of memory has been initialized;
 * only used when Target::MSAN is enabled.
 *
 * The default implementation simply calls the LLVM-provided __msan_unpoison()
 * function.
 *
 * The return value should always be zero.
 */
extern int halide_msan_annotate_memory_is_initialized(void *user_context,
                                                      const void *ptr,
                                                      uint64_t len);

/** Mark the data pointed to by the halide_buffer_t as initialized (but *not*
 * the halide_buffer_t itself), using
 * halide_msan_annotate_memory_is_initialized() for marking.
 *
 ...
 */
extern int
halide_msan_annotate_buffer_is_initialized(void *user_context,
                                           struct halide_buffer_t *buffer);
extern void
halide_msan_annotate_buffer_is_initialized_as_destructor(void *user_context,
                                                         void *buffer);
...
enum halide_error_code_t
{
  ...
  /** The dimensions field of a halide_buffer_t does not match the dimensions of
     that ImageParam. */
...
extern int halide_error_bounds_inference_call_failed(
    void *user_context, const char *extern_stage_name, int result);

/** A call to an extern stage failed. Returned the error code given by
 * the extern stage. */
extern int halide_error_extern_stage_failed(void *user_context,
 ...
 * description of each. */
// @{
extern int halide_error_explicit_bounds_too_small(
    void *user_context, const char *func_name, const char *var_name,
    int min_bound, int max_bound, int min_required, int max_required);
extern int halide_error_bad_type(
    void *user_context, const char *func_name, uint32_t type_given,
    uint32_t correct_type); // N.B. The last two args are the bit representation
                            // of a halide_type_t
extern int halide_error_bad_dimensions(void *user_context,
                                       const char *func_name,
                                       int32_t dimensions_given,
                                       int32_t correct_dimensions);
extern int halide_error_access_out_of_bounds(void *user_context,
                                             const char *func_name,
                                             int dimension, int min_touched,
                                             int max_touched, int min_valid,
                                             int max_valid);
extern int halide_error_buffer_allocation_too_large(void *user_context,
                                                    const char *buffer_name,
                                                    uint64_t allocation_size,
                                                    uint64_t max_size);
extern int halide_error_buffer_extents_negative(void *user_context,
                                                const char *buffer_name,
                                                int dimension, int extent);
extern int halide_error_buffer_extents_too_large(void *user_context,
                                                 const char *buffer_name,
                                                 int64_t actual_size,
                                                 int64_t max_size);
extern int halide_error_constraints_make_required_region_smaller(
    void *user_context, const char *buffer_name, int dimension,
    int constrained_min, int constrained_extent, int required_min,
    int required_extent);
extern int halide_error_constraint_violated(void *user_context, const char *var,
                                            int val,
                                            const char *constrained_var,
                                            int constrained_val);
extern int halide_error_param_too_small_i64(void *user_context,
                                            const char *param_name, int64_t val,
                                            int64_t min_val);
extern int halide_error_param_too_small_u64(void *user_context,
                                            const char *param_name,
                                            uint64_t val, uint64_t min_val);
extern int halide_error_param_too_small_f64(void *user_context,
                                            const char *param_name, double val,
                                            double min_val);
extern int halide_error_param_too_large_i64(void *user_context,
                                            const char *param_name, int64_t val,
                                            int64_t max_val);
extern int halide_error_param_too_large_u64(void *user_context,
                                            const char *param_name,
                                            uint64_t val, uint64_t max_val);
extern int halide_error_param_too_large_f64(void *user_context,
                                            const char *param_name, double val,
                                            double max_val);
extern int halide_error_out_of_memory(void *user_context);
extern int halide_error_buffer_argument_is_null(void *user_context,
                                                const char *buffer_name);
extern int halide_error_debug_to_file_failed(void *user_context,
                                             const char *func,
                                             const char *filename,
                                             int error_code);
...
extern int halide_error_bad_extern_fold(void *user_context,
                                        const char *func_name, int dim, int min,
                                        int extent, int valid_min,
                                        int fold_factor);

extern int
halide_error_fold_factor_too_small(void *user_context, const char *func_name,
                                   const char *var_name, int fold_factor,
                                   const char *loop_name, int required_extent);
extern int halide_error_requirement_failed(void *user_context,
                                           const char *condition,
                                           const char *message);
extern int halide_error_specialize_fail(void *user_context,
                                        const char *message);
...
extern int
halide_error_device_dirty_with_no_device_support(void *user_context,
                                                 const char *buffer_name);
extern int halide_error_storage_bound_too_small(void *user_context,
                                                const char *func_name,
                                                const char *var_name,
                                                int provided_size,
                                                int required_size);
 ...
 * Be sure to keep this in sync with the Feature enum in Target.h and the
 * implementation of get_runtime_compatible_target in Target.cpp if you add a
 * new feature.
 */
typedef enum halide_target_feature_t
{
  ...
  halide_target_feature_debug,   ///< Turn on debug info and output for runtime
                                 ///< code.
  halide_target_feature_no_asserts,      ///< Disable all runtime checks, for
                                         ///< slightly tighter code.
  halide_target_feature_no_bounds_query, ///< Disable the bounds querying
                                         ///< functionality.

  halide_target_feature_sse41, ///< Use SSE 4.1 and earlier instructions. Only
                               ///< relevant on x86.
  ...
  halide_target_feature_armv7s, ///< Generate code for ARMv7s. Only relevant for
                                ///< 32-bit ARM.
  halide_target_feature_no_neon, ///< Avoid using NEON instructions. Only
                                 ///< relevant for 32-bit ARM.

  halide_target_feature_vsx, ///< Use VSX instructions. Only relevant on
                             ///< POWERPC.
  halide_target_feature_power_arch_2_07, ///< Use POWER ISA 2.07 new
                                         ///< instructions. Only relevant on
                                         ///< POWERPC.

  halide_target_feature_cuda, ///< Enable the CUDA runtime. Defaults to compute
                              ///< capability 2.0 (Fermi)
  halide_target_feature_cuda_capability30, ///< Enable CUDA compute
                                           ///< capability 3.0 (Kepler)
  halide_target_feature_cuda_capability32, ///< Enable CUDA compute
                                           ///< capability 3.2 (Tegra K1)
  halide_target_feature_cuda_capability35, ///< Enable CUDA compute
                                           ///< capability 3.5 (Kepler)
  halide_target_feature_cuda_capability50, ///< Enable CUDA compute
                                           ///< capability 5.0 (Maxwell)
  halide_target_feature_cuda_capability61, ///< Enable CUDA compute
                                           ///< capability 6.1 (Pascal)
  halide_target_feature_cuda_capability70, ///< Enable CUDA compute
                                           ///< capability 7.0 (Volta)
  halide_target_feature_cuda_capability75, ///< Enable CUDA compute
                                           ///< capability 7.5 (Turing)
  halide_target_feature_cuda_capability80, ///< Enable CUDA compute
                                           ///< capability 8.0 (Ampere)
  halide_target_feature_cuda_capability86, ///< Enable CUDA compute
  ...
  halide_target_feature_cl_atomic64, ///< Enable 64-bit atomics operations on
                                     ///< OpenCL targets

  halide_target_feature_openglcompute, ///< Enable OpenGL Compute runtime.

  halide_target_feature_user_context, ///< Generated code takes a user_context
                                      ///< pointer as first argument

  halide_target_feature_profile, ///< Launch a sampling profiler alongside the
                                 ///< Halide pipeline that monitors and reports
                                 ///< the runtime used by each Func
  halide_target_feature_no_runtime, ///< Do not include a copy of the Halide
                                    ///< runtime in any generated object file or
                                    ///< assembly

  halide_target_feature_metal, ///< Enable the (Apple) Metal runtime.

  halide_target_feature_c_plus_plus_mangling, ///< Generate C++ mangled names
                                              ///< for result function, et al

  halide_target_feature_large_buffers, ///< Enable 64-bit buffer indexing to
                                       ///< support buffers > 2GB. Ignored if
                                       ///< bits != 64.

  halide_target_feature_hvx_128,           ///< Enable HVX 128 byte mode.
  halide_target_feature_hvx_v62,           ///< Enable Hexagon v62 architecture.
  halide_target_feature_fuzz_float_stores, ///< On every floating point store,
                                           ///< set the last bit of the mantissa
                                           ///< to zero. Pipelines for which the
                                           ///< output is very different with
                                           ///< this feature enabled may also
                                           ///< produce very different output on
                                           ///< different processors.
  halide_target_feature_soft_float_abi,    ///< Enable soft float ABI. This only
                                        ///< enables the soft float ABI calling
                                        ///< convention, which does not
                                        ///< necessarily use soft floats.
  halide_target_feature_msan,   ///< Enable hooks for MSAN support.
  halide_target_feature_avx512, ///< Enable the base AVX512 subset supported by
                                ///< all AVX512 architectures. The specific
                                ///< feature sets are AVX-512F and AVX512-CD.
                                ///< See https://en.wikipedia.org/wiki/AVX-512
                                ///< for a description of each AVX subset.
  halide_target_feature_avx512_knl, ///< Enable the AVX512 features supported by
                                    ///< Knight's Landing chips, such as the
                                    ///< Xeon Phi x200. This includes the base
                                    ///< AVX512 set, and also AVX512-CD and
                                    ///< AVX512-ER.
  halide_target_feature_avx512_skylake, ///< Enable the AVX512 features
                                        ///< supported by Skylake Xeon server
                                        ///< processors. This adds AVX512-VL,
                                        ///< AVX512-BW, and AVX512-DQ to the
                                        ///< base set. The main difference from
                                        ///< the base AVX512 set is better
                                        ///< support for small integer ops. Note
                                        ///< that this does not include the
                                        ///< Knight's Landing features. Note
                                        ///< also that these features are not
                                        ///< available on Skylake desktop and
                                        ///< mobile processors.
  halide_target_feature_avx512_cannonlake, ///< Enable the AVX512 features
                                           ///< expected to be supported by
                                           ///< future Cannonlake processors.
                                           ///< This includes all of the Skylake
                                           ///< features, plus AVX512-IFMA and
                                           ///< AVX512-VBMI.
  halide_target_feature_avx512_sapphirerapids, ///< Enable the AVX512 features
                                               ///< supported by Sapphire Rapids
                                               ///< processors. This include all
                                               ///< of the Cannonlake features,
                                               ///< plus AVX512-VNNI and
                                               ///< AVX512-BF16.
  halide_target_feature_hvx_use_shared_object, ///< Deprecated
  halide_target_feature_trace_loads, ///< Trace all loads done by the pipeline.
                                     ///< Equivalent to calling
                                     ///< Func::trace_loads on every non-inlined
                                     ///< Func.
  halide_target_feature_trace_stores,       ///< Trace all stores done by the
                                            ///< pipeline. Equivalent to calling
                                            ///< Func::trace_stores on every
                                            ///< non-inlined Func.
  halide_target_feature_trace_realizations, ///< Trace all realizations done by
                                            ///< the pipeline. Equivalent to
                                            ///< calling
                                            ///< Func::trace_realizations on
                                            ///< every non-inlined Func.
  ...
  halide_target_feature_strict_float, ///< Turn off all non-IEEE floating-point
                                      ///< optimization. Currently applies only
                                      ///< to LLVM targets.
  ...
  halide_target_feature_check_unsafe_promises, ///< Insert assertions for
                                               ///< promises.
  ...
  halide_target_feature_enable_llvm_loop_opt, ///< Enable loop vectorization +
                                              ///< unrolling in LLVM. Overrides
                                              ///< halide_target_feature_disable_llvm_loop_opt.
                                              ///< (Ignored for non-LLVM
                                              ///< targets.)
  halide_target_feature_wasm_simd128, ///< Enable +simd128 instructions for
                                      ///< WebAssembly codegen.
  halide_target_feature_wasm_signext, ///< Enable +sign-ext instructions for
                                      ///< WebAssembly codegen.
  halide_target_feature_wasm_sat_float_to_int, ///< Enable saturating
                                               ///< (nontrapping) float-to-int
                                               ///< instructions for WebAssembly
                                               ///< codegen.
  halide_target_feature_wasm_threads, ///< Enable use of threads in WebAssembly
                                      ///< codegen. Requires the use of a wasm
                                      ///< runtime that provides
                                      ///< pthread-compatible wrappers
                                      ///< (typically, Emscripten with the
                                      ///< -pthreads flag). Unsupported under
                                      ///< WASI.
  halide_target_feature_wasm_bulk_memory, ///< Enable +bulk-memory instructions
                                          ///< for WebAssembly codegen.
  ...
  halide_target_feature_arm_dot_prod, ///< Enable ARMv8.2-a dotprod extension
                                      ///< (i.e. udot and sdot instructions)
  halide_target_feature_arm_fp16, ///< Enable ARMv8.2-a half-precision floating
  ...
  halide_target_feature_sanitizer_coverage, ///< Enable hooks for
                                            ///< SanitizerCoverage support.
  halide_target_feature_profile_by_timer,   ///< Alternative to
                                            ///< halide_target_feature_profile
  ///< using timer interrupt for systems
  ///< without threads or applicartions
  ///< that need to avoid them.
  halide_target_feature_spirv, ///< Enable SPIR-V code generation support.
  halide_target_feature_end ///< A sentinel. Every target is considered to have
                            ///< this feature, and setting this feature does
                            ///< nothing.
...
struct halide_filter_argument_t {
  ...
  const struct halide_scalar_value_t *scalar_def, *scalar_min, *scalar_max,
      *scalar_estimate;
 ...
 * by Generators when the 'registration' output is requested. Each
 * registration.cc file provides a static initializer that calls this function
 * with the given filter's argv-call variant, its metadata, and (optionally) and
 * additional textual data that the build system chooses to tack on for its own
 * purposes. Note that this will be called at static-initializer time (i.e.,
 * before main() is called), and in an unpredictable order. Note that
 * extra_key_value_pairs may be nullptr; if it's not null, it's expected to be a
 * null-terminated list of strings, with an even number of entries. */
...
extern int halide_profiler_sample(struct halide_profiler_state *s,
                                  uint64_t *prev_t);
...
extern void
halide_register_device_allocation_pool(struct halide_device_allocation_pool *);
...
template <> HALIDE_ALWAYS_INLINE constexpr halide_type_t halide_type_of<float>()
{
...
template <> HALIDE_ALWAYS_INLINE constexpr halide_type_t halide_type_of<bool>()
{
