 ...
 */
template <typename T>
void addCostSegLensSegTypes(
    T *distances, T *segmentLen, T *segmentType,
    std::vector<autonomous::reedsshepp::ReedsSheppPath> path, const uint32_T m,
    const uint32_T numPaths, const T turningRadius, const uint32_T numInputs)
{
  // Add cost
 ...
 * @see autonomousReedsSheppSegmentsCodegen_real64,
 * autonomousReedsSheppSegmentsCodegen_tbb_real64
 */
template <typename T> class ReedsSheppSegmentsFunctor {
private:
  const T *m_startPose;
...
public:
  ReedsSheppSegmentsFunctor(const T *startPose, const uint32_T numStartPoses,
                            const T *goalPose, const uint32_T numGoalPoses,
                            const T turningRadius, const T forwardCost,
                            const T reverseCost, const boolean_T *allPathTypes,
                            const uint32_T numDisabledPathTypes,
                            const uint32_T numPaths, const boolean_T isOptimal,
                            const uint32_T nlhs, T *distance, T *segmentLen,
                            T *segmentType)
      : m_startPose(startPose),
        m_maxNumPoses(numStartPoses > numGoalPoses ? numStartPoses
                                                   : numGoalPoses),
        m_numStartPoses(numStartPoses), m_numGoalPoses(numGoalPoses),
        m_goalPose(goalPose), m_turningRadius(turningRadius),
        m_forwardCost(forwardCost), m_reverseCost(reverseCost),
        m_allPathTypes(allPathTypes),
        m_numTotalPathTypes(
            static_cast<const uint32_T>(autonomous::reedsshepp::TotalNumPaths) -
            numDisabledPathTypes),
        m_numPaths(numPaths), m_isOptimal(isOptimal), m_nlhs(nlhs),
        m_distance(distance), m_segmentLen(segmentLen),
  ...
  void operator()(const autonomous::ParallelRange<uint32_T> &range) const
  {
    ...
    if (m_numStartPoses >= m_numGoalPoses && m_numGoalPoses == 1) {
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        std::vector<autonomous::reedsshepp::ReedsSheppPath> path =
            autonomous::reedsshepp::computeAllReedsSheppPaths(
                &a[0], m_goalPose, m_turningRadius, m_forwardCost,
                m_reverseCost, m_isOptimal, &m_allPathTypes[0],
                m_numTotalPathTypes);

        // Add cost, segment lengths & segment types
        addCostSegLensSegTypes(m_distance, m_segmentLen, m_segmentType, path, i,
                               m_numPaths, m_turningRadius, m_nlhs);
      }
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        std::vector<autonomous::reedsshepp::ReedsSheppPath> path =
            autonomous::reedsshepp::computeAllReedsSheppPaths(
                m_startPose, &b[0], m_turningRadius, m_forwardCost,
                m_reverseCost, m_isOptimal, &m_allPathTypes[0],
                m_numTotalPathTypes);
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        std::vector<autonomous::reedsshepp::ReedsSheppPath> path =
            autonomous::reedsshepp::computeAllReedsSheppPaths(
                &a[0], &b[0], m_turningRadius, m_forwardCost, m_reverseCost,
...
/**
 * @brief Functor for calculating the distance along ReedsShepp motion primitive
 * segments
 *
 * This function is used to allow both parallel (with TBB) and serial execution.
 * @see autonomousReedsSheppDistanceCodegen_real64,
 * autonomousReedsSheppDistanceCodegen_tbb_real64
 */
template <typename T> class ReedsSheppDistanceFunctor {
private:
  const T *m_startPose;
  const uint32_T m_maxNumPoses;
  const uint32_T m_numStartPoses;
...
public:
  ReedsSheppDistanceFunctor(const T *startPose, const uint32_T numStartPoses,
                            const T *goalPose, const uint32_T numGoalPoses,
                            const T turningRadius, const T reverseCost,
                            T *distance)
      : m_startPose(startPose),
        m_maxNumPoses(numStartPoses > numGoalPoses ? numStartPoses
                                                   : numGoalPoses),
        m_numStartPoses(numStartPoses), m_numGoalPoses(numGoalPoses),
        m_goalPose(goalPose), m_turningRadius(turningRadius),
        m_reverseCost(reverseCost), m_distance(distance)
  {
  }
    ...
    if (m_numStartPoses >= m_numGoalPoses) {
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        autonomous::reedsshepp::ReedsSheppPath path =
            autonomous::reedsshepp::computeReedsSheppPath(
                &a[0], m_goalPose, m_turningRadius, m_reverseCost);
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        autonomous::reedsshepp::ReedsSheppPath path =
            autonomous::reedsshepp::computeReedsSheppPath(
                m_startPose, &b[0], m_turningRadius, m_reverseCost);
        m_distance[i] = path.cost() * m_turningRadius;
      }
    }
  }
