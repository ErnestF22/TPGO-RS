...
const DubinsSegmentType pathToSegment[dubins::TotalNumPaths][3] = {
    {autonomous::dubins::Left, autonomous::dubins::Straight,
     autonomous::dubins::Left},
    {autonomous::dubins::Left, autonomous::dubins::Straight,
     autonomous::dubins::Right},
    {autonomous::dubins::Right, autonomous::dubins::Straight,
     autonomous::dubins::Left},
    {autonomous::dubins::Right, autonomous::dubins::Straight,
     autonomous::dubins::Right},
    {autonomous::dubins::Right, autonomous::dubins::Left,
     autonomous::dubins::Right},
    {autonomous::dubins::Left, autonomous::dubins::Right,
     autonomous::dubins::Left}};
...
struct FindShortestDubinsPath {
  bool operator()(DubinsPath p1, DubinsPath p2)
  {
    ...
    return (
        !autonomous::queries::isNaN(p1.length()) &&
        (autonomous::queries::isNaN(p2.length()) || p1.length() < p2.length()));
  }
};
 ...
 */
inline DubinsPath dubinsLSL(const real64_T d, const real64_T alpha,
                            const real64_T beta)
{
  ...
  return DubinsPath(autonomous::nan, autonomous::nan, autonomous::nan,
                    autonomous::dubins::LSL);
}

/*
 * dubinsLSR - compute DubinsPath for LSR path.
 */
inline DubinsPath dubinsLSR(const real64_T d, const real64_T alpha,
                            const real64_T beta)
{
  real64_T ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
  real64_T tmp = -2. + d * d + 2. * (ca * cb + sa * sb + d * (sa + sb));
  ...
  return DubinsPath(autonomous::nan, autonomous::nan, autonomous::nan,
                    autonomous::dubins::LSR);
}

/*
 * dubinsRSL - compute DubinsPath for RSL path.
 */
inline DubinsPath dubinsRSL(const real64_T d, const real64_T alpha,
                            const real64_T beta)
{
  real64_T ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
  ...
  return DubinsPath(autonomous::nan, autonomous::nan, autonomous::nan,
                    autonomous::dubins::RSL);
}

/*
 * dubinsRSR - compute DubinsPath for RSR path.
 */
inline DubinsPath dubinsRSR(const real64_T d, const real64_T alpha,
                            const real64_T beta)
{
  real64_T ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
  ...
  return DubinsPath(autonomous::nan, autonomous::nan, autonomous::nan,
                    autonomous::dubins::RSR);
}

/*
 * dubinsRLR - compute DubinsPath for RLR path.
 */
inline DubinsPath dubinsRLR(const real64_T d, const real64_T alpha,
                            const real64_T beta)
{
  real64_T ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
  ...
  return DubinsPath(autonomous::nan, autonomous::nan, autonomous::nan,
                    autonomous::dubins::RLR);
}

/*
 * dubinsLRL - compute DubinsPath for LRL path.
 */
inline DubinsPath dubinsLRL(const real64_T d, const real64_T alpha,
                            const real64_T beta)
{
  real64_T ca = cos(alpha), sa = sin(alpha), cb = cos(beta), sb = sin(beta);
  real64_T tmp = .125 * (6. - d * d + 2. * (ca * cb + sa * sb - d * (sa - sb)));
  ...
  return DubinsPath(autonomous::nan, autonomous::nan, autonomous::nan,
                    autonomous::dubins::LRL);
}

/*
 * shortestDubins return shortest normalized Dubins path.
 */
inline DubinsPath shortestDubins(const real64_T d, const real64_T alpha,
                                 const real64_T beta)
{
  DubinsPath path;
 ...
 */
inline std::vector<DubinsPath>
allDubins(const real64_T d, const real64_T alpha, const real64_T beta,
          ...
          const boolean_T allPathTypes[dubins::TotalNumPaths])
{
  ...
  if (d < tooSmall && fabs(alpha - beta) < tooSmall) {
    static const real64_T arr[] = {
        d, d, d, d, autonomous::nan, autonomous::nan};
    std::vector<real64_T> cost(arr, arr + sizeof(arr) / sizeof(arr[0]));
    DubinsPathType allDubinsPathType[dubins::TotalNumPaths] = {
        autonomous::dubins::LSL, autonomous::dubins::LSR,
        autonomous::dubins::RSL, autonomous::dubins::RSR,
        autonomous::dubins::RLR, autonomous::dubins::LRL};

    for (uint32_T j = 0; j < dubins::TotalNumPaths; ++j) {
      if (allPathTypes[j]) {
        allPaths[i++] = DubinsPath(0, cost[j], 0, allDubinsPathType[j]);
      } else {
        allPaths[i++] = DubinsPath(autonomous::nan, autonomous::nan,
                                   autonomous::nan, allDubinsPathType[j]);
      }
    }
  }
  ...
  // object with segments lengths [nan, nan, nan].
    ...
    if (allPathTypes[0]) {
      ...
      allPaths[i++] = DubinsPath(autonomous::nan, autonomous::nan,
                                 autonomous::nan, autonomous::dubins::LSL);
    }
    ...
    if (allPathTypes[1]) {
      ...
      allPaths[i++] = DubinsPath(autonomous::nan, autonomous::nan,
                                 autonomous::nan, autonomous::dubins::LSR);
    }
    ...
    if (allPathTypes[2]) {
      ...
      allPaths[i++] = DubinsPath(autonomous::nan, autonomous::nan,
                                 autonomous::nan, autonomous::dubins::RSL);
    }
    ...
    if (allPathTypes[3]) {
      ...
      allPaths[i++] = DubinsPath(autonomous::nan, autonomous::nan,
                                 autonomous::nan, autonomous::dubins::RSR);
    }
    ...
    if (allPathTypes[4]) {
      ...
      allPaths[i++] = DubinsPath(autonomous::nan, autonomous::nan,
                                 autonomous::nan, autonomous::dubins::RLR);
    }
    ...
    if (allPathTypes[5]) {
      ...
      allPaths[i++] = DubinsPath(autonomous::nan, autonomous::nan,
                                 autonomous::nan, autonomous::dubins::LRL);
    }
  }
  if (isOptimal) {
    ...
    singlePath.push_back(
        *(std::min_element(allPaths.begin(), allPaths.end(), findShortest)));
    return singlePath;
  }
...
inline std::vector<DubinsPath>
computeAllDubinsPaths(const real64_T startPose[3], const real64_T goalPose[3],
                      const real64_T turningRadius, const boolean_T isOptimal,
                      const boolean_T allPathTypes[dubins::TotalNumPaths])
{
 ...
 */
inline void interpolateAlongInitializedDPath(
    const real64_T pathLength, const DubinsSegmentType *segments,
    const real_T *segmentLengths, const real64_T *from, const real64_T *towards,
    const real64_T t, const real64_T turningRadius, real64_T *state)
{
 ...
 */
inline void interpolateDubins(const real64_T *from, const real64_T *towards,
                              const real64_T maxDistance,
                              const uint32_T numSteps,
                              const real64_T turningRadius, real64_T *state)
{
  // Compute Dubins path.
  ...
  for (uint32_T n = 0; n < numTransitions; ++n) {
    ...
    interpolateAlongInitializedDPath(pathLength, segments, segLengths, from,
 ...
 */
inline void
interpolateDubinsSegments(const real64_T *from, const real64_T *towards,
                          const real64_T *samples, const uint32_T numSamples,
                          const real64_T turningRadius,
                          const real64_T *segmentLengths,
                          const uint32_T *segmentTypes, real64_T *state)
{
  DubinsSegmentType segments[3] = {
      static_cast<DubinsSegmentType>(segmentTypes[0]),
      static_cast<DubinsSegmentType>(segmentTypes[1]),
      static_cast<DubinsSegmentType>(segmentTypes[2])};

  real64_T temp[3] = {0, 0, 0};

  const real64_T pathLength =
  ...
  const real64_T normalizedPathLength = normalizedSegmentLengths[0] +
  ...
  for (uint32_T n = 0; n < numSamples; ++n) {
    // Compute fraction along path corresponding to n-th transition
    // point
    interpolateAlongInitializedDPath(
        normalizedPathLength, segments, normalizedSegmentLengths, from, towards,
        samples[n] / pathLength, turningRadius, &temp[0]);
