 ...
 * Dubins curve. If @p startPose and @p goalPose are both 1-by-3 & paths is
 * optimal, @p distance is a scalar. If @p startPose or @p goalPose is M-by-3,
 * @p distance is an N-by-M matrix. where N = (6-length(disablePathType))-->
 * (all), 1-->(optimal)
 ...
 * 3-by-N-by-M matrix. where N = (6-@p numDisabledPathTypes)--> (all),
 *    1-->(optimal)
...
EXTERN_C AUTONOMOUS_CODEGEN_API void autonomousDubinsSegmentsCodegen_real64(
    ...
    const real64_T turningRadius, const boolean_T *allPathTypes,
 ...
 * range of the planner, specified as a real, double, scalar.
 * @param[in] numSteps Number of interpolation steps, specified as a real,
 *   double scalar.
 * @param[in] turningRadius Turning radius (in radians)
 * @param[out] interpPoses Interpolated poses along the Dubins curve, returned
 * as a (2 + @p numSteps)-by-3 double-precision matrix. In addition to the
...
EXTERN_C AUTONOMOUS_CODEGEN_API void autonomousDubinsInterpolateCodegen_real64(
    ...
    const real64_T turningRadius, real64_T *interpPoses);
 ...
 * @param[in] turningRadius Turning radius (in radians)
 * @param[out] distance Distance between @p startPose and @p goalPose, along
 * Dubins curve. If @p startPose and @p goalPose are both 1-by-3, @p distance is
 * a scalar. If @p startPose or @p goalPose is M-by-3, @p distance is an M-by-1
 * column vector.
 */
EXTERN_C AUTONOMOUS_CODEGEN_API void autonomousDubinsDistanceCodegen_real64(
    const real64_T *startPose, const uint32_T numStartPoses,
    const real64_T *goalPose, const uint32_T numGoalPoses,
    const real64_T turningRadius, real64_T *distance);
