 ...
 */
template <typename T> class ParallelRange {
public:
  ...
  ParallelRange()
      : m_end(static_cast<T>(0)), m_begin(static_cast<T>(0)), m_grainSize(1)
  {
  }
   ...
   * @param grainSize Allow subdivision of the full range based on the grain
   * size.
   */
  ParallelRange(T begin, T end, uint32_Type grainSize = 1)
      : m_end(end), m_begin(begin), m_grainSize(grainSize)
  {
  }
  ...
  ParallelRange(ParallelRange &r, tbb::split)
      : m_end(r.m_end), m_begin(splitRange(r)), m_grainSize(r.m_grainSize)
  {
  }
