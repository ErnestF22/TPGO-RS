 ...
 */
template <typename T>
void addCostSegLensSegTypes(T *distances, T *segmentLen, T *segmentType,
                            std::vector<DubinsPath> path, const uint32_T m,
                            const uint32_T numPaths, const T turningRadius,
 ...
 * @see autonomousDubinsSegmentsCodegen_real64,
 * autonomousDubinsSegmentsCodegen_tbb_real64
 */
template <typename T> class DubinsSegmentsFunctor {
private:
  const T *m_startPose;
...
public:
  DubinsSegmentsFunctor(const T *startPose, const uint32_T numStartPoses,
                        const T *goalPose, const uint32_T numGoalPoses,
                        const T turningRadius, const boolean_T *allPathTypes,
                        const boolean_T isOptimal, const uint32_T nlhs,
                        T *distance, T *segmentLen, T *segmentType)
      : m_startPose(startPose),
        ...
        m_isOptimal(isOptimal), m_nlhs(nlhs), m_distance(distance),
        m_segmentLen(segmentLen), m_segmentType(segmentType)
    ...
    if (m_numStartPoses >= m_numGoalPoses && m_numGoalPoses == 1) {
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        std::vector<DubinsPath> path =
            computeAllDubinsPaths(&a[0], m_goalPose, m_turningRadius,
                                  m_isOptimal, &m_allPathTypes[0]);
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        std::vector<DubinsPath> path =
            computeAllDubinsPaths(m_startPose, &b[0], m_turningRadius,
                                  m_isOptimal, &m_allPathTypes[0]);
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        std::vector<DubinsPath> path = computeAllDubinsPaths(
            &a[0], &b[0], m_turningRadius, m_isOptimal, &m_allPathTypes[0]);
...
/**
 * @brief Functor for calculating the distance along Dubins motion primitive
 * segments
 *
 * This function is used to allow both parallel (with TBB) and serial execution.
 * @see autonomousDubinsDistanceCodegen_real64,
...
public:
  DubinsDistanceFunctor(const T *startPose, const uint32_T numStartPoses,
                        const T *goalPose, const uint32_T numGoalPoses,
                        const T turningRadius, T *distance)
      : m_startPose(startPose),
        m_maxNumPoses(numStartPoses > numGoalPoses ? numStartPoses
                                                   : numGoalPoses),
        m_numStartPoses(numStartPoses), m_numGoalPoses(numGoalPoses),
  ...
  void operator()(const autonomous::ParallelRange<uint32_T> &range) const
  {
    ...
    if (m_numStartPoses >= m_numGoalPoses) {
      ...
      for (uint32_T i = range.begin(); i < range.end(); ++i) {
        ...
        DubinsPath path =
            computeDubinsPath(m_startPose, &b[0], m_turningRadius);
        m_distance[i] = path.length() * m_turningRadius;
      }
    }
  }
