...
const RSSegmentType pathToSegment[18][5] = {
    ...
    {autonomous::reedsshepp::Right, autonomous::reedsshepp::Left,
     ...
     autonomous::reedsshepp::NOP},
    {autonomous::reedsshepp::Left, autonomous::reedsshepp::Right,
     autonomous::reedsshepp::Straight, autonomous::reedsshepp::Left,
     autonomous::reedsshepp::NOP},
    {autonomous::reedsshepp::Right, autonomous::reedsshepp::Left,
     autonomous::reedsshepp::Straight, autonomous::reedsshepp::Right,
     autonomous::reedsshepp::NOP},
    {autonomous::reedsshepp::Left, autonomous::reedsshepp::Straight,
     autonomous::reedsshepp::Right, autonomous::reedsshepp::Left,
     autonomous::reedsshepp::NOP},
    {autonomous::reedsshepp::Right, autonomous::reedsshepp::Straight,
     autonomous::reedsshepp::Left, autonomous::reedsshepp::Right,
     autonomous::reedsshepp::NOP},
    {autonomous::reedsshepp::Left, autonomous::reedsshepp::Right,
     autonomous::reedsshepp::Straight, autonomous::reedsshepp::Right,
    ...
    {autonomous::reedsshepp::Left, autonomous::reedsshepp::Right,
     autonomous::reedsshepp::Straight, autonomous::reedsshepp::Left,
     autonomous::reedsshepp::Right},
    {autonomous::reedsshepp::Right, autonomous::reedsshepp::Left,
     autonomous::reedsshepp::Straight, autonomous::reedsshepp::Right,
     autonomous::reedsshepp::Left}};

/*
 * shortestReedsSheppPath returns shortest normalized Reeds-Shepp path.
 */
inline autonomous::reedsshepp::ReedsSheppPath
shortestReedsSheppPath(real64_T x, real64_T y, real64_T phi,
 ...
 */
inline autonomous::reedsshepp::ReedsSheppPath
computeReedsSheppPath(const real64_T initialConfiguration[3],
                      const real64_T finalConfiguration[3],
                      const real64_T turningRadius, const real64_T reverseCost)
{
  ...
  return shortestReedsSheppPath(x / turningRadius, y / turningRadius, phi,
                                reverseCost);
}
...
inline std::vector<autonomous::reedsshepp::ReedsSheppPath> AllReedsSheppPath(
    real64_T x, real64_T y, real64_T phi, real64_T forwardCost,
    real64_T reverseCost, const boolean_T isOptimal,
    ...
    const uint32_T numTotalPathTypes)
{
  ...
  autonomous::reedsshepp::computeCSCPathAll(x, y, phi, forwardCost, reverseCost,
                                            &allPathTypes[0], &allPaths);
  autonomous::reedsshepp::computeCCCPathAll(x, y, phi, forwardCost, reverseCost,
                                            &allPathTypes[0], &allPaths);
  autonomous::reedsshepp::computeCCCCPathAll(
      x, y, phi, forwardCost, reverseCost, &allPathTypes[0], &allPaths);
  autonomous::reedsshepp::computeCCSCPathAll(
      x, y, phi, forwardCost, reverseCost, &allPathTypes[0], &allPaths);
  autonomous::reedsshepp::computeCCSCCPathAll(
      x, y, phi, forwardCost, reverseCost, &allPathTypes[0], &allPaths);
 ...
 */
inline std::vector<autonomous::reedsshepp::ReedsSheppPath>
computeAllReedsSheppPaths(
    const real64_T initialConfiguration[3],
    const real64_T finalConfiguration[3], const real64_T turningRadius,
    const real64_T forwardCost, const real64_T reverseCost,
    ...
    const uint32_T numTotalPathTypes)
{
  ...
  return AllReedsSheppPath(x / turningRadius, y / turningRadius, phi,
                           forwardCost, reverseCost, isOptimal,
                           &allPathTypes[0], numTotalPathTypes);
}
 ...
 *
 */
inline real64_T computeDirectionAtEdges(const real64_T *segmentLengths,
                                        boolean_T startFlag = true)
{
 ...
 */
inline real64_T interpolateAlongInitializedRSPath(
    const real64_T pathLength, const RSSegmentType *segments,
    const real64_T *segmentLengths, const real64_T *from,
    const real64_T *towards, const real64_T t, const real64_T turningRadius,
    real64_T *state)
{
 ...
 */
inline void interpolateRS(const real64_T *from, const real64_T *towards,
                          const real64_T maxDistance, const uint32_T numSteps,
                          const real64_T turningRadius,
                          const real64_T reverseCost, real64_T *state)
{
  ...
  for (uint32_T n = 0; n < numTransitions; ++n) {
    ...
    interpolateAlongInitializedRSPath(pathLength, segments, segLengths, from,
                                      towards, fraction, turningRadius,
                                      &temp[0]);
...
/*
 * interpolateReedsSheppSegments - interpolate along given states and
 * Reeds-Shepp curve.
 */
inline void interpolateReedsSheppSegments(
    const real64_T *from, const real64_T *towards, const real64_T *samples,
    const uint32_T numSamples, const real64_T turningRadius,
    const real64_T *segmentLengths, const int32_T *segmentDirections,
    const uint32_T *segmentTypes, real64_T *state, real64_T *directions)
{
  ...
  const real64_T pathLength = segmentLengths[0] + segmentLengths[1] +
                              segmentLengths[2] + segmentLengths[3] +
                              segmentLengths[4];

  real64_T normalizedSegmentLengths[5] = {
      segmentLengths[0] / turningRadius, segmentLengths[1] / turningRadius,
      segmentLengths[2] / turningRadius, segmentLengths[3] / turningRadius,
  ...
  normalizedSegmentLengths[0] =
      round(normalizedSegmentLengths[0] * round_val) / round_val;
  normalizedSegmentLengths[1] =
      round(normalizedSegmentLengths[1] * round_val) / round_val;
  normalizedSegmentLengths[2] =
      round(normalizedSegmentLengths[2] * round_val) / round_val;
  normalizedSegmentLengths[3] =
      round(normalizedSegmentLengths[3] * round_val) / round_val;
  normalizedSegmentLengths[4] =
      round(normalizedSegmentLengths[4] * round_val) / round_val;

  // Round up to 15 decimal point to get same optimal path in all system
  // (maci, linux, windows). (Geck --> g1770360)
  normalizedSegmentLengths[0] =
      normalizedSegmentLengths[0] * static_cast<real64_T>(segmentDirections[0]);
  normalizedSegmentLengths[1] =
      normalizedSegmentLengths[1] * static_cast<real64_T>(segmentDirections[1]);
  normalizedSegmentLengths[2] =
      normalizedSegmentLengths[2] * static_cast<real64_T>(segmentDirections[2]);
  normalizedSegmentLengths[3] =
      normalizedSegmentLengths[3] * static_cast<real64_T>(segmentDirections[3]);
  normalizedSegmentLengths[4] =
      normalizedSegmentLengths[4] * static_cast<real64_T>(segmentDirections[4]);
  ...
  for (uint32_T n = 0; n < numSamples; ++n) {
    ...
    if (directions) {
      directions[n] = interpolateAlongInitializedRSPath(
          normalizedPathLength, segments, normalizedSegmentLengths, from,
          towards, roundedSample, turningRadius, &temp[0]);
    } else {
      interpolateAlongInitializedRSPath(normalizedPathLength, segments,
                                        normalizedSegmentLengths, from, towards,
                                        roundedSample, turningRadius, &temp[0]);
    }
