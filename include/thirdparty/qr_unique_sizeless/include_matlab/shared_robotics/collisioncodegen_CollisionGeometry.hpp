...
/**
 * @file
 * @brief Interface for collision geometry representation based on support
 * mapping
 */
 ...
 * @details A collision geometry can be specified as a primitive (box, cylinder
 * or sphere) or a convex mesh (vertices only, or vertices and faces). ccd
 * algorithm interacts with a collision geometry through its support function
 */
...
public:
  ...
  CollisionGeometry(ccd_real_t *vertices, int numVertices,
                    bool isColumnMajor = true)
  {
    ...
    if (isColumnMajor) {
      for (std::size_t k = 0; k < numV; k++) {
        ccdVec3Set(&vert, vertices[k], vertices[numV + k],
                   vertices[2 * numV + k]);
        m_vertices.push_back(vert);
      }
    } else {
      for (std::size_t k = 0; k < numV; k++) {
        ccdVec3Set(&vert, vertices[3 * k], vertices[3 * k + 1],
                   vertices[3 * k + 2]);
        m_vertices.push_back(vert);
      }
    }
  ...
  CollisionGeometry(ccd_real_t *vertices, int *faces, int numVertices,
                    int numFaces, bool isColumnMajor = true)
  {
    initializePositionAndOrientation();
...
protected:
  ...
  /// radius of the sphere, if the geometry is a sphere primitive, or radius of
  /// the bottom of a cylinder, if the geometry is a cylinder primitive
  ccd_real_t m_radius;

  /// height of the cylinder, if the geometry is a cylinder primitive
  ccd_real_t m_height;

  /// number of vertices in the convex mesh, if the geometry is a convex
  /// primitive
  ...
  /// vertices is saved as a vector of ccd_vec3_t structs with m_numVertices
  /// elements
  std::vector<ccd_vec3_t> m_vertices;

  /// faces of the convex mesh (if the geometry is a mesh primitive)
  /// faces is save as an (m_numFaces)-by-1 vector of idx_vec3_t structs
  /// consisting of node indices
...
private:
  void initializePositionAndOrientation()
  {
    ...
    /// orientation of the geometry which is an array of size 4 (ccd assuming
    /// [x, y, z, w] sequence)
    for (int i = 0; i < 3; i++) {
      m_quat.q[i] = 0;
    }
...
inline shared_robotics::CollisionGeometry::Type
shared_robotics::CollisionGeometry::getEnumType() const
{
  return m_type;
}
...
inline const std::vector<ccd_vec3_t> &
shared_robotics::CollisionGeometry::getVertices() const
{
  return m_vertices;
}
inline const std::vector<idx_vec3_t> &
shared_robotics::CollisionGeometry::getFaces() const
{
  return m_faces;
}
