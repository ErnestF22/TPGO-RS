...
struct RigidBodyJoint {
  ...
  /// Maximum velocity degree of freedom of a joint. This corresponds to a
  /// floating joint.
  constexpr static const size_t MAX_VELOCITY_NUMBER = 6;

  /// Maximum position degree of freedom of a joint. This corresponds to a
  /// floating joint.
  ...
  transformBodyToParent(const std::vector<real64_T> &q) const
  {
  ...
  HomogeneousTransform jointTransform(const std::vector<real64_T> &q) const
  {
    ...
    if (m_Type == JointType::REVOLUTE) {
      ...
      auto trvec =
          [](const std::vector<real64_T> &qfloat) -> std::array<real64_T, 3> {
        return {qfloat[4], qfloat[5], qfloat[6]};
      };
      auto quat =
          [](const std::vector<real64_T> &qfloat) -> std::array<real64_T, 4> {
