...
public:
  Exception() = default;
  /**
   * Rule of Five: define all five of these (as default). Don't worry about
   * slicing, since the classes derived from this one either have additional
   * data members that are trivial to construct/destroy, or have no additional
   * data members at all. We can't simply omit the definitions, since we need to
   * specify that the destructor is virtual.
   */
