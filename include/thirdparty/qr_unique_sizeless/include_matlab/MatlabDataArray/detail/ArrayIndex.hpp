...
/**
 * This class keeps track of the current index into a TypedArray. It is a helper
 * class for iterators that iterate in an order that is different than memory
 * order.
 * @template IT_ORDER - specifies which order to move index
 */
template <typename IT_ORDER> class ArrayIndex {
public:
  /**
   ...
   * @throw none
   */
  ArrayIndex(const matlab::data::ArrayDimensions &dims, bool isEnd) noexcept
      : nDims_(dims.size()), index_(nDims_, 0), dims_(dims),
        currentInMemoryLinearIndex_(isEnd ? matlab::data::getNumElements(dims)
                                          : 0)
  {
    if (isEnd) {
  ...
  ArrayIndex(ArrayIndex &&rhs) noexcept
      : nDims_(std::move(rhs.nDims_)), index_(std::move(rhs.index_)),
  ...
  ArrayIndex(const ArrayIndex &rhs) noexcept
      : nDims_(rhs.nDims_), index_(rhs.index_), dims_(rhs.dims_),
   ...
   * @param rhs the other ArrayIndex
   * @returns true if this one is less than or equal to the input, otherwise
   * false
   * @throw none
   */
  bool operator<=(const ArrayIndex<IT_ORDER> &rhs) const noexcept
   ...
   * @param rhs the other ArrayIndex
   * @returns true if this one is less than or greater to the input, otherwise
   * false
   * @throw none
   */
  bool operator>=(const ArrayIndex<IT_ORDER> &rhs) const noexcept
  ...
  std::ptrdiff_t advance(std::ptrdiff_t d) noexcept
  {
    ...
    currentInMemoryLinearIndex_ =
        IT_ORDER::getInMemoryIndex(nDims_, index_, dims_);
