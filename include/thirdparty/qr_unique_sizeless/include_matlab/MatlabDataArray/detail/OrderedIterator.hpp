...
public:
  ...
  static void setEnd(const ArrayDimensions &dims, const size_t nDims,
  ...
  static bool lessThan(const std::vector<ptrdiff_t> &lhs,
                       const std::vector<ptrdiff_t> &rhs, const size_t nDims,
                       bool equal)
  {
   ...
   * @returns nothing
   */
  static void next(const size_t nDims, const ArrayDimensions &dims,
                   std::vector<ptrdiff_t> &index)
  {

    // since this is row major order, start by incrementing the last index and
    // move towards the first index
   ...
   * @returns nothing
   */
  static void prev(const size_t nDims, const ArrayDimensions &dims,
                   std::vector<ptrdiff_t> &index)
  {
    // since this is row major order, start by decrementing the last index and
    // move towards the first index
    for (size_t i = nDims; i > 0; --i) {
      if (index[i - 1] == 0) {
        // if this index is at 0, reset it to its max value (which is one less
        // that its dimension)
...
public:
  ...
  static void next(const size_t nDims, const ArrayDimensions &dims,
                   std::vector<ptrdiff_t> &index)
  {
    // since this is column major order, start by incrementing the first index
    // and move towards the last index
    for (size_t i = 0; i < nDims; ++i) {
  ...
  static void prev(const size_t nDims, const ArrayDimensions &dims,
                   std::vector<ptrdiff_t> &index)
  {
    // since this is column major order, start by decrementing the first index
    // and move towards the last index
    for (size_t i = 0; i < nDims; ++i) {
      if (index[i] == 0) {
        index[i] = dims[i] - 1;
...
template <typename T, typename ITORDER> class OrderedIterator {
public:
   ...
   * @throw none
   */
  OrderedIterator(matlab::data::TypedIterator<T> &&it, bool isEnd,
                  const matlab::data::ArrayDimensions &dims,
                  MemoryLayout arrayOrder)
      : it_(std::move(it)),
        matchingOrder_((arrayOrder == ITORDER::order) || (isVector(dims))),
        currentArrayIndex_(dims, isEnd), ref_(createTempRef<impl_type>())
  {
  }
  ...
  OrderedIterator(const OrderedIterator<T, ITORDER> &rhs) noexcept
      : it_(rhs.it_), matchingOrder_(rhs.matchingOrder_),
        currentArrayIndex_(rhs.currentArrayIndex_), ref_(rhs.ref_)
  {
  }
  ...
  OrderedIterator(OrderedIterator<T, ITORDER> &&rhs) noexcept
      : it_(std::move(rhs.it_)), matchingOrder_(std::move(rhs.matchingOrder_)),
  ...
  typename std::enable_if<std::is_arithmetic<U>::value,
                          OrderedIterator<T, ITORDER> &>::type
  operator++() noexcept
  {
    if (matchingOrder_) {
      ++ref_;
...
private:
  ...
  template <typename U>
  typename std::enable_if<!std::is_arithmetic<U>::value &&
                              std::is_same<matlab::data::Struct, U>::value &&
                              std::is_const<T>::value,
                          ref_type>::type
  createTempRef() const noexcept
  {
    return matlab::data::detail::Access::createObj<U>(
        std::shared_ptr<matlab::data::detail::ReferenceImpl>());
  }
