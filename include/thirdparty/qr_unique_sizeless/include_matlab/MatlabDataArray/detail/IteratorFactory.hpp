 ...
 */
template <typename T> class NonConst {
public:
  using type = T;
...
/**
 * This class can create either row major or column major const or non-const
 * iterators
 */
template <template <class> class IteratorType> class IteratorFactory {
private:
  static MemoryLayout getMemoryLayout(const matlab::data::Array &arr)
  {
    ...
    typedef int (*ArrayGetMemoryLayoutFcnPtr)(matlab::data::impl::ArrayImpl *,
                                              int *);
    static const ArrayGetMemoryLayoutFcnPtr fcn =
        (ArrayGetMemoryLayoutFcnPtr)get_function_ptr(
            static_cast<int>(FunctionType::ARRAY_GET_MEMORY_LAYOUT));
    if (fcn == nullptr) {
      // if not supported, this must be column major
      return MemoryLayout::COLUMN_MAJOR;
    }
    int layout;
    fcn(matlab::data::detail::Access::getImpl<matlab::data::impl::ArrayImpl>(
            arr),
        &layout);
  ...
  static MemoryLayout
  getMemoryLayout(const matlab::data::Reference<matlab::data::Array> &ref)
  {
...
public:
   ...
   */
  template <typename T>
  static IteratorType<typename NonConst<T>::type>
  begin(matlab::data::Array &arr)
  {
    if (TypedArray<T>::type != arr.getType()) {
      detail::throwIfError(
          static_cast<int>(matlab::data::ExceptionType::InvalidDataType));
    }
    auto r = matlab::data::getWritableElements<T>(arr);
    return IteratorType<typename NonConst<T>::type>(
        std::move(r.begin()), false, arr.getDimensions(), getMemoryLayout(arr));
  }
  ...
  {
    typedef int (*TypedArrayReferenceValidateTypeFcnPtr)(
        matlab::data::detail::ReferenceImpl *, int, bool);
    static const TypedArrayReferenceValidateTypeFcnPtr fcn =
        matlab::data::detail::resolveFunction<
            TypedArrayReferenceValidateTypeFcnPtr>(
            matlab::data::detail::FunctionType::
                TYPED_ARRAY_REFERENCE_VALIDATE_TYPE);
    matlab::data::detail::throwIfError(
        fcn(matlab::data::detail::Access::getImpl<
                matlab::data::detail::ReferenceImpl>(ref),
            static_cast<int>(GetArrayType<T>::type), true));
    auto r = matlab::data::getWritableElements<T>(ref);
    return IteratorType<typename NonConst<T>::type>(
        std::move(r.begin()), false, ref.getDimensions(), getMemoryLayout(ref));
  }
  ...
  static IteratorType<typename NonConst<T>::type>
  begin(matlab::data::ArrayElementTypedRef<Array, false> &&ref)
  {
   ...
   */
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cbegin(const matlab::data::TypedArray<T> &arr)
  {
    return begin(arr);
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cbegin(const matlab::data::Array &arr)
  {
    if (TypedArray<T>::type != arr.getType()) {
      detail::throwIfError(
          static_cast<int>(matlab::data::ExceptionType::InvalidDataType));
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cbegin(const matlab::data::Reference<matlab::data::Array> &arr)
  {
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  begin(const matlab::data::TypedArray<T> &arr)
  {
    return IteratorType<typename Const<T>::type>(
        Const<T>::begin(arr), false, arr.getDimensions(), getMemoryLayout(arr));
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  begin(const matlab::data::Array &arr)
  {
    if (TypedArray<T>::type != arr.getType()) {
      detail::throwIfError(
          static_cast<int>(matlab::data::ExceptionType::InvalidDataType));
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  begin(const matlab::data::Reference<matlab::data::Array> &arr)
  {
    const TypedArrayRef<T> t_arr(arr);
    return begin(t_arr);
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cbegin(const matlab::data::TypedArrayRef<T> &arr)
  {
    return begin(arr);
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  begin(const matlab::data::TypedArrayRef<T> &arr)
  {
    return IteratorType<typename Const<T>::type>(
        Const<T>::begin(arr), false, arr.getDimensions(), getMemoryLayout(arr));
  }
   ...
   * @throw TypeMismatchException - if the array does not contain T's
   */
  template <typename T, typename ARRAY_TYPE = typename GetReturnType<T>::type>
  static IteratorType<typename Const<T>::type>
  begin(matlab::data::ArrayElementTypedRef<Array, true> &&ref)
  {
  ...
  static IteratorType<typename Const<T>::type>
  cbegin(matlab::data::ArrayElementTypedRef<Array, isConst> &&ref)
  {
    const matlab::data::TypedArray<T> arr = std::move(ref);
    return IteratorType<typename Const<T>::type>(
        Const<T>::begin(arr), false, arr.getDimensions(), getMemoryLayout(arr));
  ...
  static IteratorType<typename NonConst<T>::type> end(matlab::data::Array &arr)
  {
    ...
    auto r = matlab::data::getWritableElements<T>(arr);
    return IteratorType<typename NonConst<T>::type>(
        std::move(r.end()), true, arr.getDimensions(), getMemoryLayout(arr));
  }
  ...
  template <typename T>
  static IteratorType<typename NonConst<T>::type>
  end(matlab::data::TypedArray<T> &arr)
  {
    return IteratorType<typename NonConst<T>::type>(
        NonConst<T>::end(arr), true, arr.getDimensions(), getMemoryLayout(arr));
  }
  ...
  static IteratorType<typename NonConst<T>::type> end(Reference<Array> &ref)
  {
    ...
    auto r = matlab::data::getWritableElements<T>(ref);
    return IteratorType<typename NonConst<T>::type>(
        std::move(r.end()), true, ref.getDimensions(), getMemoryLayout(ref));
  }
  ...
  template <typename T,
            typename ARRAY_TYPE = typename matlab::data::GetReturnType<T>::type>
  static IteratorType<typename NonConst<T>::type>
  end(matlab::data::ArrayElementTypedRef<Array, false> &&ref)
  {
    matlab::data::TypedArrayRef<T> arr = std::move(ref);
    return IteratorType<typename NonConst<T>::type>(
        NonConst<T>::end(arr), true, arr.getDimensions(), getMemoryLayout(arr));
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  end(const matlab::data::TypedArray<T> &arr)
  {
    return cend(arr);
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cend(const matlab::data::TypedArray<T> &arr)
  {
    return IteratorType<typename Const<T>::type>(
        Const<T>::end(arr), true, arr.getDimensions(), getMemoryLayout(arr));
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cend(const matlab::data::Array &arr)
  {
    if (TypedArray<T>::type != arr.getType()) {
      detail::throwIfError(
          static_cast<int>(matlab::data::ExceptionType::InvalidDataType));
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cend(const matlab::data::Reference<matlab::data::Array> &arr)
  {
    const TypedArrayRef<T> t_arr(arr);
    return end(t_arr);
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  end(const matlab::data::TypedArrayRef<T> &arr)
  {
    return cend(arr);
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  cend(const matlab::data::TypedArrayRef<T> &arr)
  {
    return IteratorType<typename Const<T>::type>(
        Const<T>::end(arr), true, arr.getDimensions(), getMemoryLayout(arr));
  }
  ...
  template <typename T>
  static IteratorType<typename Const<T>::type>
  end(matlab::data::ArrayElementTypedRef<Array, true> &&ref)
  {
    const matlab::data::TypedArray<T> arr = std::move(ref);
    return IteratorType<typename Const<T>::type>(
        Const<T>::end(arr), true, arr.getDimensions(), getMemoryLayout(arr));
  ...
  template <typename T, bool isConst>
  static IteratorType<typename Const<T>::type>
  cend(matlab::data::ArrayElementTypedRef<Array, isConst> &&ref)
  {
    const matlab::data::TypedArray<T> arr = std::move(ref);
    return IteratorType<typename Const<T>::type>(
        Const<T>::end(arr), true, arr.getDimensions(), getMemoryLayout(arr));
  ...
  {
    return Range<IteratorType, typename Const<T>::type>(begin(arr), end(arr));
  }
  ...
  readOnlyElements(matlab::data::ArrayElementTypedRef<Array, false> &&ref)
  {
    ...
    return Range<IteratorType, typename Const<T>::type>(cbegin(arr), cend(arr));
  }
  ...
  writableElements(matlab::data::Reference<Array> &ref)
  {
    ...
    return Range<IteratorType, typename NonConst<T>::type>(
        IteratorType<typename NonConst<T>::type>(std::move(r.begin()), false,
                                                 ref.getDimensions(),
                                                 getMemoryLayout(ref)),
        IteratorType<typename NonConst<T>::type>(std::move(r.end()), true,
                                                 ref.getDimensions(),
                                                 getMemoryLayout(ref)));
  }
