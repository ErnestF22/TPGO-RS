...
{
  return (std::find_if(str.begin(), str.end(), [](CHAR16_T ch) {
            return (ch & 0xFF80) != 0;
          }) == str.end());
}
...
inline bool isAscii7(const std::string &str)
{
  return (std::find_if(str.begin(), str.end(),
                       [](char ch) { return (ch & 0x80) != 0; }) == str.end());
}
...
fromUTF8Helper(const char *str, size_t strLen, char16_t *out, size_t *strLenOut)
{
  ...
  std::codecvt_base::result r = cvt.in(
      state, str, str + strLen, nextIn, static_cast<char16_t *>(bufferStart),
      static_cast<char16_t *>(bufferEnd), nextOut);
