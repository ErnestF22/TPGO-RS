...
inline void throwIfError(int type, const std::string &customMsg = std::string())
{
  ...
  case ExceptionType::IllegalFieldName:
    throw IllegalFieldNameException(
  ...
  case ExceptionType::WrongNumberOfEnumsSupplied:
    throw WrongNumberOfEnumsSuppliedException(
        "Wrong number of enums supplied.");
  case ExceptionType::OutOfMemory:
    throw matlab::OutOfMemoryException(
  ...
  case ExceptionType::StringIndexNotValid:
    throw StringIndexNotValidException(
        "String index not valid for this array.");
  case ExceptionType::CanOnlyUseOneStringIndex:
    throw CanOnlyUseOneStringIndexException(
        "Can only provide one string index.");
  ...
  case ExceptionType::DuplicateFieldNameInStructArray:
    throw DuplicateFieldNameInStructArrayException(
  ...
  case ExceptionType::NumberOfElementsExceedsMaximum:
    throw NumberOfElementsExceedsMaximumException(
        "Number of elements exceeds max.");
  case ExceptionType::InvalidHeterogeneousArray:
    throw ObjectArrayIncompatibleTypesException(
        "ObjectArray not created. Elements must belong to the same class, or "
        "to classes that are part of a heterogeneous hierarchy.");
  ...
  case ExceptionType::WrongVectorSizeForStruct:
    throw WrongVectorSizeForStructException(
        "Vector size is wrong for the struct.");
  ...
  case ExceptionType::CouldNotCallConstructor:
    throw CouldNotCallConstructorException(
        "Failed to execute class constructor.");
  ...
  case ExceptionType::InvalidHeterogeneousClass:
    throw InvalidHeterogeneousClassException(
        "ObjectArray not created. Elements must belong to the same class, or "
        "to classes that are part of a heterogeneous hierarchy.");
  ...
  case ExceptionType::InvalidDimensionsInSparseArray:
    throw InvalidDimensionsInSparseArrayException(
        "Sparse Array can only have two dimensions.");
  ...
  case ExceptionType::InvalidAssignmentForContainerArray:
    throw InvalidAssignmentForContainerArrayException(
        "Cannot assign a container array to an element of an array created for "
        "optimization ");
  case ExceptionType::InvalidUTF8Input:
    throw InvalidUTF8InputException("Input data must contain UTF8 characters.");
  case ExceptionType::InvalidUTF16toUTF8Conversion:
    throw InvalidUTF16toUTF8ConversionException(
        "Invalid conversion from UTF-16 input to UTF-8 encoding.");
...
template <class T> T resolveFunctionNoExcept(FunctionType fcn)
{
...
template <class T> T resolveFunction(FunctionType fcn)
{
  ...
  if (ptr == nullptr) {
    ...
    auto it = std::find_if(minRelease.rbegin(), minRelease.rend(),
                           [fcn](std::pair<FunctionType, std::string> elem) {
