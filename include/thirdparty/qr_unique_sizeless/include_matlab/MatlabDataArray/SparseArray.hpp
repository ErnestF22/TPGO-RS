...
public:
   ...
   * @throw none
   */
  SparseArray(const SparseArray<T> &rhs) MW_NOEXCEPT : Array(rhs)
  {
  }
   ...
   * @throw none
   */
  SparseArray(SparseArray &&rhs) MW_NOEXCEPT : Array(std::move(rhs))
  {
  }
  ...
  SparseArray(const Array &rhs) : Array(rhs)
  {
    ...
    bool result;
    detail::throwIfError(
        fcn(static_cast<int>(type), static_cast<int>(rhs.getType()), &result));
    if (!result) {
      throw InvalidArrayTypeException(
          "Can't convert the Array to this SparseArray");
    }
  }
   ...
   */
  SparseArray(Array &&rhs) : Array(std::move(rhs))
  {
    typedef int (*TypedArrayIsValidConversionFcnPtr)(int, int, bool *);
    static const TypedArrayIsValidConversionFcnPtr fcn =
        detail::resolveFunction<TypedArrayIsValidConversionFcnPtr>(
    ...
    bool result;
    detail::throwIfError(
        fcn(static_cast<int>(type), static_cast<int>(getType()), &result));
    if (!result) {
      rhs = std::move(*this);
      throw InvalidArrayTypeException(
          "Can't convert the Array to this SparseArray");
   ...
   * @throw InvalidArrayTypeException if type of rhs doesn't match or is not
   * recognized
   */
  ...
  iterator end() MW_NOEXCEPT
  {
    ...
    typedef detail::IteratorImpl *(*TypedArrayEndFcnPtr)(impl::ArrayImpl *,
                                                         bool);
    static const TypedArrayEndFcnPtr fcn =
        detail::resolveFunctionNoExcept<TypedArrayEndFcnPtr>(
            detail::FunctionType::TYPED_ARRAY_END_V2);
    if (fcn == nullptr) {
      static const TypedArrayEndFcnPtr fcn2 =
  ...
  {
    size_t val;
    typedef void (*SparseArrayGetNumNonZeroElementsFcnPtr)(impl::ArrayImpl *,
                                                           size_t *);
    static const SparseArrayGetNumNonZeroElementsFcnPtr fcn =
        detail::resolveFunction<SparseArrayGetNumNonZeroElementsFcnPtr>(
  ...
  /**
   * This API returns the row-column coordinates of the non-zero entry that the
   * iterator is pointing to.
   *
   * @param it - Iterator pointing to the current entry in the Sparse matrix
   * @return - {m,n} of the entry
   * @throw FeatureNotSupportedException if running against a MATLAB version
   * prior to R2018b
   */
  template <typename U = T>
  typename std::enable_if<std::is_arithmetic<U>::value || is_complex<U>::value,
                          SparseIndex>::type
  getIndex(const TypedIterator<T> &it)
  {
    size_t row, col;
    auto b = begin();
    typedef void (*SparseArrayGetIndexArithmeticFcnPtr)(
        impl::ArrayImpl *, size_t, size_t *, size_t *);
    static const SparseArrayGetIndexArithmeticFcnPtr fcn =
        detail::resolveFunction<SparseArrayGetIndexArithmeticFcnPtr>(
            detail::FunctionType::SPARSE_ARRAY_GET_INDEX_ARITHMETIC);
    fcn(detail::Access::getImpl<impl::ArrayImpl>(*this), (it - b), &row, &col);
    return SparseIndex(row, col);
  }
  template <typename U = T>
  typename std::enable_if<
      !std::is_arithmetic<U>::value && !is_complex<U>::value, SparseIndex>::type
  getIndex(const TypedIterator<T> &it) MW_NOEXCEPT
  {
    size_t row, col;
    typedef void (*SparseArrayGetIndexFcnPtr)(
        impl::ArrayImpl *, detail::IteratorImpl *, size_t *, size_t *);
    static const SparseArrayGetIndexFcnPtr fcn =
        detail::resolveFunction<SparseArrayGetIndexFcnPtr>(
            detail::FunctionType::SPARSE_ARRAY_GET_INDEX);
    fcn(detail::Access::getImpl<impl::ArrayImpl>(*this),
        detail::Access::getImpl<detail::IteratorImpl>(it), &row, &col);
    return SparseIndex(row, col);
  }
  ...
  typename std::enable_if<std::is_arithmetic<U>::value || is_complex<U>::value,
                          SparseIndex>::type
  getIndex(const TypedIterator<T const> &it) const
  {
    size_t row, col;
    auto b = cbegin();
    typedef void (*SparseArrayGetIndexArithmeticFcnPtr)(
  ...
  typename std::enable_if<
      !std::is_arithmetic<U>::value && !is_complex<U>::value, SparseIndex>::type
  getIndex(const TypedIterator<T const> &it) const MW_NOEXCEPT
  {
    size_t row, col;
    typedef void (*SparseArrayGetIndexFcnPtr)(
        impl::ArrayImpl *, detail::IteratorImpl *, size_t *, size_t *);
...
private:
  ...
  SparseArray(impl::ArrayImpl *impl) MW_NOEXCEPT : Array(impl)
  {
  }
