...
public:
  ...
  Array() MW_NOEXCEPT
  {
    ...
    static const ArrayCreateEmptyFcnPtr fcn =
        detail::resolveFunction<ArrayCreateEmptyFcnPtr>(
  ...
  /**
   * Assignment operator. The updated Array becomes a shared copy of the input
   * Array
   *
  ...
  /**
   * Copy consructor. The newly constructed Array becomes a shared copy of the
   * input Array
   *
   * @param - rhs Array value to be copied
  ...
  ArrayType getType() const
  {
    ...
    static const ArrayGetTypeFcnPtr fcn =
        detail::resolveFunction<ArrayGetTypeFcnPtr>(
            detail::FunctionType::ARRAY_GET_TYPE);
    int arrayType;
   ...
   * @throw FeatureNotSupportedException - if requesting the memory layout from
   * an application built before R2019a
   */
  ...
  ArrayDimensions getDimensions() const MW_NOEXCEPT
  {
    size_t numDims = 0;
    size_t *dims = nullptr;
    typedef void (*ArrayGetDimensionsFcnPtr)(impl::ArrayImpl *, size_t *,
                                             size_t **);
  ...
  bool isEmpty() const MW_NOEXCEPT
  {
    ...
    static const ArrayIsEmptyFcnPtr fcn =
        detail::resolveFunction<ArrayIsEmptyFcnPtr>(
            detail::FunctionType::ARRAY_IS_EMPTY);
    return fcn(pImpl.get());
  }
   ...
   *
   * The return value ArrayElementRef<false> allows the element of the array to
   * be modified or retrieved: For example: arr[1][1] = 5.5; double val =
   * arr[0][3];
   *
   ...
   * be retrieved, but not modified: For example: double val = arr[0][3];
   *
   * @param idx - the first array index
