...
namespace data {
/**
 * This specialization of Reference class adds APIs to a reference to a
 * SparseArray. Reference<> is not thread-safe - it should not be passed between
 * threads.
 *
 * @code
...
public:
   ...
   * @throw TypeMismatchException - if the element of the Array is not a
   * SparseArray<T>
   */
  Reference(const Reference<Array> &rhs) : ArrayReferenceExt(rhs)
  {
  ...
  {
    size_t val;
    typedef void (*SparseArrayRefGetNumNonZeroElementsFcnPtr)(
        detail::ReferenceImpl *, size_t *);
    static const SparseArrayRefGetNumNonZeroElementsFcnPtr fcn =
        detail::resolveFunction<SparseArrayRefGetNumNonZeroElementsFcnPtr>(
...
template <typename T> using SparseArrayRef = Reference<SparseArray<T>>;
} // namespace data
} // namespace matlab
