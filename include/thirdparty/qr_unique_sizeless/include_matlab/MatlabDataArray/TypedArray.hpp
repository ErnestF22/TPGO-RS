...
public:
  ...
  static const ArrayType type =
      GetArrayType<typename std::remove_const<T>::type>::type;
   ...
   * @return - newly constructed TypedArray
   * @throw InvalidArrayTypeException if type of rhs does not match the type for
   * TypedArray<T>
   */
  TypedArray(Array &&rhs) : Array(std::move(rhs))
  {
    ...
    detail::throwIfError(fcn(static_cast<int>(GetArrayType<T>::type),
                             static_cast<int>(getType()), &result));
    if (!result) {
      rhs = std::move(*this);
  ...
  TypedArray<T> &operator=(Array &&rhs)
  {
    ...
    bool result;
    detail::throwIfError(fcn(static_cast<int>(GetArrayType<T>::type),
                             static_cast<int>(rhs.getType()), &result));
    if (!result) {
      throw InvalidArrayTypeException(
          "Can't convert the Array to this TypedArray");
    }
   ...
   * @return - newly constructed TypedArray
   * @throw InvalidArrayTypeException if type of rhs does not match the type for
   * TypedArray<T> or is not recognized as valid.
   */
  ...
  const_iterator begin() const MW_NOEXCEPT
  {
    typedef detail::IteratorImpl *(*TypedArrayBeginFcnPtr)(impl::ArrayImpl *,
                                                           bool);
    static const TypedArrayBeginFcnPtr fcn =
        detail::resolveFunctionNoExcept<TypedArrayBeginFcnPtr>(
            detail::FunctionType::TYPED_ARRAY_BEGIN_V2);
    if (fcn == nullptr) {
      static const TypedArrayBeginFcnPtr fcn2 =
          detail::resolveFunction<TypedArrayBeginFcnPtr>(
              detail::FunctionType::TYPED_ARRAY_BEGIN);
   ...
   *
   * The return value ArrayElementTypedRef<T, std::is_const<T>::value> allows
   * the element of the array to be retrieved or modified (if not const): For
   * example: arr[1][1] = 5.5; double val = arr[0][3];
   *
   * @param idx - the first array index
   * @return ArrayElementTypedRef<T, std::is_const<T>::value> - contains the
   * index specified
  ...
  ArrayElementTypedRef<T, std::is_const<T>::value> operator[](size_t idx)
  {
    ...
    return detail::Access::createObj<
        ArrayElementTypedRef<T, std::is_const<T>::value>>(impl);
  }

  /**
   * Enables [] indexing on a TypedArray.
   *
   * The return value ArrayElementTypedRef<T, true> allows the element of the
   * array to be modified or retrieved: For example: double val = arr[0][3];
   *
   * @param idx - the first array index
   ...
   * @throw InvalidArrayTypeException - if this TypedArray does not support
  ...
  buffer_ptr_t<T> release()
  {
    ...
    typedef bool (*ArrayUnshareFcnPtr)(impl::ArrayImpl *, bool,
                                       impl::ArrayImpl **);
    static const ArrayUnshareFcnPtr fcn_unshare =
        detail::resolveFunction<ArrayUnshareFcnPtr>(
            detail::FunctionType::ARRAY_UNSHARE);
    ...
    typedef int (*ArrayReleaseBufferFcnPtr)(impl::ArrayImpl *, bool, void **,
                                            void (**deleter)(void *));
    static const ArrayReleaseBufferFcnPtr fcn =
        detail::resolveFunction<ArrayReleaseBufferFcnPtr>(
            detail::FunctionType::ARRAY_RELEASE_BUFFER);
    detail::throwIfError(
        fcn(pImpl.get(), (pImpl.use_count() == 1), &buffer, &deleter));
