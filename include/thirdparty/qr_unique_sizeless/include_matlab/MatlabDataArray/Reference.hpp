...
template <typename T> struct GetReferenceExt {
  using type = ReferenceHolder;
};
template <> struct GetReferenceExt<Array> {
  using type = IndexableArrayRef<false>;
};
template <> struct GetReferenceExt<MATLABString> {
...
public:
   ...
   * @throw InvalidHeterogeneousArrayException - if assigning an object to an
   * ObjectArray that is not valid
   */
  Reference<T> &operator=(T rhs)
  {
    static_assert(!std::is_const<T>::value,
                  "Can't modify a Reference to const data");
   ...
   *
   * @throw std::runtime_error - for a Reference<MATLABString> if the
   * MATLABString is missing
   * @throw NonAsciiCharInInputDataException for a Reference<MATLABString> if
   * the MATLABString contains non-ascii characters
   */
   ...
   * non-const and allow strings to be assigned
   *
   * @code
   *    Reference<String> ref = str_arr[0];
   *    ref = "MyString";
  ...
  /**
   * Assign a String value into a MATLABString Array. The Array being indexed
   * must be non-const.
   *
   * @code
   ...
   * @throw none
   */
  Reference(Reference<T> &&rhs) MW_NOEXCEPT : Parent(std::move(rhs))
  {
  }
   ...
   * @throw none
   */
  Reference(const Reference<T> &rhs) MW_NOEXCEPT : Parent(rhs)
  {
  }
...
protected:
  ...
  Reference(detail::ReferenceImpl *impl) MW_NOEXCEPT : Parent(impl)
  {
  }
                       ...
                       String const &rhs) MW_NOEXCEPT
{
  typedef bool (*ReferenceMATLABStringEqualString16FcnPtr)(
      detail::ReferenceImpl *, const char16_t *, size_t idx);
...
inline bool operator==(String const &lhs,
                       Reference<MATLABString> const &rhs) MW_NOEXCEPT
{
  typedef bool (*ReferenceMATLABStringEqualString16FcnPtr)(
      detail::ReferenceImpl *, const char16_t *, size_t idx);
