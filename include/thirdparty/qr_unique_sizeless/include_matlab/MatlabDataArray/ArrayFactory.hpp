...
public:
   ...
   *
   * @throw FailedToLoadLibMatlabDataArrayException if necessary libraries are
   * not found
   */
  ArrayFactory()
  {
    ...
    pImpl = std::shared_ptr<impl::ArrayFactoryImpl>(
        impl, [](impl::ArrayFactoryImpl *ptr) {
          typedef void (*DestroyArrayFactoryFcnPtr)(
              typename impl::ArrayFactoryImpl *);
   ...
   * @throw InvalidArrayType - if array type of the new array is not recognized
   * as valid
   * @throw NumberOfElementsExceedsMaximumException - if the ArrayDimensions
   * create an Array which exceeds the max
   */
  template <typename T> TypedArray<T> createArray(ArrayDimensions dims)
  {
    typedef int (*CreateArrayWithDimsFcnPtr)(typename impl::ArrayFactoryImpl *,
                                             int, size_t *, size_t,
    ...
    detail::throwIfError(fcn(pImpl.get(),
                             static_cast<int>(GetArrayType<T>::type), &dims[0],
                             dims.size(), &impl));
   ...
   * data specified by the begin and end iterators. Data is copied and can be in
   * either row major or column order. The data type for the array is determined
   * by the value_type of the iterator.
   *
   * @param dims - the dimensions for the Array
   * @param begin - start of the user supplied data
   * @param end - end of the user supplied data
   * @param inputLayout - the layout of the input data; either row major or
   * column major
   *
   * @return TypedArray<T> - an Array with the appropriate type, dimensions and
   * data
   *
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   * @throw InvalidArrayType - if array type of the new array is not recognized
  ...
  template <typename DATA_IT,
            typename T = typename std::remove_cv<
                typename std::iterator_traits<DATA_IT>::value_type>::type,
            typename ARRAY_TYPE = typename matlab::data::GetReturnType<T>::type>
  typename std::enable_if<!std::is_same<T, Object>::value,
                          TypedArray<ARRAY_TYPE>>::type
  createArray(ArrayDimensions dims, DATA_IT begin, const DATA_IT &end,
              ...
              InputLayout inputLayout = InputLayout::COLUMN_MAJOR)
  {
    ...
    if (begin == end) {
      detail::throwIfError(
          static_cast<int>(ExceptionType::InvalidNumberOfElementsProvided));
    }

    matlab::data::impl::ArrayImpl *aImpl = nullptr;

    typedef int (*CreateObjectArrayWithObjsFcnPtr)(
        matlab::data::impl::ArrayFactoryImpl *, size_t *, size_t,
        matlab::data::impl::ObjectImpl *, matlab::data::impl::ArrayImpl **);

    static const CreateObjectArrayWithObjsFcnPtr fcn =
        detail::resolveFunction<CreateObjectArrayWithObjsFcnPtr>(
            detail::FunctionType::CREATE_OBJECT_ARRAY_WITH_OBJ);
    detail::throwIfError(
        fcn(pImpl.get(), &dims[0], dims.size(),
   ...
   */
  template <typename T>
  TypedArray<typename GetReturnType<T>::type>
  createArray(ArrayDimensions dims, std::initializer_list<T> data)
  {
    return detail::createArrayWithIterator(pImpl.get(), std::move(dims),
                                           data.begin(), data.end());
  };

  /**
   * Creates a TypedArray<T> with the given dimensions and filled in with the
   * data specified by C-style begin and end pointers. Data is copied and must
   * be in column major order. This methods supports all arithmetic types.
   *
   * @param dims - the dimensions for the Array
   ...
   */
  template <typename T>
  typename std::enable_if<std::is_arithmetic<T>::value || is_complex<T>::value,
                          TypedArray<T>>::type
  createArray(ArrayDimensions dims, const T *const begin, const T *const end)
  {
    typedef int (*CreateArrayWithDimsAndDataFcnPtr)(
        typename impl::ArrayFactoryImpl *impl, int arrayType, size_t *dims,
        size_t numDims, const void *const dataStart, size_t numEl,
        typename impl::ArrayImpl **);
  ...
  {
    typedef int (*CreateScalarArrayFcnPtr)(
        typename impl::ArrayFactoryImpl *impl, int arrayType, const void *data,
        typename impl::ArrayImpl **);
    static const CreateScalarArrayFcnPtr fcn =
        detail::resolveFunction<CreateScalarArrayFcnPtr>(
   ...
   *
   * @return TypedArray<MATLABString> - a newly created TypedArray of
   * MATLABString
   *
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   */
  ...
  {
    typedef int (*CreateScalarStringFcnPtr)(
        typename impl::ArrayFactoryImpl *impl, const char16_t *data,
   ...
   * @param val - the string to be inserted into the scalar
   *
   * @return TypedArray<MATLABString> - an Array containing data of MATLABString
   * type
   *
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   */
  TypedArray<MATLABString> createScalar(const MATLABString val)
  {
    ...
    if (val) {
      ...
      typedef int (*CreateScalarMissingStringFcnPtr)(
          typename impl::ArrayFactoryImpl *impl, typename impl::ArrayImpl **);
  ...
  /**
   * Creates a CellArray with the data specified. Data is specified in column
   * major order
   *
   * @param dims - the dimensions of the CellArray
   * @params data - elements to be inserted into the cell array. Primitive
   * types, strings and Arrays are allowable inputs.
   *
   * @return CellArray - newly created Cell Array
   *
   * @throw NumberOfElementsExceedsMaximumException - if the ArrayDimensions
   ...
   *
   * @return TypedArray<MATLABString> - a TypedArray with data of type
   * MATLABString
   *
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
  ...
  {
    impl::ArrayImpl *impl = nullptr;
    typedef int (*CreateScalarObjectFcnPtr)(
        typename impl::ArrayFactoryImpl *impl, impl::ObjectImpl *objImpl,
        typename impl::ArrayImpl **);
    ...
    detail::throwIfError(fcn(
        pImpl.get(), detail::Access::getImpl<impl::ObjectImpl>(val), &impl));
   ...
   *
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   * @throw NonAsciiCharInInputDataException - if data contains non-ascii
   * characters
   */
   ...
   *
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   * @throw matlab::InvalidUTF8InputException - input string contains a non-UTF8
   * character
   * @throw FeatureNotSupportedException - if customer code is running version
   * older than 2024b
   */
   ...
   *
   * @return StructArray - an Array with the appropriate dimensions and
   * fieldNames
   *
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   * @throw NumberOfElementsExceedsMaximumException - if the ArrayDimensions
   * create an Array which exceeds the max
   * @throw DuplicateFieldNameInStructArrayException - if duplicate field names
   * are specified
   */
                                ...
                                std::vector<std::string> fieldNames)
  {
    ...
    static const CreateNamesFcnPtr fcn3 =
        detail::resolveFunction<CreateNamesFcnPtr>(
            detail::FunctionType::CREATE_NAMES);
    NameList field_names(fcn3(fieldNames.size()));
    for (const auto &field : fieldNames) {
      typedef void (*AddNameFcnPtr)(typename detail::NameListImpl *impl,
                                    const char *name, size_t nameLen);
      static const AddNameFcnPtr fcn = detail::resolveFunction<AddNameFcnPtr>(
          detail::FunctionType::ADD_NAME);
      fcn(field_names.getImpl(), field.c_str(), field.size());
    }
    impl::ArrayImpl *impl = nullptr;
    typedef int (*CreateStructArrayFcnPtr)(
        typename impl::ArrayFactoryImpl *impl, size_t *dims, size_t numDims,
        typename detail::NameListImpl *names, typename impl::ArrayImpl **);
  ...
  /**
   * Creates an Enumeration array. The class of the EnumArray is indicated by
   * className. The array is initialized with the list of enumeration names
   ...
   * @throw WrongNumberOfEnumsSuppliedException - if the wrong number of
                            ...
                            std::vector<std::string> enums)
  {
    ...
    typedef int (*CreateEnumArrayWithEnumsFcnPtr)(
        typename impl::ArrayFactoryImpl *impl, size_t *dims, size_t numDims,
        const char *cls, size_t clsSize, typename detail::NameListImpl *names,
        typename impl::ArrayImpl **retVal);
    ...
    detail::throwIfError(fcn2(pImpl.get(), &dims[0], dims.size(),
                              className.c_str(), className.size(),
                              enum_names.getImpl(), &impl));
  ...
  EnumArray createEnumArray(ArrayDimensions dims, std::string className)
  {
    typedef int (*CreateEnumArrayFcnPtr)(typename impl::ArrayFactoryImpl *,
                                         size_t *, size_t, const char *, size_t,
                                         typename impl::ArrayImpl **);
    static const CreateEnumArrayFcnPtr fcn =
        detail::resolveFunction<CreateEnumArrayFcnPtr>(
            detail::FunctionType::CREATE_ENUM_ARRAY);
    impl::ArrayImpl *impl = nullptr;
    detail::throwIfError(fcn(pImpl.get(), &dims[0], dims.size(),
                             className.c_str(), className.size(), &impl));
    return detail::Access::createObj<EnumArray>(impl);
  }
   ...
   *
   * @param numberOfElements - the number of elements, not the actual buffer
   * size
   *
   * @return buffer_ptr<T> - unique_ptr containing the buffer
   *
   * @throw InvalidArrayTypeException - if buffer type is not recognized as
   * valid
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   */
  template <typename T> buffer_ptr_t<T> createBuffer(size_t numberOfElements)
  {
    ...
    typedef int (*CreateBufferFcnPtr)(typename impl::ArrayFactoryImpl *impl,
                                      void **buffer, void (**deleter)(void *),
                                      int dataType, size_t numElements);
    static const CreateBufferFcnPtr fcn =
        detail::resolveFunction<CreateBufferFcnPtr>(
            detail::FunctionType::CREATE_BUFFER);
    detail::throwIfError(fcn(pImpl.get(), &buffer, &deleter,
                             static_cast<int>(GetArrayType<T>::type),
                             numberOfElements));
    return buffer_ptr_t<T>(static_cast<T *>(buffer),
                           reinterpret_cast<buffer_deleter_t<T>>(deleter));
  }

  /**
   * Creates a TypedArray<T> using the given buffer
   * No data copies are made when creating an Array from a buffer. The
   * TypedArray<T> takes ownership of the buffer.
   *
   * @param dims - the dimensions for the Array
   * @param buffer - buffer containing the data. Buffer will not be copied.
   ...
   * @throw InvalidArrayTypeException - if buffer type is not recognized as
   * valid
   * @throw FeatureNotSupportedException - if creating an array with a row major
   * layout using a release prior to R2019a
   * @throw matlab::InvalidMemoryLayoutException - if the array could not be
   * allocated
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   */
  template <typename T>
  TypedArray<T>
  createArrayFromBuffer(ArrayDimensions dims, buffer_ptr_t<T> buffer,
                        MemoryLayout memoryLayout = MemoryLayout::COLUMN_MAJOR)
  {
    buffer_deleter_t<> deleter =
        reinterpret_cast<buffer_deleter_t<>>(buffer.get_deleter());
    impl::ArrayImpl *impl = nullptr;
    typedef int (*CreateArrayFromBufferV2FcnPtr)(
        typename impl::ArrayFactoryImpl *impl, int arrayType, size_t *dims,
        size_t numDims, void *buffer, void (*deleter)(void *),
        typename impl::ArrayImpl **, int memoryLayout);
    ...
    if (fcn != nullptr) {
      detail::throwIfError(fcn(pImpl.get(),
                               static_cast<int>(GetArrayType<T>::type),
                               &dims[0], dims.size(), buffer.release(), deleter,
                               &impl, static_cast<int>(memoryLayout)));
    } else {
      // new version is not available
      // if asking for a row-major need to throw
      if (memoryLayout == MemoryLayout::ROW_MAJOR) {
        throw FeatureNotSupportedException(
            std::string("Row-major buffers require ") + std::string("2019a"));
      }
      ...
      detail::throwIfError(
          fcn(pImpl.get(), static_cast<int>(GetArrayType<T>::type), &dims[0],
   ...
   * @param data - buffer containing the non-zero elements. Buffer will not be
   * copied. Array takes ownership
   * @param rows - buffer containing the row value for each element. Buffer will
   * not be copied. Array takes ownership
   * @param cols - buffer containing the column value for each element. Buffer
   * will not be copied. Array takes ownership
   *
   * @return SparseArray<T> - a sparse array with the appropriate type and
   * dimensions, and data
   *
   * @throw InvalidArrayTypeException - if array type is not recognized as valid
   * @throw InvalidDimensionsInSparseArrayException if more than two dimensions
   * are specified.
   * @throw matlab::OutOfMemoryException - if the array could not be allocated
   *
   */
  ...
  typename std::enable_if<std::is_same<double, T>::value ||
                          ...
                          SparseArray<T>>::type
  createSparseArray(ArrayDimensions dims, size_t nnz, buffer_ptr_t<T> data,
                    buffer_ptr_t<size_t> rows, buffer_ptr_t<size_t> cols)
  {
    ...
    buffer_deleter_t<> data_deleter =
        reinterpret_cast<buffer_deleter_t<>>(data.get_deleter());
    buffer_deleter_t<> rows_deleter =
        reinterpret_cast<buffer_deleter_t<>>(rows.get_deleter());
    buffer_deleter_t<> cols_deleter =
        reinterpret_cast<buffer_deleter_t<>>(cols.get_deleter());

    impl::ArrayImpl *impl = nullptr;
    typedef int (*CreateSparseArrayFromBufferFcnPtr)(
        typename impl::ArrayFactoryImpl *impl, int arrayType, size_t *dims,
        size_t numDims, size_t nnz, void *dataBuffer,
        void (*dataDeleter)(void *), size_t *rowsBuffer,
        void (*rowsDeleter)(void *), size_t *colsBuffer,
        void (*colsDeleter)(void *), typename impl::ArrayImpl **);
    ...
    detail::throwIfError(
        fcn(pImpl.get(), static_cast<int>(GetSparseArrayType<T>::type),
            &dims[0], dims.size(), nnz, data.release(), data_deleter,
            rows.release(), rows_deleter, cols.release(), cols_deleter, &impl));
...
private:
    ...
    NameList(detail::NameListImpl *impl)
        : pImpl(std::shared_ptr<detail::NameListImpl>(
              impl, [](detail::NameListImpl *ptr) {
                typedef void (*DestroyNamesFcnPtr)(
                    typename detail::NameListImpl *impl);
  ...
  template <typename T>
  typename std::enable_if<!std::is_base_of<Array, T>::value, Array>::type
  createValue(T value)
  {
  ...
  template <typename ORDER, typename DATA_IT>
  void fillInObjectArrayImpl(ObjectArray &retVal, const DATA_IT &begin,
                             const DATA_IT &end)
  {
    ...
    for (auto elem : values) {
      if (it == end) {
        detail::throwWrongNumberOfElementsException(begin, end,
                                                    retVal.getDimensions());
      }
      elem = Object(*it);
      ++it;
    }
  ...
  template <template <class> class MDA_IT, typename ORDER, typename ARRAY_TYPE,
            typename DATA_IT,
            typename IT_VALUE = typename std::remove_cv<
                typename std::iterator_traits<DATA_IT>::value_type>::type>
  typename std::enable_if<!std::is_same<IT_VALUE, MATLABString>::value,
                          void>::type
  fillInArrayImpl(matlab::data::TypedArray<ARRAY_TYPE> &retVal, DATA_IT begin,
                  const DATA_IT &end)
  {
  ...
  template <template <class> class MDA_IT, typename ORDER, typename DATA_IT>
  void fillInArrayImpl(matlab::data::TypedArray<matlab::data::Array> &retVal,
                       DATA_IT begin, const DATA_IT &end)
  {
