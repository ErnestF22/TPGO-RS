...
public:
  ...
  typedef typename std::conditional<std::is_arithmetic<T>::value, T &,
                                    ref_type>::type reference;
  typedef typename std::conditional<std::is_arithmetic<T>::value, T *,
                                    ref_type *>::type pointer;
...
private:
  typedef typename std::conditional<std::is_arithmetic<T>::value, void *,
                                    ref_type>::type TempRef;
  ...
  template <typename U>
  typename std::enable_if<std::is_arithmetic<U>::value, reference>::type
  getValueOrRef() const MW_NOEXCEPT
  {
    void *value = nullptr;
  ...
  template <typename U>
  typename std::enable_if<std::is_arithmetic<U>::value, pointer>::type
  getPointerOrRef() const MW_NOEXCEPT
  {
    void *value = nullptr;
    typedef void (*TypedIteratorGetPodValueFcnPtr)(detail::IteratorImpl *,
                                                   void **);
  ...
  createTempRef() const MW_NOEXCEPT
  {
    return nullptr;
  }
  ...
  createTempRef() const MW_NOEXCEPT
  {
    return detail::Access::createObj<U>(
        std::shared_ptr<detail::ReferenceImpl>());
  }
};
} // namespace data
} // namespace matlab
