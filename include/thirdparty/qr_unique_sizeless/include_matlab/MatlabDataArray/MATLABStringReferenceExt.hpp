...
/**
 * The MATLABStringReferenceExt class is a base class for all reference types
 * that refer to MATLABString. It provides APIs for MATLABString that are not
 * supported by generic Reference class.
 */
...
protected:
  ...
  MATLABStringReferenceExt(std::shared_ptr<detail::ReferenceImpl> impl)
      MW_NOEXCEPT : ReferenceHolder(impl)
  {
  }
  ...
  MATLABStringReferenceExt &
  operator=(MATLABStringReferenceExt &&rhs) MW_NOEXCEPT
  {
  ...
  MATLABStringReferenceExt &
  operator=(const MATLABStringReferenceExt &rhs) MW_NOEXCEPT
  {
