 ...
 * cast to an element if the array holds primitive data or can be used to create
 * a Reference<T> instance. The template argument indicates if this reference is
 ...
 * @endcode
 */
template <bool is_const_ref> class ArrayElementRef {
public:
  /**
   ...
   *
   * @return ArrayElementRef<is_const_ref> - a new ArrayElementRef<is_const_ref>
   * with the additional index
   *
  ...
  {
    typedef int (*ReferenceAddStringIndexFcnPtr)(detail::ReferenceImpl *,
  ...
  /**
   * Cast a value. Can be cast to the element type of the array, or to a
   * reference to an element of the array if the element type is not a primitive
   * type
   *
   * @code
   ...
   *
   * @throw InvalidArrayIndexException - if one of the indices is out of range
   * for this Array
   * @throw NotEnoughIndicesProvidedException - if not enough indices were
   * provided
   * @throw TypeMismatchException - if the element of the Array does not contain
   * T's
   */
  template <typename T> ArrayElementRef<is_const_ref> &operator=(T rhs)
  {
    ...
    typedef typename std::conditional<
        std::is_same<std::string, T>::value || std::is_same<String, T>::value,
        GetStringType, GetArrayType<T>>::type GetNonArrayType;
    typedef
        typename std::conditional<std::is_base_of<Array, T>::value, GetCellType,
                                  GetNonArrayType>::type GetType;
    detail::setElement(pImpl.get(), std::move(rhs),
                       static_cast<int>(GetType::type));
    return *this;
  }
...
private:
  ...
  ArrayElementRef<is_const_ref> &
  operator=(const ArrayElementRef<is_const_ref> &rhs) = delete;
  ArrayElementRef<is_const_ref> &
  operator=(ArrayElementRef<is_const_ref> &&rhs) = delete;
};
} // namespace data
} // namespace matlab
