...
public:
  ...
  /**
   * @param outputs - Collection of matlab::data::Array objects that will
   * returned back to MATLAB
   * @param inputs - Collection of matlab::data::Array objects that are passed
   * to the MEX-file from MATLAB
   */
  ...
  /**
   * Locks the MEX-file preventing MATLAB's 'clear' commands to remove it from
   * memory.
   */
  void mexLock();

  /**
   * Unlocks the MEX-file allowing MATLAB's 'clear' commands to remove it from
   * memory
   */
  void mexUnlock();
