...
/* Copied from
 * fullfile(matlabroot,'extern','include','coder','coder_array','coder_array_mex.h')
 */
#pragma once
...
public:
  ...
  data_ptr(const T *_data, SZ _sz)
      : data_(const_cast<T *>(_data)), size_(_sz), capacity_(_sz), owner_(false)
  {
  }
  ...
  T *allocate(SZ _cap)
  {
    T *t = reinterpret_cast<T *>(
        emlrtMallocMex(static_cast<size_t>(_cap) * sizeof(T)));
    return t;
  }
  ...
  void reserve(SZ _n, emlrtRTEInfo *_rteInfo = nullptr,
  ...
  void resize(SZ _n, emlrtRTEInfo *_rteInfo = nullptr,
              emlrtConstCTX _ctx = nullptr)
  {
    SZ old_size = size_;
    if (_n > old_size) {
...
// : Compile-time product of the given size vector of length N with runtime
// check.
template <int N> class numel_check {
public:
  template <typename SZ>
  static SZ compute(emlrtRTEInfo *_rteInfo, emlrtConstCTX _ctx, SZ _size[])
  {
    return static_cast<SZ>(
        emlrtSizeMulR2012b(static_cast<uint32_T>(_size[N - 1]),
                           static_cast<uint32_T>(numel_check<N - 1>::compute(
                               _rteInfo, _ctx, _size)),
                           _rteInfo, _ctx));
  }
};
template <> class numel_check<0> {
public:
  template <typename SZ> static SZ compute(emlrtRTEInfo *, emlrtConstCTX, SZ[])
  {
    return 1;
  }
};
...
template <> struct match_dimensions<false> {
  static void check()
  {
...
public:
  ...
  // This is for MEX only. To prevent memory allocation when set_size is called
  // later.
  void prealloc(SZ _numel)
  {
  ...
  template <typename... Dims> void set(T const *_data, Dims... dims)
  {
...
public:
  ...
  template <typename... Dims> void set_size(SZ dim, Dims... dims)
  {
...
private:
  void ensureCapacity(SZ _newNumel, emlrtRTEInfo *_rteInfo = nullptr,
                      emlrtConstCTX _ctx = nullptr)
  {
...
template <> class array<char_T, 2> : public array_base<char_T, SizeType, 2> {
private:
...
public:
  ...
  array(array<char_T, 2> const &_other) : Base(_other)
  {
  }
  ...
  array(char_T *_data, SizeType const *_sz) : Base(_data, _sz)
  {
  }
