...
[A_nl_stiefel, b_nl_stiefel] = make_A_b_noloops_stiefel(R_truth_stiefel, T_globalframe_stiefel, Tijs_vec, edges, d_stiefel, som_params);
...
A_nl_stiefel = remove_quasi_zeros(A_nl_stiefel);
b_nl_stiefel = remove_quasi_zeros(b_nl_stiefel);

disp(max(max(abs(A_stiefel-A_nl_stiefel))))
disp(max(max(abs(b_stiefel-b_nl_stiefel))))
