...
[A, b] = make_A_b(R, T_globalframe, Tijs_vec, edges, som_params);

[A_nl, b_nl] = make_A_b_noloops(R, T_globalframe, Tijs_vec, edges, som_params);

% [Lnl, Pnl] = make_LT_PT_noloops(T_globalframe, Tijs_mat, edges, som_params);
...
disp("Cost with A,b computed with for loops");
disp(norm(A * T_globalframe(:) + b)) %this should be squared norm but w/e

disp("Cost with A,b computed WITHOUT for loops");
disp(norm(A_nl * T_globalframe(:) + b_nl)) %this should be squared norm but w/e
